...
stress_point_color = vector(1.0, 0.1, 0.1)  # Red color for stress points
pause_fork_color = vector(1.0, 0.5, 0.0)   # Orange color for paused forks
...
# --- New behavior: Localized stress points and fork pauses with repair enzyme recruitment ---

# Stress points along the DNA where forks may pause
stress_point_indices = []
stress_point_markers = []
stress_pause_duration = 1.8  # seconds of pause at stress point
stress_pause_chance = 0.75   # chance fork hits stress point and pauses
stress_pause_progress_threshold = 0.05  # minimum fork distance to start stress point effect

# Repair enzyme recruitment visual markers
repair_recruitment_markers = []

# Parameters controlling stress point spacing and count
num_stress_points = 5
stress_point_min_gap = 10  # minimum index gap between stress points

# Initialize stress points at reset and start
def initialize_stress_points():
    global stress_point_indices, stress_point_markers, stress_pause_timers, repair_recruitment_markers
    global resolved_stress_points, active_stress_point
    stress_point_indices = []
    stress_pause_timers.clear()
    resolved_stress_points["left"].clear()
    resolved_stress_points["right"].clear()
    active_stress_point["left"] = None
    active_stress_point["right"] = None
    # Clear old markers
    for m in stress_point_markers:
        m.visible = False
        del m
    stress_point_markers.clear()
    for m in repair_recruitment_markers:
        m.visible = False
        del m
    repair_recruitment_markers.clear()

    # Choose stress points spaced along the DNA indices, but keep them away
    # from the origin so the fork can visibly begin moving before the first pause.
    chosen_indices = []
    candidate_pool = [
        i for i in range(5, N - 5)
        if abs(x_positions[i]) > 1.75
    ]
    attempts = 0
    while len(chosen_indices) < num_stress_points and attempts < 600:
        attempts += 1
        candidate = random.choice(candidate_pool)
        if all(abs(candidate - c) >= stress_point_min_gap for c in chosen_indices):
            chosen_indices.append(candidate)
    if len(chosen_indices) < num_stress_points:
        for candidate in candidate_pool:
            if len(chosen_indices) >= num_stress_points:
                break
            if candidate not in chosen_indices and all(abs(candidate - c) >= max(4, stress_point_min_gap // 2) for c in chosen_indices):
                chosen_indices.append(candidate)
    chosen_indices.sort()
    stress_point_indices = chosen_indices

    # Create visual markers (rings) at stress points around the DNA axis
    for idx in stress_point_indices:
        pos = vector(x_positions[idx], 0, 0)
        ring_marker = ring(
            pos=pos,
            axis=vector(1, 0, 0),
            radius=1.3,
            thickness=0.04,
            color=stress_point_color,
            opacity=0.0
        )
        stress_point_markers.append(ring_marker)

    # Initialize repair recruitment markers (small spheres that appear near forks when paused)
    for _ in range(2):  # one per fork
        marker = sphere(
            pos=vector(0, 0, 0),
            radius=0.22,
            color=vector(0.2, 0.9, 1.0),
            emissive=True,
            opacity=0.0
        )
        repair_recruitment_markers.append(marker)

# Track pause timers per fork: left and right fork
stress_pause_timers = {
    "left": 0.0,
    "right": 0.0
}

# Track if fork is currently paused
fork_paused = {
    "left": False,
    "right": False
}

# Track repair enzyme recruitment cooldown to avoid spamming
repair_recruitment_cooldown = {
    "left": 0.0,
    "right": 0.0
}

# Stress points are resolved once a fork has paused and repaired them.
# Without this, a fork can unpause while still sitting on the same stress point
# and immediately pause again forever, especially near the origin at startup.
resolved_stress_points = {
    "left": set(),
    "right": set()
}
active_stress_point = {
...
# New variable to track if forks are paused due to stress points
forks_currently_paused = {
    "left": False,
    "right": False
}
...
def reset_simulation(ritual=False):
    ...
    global simulation_mode_note
    global stress_pause_timers, forks_currently_paused, repair_recruitment_cooldown
    global resolved_stress_points, active_stress_point
    ...
    # Initialize stress points and pause timers on reset
    initialize_stress_points()
    stress_pause_timers["left"] = 0.0
    stress_pause_timers["right"] = 0.0
    forks_currently_paused["left"] = False
    forks_currently_paused["right"] = False
    resolved_stress_points["left"].clear()
    resolved_stress_points["right"].clear()
    active_stress_point["left"] = None
    active_stress_point["right"] = None
    repair_recruitment_cooldown["left"] = 0.0
    repair_recruitment_cooldown["right"] = 0.0
...
def update_forks(now):
    ...
    def nearest_unresolved_stress(fork_side, fork_x):
        if abs(fork_x) < stress_pause_progress_threshold:
            return None
        # Each fork should only react to stress points on its own side.
        if fork_side == "left":
            side_indices = [idx for idx in stress_point_indices if x_positions[idx] < -0.05]
        else:
            side_indices = [idx for idx in stress_point_indices if x_positions[idx] > 0.05]
        side_indices = [idx for idx in side_indices if idx not in resolved_stress_points[fork_side]]
        if not side_indices:
            return None
        nearest_idx = min(side_indices, key=lambda idx: abs(x_positions[idx] - fork_x))
        if abs(x_positions[nearest_idx] - fork_x) < 0.25:
            return nearest_idx
        return None

    def set_marker_opacity(stress_idx, opacity):
        if stress_idx in stress_point_indices:
            stress_point_markers[stress_point_indices.index(stress_idx)].opacity = opacity

    def handle_fork_pause(fork_side, fork_x, marker_index):
        if forks_currently_paused[fork_side]:
            stress_pause_timers[fork_side] -= DT
            active_idx = active_stress_point[fork_side]
            if active_idx is not None:
                set_marker_opacity(active_idx, 0.75 + 0.2 * math.sin(now * 10.0))
                repair_recruitment_markers[marker_index].pos = vector(fork_x, 0.0, 0.0)
                repair_recruitment_markers[marker_index].opacity = 0.75 + 0.2 * math.sin(now * 12.0 + marker_index)
            if stress_pause_timers[fork_side] <= 0.0:
                if active_idx is not None:
                    resolved_stress_points[fork_side].add(active_idx)
                    set_marker_opacity(active_idx, 0.18)
                forks_currently_paused[fork_side] = False
                stress_pause_timers[fork_side] = 0.0
                active_stress_point[fork_side] = None
                repair_recruitment_markers[marker_index].opacity = 0.0
                repair_recruitment_cooldown[fork_side] = 0.75
            return

        repair_recruitment_cooldown[fork_side] = max(0.0, repair_recruitment_cooldown[fork_side] - DT)
        stress_idx = nearest_unresolved_stress(fork_side, fork_x)
        if stress_idx is not None and repair_recruitment_cooldown[fork_side] <= 0.0:
            if random.random() < stress_pause_chance:
                forks_currently_paused[fork_side] = True
                stress_pause_timers[fork_side] = stress_pause_duration
                active_stress_point[fork_side] = stress_idx
                set_marker_opacity(stress_idx, 0.85)
                repair_recruitment_markers[marker_index].pos = vector(fork_x, 0.0, 0.0)
                repair_recruitment_markers[marker_index].opacity = 0.9

    handle_fork_pause("left", left_x, 0)
    handle_fork_pause("right", right_x, 1)

    # Fade inactive stress markers without clearing the currently active pause markers.
    active_indices = {idx for idx in active_stress_point.values() if idx is not None}
    for i, idx in enumerate(stress_point_indices):
        if idx not in active_indices and stress_point_markers[i].opacity > 0.0:
            floor = 0.12 if (idx in resolved_stress_points["left"] or idx in resolved_stress_points["right"]) else 0.0
            stress_point_markers[i].opacity = max(floor, stress_point_markers[i].opacity - DT * 0.45)

    # Update fork cones and glows with pause visual cue.
    left_fork_cone.pos = vector(left_x + 0.36, 0, 0)
    left_fork_cone.axis = vector(-0.72, 0, 0)
    if forks_currently_paused["left"]:
        left_fork_cone.color = pause_fork_color
        left_fork_glow.color = pause_fork_color
        left_fork_glow.opacity = 0.3 + 0.3 * math.sin(now * 15)
    else:
        left_fork_cone.color = fork_color
        left_fork_glow.color = vector(1.0, 0.88, 0.2)
        left_fork_glow.opacity = 0.18

    right_fork_cone.pos = vector(right_x - 0.36, 0, 0)
    right_fork_cone.axis = vector(0.72, 0, 0)
    if forks_currently_paused["right"]:
        right_fork_cone.color = pause_fork_color
        right_fork_glow.color = pause_fork_color
        right_fork_glow.opacity = 0.3 + 0.3 * math.sin(now * 15 + 3.14)
    else:
        right_fork_cone.color = fork_color
        right_fork_glow.color = vector(1.0, 0.88, 0.2)
        right_fork_glow.opacity = 0.18

    pulse = 0.18 + 0.08 * math.sin(now * 7.0)
    if not forks_currently_paused["right"]:
        right_fork_glow.opacity = pulse
    if not forks_currently_paused["left"]:
...
def update_status(elapsed):
    # Add fork pause status to status label
    left_pause_status = "PAUSED" if forks_currently_paused["left"] else "running"
    right_pause_status = "PAUSED" if forks_currently_paused["right"] else "running"
    status_label.text = (
        f"Round: {round_count}\n"
        ...
        f"Left fork: {left_pause_status} | Right fork: {right_pause_status}\n"
...
try:
    while True:
        ...
        if not paused:
            # If forks are paused due to stress points, temporarily halt fork progression and replication speed
            if forks_currently_paused["left"] or forks_currently_paused["right"]:
                # Slow replication speed and fork distance increase during pause
                replication_speed += (0.0 - replication_speed) * 0.12
                # Do not advance fork distance while paused
                # Unfolding continues but slowed slightly
                unfold_speed += (target_unfold_speed * 0.45 - unfold_speed) * 0.12
        ...
        if CSV_ACTIVE and elapsed - last_csv_log >= 0.25:
            ...
            csv_writer.writerow([
                ...
                int(forks_currently_paused["left"]),
                int(forks_currently_paused["right"])
            ])
