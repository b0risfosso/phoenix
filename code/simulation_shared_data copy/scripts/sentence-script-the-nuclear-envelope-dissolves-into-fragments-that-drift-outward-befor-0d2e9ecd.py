...
class NuclearEnvelopeFragment:
    """A small membrane shard that dissolves from the original nucleus and later rebuilds.

    The fragments are intentionally lightweight VPython objects. They are created
    once per round, drift outward during prophase/prometaphase, then migrate to
    the two daughter nuclei to make nuclear-envelope reformation visible.
    """
    def __init__(self, sim, index, total_count):
        self.sim = sim
        self.index = index
        self.total_count = total_count
        self.phase = random.uniform(0, math.tau)
        self.spin = random.choice([-1, 1]) * random.uniform(0.8, 1.9)
        self.side = -1 if index % 2 == 0 else 1

        theta = math.acos(random.uniform(-0.82, 0.82))
        phi = (index / max(1, total_count)) * math.tau * 2.7 + random.uniform(-0.22, 0.22)
        self.surface_dir = safe_norm(vector(
            math.cos(phi) * math.sin(theta),
            math.cos(theta),
            math.sin(phi) * math.sin(theta)
        ))
        self.start_pos = self.surface_dir * random.uniform(0.74, 0.96)
        self.drift_dir = safe_norm(
            self.surface_dir + vector(
                random.uniform(-0.28, 0.28),
                random.uniform(-0.18, 0.18),
                random.uniform(-0.28, 0.28)
            )
        )
        self.tangent = safe_norm(cross(self.surface_dir, vector(1, 0.25, 0.1)), vector(0, 1, 0))

        # Rebuild targets form two incomplete shell-like rings around daughter nuclei.
        local_angle = (index // 2) * math.tau / max(1, total_count // 2) + random.uniform(-0.14, 0.14)
        y = math.cos(local_angle) * random.uniform(0.38, 0.74)
        z = math.sin(local_angle) * random.uniform(0.38, 0.74)
        x = random.uniform(-0.20, 0.20)
        self.rebuild_offset = vector(x, y, z)

        self.obj = box(
            pos=self.start_pos,
            size=vector(random.uniform(0.13, 0.22), random.uniform(0.018, 0.032), random.uniform(0.055, 0.095)),
            color=vector(0.50, 0.74, 1.0),
            opacity=0.42,
            shininess=0.7,
            emissive=True
        )
    ...
    def update(self, t, time_seconds, left_center, right_center):
        dissolve = smoothstep(0.06, 0.22, t)
        free_drift = smoothstep(0.12, 0.46, t) * (1 - smoothstep(0.56, 0.74, t))
        rebuild = smoothstep(0.58, 0.86, t)
        final_lock = smoothstep(0.82, 0.96, t)

        drift_distance = 0.30 + 0.55 * free_drift
        swirl = self.tangent * (0.10 * math.sin(time_seconds * self.spin + self.phase) * dissolve * (1 - final_lock))
        scatter_pos = self.start_pos + self.drift_dir * drift_distance * dissolve + swirl

        center = left_center if self.side < 0 else right_center
        rebuild_pos = center + self.rebuild_offset
        self.obj.pos = vlerp(scatter_pos, rebuild_pos, rebuild)

        # Rotate shard directions while free, then align them tangent to the rebuilt envelope.
        free_axis = safe_norm(self.tangent + self.drift_dir * math.sin(time_seconds * self.spin + self.phase))
        rebuilt_axis = safe_norm(cross(self.rebuild_offset, vector(1, 0.15, 0.05)), vector(0, 1, 0))
        self.obj.axis = safe_norm(vlerp(free_axis, rebuilt_axis, rebuild))
        self.obj.up = safe_norm(vlerp(self.surface_dir, self.rebuild_offset, rebuild), vector(0, 1, 0))

        visible_birth = smoothstep(0.00, 0.06, t)
        drift_glow = 0.20 * dissolve * (1 - rebuild)
        rebuild_glow = 0.22 * rebuild
        self.obj.opacity = (0.36 + drift_glow + rebuild_glow) * visible_birth
        self.obj.color = vlerp(vector(0.50, 0.74, 1.0), vector(0.74, 0.78, 1.0), rebuild)
...
class MitosisSimulation:
    ...
    def cleanup_dynamic(self):
        ...
        for w in list(self.wrappers):
            w.destroy()
        for f in list(self.envelope_fragments):
            f.destroy()
    ...
    def reset_round(self, full=False):
        ...
        self.central_nuclear_envelope = sphere(
            pos=vector(0, 0, 0),
            radius=0.96,
            color=vector(0.58, 0.75, 1.0),
            opacity=0.16,
            shininess=0.35
        )
        self.central_nuclear_envelope_ring_y = ring(
            pos=vector(0, 0, 0),
            axis=vector(0, 1, 0),
            radius=0.98,
            thickness=0.018,
            color=vector(0.50, 0.72, 1.0),
            opacity=0.44
        )
        self.central_nuclear_envelope_ring_z = ring(
            pos=vector(0, 0, 0),
            axis=vector(0, 0, 1),
            radius=0.98,
            thickness=0.018,
        ...
        self.register(self.central_nuclear_envelope)
        self.register(self.central_nuclear_envelope_ring_y)
        self.register(self.central_nuclear_envelope_ring_z)
        ...
        self.left_nucleus_ring_y = ring(
            pos=self.left_nucleus.pos,
            axis=vector(0, 1, 0),
            radius=0.89,
            thickness=0.020,
            color=vector(0.62, 0.76, 1.0),
            opacity=0
        )
        self.left_nucleus_ring_z = ring(
            pos=self.left_nucleus.pos,
            axis=vector(0, 0, 1),
            radius=0.89,
            thickness=0.018,
        ...
        self.right_nucleus_ring_y = ring(
            pos=self.right_nucleus.pos,
            axis=vector(0, 1, 0),
            radius=0.89,
            thickness=0.020,
            color=vector(0.62, 0.76, 1.0),
        ...
        self.right_nucleus_ring_z = ring(
            pos=self.right_nucleus.pos,
            axis=vector(0, 0, 1),
            radius=0.89,
            thickness=0.018,
        ...
        self.register(self.left_nucleus_ring_y)
        self.register(self.left_nucleus_ring_z)
        self.register(self.right_nucleus_ring_y)
        self.register(self.right_nucleus_ring_z)
        ...
        for i in range(fragment_count):
            self.envelope_fragments.append(NuclearEnvelopeFragment(self, i, fragment_count))

        self.mark(vector(0, 0.95, 0), "duplicated chromosome cluster", vector(0.45, 0.25, 0.85), life=4.0)
        self.mark(vector(0, -0.95, 0), "nuclear envelope intact", vector(0.42, 0.63, 1.0), life=3.2)

    def stage_name(self):
        t = self.progress
        if t < 0.16:
            return "Prophase: chromosomes condense as nuclear envelope starts dissolving"
        if t < 0.34:
            return "Metaphase: envelope fragments drift while spindle fibers organize"
        if t < 0.58:
            return "Anaphase: chromosomes are pulled apart"
        if t < 0.82:
            return "Telophase/Cytokinesis: envelopes rebuild around new chromosome groups"
        if t < 0.98:
    ...
    def update(self, dt):
        ...
        envelope_dissolve = smoothstep(0.06, 0.24, t)
        envelope_rebuild = smoothstep(0.58, 0.88, t)

        self.central_nuclear_envelope.opacity = 0.16 * (1 - envelope_dissolve)
        self.central_nuclear_envelope.size = vector(1.92 + 0.22 * envelope_dissolve, 1.92 + 0.14 * envelope_dissolve, 1.92 + 0.14 * envelope_dissolve)
        self.central_nuclear_envelope_ring_y.opacity = 0.44 * (1 - envelope_dissolve)
        self.central_nuclear_envelope_ring_z.opacity = 0.36 * (1 - envelope_dissolve)
        self.central_nuclear_envelope_ring_y.axis = norm(vector(0.05 * math.sin(self.time_seconds * 1.8), 1, 0.15 * math.cos(self.time_seconds * 1.4)))
        self.central_nuclear_envelope_ring_z.axis = norm(vector(0.08 * math.cos(self.time_seconds * 1.6), 0.15 * math.sin(self.time_seconds * 1.2), 1))
        ...
        ring_op = 0.56 * envelope_rebuild
        for ring_obj, center, axis_base, phase in [
            (self.left_nucleus_ring_y, self.left_nucleus.pos, vector(0, 1, 0), 0.0),
            (self.left_nucleus_ring_z, self.left_nucleus.pos, vector(0, 0, 1), 1.1),
            (self.right_nucleus_ring_y, self.right_nucleus.pos, vector(0, 1, 0), 2.0),
            (self.right_nucleus_ring_z, self.right_nucleus.pos, vector(0, 0, 1), 3.1),
        ]:
            ring_obj.pos = center
            ring_obj.radius = lerp(0.70, 0.91, envelope_rebuild) + 0.018 * math.sin(self.time_seconds * 2.0 + phase)
            ring_obj.opacity = ring_op
            ring_obj.axis = norm(axis_base + vector(0.05 * math.sin(self.time_seconds + phase), 0.04 * math.cos(self.time_seconds * 1.2 + phase), 0.04 * math.sin(self.time_seconds * 1.5 + phase)))

        for f in self.envelope_fragments:
            f.update(t, self.time_seconds, self.left_nucleus.pos, self.right_nucleus.pos)

        if 0.19 < t < 0.205 and int(self.time_seconds * 10) % 7 == 0:
            self.mark(vector(0, 1.18, 0), "nuclear envelope dissolving", vector(0.48, 0.68, 1.0), life=1.7)
        if 0.70 < t < 0.715 and int(self.time_seconds * 10) % 7 == 0:
            self.mark(vector(0, 1.25, 0), "envelopes rebuilding", vector(0.58, 0.76, 1.0), life=1.7)
        ...
        envelope_note = "envelope intact"
        if 0.08 <= t < 0.30:
            envelope_note = "envelope dissolving into fragments"
        elif 0.30 <= t < 0.58:
            envelope_note = "fragments drifting outward"
        elif 0.58 <= t < 0.90:
            envelope_note = "envelopes rebuilding"
        elif t >= 0.90:
            envelope_note = "new nuclear envelopes formed"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}% | {envelope_note}"
