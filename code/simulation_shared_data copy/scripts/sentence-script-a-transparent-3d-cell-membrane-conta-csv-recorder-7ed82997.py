...
# Anti-freeze motion safeguards.
# The AI can temporarily close channels or reduce global fields to zero; these
# values keep ions from settling into a visually frozen/stuck state.
MIN_ION_SPEED = 0.08
STUCK_SPEED_THRESHOLD = 0.035
STUCK_TIME_LIMIT = 1.25
MAX_ATTACH_SECONDS = 0.95

# -----------------------------
# CSV storage settings
# -----------------------------
# Web app compatibility:
#   SIMULATION_CSV_OUTPUT_DIR = folder created by the CSV storage web app
#   SIMULATION_CSV_RUN_ID = web app run identifier
#   SIMULATION_CSV_RUN_SECONDS = requested run length
# Fallback compatibility:
#   SIM_STATE_CSV_PATH can still be used when this script is run directly.
CSV_RUN_SECONDS = float(os.environ.get("SIMULATION_CSV_RUN_SECONDS", "60"))
CSV_SAMPLE_INTERVAL = 0.10  # seconds between saved state snapshots
...
if _csv_output_dir:
    CSV_OUTPUT_PATH = os.path.join(
        _csv_output_dir,
        f"{_csv_run_id}-cell-membrane-state-log.csv"
    )
else:
    CSV_OUTPUT_PATH = os.environ.get(
        "SIM_STATE_CSV_PATH",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "cell_membrane_state_log.csv")
    )
...
csv_writer = None
csv_closed = False
csv_recording_complete = False
csv_exit_when_done = bool(os.environ.get("SIMULATION_CSV_OUTPUT_DIR"))
...
class Ion:
    ...
    def update(self, dt):
        ...
        if self.attached_channel is not None:
            ...
            # Attached ions should look like they are sticking at a pore, not
            # permanently freezing. Give them a tiny surface wobble and force
            # release after MAX_ATTACH_SECONDS or when the gate opens.
            wobble = tangent_swirl(ch.n + vector(0.03, 0.01, 0.02), 0.08 * math.sin(frame_count * 0.2))
            self.pos = ch.n * (R + self.attach_side * 0.21) + wobble
            self.vel = wobble * 0.35
            self.obj.pos = self.pos

            if self.attach_timer <= 0 or ch.open:
                outward = ch.n * self.attach_side
                tang = tangent_swirl(ch.n, pyrandom.uniform(0.18, 0.55))
                jitter = random_unit() * 0.08
                self.vel = outward * pyrandom.uniform(0.58, 1.05) + tang + jitter
                self.attached_channel = None
                self.attach_timer = 0
            return
        ...
        brownian = random_unit() * pyrandom.uniform(0.05, 0.38)
        ...
        # If an ion has been nearly motionless for more than a moment, give it
        # a small thermal kick. This prevents AI modes with low field strength
        # or closed channels from visually freezing the particle population.
        if speed < STUCK_SPEED_THRESHOLD:
            self.stuck_timer += dt
        else:
            self.stuck_timer = max(0.0, self.stuck_timer - dt * 2.0)

        if speed < MIN_ION_SPEED or self.stuck_timer > STUCK_TIME_LIMIT:
            radial_kick = safe_norm(self.pos) * pyrandom.uniform(-0.05, 0.05)
            surface_kick = tangent_swirl(self.pos + random_unit() * 0.05, pyrandom.uniform(-0.18, 0.18))
            self.vel += random_unit() * pyrandom.uniform(0.06, 0.16) + radial_kick + surface_kick
            self.stuck_timer = 0.0
        ...
        if crossed_membrane:
            ...
            if selected is not None:
                ...
                if ch.open:
                    ...
                    if dist_to_channel < CHANNEL_APERTURE * 0.82 and pyrandom.random() < 0.45:
                        self.attached_channel = ch
                        self.attach_side = side
                        self.attach_timer = min(MAX_ATTACH_SECONDS, pyrandom.uniform(0.18, 0.62))
                        ch.attached_count += 1
                        self.vel = tangent_swirl(ch.n, pyrandom.uniform(-0.04, 0.04))
...
class MembraneAI:
    ...
    def detect_stagnation_and_completion(self, dt, state):
        ...
        if self.stagnant_time > 8:
            self.set_mode("PULSE")
            self.stagnant_time = 0
        elif self.completion_time > 5 or too_long:
            self.set_mode("MIX")
...
def init_csv_log():
    ...
    global csv_file, csv_writer, csv_closed

    out_dir = os.path.dirname(os.path.abspath(CSV_OUTPUT_PATH))
    if out_dir:
    ...
    csv_file.flush()
    csv_closed = False
    print(f"CSV state logging started: {CSV_OUTPUT_PATH}")

def write_csv_state(t):
    """Write one simulation snapshot as one row per ion."""
    global csv_sample_index

    if csv_writer is None or csv_closed:
        return

    inside_count = sum(1 for ion in ions if ion.is_inside())
    outside_count = len(ions) - inside_count
    open_channel_count = sum(1 for ch in channels if ch.open)
    electric_x, electric_y, electric_z = vector_components(global_electric_field)
    channel_open_mask = "".join("1" if ch.open else "0" for ch in channels)
    channel_attached_counts = "|".join(str(ch.attached_count) for ch in channels)

    for ion_index, ion in enumerate(ions):
        vx, vy, vz = vector_components(ion.vel)
        x, y, z = vector_components(ion.pos)
        attached_idx = "" if ion.attached_channel is None else ion.attached_channel.idx
        ...
        csv_writer.writerow({
            ...
            "electric_x": round(electric_x, 6),
            "electric_y": round(electric_y, 6),
            "electric_z": round(electric_z, 6),
            "channel_open_mask": channel_open_mask,
            "channel_attached_counts": channel_attached_counts
        })

    csv_sample_index += 1

    # Flush every simulated second so data is preserved during long runs.
    if csv_sample_index % max(1, int(1.0 / CSV_SAMPLE_INTERVAL)) == 0:
        csv_file.flush()

def close_csv_log():
    """Flush and close the CSV file once the configured recording is complete."""
    global csv_closed

    if csv_file is not None and not csv_closed:
        csv_file.flush()
        csv_file.close()
        csv_closed = True
        print(f"CSV state logging complete: {CSV_OUTPUT_PATH}")

atexit.register(close_csv_log)
...
while True:
    ...
    if sim_elapsed >= csv_next_sample_time:
        write_csv_state(sim_elapsed)
        csv_next_sample_time += CSV_SAMPLE_INTERVAL

    if (not csv_recording_complete) and sim_elapsed >= CSV_RUN_SECONDS:
        csv_recording_complete = True
        close_csv_log()
        status_label.text = (
            f"CSV recording complete: {CSV_RUN_SECONDS:0.0f}s saved to "
            f"{os.path.basename(CSV_OUTPUT_PATH)}"
        )
        print(f"Recorded {csv_sample_index} snapshots over {CSV_RUN_SECONDS:0.0f} simulated seconds.")

        # When the CSV web app launches the script, exit so the run can finish.
        # When run directly, keep the visual simulation alive instead of leaving
        # the ions frozen on the final recorded frame.
        if csv_exit_when_done:
            break
