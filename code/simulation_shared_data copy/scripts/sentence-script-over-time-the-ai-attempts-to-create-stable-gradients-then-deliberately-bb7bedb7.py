...
Controls:
  ...
  G = AI gradient cycle       X = disrupt gradient now
"""
...
# Gradient cycle state used by the AI when it builds, stabilizes, disrupts,
# and recovers ion gradients across the membrane.
gradient_disruption_flash = 0.0
...
def gradient_strength_counts(inside, outside):
    total = max(1, inside + outside)
    return abs(inside - outside) / total

def gradient_direction_counts(inside, outside):
    if inside > outside:
        return "inside-rich"
    if outside > inside:
        return "outside-rich"
...
# Gradient cycle display. The AI uses this gauge to show whether it is
# building a stable inside/outside imbalance, disrupting it, or recovering it.
gradient_back = box(
    pos=vector(8.45, 0, 0),
    size=vector(0.12, 5.2, 0.22),
    color=vector(0.84, 0.86, 0.92),
    opacity=0.42
)

gradient_strength_bar = box(
    pos=vector(8.45, -2.5, 0),
    size=vector(0.42, 0.01, 0.42),
    color=vector(0.74, 0.28, 1.0),
    opacity=0.75
)

gradient_target_marker = ring(
    pos=vector(8.45, 2.15, 0),
    axis=vector(0, 1, 0),
    radius=0.34,
    thickness=0.025,
    color=vector(1.0, 0.72, 0.12),
    opacity=0.72
)

gradient_label = label(
    pos=vector(8.45, 3.15, 0),
    text="gradient",
    height=10,
    color=vector(0.22, 0.12, 0.36),
    box=False,
    opacity=0
)

disruption_shell = sphere(
    pos=vector(0, 0, 0),
    radius=R * 1.045,
    color=vector(1.0, 0.32, 0.18),
    opacity=0.0
)
...
class MembraneAI:
    def __init__(self):
        ...
        self.reset_wait = 0.0
        self.gradient_target_side = "inside"
        self.gradient_stable_timer = 0.0
        self.gradient_recovery_timer = 0.0
        self.gradient_cycle_count = 0
        self.disruption_burst_timer = 0.0
    ...
    def read_state(self):
        ...
        return {
            ...
            "gradient_strength": gradient_strength_counts(inside, outside),
            "gradient_direction": gradient_direction_counts(inside, outside),
            "round": round_number
        }
    ...
    def set_mode(self, mode):
        ...
        if mode == "GRADIENT_BUILD":
            self.mode_duration = pyrandom.uniform(16.0, 24.0)
            self.gradient_target_side = "outside" if self.gradient_target_side == "inside" else "inside"
            self.gradient_stable_timer = 0.0
            self.gradient_cycle_count += 1
        if mode == "GRADIENT_HOLD":
            self.mode_duration = pyrandom.uniform(4.0, 7.0)
            self.gradient_stable_timer = 0.0
        if mode == "GRADIENT_DISRUPT":
            self.mode_duration = pyrandom.uniform(5.0, 8.0)
            self.disruption_burst_timer = 0.0
        if mode == "RECOVER":
            self.mode_duration = pyrandom.uniform(10.0, 16.0)
            self.gradient_recovery_timer = 0.0
        if mode == "CHAOS":
    ...
    def choose_new_mode(self, state, force=False):
        ...
        candidates = ["BALANCE", "GRADIENT_BUILD", "MIX", "PULSE", "ORBIT", "ARTIST", "CAREFUL"]
    ...
    def detect_stagnation_and_completion(self, dt, state):
        ...
        if self.mode not in ["GRADIENT_BUILD", "GRADIENT_HOLD", "GRADIENT_DISRUPT", "RECOVER"]:
    ...
    def step(self, dt):
        ...
        if self.time_in_mode > self.mode_duration and self.mode not in ["RESET", "GRADIENT_BUILD", "GRADIENT_HOLD", "GRADIENT_DISRUPT", "RECOVER"]:
        ...
        if self.mode == "BALANCE":
        ...
        elif self.mode == "GRADIENT_BUILD":
            # Build a deliberate inside-rich or outside-rich ion gradient. The AI
            # keeps only a few moving pores open so crossings look like directed
            # transport rather than random mixing.
            self.focus_index = int((self.phase * 0.85) % len(channels))
            self.apply_channel_pattern("focus", state)
            target_inside = self.gradient_target_side == "inside"
            goal_reached = inside > total * 0.72 if target_inside else outside > total * 0.72
            global_pressure_bias = -1.18 if target_inside else 1.18
            global_swirl_strength = 0.10
            focus_dir = safe_norm(channels[self.focus_index].n)
            global_electric_field = focus_dir * (-0.12 if target_inside else 0.12)
            for ion in ions:
                if abs(mag(ion.pos) - R) < 1.25 and pyrandom.random() < 0.006:
                    ion.set_marked(True)
            if goal_reached:
                self.gradient_stable_timer += dt
            else:
                self.gradient_stable_timer = max(0.0, self.gradient_stable_timer - dt * 0.5)
            if self.gradient_stable_timer > 1.8 or self.time_in_mode > self.mode_duration:
                self.set_mode("GRADIENT_HOLD")

        elif self.mode == "GRADIENT_HOLD":
            # Stabilize the gradient briefly. A few channels remain open while the
            # pressure bias gently resists immediate equalization.
            self.apply_channel_pattern("careful", state)
            target_inside = self.gradient_target_side == "inside"
            global_pressure_bias = -0.34 if target_inside else 0.34
            global_swirl_strength = 0.025
            global_electric_field = vector(0.015 * math.sin(self.phase), 0.015 * math.cos(self.phase), 0)
            self.gradient_stable_timer += dt
            if self.gradient_stable_timer > self.mode_duration:
                self.set_mode("GRADIENT_DISRUPT")

        elif self.mode == "GRADIENT_DISRUPT":
            # Deliberately break the stable gradient: open many channels, alternate
            # pressure, add swirl, and kick nearby ions so the imbalance visibly
            # collapses.
            global gradient_disruption_flash
            self.apply_channel_pattern("all_open", state)
            pulse = math.sin(self.phase * 5.2)
            global_pressure_bias = 1.45 * pulse
            global_swirl_strength = 1.15 + 0.35 * math.sin(self.phase * 2.8)
            global_electric_field = random_unit() * 0.18
            gradient_disruption_flash = max(gradient_disruption_flash, 0.85)
            if self.action_timer <= 0:
                self.action_timer = 0.35
                for ch in channels:
                    if pyrandom.random() < 0.34:
                        ch.toggle()
                if len(ions) < MAX_IONS and pyrandom.random() < 0.45:
                    spill_ions(2)
                for ion in ions:
                    if abs(mag(ion.pos) - R) < 1.4:
                        ion.vel += random_unit() * pyrandom.uniform(0.3, 1.1)
            ...
            # Recovery phase: reopen a controlled pattern and push toward balanced
            # inside/outside counts until the gradient is mostly erased.
            self.apply_channel_pattern("alternating", state)
            diff = inside - outside
            if diff > 3:
                global_pressure_bias = 0.86
            elif diff < -3:
                global_pressure_bias = -0.86
            else:
                global_pressure_bias = 0.06 * math.sin(self.phase * 1.4)
            global_swirl_strength = 0.20
            global_electric_field = vector(0.04 * math.sin(self.phase * 0.9), 0.02, 0.04 * math.cos(self.phase * 0.9))
            if abs(diff) < max(4, total * 0.15):
                self.gradient_recovery_timer += dt
            else:
                self.gradient_recovery_timer = max(0.0, self.gradient_recovery_timer - dt * 0.5)
            if self.gradient_recovery_timer > 2.2 or self.time_in_mode > self.mode_duration:
                self.set_mode("GRADIENT_BUILD")
    ...
    def update_wand(self, dt, state):
        mode_col = {
            ...
            "GRADIENT_BUILD": vector(0.95, 0.72, 0.12),
            "GRADIENT_HOLD": vector(0.86, 0.36, 1.0),
            "GRADIENT_DISRUPT": vector(1.0, 0.16, 0.08),
            "RECOVER": vector(0.18, 0.88, 0.72),
        ...
        if channels:
            if self.mode in ["PUMP_IN", "PUMP_OUT", "ARTIST", "GRADIENT_BUILD", "GRADIENT_HOLD"]:
                target_dir = channels[self.focus_index % len(channels)].n
                target = target_dir * (R + 1.05)
                self.wand.pos = self.wand.pos * 0.86 + target * 0.14
            else:
                a = self.phase * (0.55 if self.mode not in ["CHAOS", "GRADIENT_DISRUPT"] else 1.7)
                b = self.phase * 0.37
...
def reset_simulation(round_increment=False):
    global ions, round_number, total_crossings, global_pressure_bias, global_swirl_strength, global_electric_field, gradient_disruption_flash
...
def update_meter_and_labels():
    global last_counts, gradient_disruption_flash
    inside = sum(1 for ion in ions if ion.is_inside())
    outside = len(ions) - inside
    ...
    strength = gradient_strength_counts(inside, outside)
    gradient_h = max_h * strength
    gradient_strength_bar.size = vector(0.42, max(0.03, gradient_h), 0.42)
    gradient_strength_bar.pos = vector(8.45, -2.5 + gradient_h / 2, 0)
    gradient_strength_bar.color = vector(0.74, 0.28, 1.0) if inside >= outside else vector(1.0, 0.55, 0.18)
    target_y = -2.5 + max_h * 0.72
    gradient_target_marker.pos = vector(8.45, target_y, 0)
    gradient_target_marker.radius = 0.32 + 0.08 * math.sin(frame_count * 0.12)
    gradient_target_marker.color = vector(0.22, 0.48, 1.0) if getattr(ai, "gradient_target_side", "inside") == "inside" else vector(1.0, 0.55, 0.18)
    disruption_shell.opacity = 0.18 * gradient_disruption_flash
    disruption_shell.radius = R * (1.025 + 0.075 * gradient_disruption_flash * (0.5 + 0.5 * math.sin(frame_count * 0.55)))
    gradient_disruption_flash = max(0.0, gradient_disruption_flash - DT * 1.4)

    open_count = sum(1 for ch in channels if ch.open)
    status_label.text = (
        f"Round {round_number}   Inside: {inside}   Outside: {outside}   "
        f"Open channels: {open_count}/{len(channels)}   Transfers: {total_crossings}   "
        f"Gradient: {gradient_strength_counts(inside, outside):0.2f}"
    )
    ...
    gradient_label.text = (
        f"gradient {gradient_direction_counts(inside, outside)}\n"
        f"target {getattr(ai, 'gradient_target_side', 'inside')}"
    )

    ai_label.text = (
        f"AI {ai_state}: {ai.mode}   gradient cycle {getattr(ai, 'gradient_cycle_count', 0)}   "
        f"stagnation {ai.stagnant_time:0.1f}s"
...
def on_keydown(evt):
    ...
    if k == " ":
        ...
        ai.set_mode("GRADIENT_BUILD")
    elif k == "x":
        ai.set_mode("GRADIENT_DISRUPT")
    elif k == "h":
