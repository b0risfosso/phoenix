...
Controls:
  ...
  V = AI channel scout mode    N = select next channel
"""
...
SELECT_COLOR = vector(1.0, 0.86, 0.12)
TRAFFIC_LOW_COLOR = vector(0.24, 0.58, 1.0)
TRAFFIC_HIGH_COLOR = vector(1.0, 0.32, 0.10)
...
def move_direction_toward(current, target, fraction):
    """Slide a direction vector over the membrane without leaving the sphere."""
    blended = safe_norm(current) * (1 - fraction) + safe_norm(target) * fraction
    return safe_norm(blended)
...
traffic_label = label(
    pos=vector(7.35, -6.05, 0),
    text="",
    height=11,
    color=vector(0.18, 0.16, 0.34),
    box=False,
    opacity=0
)
...
class IonChannel:
    def __init__(self, idx, direction):
        ...
        self.selected = False
        self.target_n = vector(self.n)
        self.local_traffic = 0.0
        self.recent_crossings = 0.0
        self.local_in = 0
        self.local_out = 0
        self.scan_score = 0.0
        ...
        self.selection_ring = ring(
            pos=self.n * (R + 0.58),
            axis=self.n,
            radius=CHANNEL_RADIUS * 2.15,
            thickness=0.035,
            color=SELECT_COLOR,
            opacity=0.0
        )

        self.traffic_beacon = sphere(
            pos=self.n * (R + 0.98),
            radius=0.09,
            color=TRAFFIC_LOW_COLOR,
            emissive=True,
            opacity=0.35
        )
    ...
    def transfer_flash(self):
        self.flash = 1.0
        self.recent_crossings += 1.0

    def set_selected(self, value):
        self.selected = bool(value)

    def set_target_direction(self, direction):
        self.target_n = safe_norm(direction)

    def update_geometry(self, dt):
        # The AI can drag a channel across the spherical membrane by changing target_n.
        # All visual parts are reprojected back onto the membrane after the direction changes.
        if mag(self.target_n - self.n) > 0.002:
            self.n = move_direction_toward(self.n, self.target_n, clamp(dt * 1.4, 0.0, 0.08))

        self.body.pos = self.n * (R - CHANNEL_LENGTH / 2)
        self.body.axis = self.n * CHANNEL_LENGTH
        self.pore_core.pos = self.n * (R - CHANNEL_LENGTH / 2 + 0.04)
        self.pore_core.axis = self.n * (CHANNEL_LENGTH - 0.08)
        self.gate.pos = self.n * R
        self.gate.axis = self.n * 0.105
        self.halo.pos = self.n * (R + 0.42)
        self.halo.axis = self.n
        self.selection_ring.pos = self.n * (R + 0.58)
        self.selection_ring.axis = self.n
        self.traffic_beacon.pos = self.n * (R + 0.98)
        self.lbl.pos = self.n * (R + 0.82)

    def update_visual(self, dt):
        self.update_geometry(dt)
        self.pulse_phase += dt * 2.0
        self.flash = max(0.0, self.flash - dt * 2.2)
        self.recent_crossings *= max(0.0, 1.0 - dt * 1.6)
        breathe = 1.0 + 0.07 * math.sin(self.pulse_phase)
        traffic_heat = clamp(self.local_traffic / 8.0 + self.recent_crossings * 0.18, 0.0, 1.0)
        traffic_col = TRAFFIC_LOW_COLOR * (1 - traffic_heat) + TRAFFIC_HIGH_COLOR * traffic_heat

        if self.open:
            base_col = OPEN_COLOR
            self.body.color = base_col + vector(0.45, 0.35, 0.15) * self.flash + traffic_col * (0.18 * traffic_heat)
            self.body.opacity = 0.34 + 0.25 * self.flash + 0.10 * traffic_heat
            self.body.radius = CHANNEL_RADIUS * (1.06 + 0.06 * breathe + 0.12 * self.flash + 0.08 * traffic_heat)
            self.pore_core.opacity = 0.25 + 0.35 * self.flash + 0.22 * traffic_heat
            self.gate.opacity = 0.0
            self.halo.color = base_col * (1 - traffic_heat) + traffic_col * traffic_heat
            self.halo.opacity = 0.35 + 0.35 * self.flash + 0.25 * traffic_heat
            self.halo.radius = CHANNEL_RADIUS * (1.55 + 0.14 * math.sin(self.pulse_phase) + 0.22 * traffic_heat)
        else:
            base_col = CLOSED_COLOR
            self.body.color = base_col * (1 - 0.25 * traffic_heat) + traffic_col * (0.25 * traffic_heat)
            self.body.opacity = 0.55 + 0.08 * traffic_heat
            self.body.radius = CHANNEL_RADIUS * (0.92 + 0.03 * breathe + 0.04 * traffic_heat)
            self.pore_core.opacity = 0.03 + 0.08 * traffic_heat
            self.gate.opacity = 0.78
            self.gate.color = CLOSED_COLOR
            self.halo.color = CLOSED_COLOR * (1 - 0.35 * traffic_heat) + traffic_col * (0.35 * traffic_heat)
            self.halo.opacity = 0.30 + 0.18 * traffic_heat
            self.halo.radius = CHANNEL_RADIUS * (1.32 + 0.12 * traffic_heat)

        self.selection_ring.opacity = 0.68 + 0.22 * math.sin(self.pulse_phase * 2.2) if self.selected else 0.0
        self.selection_ring.radius = CHANNEL_RADIUS * (2.05 + 0.20 * math.sin(self.pulse_phase * 1.6))
        self.traffic_beacon.color = traffic_col
        self.traffic_beacon.radius = 0.08 + 0.20 * traffic_heat
        self.traffic_beacon.opacity = 0.30 + 0.55 * traffic_heat
...
class MembraneAI:
    ...
    def read_state(self):
        ...
        selected = channels[self.focus_index % len(channels)] if channels else None
        return {
            "inside": inside,
            "outside": outside,
            "total": len(ions),
            ...
            "attached": attached,
            "marked": marked,
            "selected_channel": selected.idx + 1 if selected is not None else 0,
            "selected_traffic": selected.local_traffic if selected is not None else 0,
            "selected_local_in": selected.local_in if selected is not None else 0,
            "selected_local_out": selected.local_out if selected is not None else 0,
    ...
    def set_mode(self, mode):
        ...
        if mode == "SCOUT":
            self.mode_duration = pyrandom.uniform(16.0, 28.0)
        if mode == "RESET":
    ...
    def choose_new_mode(self, state, force=False):
        ...
        candidates = ["BALANCE", "MIX", "PULSE", "ORBIT", "ARTIST", "SCOUT", "CAREFUL"]
        ...
        if self.stagnant_time > 10:
            candidates += ["CHAOS", "PULSE", "ARTIST", "SCOUT"]
    ...
    def select_channel(self, idx):
        if not channels:
            return
        self.focus_index = idx % len(channels)
        for i, ch in enumerate(channels):
            ch.set_selected(i == self.focus_index)

    def select_next_channel(self):
        self.select_channel(self.focus_index + 1)

    def move_selected_channel_to(self, direction):
        if not channels:
            return
        ch = channels[self.focus_index % len(channels)]
        ch.set_target_direction(direction)
        ch.set_selected(True)

    def scout_channel_traffic(self, state):
        # The AI studies one pore at a time. It slides that pore to a nearby membrane patch,
        # opens it, reads the local ion traffic, then chooses another patch if traffic stalls.
        if not channels:
            return
        ch = channels[self.focus_index % len(channels)]
        self.select_channel(self.focus_index)
        for i, other in enumerate(channels):
            if i == self.focus_index:
                other.set_open(True)
            elif pyrandom.random() < 0.015:
                other.set_open(pyrandom.random() < 0.35)

        if self.action_timer <= 0:
            self.action_timer = pyrandom.uniform(2.0, 3.8)
            # Prefer moving toward the densest nearby ion cluster, with occasional exploratory jumps.
            candidates = [ion.pos for ion in ions if abs(mag(ion.pos) - R) < 2.2]
            if candidates and pyrandom.random() < 0.78:
                candidates.sort(key=lambda p: mag(p - ch.surface_point()))
                target = safe_norm(pyrandom.choice(candidates[:min(12, len(candidates))]))
            else:
                target = move_direction_toward(ch.n, random_unit(), pyrandom.uniform(0.25, 0.55))
            self.move_selected_channel_to(target)

        # If the selected channel finds little local traffic for a while, switch to a new channel.
        if ch.local_traffic < 1.0 and self.time_in_mode > 4 and pyrandom.random() < 0.01:
            self.select_next_channel()
        # If the selected channel becomes a hot spot, let the wand mark the activity.
        if ch.local_traffic > 5.0 and pyrandom.random() < 0.04:
            self.mark_near_wand()
    ...
    def step(self, dt):
        ...
        if self.mode == "BALANCE":
            ...
            self.scout_channel_traffic(state)
            ch = channels[self.focus_index % len(channels)]
            # bias ions gently toward the selected pore so traffic changes are easy to see.
            pull_sign = -1 if outside > inside else 1
            global_pressure_bias = 0.46 * pull_sign
            global_swirl_strength = 0.20 + clamp(ch.local_traffic / 20.0, 0.0, 0.35)
            global_electric_field = safe_norm(ch.surface_point()) * (0.08 * pull_sign)
    ...
    def update_wand(self, dt, state):
        ...
        if channels:
            if self.mode in ["PUMP_IN", "PUMP_OUT", "ARTIST", "SCOUT"]:
...
def reset_simulation(round_increment=False):
    ...
    for i, ch in enumerate(channels):
        ...
        ch.recent_crossings = 0.0
        ch.scan_score = 0.0
        ch.set_selected(i == ai.focus_index % len(channels) if channels else False)
        ch.set_target_direction(ch.n)
...
# -----------------------------
# Local channel traffic measurement
# -----------------------------
def update_local_channel_traffic(dt):
    for ch in channels:
        local_in = 0
        local_out = 0
        for ion in ions:
            if abs(mag(ion.pos) - R) < 1.15 and angular_surface_distance(ion.pos, ch.n) < CHANNEL_APERTURE * 1.6:
                if ion.is_inside():
                    local_in += 1
                else:
                    local_out += 1
        instant = local_in + local_out
        ch.local_in = local_in
        ch.local_out = local_out
        ch.local_traffic = ch.local_traffic * 0.86 + instant * 0.14
        ch.scan_score = ch.scan_score * 0.985 + (instant + ch.recent_crossings * 3.0) * dt
...
def update_meter_and_labels():
    ...
    open_count = sum(1 for ch in channels if ch.open)
    selected = channels[ai.focus_index % len(channels)] if channels else None
    selected_text = ""
    if selected is not None:
        selected_text = (
            f"   Selected channel: {selected.idx + 1}"
            f"   Local traffic: {selected.local_traffic:0.1f}"
        )
    status_label.text = (
    ...
    if selected is not None:
        traffic_label.text = (
            f"AI watching channel {selected.idx + 1}\n"
            f"near pore: in {selected.local_in} | out {selected.local_out}\n"
            f"moving target: ({selected.target_n.x:0.1f}, {selected.target_n.y:0.1f}, {selected.target_n.z:0.1f})"
        )
    else:
        traffic_label.text = ""
...
def update_channel_visuals(dt):
    update_local_channel_traffic(dt)
    for ch in channels:
        ch.update_visual(dt)
