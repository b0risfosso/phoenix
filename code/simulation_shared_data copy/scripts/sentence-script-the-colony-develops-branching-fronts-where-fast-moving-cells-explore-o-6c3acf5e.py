...
scene = canvas(
    title="3D Bacterial Colony Growth - Branching Fronts",
    width=1200,
    height=780,
...
TRAIL_LIMIT = 520
PARTICLE_LIMIT = 250
BRANCH_MARKER_LIMIT = 260

# Branching-front behavior:
# Some cells become fast scouts at the colony edge. They lay faint branch
# trails while the slower central mass follows outward through slime paths.
FRONT_CELL_FRACTION = 0.18
FRONT_ACTIVATION_SPREAD = 1.15
FRONT_SPEED_BOOST = 1.95
MASS_FOLLOW_STRENGTH = 0.22
BRANCH_TRAIL_STRENGTH = 0.085
...
def add_branch_marker(start, end, strength=1.0):
    """Draw a short, bright guide line left by fast exploratory front cells."""
    if mag(end - start) < 0.045:
        return
    if len(branch_markers) >= BRANCH_MARKER_LIMIT:
        old = branch_markers.pop(0)
        hide_object(old["obj"])

    c = curve(
        pos=[vector(start.x, 0.058, start.z), vector(end.x, 0.058, end.z)],
        radius=0.017 + 0.006 * clamp(strength),
        color=vector(1.0, 0.78, 0.28)
    )
    try:
        c.opacity = 0.28 + 0.20 * clamp(strength)
    except Exception:
        pass
    branch_markers.append({"obj": c, "life": 15.0 + 10.0 * clamp(strength), "maxlife": 15.0 + 10.0 * clamp(strength)})


def update_branch_markers(dt):
    alive = []
    for bm in branch_markers:
        ...
        if bm["life"] <= 0:
            ...
            continue
        try:
            bm["obj"].opacity = max(0.025, 0.42 * bm["life"] / bm["maxlife"])
        except Exception:
            pass
        alive.append(bm)
    branch_markers[:] = alive
...
class Bacterium:
    ...
    def __init__(self, pos=None, theta=None, energy=1.0, attached=False):
        ...
        # Branching front state.  Front cells are temporary explorers at the
        # colony perimeter; the slower mass follows the paths they reinforce.
        self.front_cell = random.random() < FRONT_CELL_FRACTION
        self.front_active = False
        self.branch_memory = random.uniform(0.0, 1.0)
        self.branch_clock = random.uniform(0.1, 0.7)
        self.mass_follow_bias = random.uniform(0.65, 1.25)
    ...
    def update_visual(self):
        ...
        if self.front_active:
            c = mix_color(c, vector(1.0, 0.78, 0.22), 0.55)
            self.halo.color = vector(1.0, 0.72, 0.18)
            self.halo.opacity = 0.16 + 0.05 * math.sin(sim_time * 5.0 + self.id)
            self.halo.radius = self.radius * (2.35 + 0.30 * math.sin(sim_time * 3.4 + self.id))
        elif self.attached:
    ...
    def update(self, dt):
        ...
        # Branching-front behavior: a subset of perimeter cells accelerates
        # outward toward nutrients and leaves reinforced slime/branch trails.
        ccenter = colony_center()
        outward = safe_norm(flat(self.pos - ccenter))
        radial_distance = mag(flat(self.pos - ccenter))
        spread_now = colony_spread(ccenter)
        at_front = radial_distance > max(FRONT_ACTIVATION_SPREAD, spread_now * 0.82)
        can_explore = (self.front_cell or self.energy > 1.35) and (not self.attached) and local_n > 0.20
        self.front_active = can_explore and at_front and len(bacteria) > 8

        if self.front_active:
            nutrient_pull = safe_norm(grad_n) if mag(grad_n) > 0.01 else vector(0, 0, 0)
            exploratory = safe_norm(outward * 0.78 + nutrient_pull * 1.15 + perpendicular_xz(outward) * math.sin(sim_time * 0.8 + self.id) * 0.28)
            desired += exploratory * 1.75
            self.branch_memory = clamp(self.branch_memory + 0.40 * dt, 0.0, 1.0)
        else:
            # Slower mass follows established slime/branch corridors without
            # racing outward as quickly as the front cells.
            if mag(grad_s) > 0.006:
                desired += safe_norm(grad_s) * MASS_FOLLOW_STRENGTH * self.mass_follow_bias
            self.branch_memory = clamp(self.branch_memory - 0.08 * dt, 0.0, 1.0)
        ...
        if self.attached:
            ...
            if self.front_active:
                speed *= FRONT_SPEED_BOOST
                # Explorers stay lean: they spend energy to probe outward.
                self.energy -= 0.010 * dt
            else:
                # The mass advances more slowly but is pulled along reinforced
                # slime corridors left by explorers.
                speed *= 0.78 + 0.26 * clamp(local_s)
            self.theta += self.omega * dt
            self.pos += self.forward() * speed * dt + self.vel * dt
            slime_rate = 0.030 + 0.030 * clamp(1.0 - local_n)
            if self.front_active:
                slime_rate += BRANCH_TRAIL_STRENGTH
        ...
        add_to_field(S, self.pos, slime_rate * dt, radius=0.68 if not self.front_active else 0.82, cap=1.35)

        if self.front_active:
            self.branch_clock -= dt
            if self.branch_clock <= 0 and mag(self.pos - self.prev_pos) > 0.035:
                add_branch_marker(self.prev_pos, self.pos, strength=0.75 + 0.25 * self.branch_memory)
                # Seed a faint patch of slime behind the fast cell so the
                # slower colony mass can follow the newly opened branch.
                add_to_field(S, self.prev_pos, 0.030 + 0.040 * self.branch_memory, radius=0.55, cap=1.35)
                self.branch_clock = random.uniform(0.22, 0.50)
    ...
    def replicate(self):
        ...
        if child:
            ...
            # Edge lineages are more likely to produce additional explorers,
            # creating visible branching fronts before the central mass catches up.
            child.front_cell = self.front_cell or (self.front_active and random.random() < 0.55) or random.random() < FRONT_CELL_FRACTION
...
class AIController:
    ...
    def choose_mode(self, state):
        ...
        if state["crowded"] and self.mode not in ("SCRAPE", "ORGANIZE", "ORBIT"):
            return random.choice(["SCRAPE", "ORGANIZE", "ORBIT"])
...
def reset_simulation(seed_count=START_BACTERIA, from_ai=False):
    global bacteria, trails, particles, branch_markers, sim_time, frame_count
...
def update_labels():
    state = ai.read_state()
    active_fronts = sum(1 for b in bacteria if getattr(b, "front_active", False))
    scout_cells = sum(1 for b in bacteria if getattr(b, "front_cell", False))
    stats_label.text = (
        f"Round {ai.round}\n"
        ...
        f"Branch fronts: {active_fronts}/{scout_cells}\n"
