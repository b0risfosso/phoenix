...
class SoftCapsuleCell:
    def __init__(self, n_beads=88, n_particles=28):
        ...
        self.pore_timer = 0
        # VPython compatibility: use ring instead of torus.
        self.pore_ring = ring(pos=vector(0, 0, 0), axis=vector(1, 0, 0),
    ...
    def update_visuals(self):
        ...
        if self.center_trail is not None:
            c = self.center()
            # VPython curve objects do not expose a stable public _points list.
            # npoints is the supported way to check trail length.
            if getattr(self.center_trail, "npoints", 0) < 900:
...
class ExpressiveAIController:
    ...
    def __init__(self, cell):
        ...
        self.wrap_trails = []
        # VPython does not allow curve(pos=[]). Seed with one hidden point,
        # then reveal the curve only after the first real append.
        self.current_wrap = curve(pos=[self.probe.pos],
                                  radius=0.018,
    ...
    def append_wrap_point(self, pos):
        """Append to the AI wrap trail without constructing an empty VPython curve."""
        if self.current_wrap is None:
            return
        if not self.current_wrap.visible:
            try:
                self.current_wrap.modify(0, pos=pos)
            except Exception:
                pass
            self.current_wrap.visible = True
        self.current_wrap.append(pos=pos)
    ...
    def choose_mode(self, s, forced=False):
        ...
        if self.mode == "WRAP_ORBIT":
            self.current_wrap = curve(pos=[self.probe.pos], radius=0.018,
    ...
    def update_probe(self, dt, s):
        ...
        if self.mode in ["WRAP_ORBIT", "MIX", "DIP_PROBE", "CHAOS", "MARK_ART"]:
            ...
            if self.mode == "WRAP_ORBIT":
                self.append_wrap_point(self.probe.pos)
