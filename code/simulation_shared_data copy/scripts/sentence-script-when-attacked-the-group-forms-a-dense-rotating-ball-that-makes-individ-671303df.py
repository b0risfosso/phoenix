...
scene.title = "Two-Predator Fish School in a Reef - Active Bait Ball Defense"
...
# Dense rotating ball behavior: the school now actively forms a rotating bait ball
# whenever predators get close, not only when cornered. The ball blocks ordinary
# captures until both predators coordinate from different sides to disrupt it.
BAIT_BALL_TRIGGER_DISTANCE = 9.4
BAIT_BALL_RELEASE_DISTANCE = 12.2
BAIT_BALL_MIN_TIME = 5.2
BAIT_BALL_RADIUS = 2.65
BAIT_BALL_STEER = 12.8
BAIT_BALL_TANGENT = 5.2
BAIT_BALL_CLOSE_FISH_TRIGGER = 7.8
BAIT_BALL_DISRUPT_DISTANCE = 7.1
BAIT_BALL_CAPTURE_DISRUPTION = 0.68
...
# -----------------------------
# Cornered rotating bait-ball defense
# -----------------------------

class BaitBallSystem:
    def __init__(self):
        self.active = False
        self.timer = 0.0
        self.cooldown = 0.0
        self.center = vector(0, 1, 0)
        self.axis = vector(0, 1, 0)
        self.radius = BAIT_BALL_RADIUS
        self.rotation = 0.0
        self.threat_pos = vector(0, 0, 0)
        self.corner_pressure = 0.0
        self.formation_pressure = 0.0
        self.disruption = 0.0
        self.coordinated_predators = False
        self.member_count = 0
        self.shell = ring(pos=self.center, axis=self.axis, radius=self.radius, thickness=0.045, color=vector(1.0, 0.82, 0.18), opacity=0.0)
        self.inner = sphere(pos=self.center, radius=self.radius * 0.72, color=vector(1.0, 0.86, 0.28), opacity=0.0)
        self.core_label = label(pos=self.center + vector(0, self.radius + 0.75, 0), text="", height=10, color=vector(0.85, 0.55, 0.05), box=False, opacity=0)
    ...
    def reset(self):
        ...
        self.disruption = 0.0
        self.coordinated_predators = False
        self.shell.opacity = 0.0
        self.inner.opacity = 0.0
        self.core_label.text = ""
    ...
    def update(self, dt, prey_list, predator_list):
        ...
        self.center = lerp(self.center, center, 0.09 if self.active else 0.22)

        nearest_pred = min(predator_list, key=lambda pr: mag(pr.pos - self.center))
        self.threat_pos = nearest_pred.pos
        threat_dist = mag(nearest_pred.pos - self.center)
        closest_fish_threat = min(mag(f.pos - pr.pos) for f in active_prey for pr in predator_list)
        close_predators = sum(1 for pr in predator_list if mag(pr.pos - self.center) < BAIT_BALL_TRIGGER_DISTANCE + 1.4)

        # Corner pressure still helps, but the defense now forms even in open water
        # as soon as predators press close enough to threaten the school.
        wall_x = clamp((abs(self.center.x) - (HALF.x - 5.5)) / 5.5, 0, 1)
        wall_z = clamp((abs(self.center.z) - (HALF.z - 4.5)) / 4.5, 0, 1)
        floor_pressure = clamp(((FLOOR_Y + 2.3) - self.center.y) / 2.3, 0, 1)
        self.corner_pressure = clamp(wall_x + wall_z + floor_pressure * 0.35, 0, 1.65)
        packed = sum(1 for f in active_prey if mag(f.pos - self.center) < 6.4) / max(1, len(active_prey))
        threat_pressure = clamp((BAIT_BALL_TRIGGER_DISTANCE - threat_dist) / BAIT_BALL_TRIGGER_DISTANCE, 0, 1)
        fish_pressure = clamp((BAIT_BALL_CLOSE_FISH_TRIGGER - closest_fish_threat) / BAIT_BALL_CLOSE_FISH_TRIGGER, 0, 1)
        pack_pressure = clamp(close_predators / max(1, NUM_PREDATORS), 0, 1)
        self.formation_pressure = clamp(threat_pressure * 0.85 + fish_pressure * 0.95 + pack_pressure * 0.45 + self.corner_pressure * 0.18, 0, 2.2)

        self.coordinated_predators = False
        self.disruption = 0.0
        if len(predator_list) >= 2:
            p0, p1 = predator_list[0], predator_list[1]
            v0 = p0.pos - self.center
            v1 = p1.pos - self.center
            d0 = mag(v0)
            d1 = mag(v1)
            opposition = 0.0
            if d0 > EPS and d1 > EPS:
                # High opposition means predators are on different sides of the ball.
                opposition = clamp((-dot(safe_norm(v0), safe_norm(v1)) + 0.15) / 1.15, 0, 1)
            closeness = clamp((BAIT_BALL_DISRUPT_DISTANCE - max(d0, d1)) / BAIT_BALL_DISRUPT_DISTANCE, 0, 1)
            approach0 = clamp(dot(safe_norm(p0.vel, self.center - p0.pos), safe_norm(self.center - p0.pos, random_unit_vector())), 0, 1)
            approach1 = clamp(dot(safe_norm(p1.vel, self.center - p1.pos), safe_norm(self.center - p1.pos, random_unit_vector())), 0, 1)
            approach = (approach0 + approach1) * 0.5
            self.disruption = clamp(opposition * 0.55 + closeness * 0.70 + approach * 0.35, 0, 1)
            self.coordinated_predators = self.disruption >= BAIT_BALL_CAPTURE_DISRUPTION

        should_form = (self.formation_pressure > 0.42 and packed > 0.24) or (closest_fish_threat < BAIT_BALL_CLOSE_FISH_TRIGGER and packed > 0.18)
        should_release = (threat_dist > BAIT_BALL_RELEASE_DISTANCE and closest_fish_threat > BAIT_BALL_CLOSE_FISH_TRIGGER + 2.0 and self.timer > BAIT_BALL_MIN_TIME)

        if should_form and not self.active and self.cooldown <= 0:
            self.active = True
            self.timer = 0.0
            add_mark(self.center, 1.35, vector(1.0, 0.78, 0.18), 5.0, "ball up")
            spawn_particles(self.center, vector(1.0, 0.78, 0.18), count=30, speed=2.8)
        elif self.active and should_release:
            self.active = False
            self.cooldown = 1.15
            add_mark(self.center, 0.7, vector(0.38, 0.96, 0.78), 4.0, "release")
            spawn_particles(self.center, vector(0.38, 0.96, 0.78), count=14, speed=2.0)
        elif self.active and self.coordinated_predators and int(self.timer * 2.0) != int((self.timer - dt) * 2.0):
            add_mark(self.center, 0.75, vector(1.0, 0.22, 0.18), 1.8, "disrupt")

        if self.active:
            self.timer += dt
            away = safe_norm(self.center - nearest_pred.pos, vector(0, 0, 1))
            inward = vector(0, 0, 0)
            if self.center.x > HALF.x - 4.8:
                inward.x -= 1
            elif self.center.x < -HALF.x + 4.8:
                inward.x += 1
            if self.center.z > HALF.z - 4.2:
                inward.z -= 1
            elif self.center.z < -HALF.z + 4.2:
                inward.z += 1
            if mag(inward) > 0.01:
                away = safe_norm(away * 0.7 + safe_norm(inward) * 0.8)
            # Keep the rotation axis mostly vertical but tilted away from the nearest predator.
            self.axis = safe_norm(vector(away.x * 0.32, 1.0, away.z * 0.32), vector(0, 1, 0))
            self.rotation += dt * (3.0 + 2.0 * self.formation_pressure + 1.2 * self.disruption)
            density_t = clamp((len(active_prey) - 4) / max(1, NUM_PREY), 0.25, 1.0)
            # The ball tightens under ordinary pressure but swells/wobbles when both predators disrupt it.
            target_radius = BAIT_BALL_RADIUS * (0.74 + 0.22 * density_t + 0.22 * self.disruption)
            self.radius = lerp(self.radius, target_radius, 0.07)

        self.update_visual(dt)

    def steering_for(self, fish, nearest_threat, pred_dist):
        if not self.active or fish.caught:
            return vector(0, 0, 0)
        radial_seed = fish.idx * 2.399963 + self.rotation
        height_seed = math.sin(fish.idx * 1.731 + self.rotation * 0.65)
        layer = 0.42 + 0.58 * ((fish.idx % 7) / 6.0)
        side = safe_norm(cross(self.axis, vector(1, 0.2, 0.3)), vector(1, 0, 0))
        upish = safe_norm(cross(side, self.axis), vector(0, 0, 1))
        orbit_vec = side * math.cos(radial_seed) + upish * math.sin(radial_seed)
        target = self.center + orbit_vec * (self.radius * layer) + self.axis * (height_seed * self.radius * 0.38)
        to_target = target - fish.pos
        tangent = safe_norm(cross(self.axis, orbit_vec), random_unit_vector())
        inward = safe_norm(self.center - fish.pos, random_unit_vector())
        anti_pred = safe_norm(fish.pos - nearest_threat.pos, random_unit_vector())
        steer = vector(0, 0, 0)
        disruption_loss = 1.0 - 0.32 * self.disruption
        steer += safe_norm(to_target, inward) * BAIT_BALL_STEER * disruption_loss * clamp(mag(to_target) / 2.0, 0.45, 2.1)
        steer += tangent * BAIT_BALL_TANGENT * (1.0 + 0.55 * self.formation_pressure)
        steer += inward * (2.8 + 1.8 * self.formation_pressure) * disruption_loss
        steer += anti_pred * (3.6 + 3.0 * clamp((8.2 - pred_dist) / 8.2, 0, 1))
        if self.disruption > 0.45:
            # Coordinated predators make the surface of the ball wobble and loosen.
            steer += random_unit_vector() * (2.2 * self.disruption)
        self.member_count += 1
        return steer

    def shields_fish(self, fish):
        return self.active and (mag(fish.pos - self.center) < self.radius * 1.55)

    def capture_allowed(self, fish, threat):
        if not self.shields_fish(fish):
            return True
        if len(predators) < 2:
            return False
        other = predators[1] if threat is predators[0] else predators[0]
        near_threat = mag(threat.pos - self.center) < BAIT_BALL_DISRUPT_DISTANCE
        near_other = mag(other.pos - self.center) < BAIT_BALL_DISRUPT_DISTANCE
        fish_near_edge = mag(fish.pos - self.center) > self.radius * 0.62
        return self.coordinated_predators and near_threat and near_other and fish_near_edge

    def update_visual(self, dt):
        self.shell.pos = self.center
        self.shell.axis = self.axis
        self.shell.radius = self.radius * (1.0 + 0.08 * math.sin(self.rotation * 1.7))
        self.inner.pos = self.center
        self.inner.radius = self.radius * 0.72
        self.core_label.pos = self.center + vector(0, self.radius + 0.75, 0)
        if self.active:
            pulse = 0.5 + 0.5 * math.sin(self.rotation * 2.2)
            self.shell.opacity = 0.32 + 0.18 * pulse
            self.shell.color = color_lerp(vector(1.0, 0.78, 0.12), vector(1.0, 0.18, 0.12), max(pulse, self.disruption))
            self.inner.opacity = 0.11 + 0.08 * pulse + 0.08 * self.disruption
            self.core_label.text = "coordinated disruption" if self.coordinated_predators else "active bait ball"
        else:
            self.shell.opacity *= 0.88
            self.inner.opacity *= 0.88
            if self.shell.opacity < 0.015:
                self.shell.opacity = 0
...
class PreyFish:
    def __init__(self, idx):
        ...
        self.bait_phase = random.uniform(0, 2 * math.pi)
        self.bait_radius_bias = random.uniform(0.55, 1.1)
    ...
    def update(self, dt, prey_list, predators):
        ...
        if bait_ball.active:
            # In a cornered rotating ball, ordinary alignment/cohesion is overruled by
            # dense inward packing plus tangential rotation, making single targets harder to isolate.
            acceleration += bait_ball.steering_for(self, nearest_threat, pred_dist)
            if self.mark_cooldown <= 0.15 and random.random() < 0.015:
                add_mark(self.pos, 0.25, vector(1.0, 0.78, 0.18), 1.8, "spin")
        ...
        ball_boost = 1.32 if bait_ball.active else 1.0
        max_force = (7.2 + 3.2 * self.fear) * PREY_ESCAPE_DAMPING * ball_boost
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.34 * self.fear) * PREY_ESCAPE_DAMPING * ball_boost)
        self.pos += self.vel * dt
        ...
        for threat in predators:
            current_capture_radius = capture_radius
            if bait_ball.shields_fish(self):
                # A bait ball prevents ordinary simple kills. Captures inside the ball
                # require both predators to coordinate from different sides and disrupt it.
                if not bait_ball.capture_allowed(self, threat):
                    continue
                current_capture_radius *= 0.62
            if mag(self.pos - threat.pos) < current_capture_radius:
    ...
    def update_visual(self):
        ...
        if not self.caught:
            ...
            if bait_ball.shields_fish(self):
                spin_t = 0.5 + 0.5 * math.sin(sim_time * 8.0 + self.idx * 0.7)
                fear_col = color_lerp(fear_col, vector(1.0, 0.55, 0.16), 0.55 + 0.25 * spin_t)
            self.body.color = fear_col
            self.fin.color = color_lerp(fear_col, color.white, 0.30)
            trail_target = vector(1.0, 0.62, 0.18) if bait_ball.shields_fish(self) else vector(1.0, 0.86, 0.22)
            self.trail.color = color_lerp(color_lerp(self.base_col, vector(1, 1, 1), 0.35), trail_target, self.fear if not bait_ball.shields_fish(self) else 0.9)
...
def secondary_predator_accel(pr, idx, active_prey):
    ...
    if bait_ball.active:
        # Secondary predator coordinates from the opposite side of the primary predator,
        # creating a pincer that can disrupt the rotating defense.
        primary_offset = predators[0].pos - bait_ball.center
        far_side = bait_ball.center - safe_norm(primary_offset, random_unit_vector()) * (bait_ball.radius + 1.8)
        tangent = safe_norm(cross(vector(0, 1, 0), primary_offset), random_unit_vector())
        pincer_target = far_side + tangent * (0.8 * math.sin(sim_time * 2.0 + idx))
        acc += (pincer_target - pr.pos) * 0.95
...
class ReefAIController:
    ...
    def read_state(self):
        ...
        return {
            ...
            "bait_ball_active": bait_ball.active,
            "bait_ball_center": bait_ball.center,
            "bait_ball_members": bait_ball.member_count,
        }
    ...
    def choose_action(self, dt, state):
        ...
        if self.mode == "HUNT":
            if state.get("bait_ball_active", False):
                # Primary predator presses one edge while the second predator takes the far side.
                offset = predator.pos - state["bait_ball_center"]
                edge_target = state["bait_ball_center"] + safe_norm(offset, random_unit_vector()) * (bait_ball.radius + 1.05)
                edge_target += safe_norm(cross(vector(0, 1, 0), offset), random_unit_vector()) * 0.8
                acc = self.steering_to(edge_target, arrive_radius=0.65, speed_scale=1.12)
            elif nearest:
...
def update_hud():
    ...
    hud.text = (
        ...
        f"Boundary: {boundary_mode} | Bait ball: {'ACTIVE' if bait_ball.active else 'off'} "
        f"({bait_ball.member_count} fish, form {bait_ball.formation_pressure:.2f}, disrupt {bait_ball.disruption:.2f}) | "
    ...
    mode_label.text = (
        ...
        f"bait ball: {'active' if bait_ball.active else 'inactive'}\n"
        f"predator coordination: {'disrupting' if bait_ball.coordinated_predators else 'not coordinated'}"
    )
...
while True:
    ...
    bait_ball.update(DT, prey, predators)
