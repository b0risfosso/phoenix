...
ATTACH_RADIUS = 0.72              # how close an ion must be to stick to a channel mouth
ATTACH_HOLD_MIN = 0.28
ATTACH_HOLD_MAX = 1.05
ATTACH_PULL_TIME = 0.42
ATTACH_KNOCK_SPEED = 0.35
...
class IonChannel:
    def __init__(self, idx, direction):
        ...
        self.live_attached = 0
        self.attach_flash = 0.0
        ...
        self.outer_mouth = ring(
            pos=self.n * (R + 0.34),
            axis=self.n,
            radius=CHANNEL_RADIUS * 1.05,
            thickness=0.018,
            color=vector(1.0, 0.86, 0.22),
            opacity=0.0
        )

        self.inner_mouth = ring(
            pos=self.n * (R - 0.34),
            axis=self.n,
            radius=CHANNEL_RADIUS * 1.05,
            thickness=0.018,
    ...
    def attachment_flash(self):
        self.attach_flash = 1.0

    def update_visual(self, dt):
        self.pulse_phase += dt * 2.0
        self.flash = max(0.0, self.flash - dt * 2.2)
        self.attach_flash = max(0.0, self.attach_flash - dt * 2.6)
        breathe = 1.0 + 0.07 * math.sin(self.pulse_phase)
        attach_glow = clamp(self.live_attached / 3.0, 0.0, 1.0)
        ...
        mouth_opacity = 0.12 + 0.72 * attach_glow + 0.50 * self.attach_flash
        mouth_radius = CHANNEL_RADIUS * (1.05 + 0.22 * attach_glow + 0.12 * math.sin(self.pulse_phase * 3.0))
        self.outer_mouth.opacity = mouth_opacity
        self.inner_mouth.opacity = mouth_opacity * 0.88
        self.outer_mouth.radius = mouth_radius
        self.inner_mouth.radius = mouth_radius
        self.outer_mouth.color = vector(1.0, 0.86, 0.22) + vector(0.0, 0.10, 0.02) * self.attach_flash
        self.inner_mouth.color = self.outer_mouth.color
...
class Ion:
    def __init__(self, pos, charge=None):
        ...
        self.attach_side = 1
        self.attach_phase = 0.0
        self.attach_state = "free"
        self.attach_pull_progress = 0.0
        self.attach_tangent = random_unit()
        self.attach_origin = vector(0, 0, 0)
        self.marked = False
    ...
    def dispose(self):
        ...
        dispose_obj(self.attach_tether)

    def is_inside(self):
        return mag(self.pos) < R

    def attach_to_channel(self, ch, side, hold_time=None):
        self.attached_channel = ch
        self.attach_side = side
        self.attach_timer = hold_time if hold_time is not None else pyrandom.uniform(ATTACH_HOLD_MIN, ATTACH_HOLD_MAX)
        self.attach_phase = pyrandom.uniform(0, 2 * math.pi)
        self.attach_state = "stuck"
        self.attach_pull_progress = 0.0
        self.attach_tangent = safe_norm(tangent_swirl(ch.n + random_unit() * 0.18, 1.0))
        if mag(self.attach_tangent) < 0.01:
            self.attach_tangent = safe_norm(cross(vector(0, 1, 0), ch.n))
        self.attach_origin = ch.n * (R + side * 0.30)
        self.vel = vector(0, 0, 0)
        ch.attached_count += 1
        ch.attachment_flash()

    def detach_from_channel(self, velocity, mark_flash=False):
        if self.attached_channel is not None:
            self.attached_channel.attachment_flash()
        self.attached_channel = None
        self.attach_timer = 0.0
        self.attach_state = "free"
        self.attach_pull_progress = 0.0
        self.vel = velocity
        self.attach_tether.opacity = 0.0
        if mark_flash:
            self.recent_transfer_timer = 0.45

    def update_attachment_tether(self, ch):
        mouth = ch.n * (R + self.attach_side * 0.34)
        self.attach_tether.modify(0, pos=mouth)
        self.attach_tether.modify(1, pos=self.pos)
        self.attach_tether.opacity = 0.60
    ...
    def update(self, dt):
        ...
        if self.attached_channel is not None:
            ...
            self.attach_phase += dt * 11.0

            mouth = ch.n * (R + self.attach_side * 0.31)
            wobble = self.attach_tangent * (0.055 * math.sin(self.attach_phase))

            if ch.open:
                self.attach_state = "pulling"
                self.attach_pull_progress = clamp(self.attach_pull_progress + dt / ATTACH_PULL_TIME, 0.0, 1.0)
                start = ch.n * (R + self.attach_side * 0.31)
                end = ch.n * (R - self.attach_side * 0.43)
                self.pos = start * (1 - self.attach_pull_progress) + end * self.attach_pull_progress + wobble * 0.45
                self.obj.color = vector(1.0, 0.88, 0.20) if self.attach_pull_progress < 0.75 else (ION_PLUS_COLOR if self.charge > 0 else ION_MINUS_COLOR)

                if self.attach_pull_progress >= 1.0:
                    ch.transfer_flash()
                    total_crossings += 1
                    exit_dir = -ch.n * self.attach_side
                    tang = tangent_swirl(ch.n, pyrandom.uniform(0.10, 0.36))
                    self.detach_from_channel(exit_dir * pyrandom.uniform(0.75, 1.20) + tang, mark_flash=True)
                    if pyrandom.random() < 0.18:
                        self.set_marked(True)
            else:
                self.attach_state = "stuck"
                self.pos = mouth + wobble
                self.obj.color = vector(1.0, 0.76, 0.15)
                if self.attach_timer <= 0:
                    side_push = ch.n * self.attach_side
                    tang = tangent_swirl(ch.n, pyrandom.uniform(-0.50, 0.50))
                    self.detach_from_channel(side_push * pyrandom.uniform(0.35, 0.78) + tang, mark_flash=True)
                    self.set_marked(False)

            self.vel = vector(0, 0, 0)
            self.obj.pos = self.pos
            self.update_attachment_tether(ch)
            return
        else:
            self.attach_tether.opacity = 0.0
        ...
        if near_membrane:
            ...
            if nearest is not None:
                ...
                # Ions can visibly stick to channel openings before the gate decides their fate.
                # Open channels tend to pull them through; closed channels hold them like a clogged pore.
                if surf_dist < ATTACH_RADIUS and abs(old_r - R) < 0.62 and self.recent_transfer_timer <= 0:
                    side = -1 if old_r < R else 1
                    approach_speed = abs(dot(self.vel, ch.n))
                    chance = 0.030 + 0.030 * clamp(approach_speed, 0, 1.5)
                    if ch.open:
                        chance += 0.040
                    if pyrandom.random() < chance:
                        hold = pyrandom.uniform(0.08, 0.28) if ch.open else pyrandom.uniform(ATTACH_HOLD_MIN, ATTACH_HOLD_MAX)
                        self.attach_to_channel(ch, side, hold)
                        self.pos = ch.n * (R + side * 0.31)
                        self.obj.pos = self.pos
                        self.update_attachment_tether(ch)
                        return
            ...
            if selected is not None:
                ...
                if ch.open:
                    ...
                    if dist_to_channel < CHANNEL_APERTURE * 0.82 and pyrandom.random() < 0.72:
                        self.attach_to_channel(ch, side, pyrandom.uniform(0.34, 1.10))
                        new_pos = ch.n * (R + side * 0.31)
                    else:
                        n = direction
...
# Particle collisions
# -----------------------------
def knock_attached_ion(attached, hitter, normal):
    ch = attached.attached_channel
    if ch is None:
        return
    incoming_speed = mag(hitter.vel)
    if incoming_speed < ATTACH_KNOCK_SPEED and pyrandom.random() > 0.18:
        return
    away_from_channel = safe_norm(attached.pos - ch.surface_point())
    if mag(away_from_channel) < 0.01:
        away_from_channel = ch.n * attached.attach_side
    scatter = away_from_channel * pyrandom.uniform(0.70, 1.25) + normal * pyrandom.uniform(0.25, 0.65) + random_unit() * 0.20
    attached.detach_from_channel(scatter, mark_flash=True)
    attached.set_marked(True)
    hitter.vel -= normal * 0.18
    ch.attachment_flash()
...
def handle_ion_collisions():
    ...
    min_d2 = min_d * min_d
    attach_hit_d = ION_RADIUS * 3.4
    attach_hit_d2 = attach_hit_d * attach_hit_d
    ...
    for ion in ions:
        if ion.attached_channel is not None:
            ion.attached_channel.live_attached += 1
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            # Nearby free particles can knock ions away from channel mouths.
            if a.attached_channel is not None or b.attached_channel is not None:
                if d2 < attach_hit_d2:
                    if a.attached_channel is not None and b.attached_channel is None:
                        knock_attached_ion(a, b, -normal)
                    elif b.attached_channel is not None and a.attached_channel is None:
                        knock_attached_ion(b, a, normal)
                continue
...
def update_meter_and_labels():
    ...
    open_count = sum(1 for ch in channels if ch.open)
    attached_now = sum(1 for ion in ions if ion.attached_channel is not None)
    status_label.text = (
        f"Round {round_number}   Inside: {inside}   Outside: {outside}   "
        f"Open channels: {open_count}/{len(channels)}   Attached: {attached_now}   Transfers: {total_crossings}"
    )
