...
scene.caption = """
Controls:
  T: toggle AI    L: sleep mode    SPACE/P: pause/resume    R: reset    M: cycle AI mode    B: wrap/bounce boundary
...
# Sleep mode is intended for long unattended runs.
# It lowers CPU/GPU demand by rendering fewer frames, batching simulation
# updates, slowing time, reducing effects, and refreshing labels/tiles less often.
SLEEP_RATE = 6
NORMAL_RATE = 45
SLEEP_UPDATE_INTERVAL = 0.75
SLEEP_TIME_SCALE = 0.28
SLEEP_TRAIL_LIMIT = 90
SLEEP_PARTICLE_LIMIT = 45
...
def update_tile_colors(force=False):
    if not force:
        refresh_every = 36 if sleep_mode else 4
...
def add_particle(pos, color_value, radius=0.045, life=1.0, vel=None, opacity=0.45):
    # In sleep mode, most decorative particles are skipped and the total
    # particle budget is much smaller. This prevents slow memory growth and
    # keeps long runs lightweight.
    limit = SLEEP_PARTICLE_LIMIT if sleep_mode else PARTICLE_LIMIT
    if sleep_mode and random.random() < 0.82:
        return
    while len(particles) >= limit:
        old = particles.pop(0)
        hide_object(old["obj"])
    if vel is None:
...
def add_slime_trail(start, end, strength=0.16):
    if mag(end - start) < 0.035:
        return
    # Sleep mode keeps only sparse trails. The slime field still records the
    # colony history, but visual curve objects are limited for low power use.
    limit = SLEEP_TRAIL_LIMIT if sleep_mode else TRAIL_LIMIT
    if sleep_mode and random.random() < 0.72:
        return
    while len(trails) >= limit:
        old = trails.pop(0)
        hide_object(old["obj"])
    c = curve(
...
def update_labels():
    ...
    stats_label.text = (
        f"Round {ai.round}\n"
        f"Sleep mode: {'ON' if sleep_mode else 'OFF'}\n"
    ...
    mode_label.text = (
        ...
        f"{'SLEEP MODE' if sleep_mode else ''} {'PAUSED' if paused else ''}"
    )
...
def on_keydown(evt):
    global paused, SIM_SPEED, boundary_mode, sleep_mode, sleep_accumulator
    k = evt.key.lower()
    ...
    if k in ("w", "up"):
        ...
        sleep_accumulator = 0.0
        # Trim existing visual effects immediately when entering sleep mode.
        if sleep_mode:
            while len(particles) > SLEEP_PARTICLE_LIMIT:
                old = particles.pop(0)
                hide_object(old["obj"])
            while len(trails) > SLEEP_TRAIL_LIMIT:
                old = trails.pop(0)
                hide_object(old["obj"])
        update_labels()
    elif k == "r":
...
def run_simulation_step(dt):
    ...
    # Pairwise interactions are the most expensive part of the simulation.
    # In sleep mode they are still present, but less frequent.
    if (not sleep_mode) or frame_count % 3 == 0:
        process_bacterial_interactions(dt)
    remove_dead_bacteria()

    if frame_count % (9 if sleep_mode else 3) == 0:
        diffuse_fields(dt * (9.0 if sleep_mode else 3.0))

    update_particles(dt)
    update_trails(dt)
    update_tile_colors(force=False)

    label_every = 24 if sleep_mode else 10
    if frame_count % label_every == 0:
        update_labels()


while True:
    rate(SLEEP_RATE if sleep_mode else NORMAL_RATE)
    now = time.time()
    real_dt = min(0.25 if sleep_mode else 0.055, max(0.001, now - last_clock))
    last_clock = now
    ...
    if sleep_mode:
        sleep_accumulator += real_dt
        # Keep the cursor gently alive, but avoid full colony updates until the
        # batch interval is reached. This is the main CPU-saving behavior.
        ai.move_cursor(real_dt * 0.35)
        if sleep_accumulator < SLEEP_UPDATE_INTERVAL:
            continue
        dt = min(1.5, sleep_accumulator) * SIM_SPEED * SLEEP_TIME_SCALE
        sleep_accumulator = 0.0
    else:
        dt = real_dt * SIM_SPEED

    run_simulation_step(dt)
