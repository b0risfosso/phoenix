...
# Scene styling
# -----------------------------
scene.title = "Cell Membrane with Ion Collisions, Sparks, Membrane Ripples, and AI Controller"
scene.width = 1200
scene.height = 760
...
SPARK_COLOR = vector(1.0, 0.86, 0.28)
RIPPLE_COLOR = vector(0.35, 0.82, 1.0)
MAX_SPARKS = 180
MAX_RIPPLES = 36
...
# -----------------------------
# Collision spark and membrane ripple effects
# -----------------------------
class CollisionSpark:
    def __init__(self, pos, vel, col, life=0.34):
        self.pos = vector(pos)
        self.vel = vector(vel)
        self.life = life
        self.max_life = life
        self.obj = sphere(
            pos=self.pos,
            radius=pyrandom.uniform(0.035, 0.075),
            color=col,
            opacity=0.95,
            emissive=True,
            make_trail=True,
    ...
    def update(self, dt):
        ...
        self.obj.opacity = 0.90 * fade
        self.obj.radius = max(0.005, self.obj.radius * (0.985 + 0.015 * fade))
        return self.life > 0

    def dispose(self):
        dispose_obj(self.obj)

class MembraneRipple:
    def __init__(self, pos, strength=1.0):
        axis = safe_norm(pos)
        self.pos = axis * (R + 0.045)
        self.axis = axis
        self.life = 0.72
        self.max_life = self.life
        self.strength = clamp(strength, 0.45, 1.8)
        self.ring = ring(
            pos=self.pos,
            axis=self.axis,
            radius=0.16,
            thickness=0.018 * self.strength,
            color=RIPPLE_COLOR,
            opacity=0.62
        )
    ...
    def update(self, dt):
        self.life -= dt
        age = 1.0 - clamp(self.life / self.max_life, 0, 1)
        self.ring.radius = 0.16 + age * (1.35 + 0.55 * self.strength)
        self.ring.thickness = max(0.004, 0.02 * (1 - age) * self.strength)
        self.ring.opacity = 0.62 * (1 - age)
        self.ring.pos = self.pos * (1.0 + 0.006 * math.sin(age * math.pi))
        return self.life > 0

    def dispose(self):
        dispose_obj(self.ring)

def create_collision_effect(pos, normal, rel_speed=1.0):
    # Sparks appear at the ion collision point; ripples are projected outward onto the transparent membrane.
    strength = clamp(rel_speed / 1.6, 0.35, 1.8)
    n = safe_norm(pos if mag(pos) > 0.05 else normal)
    spark_count = int(3 + 4 * strength)

    for _ in range(spark_count):
        if len(collision_sparks) >= MAX_SPARKS:
            old = collision_sparks.pop(0)
            old.dispose()
        tang = tangent_swirl(n + random_unit() * 0.18, pyrandom.uniform(-1.4, 1.4))
        burst_dir = safe_norm(normal * pyrandom.uniform(-0.7, 0.7) + tang + random_unit() * 0.35)
        speed = pyrandom.uniform(0.55, 1.65) * strength
        col = SPARK_COLOR + vector(0.0, pyrandom.uniform(-0.10, 0.10), pyrandom.uniform(-0.16, 0.08))
        collision_sparks.append(CollisionSpark(pos + burst_dir * 0.06, burst_dir * speed, col))

    if abs(mag(pos) - R) < 2.8 or pyrandom.random() < 0.45:
        if len(membrane_ripples) >= MAX_RIPPLES:
            old = membrane_ripples.pop(0)
            old.dispose()
        membrane_ripples.append(MembraneRipple(n * R, strength))

def update_collision_effects(dt):
    for effect_list in (collision_sparks, membrane_ripples):
        i = len(effect_list) - 1
        while i >= 0:
            if not effect_list[i].update(dt):
...
def reset_simulation(round_increment=False):
    global ions, round_number, total_crossings, total_ion_collisions, global_pressure_bias, global_swirl_strength, global_electric_field
    ...
    for ripple in membrane_ripples:
        ripple.dispose()
    membrane_ripples.clear()
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if 1e-8 < d2 < min_d2:
                ...
                rel_speed = mag(a.vel - b.vel)

                # Direction change: exchange normal velocity and add a small tangential kick.
                a.vel += (vb_n - va_n) * normal * 0.92
                b.vel += (va_n - vb_n) * normal * 0.92
                tangent_kick = tangent_swirl((a.pos + b.pos) * 0.5 + random_unit() * 0.05, pyrandom.uniform(-0.42, 0.42))
                a.vel += tangent_kick + random_unit() * pyrandom.uniform(0.015, 0.055)
                b.vel -= tangent_kick + random_unit() * pyrandom.uniform(0.015, 0.055)

                midpoint = (a.pos + b.pos) * 0.5
                create_collision_effect(midpoint, normal, rel_speed)
                total_ion_collisions += 1
...
def update_meter_and_labels():
    ...
    status_label.text = (
        ...
        f"Collisions: {total_ion_collisions}   Sparks: {len(collision_sparks)}   Ripples: {len(membrane_ripples)}"
    )
