...
# Scene styling
# -----------------------------
scene.title = "Cell Membrane with Moving Ion Channels, Drifting Ions, Flex Waves, Meter, and AI Controller"
scene.width = 1200
scene.height = 760
...
# Membrane flex/wave tuning: crossing bursts add energy that briefly expands
# the transparent membrane and launches visible ripple rings across its surface.
MEMBRANE_FLEX_MAX = 0.22
MEMBRANE_FLEX_DECAY = 2.8
MEMBRANE_WAVE_LIFE = 1.35
MEMBRANE_WAVE_MAX_RADIUS = 1.65
MEMBRANE_WAVE_THICKNESS = 0.032
...
membrane_flex_energy = 0.0
transport_burst_count = 0
transport_burst_timer = 0.0
surface_waves = []
...
# -----------------------------
# Membrane flex and transport wave visuals
# -----------------------------
class SurfaceWave:
    def __init__(self, direction, strength=1.0):
        self.n = safe_norm(direction)
        self.age = 0.0
        self.life = MEMBRANE_WAVE_LIFE * clamp(strength, 0.75, 1.75)
        self.base_strength = clamp(strength, 0.6, 2.3)
        self.obj = ring(
            pos=self.n * (R + 0.075),
            axis=self.n,
            radius=0.16,
            thickness=MEMBRANE_WAVE_THICKNESS * self.base_strength,
            color=vector(0.20, 0.72, 1.0),
            opacity=0.52
        )

    def update(self, dt, flex_offset):
        self.age += dt
        u = clamp(self.age / max(0.001, self.life), 0, 1)
        # Expanding ripple ring on the tangent plane of the channel mouth.
        self.obj.pos = self.n * (R + 0.075 + flex_offset * 0.65)
        self.obj.axis = self.n
        self.obj.radius = 0.16 + MEMBRANE_WAVE_MAX_RADIUS * (u ** 0.72) * self.base_strength
        self.obj.thickness = MEMBRANE_WAVE_THICKNESS * (1.15 - 0.55 * u) * self.base_strength
        self.obj.opacity = max(0.0, 0.56 * (1 - u))
        # Slight color shift makes fresh waves brighter and old waves softer.
        self.obj.color = vector(0.20 + 0.20 * (1 - u), 0.68 + 0.18 * (1 - u), 1.0)
...
def register_transport_wave(direction, burst_bonus=1.0):
    """Called whenever an ion crosses through an open channel."""
    global membrane_flex_energy, transport_burst_count, transport_burst_timer

    transport_burst_count += 1
    transport_burst_timer = 0.22

    # Single crossings produce small ripples; rapid crossings compound into a visible flex.
    burst_strength = 1.0 + min(transport_burst_count, 8) * 0.16
    membrane_flex_energy = clamp(
        membrane_flex_energy + 0.030 * burst_strength * burst_bonus,
        0.0,
        MEMBRANE_FLEX_MAX
    )

    # Avoid unlimited wave objects, but show enough simultaneous waves to reveal bursts.
    if len(surface_waves) > 34:
        old = surface_waves.pop(0)
        old.dispose()
    surface_waves.append(SurfaceWave(direction, burst_strength))


def update_membrane_flex(dt):
    global membrane_flex_energy, transport_burst_count, transport_burst_timer, surface_waves

    if transport_burst_timer > 0:
        transport_burst_timer -= dt
    else:
        transport_burst_count = 0

    membrane_flex_energy = max(0.0, membrane_flex_energy - MEMBRANE_FLEX_DECAY * dt * membrane_flex_energy)
    pulse = membrane_flex_energy * (0.65 + 0.35 * math.sin(frame_count * 0.42))

    membrane.radius = R + pulse
    membrane.opacity = 0.18 + min(0.12, pulse * 0.62)
    inner_glow.radius = R * 0.985 + pulse * 0.72
    inner_glow.opacity = 0.055 + min(0.06, pulse * 0.35)

    alive = []
    for wave in surface_waves:
        if wave.update(dt, pulse):
            alive.append(wave)
        else:
            wave.dispose()
    surface_waves = alive
...
def reset_simulation(round_increment=False):
    ...
    transport_burst_timer = 0.0
    membrane.radius = R
    membrane.opacity = 0.18
    inner_glow.radius = R * 0.985
    inner_glow.opacity = 0.055
...
def update_meter_and_labels():
    ...
    status_label.text = (
        ...
        f"Surface waves: {len(surface_waves)}   Flex: {membrane_flex_energy:0.2f}"
    )
