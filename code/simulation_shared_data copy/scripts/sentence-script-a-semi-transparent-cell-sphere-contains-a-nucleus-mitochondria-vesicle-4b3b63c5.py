...
# stagnation/completion detection, reset, looping, and human override controls.

scene.title = "3D Cell with Organelles, Cytoplasmic Flow, Vesicles, Trails, Labels, and AI Controller"
scene.width = 1200
scene.height = 760
...
scene.caption = """
Controls:
SPACE pause/resume | A toggle AI | M next AI mode | 1-8 choose AI mode | R reset round
...
def flow_field(p, t):
    # Soft cytoplasmic circulation: spiral, curl, vertical undulation, and local eddies.
...
cell_membrane = sphere(
    pos=vector(0, 0, 0),
    radius=CELL_RADIUS * 1.006,
    color=vector(0.35, 0.75, 0.95),
    opacity=0.055,
    shininess=0.8
)

# Stationary organelles
organelles = []

nucleus_obj = sphere(
    pos=vector(-1.15, 0.25, 0.15),
    radius=1.55,
    color=vector(0.58, 0.48, 0.92),
    opacity=0.58,
    shininess=0.9
)

nucleolus = sphere(
    pos=nucleus_obj.pos + vector(-0.25, 0.15, 0.18),
    radius=0.48,
    color=vector(0.42, 0.28, 0.72),
    opacity=0.82
)

nucleus_wrap_hint = sphere(
    pos=nucleus_obj.pos,
    radius=1.75,
    color=vector(0.82, 0.76, 1.0),
    opacity=0.06
)
...
organelles.append({
    ...
    "radius": 1.68,
    "marks": 0,
    "label": label(pos=nucleus_obj.pos + vector(0, 1.95, 0), text="Nucleus", color=vector(0.16, 0.10, 0.32),
                   box=False, opacity=0, height=13)
})

mito_positions = [
    vector(2.6, 1.4, -0.95),
    vector(2.25, -1.55, 1.15),
    vector(-3.15, -1.05, -1.3),
    vector(0.45, 2.8, 1.85),
    vector(-2.55, 1.95, 1.2),
]
...
for i, p in enumerate(mito_positions):
    m = sphere(
        pos=p,
        size=vector(1.55, 0.58, 0.58),
        color=vector(1.0, 0.58, 0.25),
        opacity=0.88,
        shininess=0.7
    )
    try:
    ...
    organelles.append({
        "name": "Mitochondrion " + str(i + 1),
        "kind": "mitochondrion",
        "obj": m,
        "radius": 0.88,
        "marks": 0,
        "spin": random.choice([-1, 1]) * random.uniform(0.25, 0.65),
        "cristae": c,
        "label": label(pos=p + vector(0, 0.82, 0), text="Mitochondrion", color=vector(0.38, 0.18, 0.02),
                       box=False, opacity=0, height=10)
    })

# Ribosomes: tiny mostly-stationary particles with slight Brownian bobbing.
ribosomes = []
ribosome_label = label(pos=vector(2.4, -3.65, 0), text="Ribosomes", color=vector(0.32, 0.10, 0.37),
                       box=False, opacity=0, height=12)
...
for i in range(RIBOSOME_COUNT):
    ...
    r = sphere(
        ...
        color=vector(0.78, 0.36, 0.88),
        opacity=random.uniform(0.48, 0.86),
        shininess=0.25
    )
    ribosomes.append({
        "obj": r,
        ...
        "amp": random.uniform(0.012, 0.04)
    })

# Cytoplasmic flow particles and arrows
flow_particles = []
for i in range(FLOW_PARTICLE_COUNT):
    p = random_free_position(0.25)
    fp = sphere(
        pos=p,
        radius=random.uniform(0.035, 0.06),
        color=vector(0.24, 0.68, 1.0),
        opacity=random.uniform(0.18, 0.36),
        shininess=0.2
    )
...
for i in range(14):
    ...
    arr = arrow(
        pos=p,
        axis=flow_field(p, 0) * 2.0,
        shaftwidth=0.025,
        headwidth=0.09,
        headlength=0.13,
        color=vector(0.42, 0.78, 1.0),
        opacity=0.26
    )
...
class Vesicle:
    def __init__(self, idx):
        ...
        self.base_color = random.choice([
            vector(0.98, 0.78, 0.28),
            vector(0.98, 0.54, 0.72),
            vector(0.36, 0.86, 0.62),
            vector(0.56, 0.78, 1.0)
        ])
        self.obj = sphere(
            pos=random_free_position(self.r + 0.25),
            radius=self.r,
        ...
        self.label = label(pos=self.obj.pos + vector(0, self.r + 0.24, 0), text="V" + str(idx + 1),
                           height=8, color=vector(0.16, 0.20, 0.26), box=False, opacity=0)
...
selector = ring(
    pos=vesicles[selected_index].obj.pos,
    axis=vector(0, 1, 0),
    radius=0.48,
    thickness=0.018,
    color=vector(0.18, 0.38, 1.0),
    opacity=0.6
)

hud = label(
    pos=vector(0, CELL_RADIUS + 0.55, 0),
    text="",
    height=12,
    color=vector(0.08, 0.14, 0.20),
    box=False,
    opacity=0
)
...
def create_tiny_collision_flash(p, n):
    ...
    spill_particles.append({
        "obj": flash,
        "vel": n * random.uniform(0.15, 0.4) + random_unit() * 0.08,
        "age": 0.0,
...
def create_mark(target, source_pos=None, mark_color=None):
    ...
    mk = sphere(
        pos=p,
        radius=0.085 if target["kind"] == "nucleus" else 0.065,
        color=col,
        opacity=0.88,
        shininess=0.4
    )
...
def spill_from_vesicle(v, count=7, color_override=None):
    ...
    for _ in range(count):
        ...
        particle = sphere(
            ...
            color=col + random_unit() * 0.03,
            opacity=random.uniform(0.42, 0.78),
            shininess=0.2
        )
        spill_particles.append({
            "obj": particle,
            "vel": v.vel * 0.35 + d * random.uniform(0.25, 0.85),
            "age": 0.0,
...
def wrap_target(target, color_override=None, life=6.0):
    ...
    shell = sphere(
        pos=target["obj"].pos,
        radius=target["radius"] + random.uniform(0.16, 0.32),
        color=col,
        opacity=0.15,
        shininess=0.9
    )
...
class CellAIController:
    ...
    def pick_next_mode(self, state=None):
        ...
        if state["completion"] or state["empty"] or self.stable_time > 7.0 or sim_time > 125.0:
    ...
    def choose_actions(self, dt, t, state):
        ...
        if self.mode == "PLAYFUL_FLOW":
            ...
            for i, v in enumerate(vesicles):
                dip = vector(0, -0.25 * math.sin(t * 1.4 + i), 0)
                v.ai_force = flow_field(v.obj.pos, t) * 1.2 + dip
            if t - self.last_action_time > random.uniform(0.7, 1.4):
                ...
                for v in random.sample(vesicles, k=min(3, len(vesicles))):
                    spill_from_vesicle(v, count=random.randint(5, 11), color_override=random.choice([
                        vector(1.0, 0.72, 0.28),
                        vector(0.54, 0.95, 1.0),
                        vector(0.94, 0.62, 1.0)
                    ]))
            ...
            if t - self.last_action_time > 3.4:
                self.last_action_time = t
                target = random.choice(organelles)
                wrap_target(target, vector(0.72, 0.92, 1.0), life=random.uniform(4.5, 7.5))
            ...
            for i, v in enumerate(vesicles):
                random_kick = vector(
                    math.sin(t * 4.1 + i * 2.3),
                    math.cos(t * 3.7 + i * 1.9),
                    math.sin(t * 4.5 - i * 1.3)
                )
...
def update_particles(dt, t):
    ...
    for fa in flow_arrows:
        drift = vector(
            0.16 * math.sin(t * 0.18 + fa["phase"]),
            0.10 * math.sin(t * 0.22 + fa["phase"] * 0.7),
            0.16 * math.cos(t * 0.18 + fa["phase"])
        )
...
def update_ribosomes_and_organelles(dt, t):
    for r in ribosomes:
        r["phase"] += dt * 0.8
        bob = vector(
            r["amp"] * math.sin(r["phase"] * 1.7),
            r["amp"] * math.cos(r["phase"] * 1.2),
            r["amp"] * math.sin(r["phase"] * 1.1 + 1.3)
        )
    ...
    nucleolus.pos = nucleus_obj.pos + vector(
        -0.25 + 0.03 * math.sin(t * 0.5),
        0.15 + 0.025 * math.cos(t * 0.6),
        0.18
    )
...
def update_labels():
    for o in organelles:
        ...
        if o["marks"] > 0:
            o["label"].text = o["name"] + "  marks:" + str(o["marks"])
        else:
            o["label"].text = o["name"]
...
def keydown(evt):
    ...
    # Human nudges can run while AI remains on; X suppresses AI force on selected briefly.
    v = vesicles[selected_index]
