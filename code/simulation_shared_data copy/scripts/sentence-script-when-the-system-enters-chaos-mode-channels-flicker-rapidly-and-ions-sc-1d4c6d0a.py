...
Controls:
  ...
  H = human override timer    F = clear trails    X = force chaos mode
"""
...
CHAOS_COLOR = vector(1.0, 0.10, 0.08)
CHAOS_ALT_COLOR = vector(1.0, 0.86, 0.14)
CHAOS_BURST_COLOR = vector(1.0, 0.22, 0.05)
...
class IonChannel:
    def __init__(self, idx, direction):
        ...
        self.chaos_flash = 0.0
        self.chaos_jitter = 0.0
    ...
    def chaos_flicker(self):
        """Rapid visual pulse used when the AI enters CHAOS mode."""
        self.open = pyrandom.random() < 0.62
        self.flash = max(self.flash, pyrandom.uniform(0.4, 1.0))
        self.chaos_flash = 1.0
        self.chaos_jitter = pyrandom.uniform(0.0, 1.0)

    def update_visual(self, dt):
        self.pulse_phase += dt * 2.0
        self.flash = max(0.0, self.flash - dt * 2.2)
        self.chaos_flash = max(0.0, self.chaos_flash - dt * 8.0)
        breathe = 1.0 + 0.07 * math.sin(self.pulse_phase)
        chaos_pulse = self.chaos_flash * (0.55 + 0.45 * math.sin(self.pulse_phase * 8.0 + self.idx))
        ...
        if self.chaos_flash > 0:
            flicker_col = CHAOS_COLOR if pyrandom.random() < 0.5 else CHAOS_ALT_COLOR
            self.body.color = flicker_col
            self.body.opacity = clamp(self.body.opacity + 0.25 * chaos_pulse, 0.2, 0.95)
            self.body.radius = CHANNEL_RADIUS * (0.95 + 0.42 * chaos_pulse + 0.08 * self.chaos_jitter)
            self.pore_core.opacity = clamp(0.15 + 0.65 * chaos_pulse, 0.0, 0.9)
            self.halo.color = flicker_col
            self.halo.opacity = clamp(0.30 + 0.55 * chaos_pulse, 0.0, 0.95)
            self.halo.radius = CHANNEL_RADIUS * (1.25 + 0.85 * chaos_pulse)
...
class MembraneAI:
    ...
    def step(self, dt):
        ...
        if self.mode == "BALANCE":
            ...
            # Chaos is intentionally unstable: channels flicker rapidly and
            # ions receive irregular membrane-centered burst impulses.
            if self.action_timer <= 0:
                self.action_timer = pyrandom.uniform(0.035, 0.095)
                for ch in channels:
                    if pyrandom.random() < 0.86:
                        ch.chaos_flicker()
                self.chaos_vector = random_unit() * pyrandom.uniform(0.32, 1.05)

                if pyrandom.random() < 0.72:
                    chaos_scatter_ions(strength=pyrandom.uniform(0.75, 1.45))

                if len(ions) < MAX_IONS and pyrandom.random() < 0.42:
                    spill_ions(pyrandom.randint(1, 4))

            # Fast-changing global forces keep the membrane environment turbulent.
            global_pressure_bias = pyrandom.uniform(-1.85, 1.85)
            global_swirl_strength = pyrandom.uniform(-1.45, 1.65)
            global_electric_field = self.chaos_vector + random_unit() * pyrandom.uniform(0.0, 0.35)
            for ion in ions:
                if abs(mag(ion.pos) - R) < 1.9 and pyrandom.random() < 0.018:
                    radial = safe_norm(ion.pos if mag(ion.pos) > 0.01 else random_unit())
                    ion.vel += radial * pyrandom.uniform(-1.6, 1.6) + random_unit() * pyrandom.uniform(0.4, 1.3)
...
# Chaos burst helpers
# -----------------------------
def add_chaos_burst(pos, radius=0.16):
    """Add a short-lived glowing burst marker near the membrane."""
    burst = sphere(
        pos=pos,
        radius=radius,
        color=CHAOS_BURST_COLOR,
        opacity=0.42,
        emissive=True
    )
    chaos_bursts.append({"obj": burst, "life": 0.45, "max_life": 0.45, "base_radius": radius})

def update_chaos_bursts(dt):
    for burst in list(chaos_bursts):
        burst["life"] -= dt
        obj = burst["obj"]
        if burst["life"] <= 0:
            obj.visible = False
            chaos_bursts.remove(burst)
        else:
            fade = burst["life"] / burst["max_life"]
            obj.opacity = 0.42 * fade
            obj.radius = burst["base_radius"] * (1.0 + 3.2 * (1.0 - fade))

def chaos_scatter_ions(strength=1.0):
    """Scatter ions in unpredictable bursts around the membrane surface."""
    global chaos_burst_count
    if not ions:
        return

    burst_dir = random_unit()
    burst_center = burst_dir * pyrandom.uniform(R - 0.25, R + 0.55)
    add_chaos_burst(burst_center, pyrandom.uniform(0.12, 0.22))
    chaos_burst_count += 1

    for ion in ions:
        distance = mag(ion.pos - burst_center)
        near_membrane = abs(mag(ion.pos) - R) < 2.3
        random_pick = pyrandom.random() < 0.10 * strength
        close_pick = distance < pyrandom.uniform(1.4, 2.8)
        if near_membrane and (close_pick or random_pick):
            radial = safe_norm(ion.pos if mag(ion.pos) > 0.01 else burst_dir)
            tangential = tangent_swirl(radial + random_unit() * 0.3, pyrandom.uniform(-1.0, 1.0))
            outward_or_inward = radial * pyrandom.uniform(-1.9, 1.9)
            jitter = random_unit() * pyrandom.uniform(0.8, 2.4)
            ion.vel += (outward_or_inward + tangential + jitter) * strength
            ion.recent_transfer_timer = 0.8
...
def update_meter_and_labels():
    ...
    status_label.text = (
        ...
        f"Chaos bursts: {chaos_burst_count}"
    )
