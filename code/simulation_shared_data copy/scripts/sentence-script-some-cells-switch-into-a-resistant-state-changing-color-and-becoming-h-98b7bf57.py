...
scene = canvas(
    title="3D Bacterial Colony Growth - Resistant Cells",
    width=1200,
    height=780,
...
Controls:
  ...
  Y: induce resistant state nearby cells
  +/-: speed up / slow down
"""
...
# Resistant-state behavior: stressed cells can harden, darken, and resist motion.
RESISTANT_TRIGGER_SLIME = 0.58
RESISTANT_TRIGGER_LOW_NUTRIENT = 0.28
RESISTANT_DURATION = 18.0
RESISTANT_FORCE_MULT = 0.24
RESISTANT_SPEED_MULT = 0.38
RESISTANT_DRAG_BONUS = 1.35
RESISTANT_SPREAD_CHANCE = 0.022
...
class Bacterium:
    ...
    def __init__(self, pos=None, theta=None, energy=1.0, attached=False):
        ...
        # Resistant cells represent hardened individuals in stressful slime/depleted zones.
        # They become darker, move less, resist pushes, and can locally seed more resistance.
        self.resistant = False
        self.resistant_timer = 0.0
        self.resistance_level = 0.0
        self.resistance_cooldown = random.uniform(3.0, 9.0)
    ...
    def apply_force(self, f):
        multiplier = RESISTANT_FORCE_MULT if self.resistant else 1.0
        self.vel += flat(f) * multiplier
    ...
    def enter_resistant_state(self, duration=RESISTANT_DURATION, level=1.0):
        self.resistant = True
        self.resistance_level = clamp(max(self.resistance_level, level), 0.0, 1.0)
        self.resistant_timer = max(self.resistant_timer, duration * (0.75 + 0.55 * self.resistance_level))
        self.vel *= 0.18
        self.omega *= 0.20
        add_to_field(S, self.pos, 0.035 + 0.035 * self.resistance_level, radius=0.48, cap=1.35)
        add_particle(
            self.pos + vector(0, 0.18, 0),
            vector(0.30, 0.18, 0.56),
            radius=0.045 + 0.018 * self.resistance_level,
            life=0.85,
            opacity=0.48
        )

    def update_resistance(self, local_n, local_s, dt):
        self.resistance_cooldown -= dt

        if self.resistant:
            self.resistant_timer -= dt
            self.resistance_level = clamp(self.resistant_timer / RESISTANT_DURATION)
            # Rich nutrient pockets slowly soften hardened cells.
            if local_n > 0.68 and local_s < 0.42:
                self.resistant_timer -= dt * 1.4
            if self.resistant_timer <= 0:
                self.resistant = False
                self.resistance_level = 0.0
                self.resistance_cooldown = random.uniform(8.0, 16.0)
            return

        stress = 0.0
        if local_s > RESISTANT_TRIGGER_SLIME:
            stress += (local_s - RESISTANT_TRIGGER_SLIME) * 1.55
        if local_n < RESISTANT_TRIGGER_LOW_NUTRIENT:
            stress += (RESISTANT_TRIGGER_LOW_NUTRIENT - local_n) * 2.2
        if self.energy < 0.42:
            stress += (0.42 - self.energy) * 0.85

        # Nearby hardened cells can trigger a small local hardening response.
        if self.resistance_cooldown <= 0 and stress > 0.06 and random.random() < stress * 0.055 * dt:
            self.enter_resistant_state(duration=random.uniform(13.0, 26.0), level=clamp(0.50 + stress))

    def set_attached(self, value=True):
        if self.resistant and not value and random.random() < 0.55:
            # Hardened cells often remain stuck even when a detach action hits them.
            self.vel *= 0.35
            return
        self.attached = value
        if value:
    ...
    def update_visual(self):
        ...
        if self.resistant:
            hard_col = vector(0.25, 0.14, 0.46)
            rim_col = vector(0.88, 0.78, 1.0)
            pulse = 0.45 + 0.20 * math.sin(sim_time * 4.0 + self.id)
            c = mix_color(c, hard_col, 0.68 + 0.18 * self.resistance_level)
            c = mix_color(c, rim_col, 0.10 * pulse)
            self.halo.color = vector(0.34, 0.18, 0.64)
            self.halo.opacity = 0.15 + 0.10 * self.resistance_level
            self.halo.radius = self.radius * (2.35 + 0.28 * pulse)
        else:
    ...
    def update(self, dt):
        ...
        self.update_resistance(local_n, local_s, dt)
        ...
        if self.resistant:
            drag += RESISTANT_DRAG_BONUS * (0.55 + self.resistance_level)
        self.vel *= max(0.0, 1.0 - drag * dt)

        if self.attached:
            crawl = BASE_SPEED * 0.045 * (0.6 + local_n)
            if self.resistant:
                crawl *= RESISTANT_SPEED_MULT
            self.theta += self.omega * dt * (0.18 if self.resistant else 0.34)
            self.pos += self.forward() * crawl * dt + self.vel * dt * (0.18 if self.resistant else 0.5)
            slime_rate = 0.055
            consume_rate = 0.010
            ...
            if self.resistant:
                speed *= RESISTANT_SPEED_MULT
            self.theta += self.omega * dt * (0.28 if self.resistant else 1.0)
            self.pos += self.forward() * speed * dt + self.vel * dt * (0.20 if self.resistant else 1.0)
        ...
        if self.resistant:
            add_to_field(S, self.pos, 0.014 * dt * (0.7 + self.resistance_level), radius=0.54, cap=1.35)
        ...
        for j in range(i + 1, n):
            ...
            if dist < neighbor_range:
                ...
                if dist < min_dist:
                    ...
                    # Contact with a hardened neighbor can induce resistance in crowded slime.
                    if local_s > 0.46 and random.random() < RESISTANT_SPREAD_CHANCE * dt:
                        if a.resistant and not b.resistant:
                            b.enter_resistant_state(duration=random.uniform(10.0, 18.0), level=0.62)
                        elif b.resistant and not a.resistant:
                            a.enter_resistant_state(duration=random.uniform(10.0, 18.0), level=0.62)
...
def induce_resistance_near(pos, radius=1.7, strength=1.0):
    for b in bacteria:
        d = mag(flat(b.pos - pos))
        if d < radius:
            w = (1 - d / radius) ** 1.2
            if random.random() < 0.38 * strength * w:
                b.enter_resistant_state(duration=random.uniform(12.0, 25.0), level=clamp(0.45 + 0.55 * strength * w))
            # Resistant-state induction makes the patch sticky and harder to disturb.
            add_to_field(S, b.pos, 0.025 * strength * w, radius=0.42, cap=1.35)
...
class AIController:
    ...
    def read_state(self):
        ...
        return {
            ...
            "resistant_fraction": resistant / pop if pop else 0.0,
    ...
    def choose_interesting_target(self):
        ...
        if self.mode == "SEED":
            return richest_cell_pos()
        if self.mode in ("HERD", "ORGANIZE", "RESIST"):
    ...
    def choose_mode(self, state):
        ...
        if state["nutrient_poor"] and self.mode not in ("FEED", "SEED"):
        ...
        if (state["high_biofilm"] or state["nutrient_poor"]) and state["population"] > 18 and state["resistant_fraction"] < 0.34:
            if random.random() < 0.36:
                return "RESIST"

        if state["crowded"] and self.mode not in ("SCRAPE", "ORGANIZE", "ORBIT", "RESIST"):
            return random.choice(["SCRAPE", "ORGANIZE", "ORBIT", "RESIST"])

        if state["spread"] < 2.2 and state["population"] > 22:
            return random.choice(["STIR", "ORBIT", "MARK", "HERD"])
        ...
        if self.mode_time > self.switch_after:
            ...
            for m in options:
                ...
                if m == "FEED":
                    ...
                    w += 1.6 if (state["high_biofilm"] or state["nutrient_poor"]) and state["resistant_fraction"] < 0.35 else 0.15
                elif m == "REST":
                    w += 0.35
                weights.append(w)
            total = sum(weights)
    ...
    def perform_mode_action(self, state, dt):
        ...
        elif self.mode == "RESIST":
            if self.action_timer <= 0:
                self.cursor_target = state["center"] + vector(random.uniform(-max(1.0, state["spread"]), max(1.0, state["spread"])), 0, random.uniform(-max(1.0, state["spread"]), max(1.0, state["spread"])))
                induce_resistance_near(self.cursor_pos, radius=random.uniform(1.2, 2.1), strength=random.uniform(0.75, 1.25))
                spill_slime(self.cursor_pos, radius=0.8, amount=0.075, particles_on=True)
                self.action_timer = random.uniform(1.0, 2.2)
...
def reset_simulation(seed_count=START_BACTERIA, from_ai=False):
    global bacteria, trails, particles, sim_time, frame_count
...
def update_labels():
    ...
    stats_label.text = (
        ...
        f"Resistant: {state['resistant_count']} ({state['resistant_fraction']*100:.0f}%)\n"
...
def on_keydown(evt):
    ...
    if k in ("w", "up"):
        ...
        induce_resistance_near(ai.cursor_pos, radius=2.0, strength=1.25)
        ai.override(4.0)
    elif k == "o":
