...
#   T     manually trigger an electrical storm
...
scene.caption = (
    ...
    "Use TAB/1-9 to select neurons; G/P/C/D/X/M/V/B/O manipulate the system; T triggers a storm.\n"
)
...
# Electrical storms create occasional synchronized network-wide firing events.
STORM_MIN_INTERVAL = 8.0
STORM_MAX_INTERVAL = 16.0
STORM_DURATION = 3.4
STORM_WAVE_PERIOD = 0.26
STORM_MAX_SIGNAL_SPAWNS_PER_WAVE = 12
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.storm_timer = random.uniform(4.5, 9.5)
        self.storm_active = False
        self.storm_elapsed = 0.0
        self.storm_duration = STORM_DURATION
        self.storm_next_wave = 0.0
        self.storm_center = vector(0, 0, 0)
        self.storm_count = 0
        self.storm_wave_count = 0
        self.last_storm_pulsed = []
        ...
        self.storm_label = label(
            pos=vector(0, 4.75, 0),
            text="",
            height=15,
            border=7,
            color=vector(0.18, 0.10, 0.28),
            background=vector(0.96, 0.92, 1.0),
            opacity=0.0,
            box=True,
            visible=False,
        )
        ...
        for i in range(4):
            shell = sphere(
                pos=vector(0, 0, 0),
                radius=0.2,
                color=vector(0.72 + 0.06 * i, 0.58, 1.0),
                opacity=0.0,
                emissive=True,
                visible=False,
            )
            self.storm_shells.append(shell)
    ...
    def hide_storm_visuals(self):
        self.storm_label.visible = False
        self.storm_label.opacity = 0.0
        for shell in self.storm_shells:
            shell.visible = False
            shell.opacity = 0.0

    def start_electrical_storm(self, manual=False):
        if self.storm_active or not self.neurons:
            return

        hot_neurons = sorted(self.neurons, key=lambda n: n.activity, reverse=True)
        seed = hot_neurons[0] if hot_neurons and hot_neurons[0].activity > 0.28 else random.choice(self.neurons)
        self.storm_center = vector(seed.pos.x, seed.pos.y, seed.pos.z)
        self.storm_active = True
        self.storm_elapsed = 0.0
        self.storm_duration = random.uniform(STORM_DURATION * 0.85, STORM_DURATION * 1.28)
        self.storm_next_wave = 0.0
        self.storm_wave_count = 0
        self.last_storm_pulsed = []
        self.storm_count += 1

        self.storm_label.visible = True
        self.storm_label.opacity = 0.55
        self.storm_label.text = "ELECTRICAL STORM: synchronized burst spreading"

        for i, shell in enumerate(self.storm_shells):
            shell.pos = self.storm_center
            shell.radius = 0.25 + 0.18 * i
            shell.opacity = 0.09 - 0.012 * i
            shell.visible = True

        seed.fire(0.95)
        self.wrap_particles(seed, count=10)
        if manual:
            self.ai_mode_text = "manual storm"

    def emit_storm_wave(self):
        if not self.neurons:
            return

        self.storm_wave_count += 1
        progress = clamp(self.storm_elapsed / max(0.1, self.storm_duration))
        wave_radius = lerp(0.25, WORLD_RADIUS * 1.35, progress)
        window = 1.55 + 0.75 * math.sin(self.storm_wave_count * 0.9) ** 2

        candidates = []
        for n in self.neurons:
            d = mag(n.pos - self.storm_center)
            if abs(d - wave_radius) <= window or random.random() < 0.18:
                candidates.append(n)

        if not candidates:
            candidates = random.sample(self.neurons, min(len(self.neurons), random.randint(1, 3)))

        if self.storm_wave_count % 3 == 0:
            candidates = list(self.neurons)

        self.last_storm_pulsed = candidates
        for n in candidates:
            amount = random.uniform(0.45, 0.95)
            n.fire(amount)
            n.vel += safe_norm(n.pos - self.storm_center, random_unit()) * random.uniform(0.015, 0.08)
            n.vel += random_unit() * random.uniform(0.015, 0.06)
            if random.random() < 0.42:
                self.wrap_particles(n, count=random.randint(2, 5))

        storm_synapses = []
        for syn in self.live_synapses():
            if syn.neuron_a in candidates or syn.neuron_b in candidates:
                storm_synapses.append(syn)

        if storm_synapses:
            sample_count = min(len(storm_synapses), STORM_MAX_SIGNAL_SPAWNS_PER_WAVE)
            for syn in random.sample(storm_synapses, sample_count):
                source = syn.neuron_a if syn.neuron_a.activity >= syn.neuron_b.activity else syn.neuron_b
                self.spawn_signal_on_synapse(syn, source=source)
                if random.random() < 0.30:
                    self.spawn_signal_on_synapse(syn, source=syn.other_neuron(source))

        for n in candidates:
            for b in random.sample(n.live_branches(), min(len(n.live_branches()), 5)):
                b.pulse(random.uniform(0.45, 1.0))

    def update_electrical_storms(self, dt):
        if not self.storm_active:
            self.storm_timer -= dt
            if self.storm_timer <= 0:
                # Storms are more likely when the network has enough synaptic structure to carry them.
                if len(self.live_synapses()) >= 4 or random.random() < 0.35:
                    self.start_electrical_storm()
                else:
                    self.storm_timer = random.uniform(3.0, 6.0)
            return

        self.storm_elapsed += dt
        self.storm_next_wave -= dt

        if self.storm_next_wave <= 0:
            self.emit_storm_wave()
            self.storm_next_wave = random.uniform(STORM_WAVE_PERIOD * 0.72, STORM_WAVE_PERIOD * 1.22)

        progress = clamp(self.storm_elapsed / max(0.1, self.storm_duration))
        for i, shell in enumerate(self.storm_shells):
            phase = clamp(progress - i * 0.08, 0, 1)
            shell.pos = self.storm_center
            shell.radius = lerp(0.35 + i * 0.18, WORLD_RADIUS * (1.0 + 0.10 * i), phase)
            shell.opacity = max(0.0, (0.12 - 0.02 * i) * (1.0 - phase))
            shell.color = lerp_vec(vector(0.55, 0.70, 1.0), vector(1.0, 0.78, 0.24), 0.5 + 0.5 * math.sin(self.time * 8.0 + i))
            shell.visible = shell.opacity > 0.005

        self.storm_label.pos = vector(0, 4.75 + 0.12 * math.sin(self.time * 10.0), 0)
        self.storm_label.opacity = 0.35 + 0.20 * math.sin(self.time * 11.0) ** 2
        self.storm_label.text = f"ELECTRICAL STORM | wave {self.storm_wave_count} | synchronized firing"

        if self.storm_elapsed >= self.storm_duration:
            self.storm_active = False
            self.storm_timer = random.uniform(STORM_MIN_INTERVAL, STORM_MAX_INTERVAL)
            self.last_storm_pulsed = []
            self.hide_storm_visuals()
    ...
    def update_meter(self):
        ...
        storm_state = "STORM" if self.storm_active else f"storm in {max(0.0, self.storm_timer):.1f}s"
        self.status_label.text = (
            ...
            f"Storms {self.storm_count} ({storm_state}) | "
    ...
    def update(self, dt):
        ...
        self.update_electrical_storms(dt)
...
class ExpressiveAIController:
    ...
    AI behavior modes implemented in this file:
      ...
      curious, ritual, destructive, artistic, storm, reset_loop
    ...
    def execute_mode(self, dt, state):
        ...
        if self.mode == "construct":
            ...
            if not sim.storm_active:
                sim.start_electrical_storm()
            sim.organize_circle(dt, radius=3.15 + 0.35 * math.sin(sim.time * 2.0))
            if self.action_timer > 0.40:
                self.action_timer = 0.0
                random.choice(sim.neurons).fire(random.uniform(0.25, 0.55))
...
def keydown(evt):
    ...
    if key == " ":
        ...
        if key == "left":
            ...
            sim.start_electrical_storm(manual=True)
...
while True:
    ...
    if not sim.paused:
        ...
        sim.status_label.text = (
            ...
            f"Synapses {len(sim.live_synapses())} | Storms {sim.storm_count} | {'AI ON' if sim.ai_enabled else 'AI OFF'}: {sim.ai_mode_text}"
        )
