...
if _csv_output_dir:
    ...
    CSV_OUTPUT_PATH = os.path.join(
        _csv_output_dir,
        f"{_csv_run_id}-reef-predator-prey-state-log.csv"
    )
else:
    CSV_OUTPUT_PATH = os.environ.get(
        "SIM_STATE_CSV_PATH",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "reef_predator_prey_state_log.csv")
    )
...
def write_csv_snapshot():
    active_prey = [f for f in prey if not f.caught]
    active_count = len(active_prey)
    base = {
        "run_id": csv_run_id,
        "time_seconds": round(sim_time, 4),
        "frame": _csv_frame,
        "round_number": round_number,
        "ai_enabled": bool(ai.enabled),
        "ai_mode": ai.mode,
        "ai_mode_time": round(ai.mode_time, 4),
        "ai_stagnant": bool(ai.stagnant),
        "ai_completed": bool(ai.completed),
        "boundary_mode": boundary_mode,
        "paused": bool(paused),
        "predator_speed_multiplier": round(predator_speed_multiplier, 4),
        "active_prey_count": active_count,
    ...
    summary.update({
        ...
        "object_id": "reef",
        "object_kind": "system",
        "extra": f"predators={len(predators)};prey={len(prey)};coral={len(obstacles)}",
    })
    _csv_writer.writerow(summary)

    for i, pr in enumerate(predators):
        px, py, pz = _csv_vec(pr.pos)
        vx, vy, vz = _csv_vec(pr.vel)
        fx, fy, fz = _csv_vec(pr.forward)
        nearest_dist = min([mag(f.pos - pr.pos) for f in active_prey], default=0.0)
        row = dict(base)
        row.update({
            "row_type": "predator",
            "object_id": i,
            "object_kind": "predator",
            "x": round(px, 5),
            "y": round(py, 5),
            "z": round(pz, 5),
            "vx": round(vx, 5),
            "vy": round(vy, 5),
            "vz": round(vz, 5),
            "speed": round(_csv_speed(pr.vel), 5),
            "forward_x": round(fx, 5),
            "forward_y": round(fy, 5),
            "forward_z": round(fz, 5),
            "nearest_prey_distance": round(nearest_dist, 5),
            "extra": f"restlessness={getattr(pr, 'restlessness', 0):.4f}",
        })
        _csv_writer.writerow(row)

    for f in prey:
        px, py, pz = _csv_vec(f.pos)
        vx, vy, vz = _csv_vec(f.vel)
        nearest_pr_dist = min([mag(f.pos - pr.pos) for pr in predators], default=0.0)
        row = dict(base)
        row.update({
            "row_type": "prey",
            "object_id": f.idx,
            ...
            "vy": round(vy, 5),
            "vz": round(vz, 5),
            "speed": round(_csv_speed(f.vel), 5),
            "caught": bool(f.caught),
            "captor": getattr(f.captor, "idx", "") if f.captor is not None else "",
            "caught_timer": round(f.caught_timer, 5),
            "fear": round(f.fear, 5),
            "orbit_radius": round(f.orbit_radius, 5),
            "max_speed": round(f.max_speed, 5),
            "nearest_predator_distance": round(nearest_pr_dist, 5),
        })
        _csv_writer.writerow(row)

    for i, o in enumerate(obstacles):
        px, py, pz = _csv_vec(o.center)
        row = dict(base)
        row.update({
            "row_type": "coral",
            "object_id": i,
            "object_kind": "coral_obstacle",
            "x": round(px, 5),
            "y": round(py, 5),
            "z": round(pz, 5),
            "coral_radius": round(o.radius, 5),
            "coral_height": round(o.height, 5),
        })
        _csv_writer.writerow(row)

    for i, m in enumerate(danger_marks):
        px, py, pz = _csv_vec(m.pos)
        row = dict(base)
        row.update({
            "row_type": "danger_mark",
            "object_id": i,
            "object_kind": "mark",
            "x": round(px, 5),
            "y": round(py, 5),
            "z": round(pz, 5),
            "mark_intensity": round(m.intensity, 5),
            "mark_life": round(m.life, 5),
        })
        _csv_writer.writerow(row)

    for i, p in enumerate(particles):
        px, py, pz = _csv_vec(p.obj.pos)
        vx, vy, vz = _csv_vec(p.vel)
        row = dict(base)
        row.update({
            "row_type": "particle",
            "object_id": i,
            "object_kind": "visual_particle",
            "x": round(px, 5),
            "y": round(py, 5),
            "z": round(pz, 5),
            "vx": round(vx, 5),
            "vy": round(vy, 5),
            "vz": round(vz, 5),
            "speed": round(_csv_speed(p.vel), 5),
            "particle_life": round(p.life, 5),
        })
        _csv_writer.writerow(row)
...
while sim_time < CSV_RUN_SECONDS:
    ...
    _csv_frame += 1
    if sim_time >= _csv_next_sample_time:
        write_csv_snapshot()
