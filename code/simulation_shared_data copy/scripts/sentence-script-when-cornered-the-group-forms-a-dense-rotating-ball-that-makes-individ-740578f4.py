...
scene.title = "Two-Predator Fish School in a Reef - Rotating Bait Ball Defense"
...
# Dense rotating ball behavior: when the school is cornered and predators press close,
# active prey temporarily collapse into a tight rotating bait-ball formation.
BAIT_BALL_TRIGGER_DISTANCE = 6.4
BAIT_BALL_RELEASE_DISTANCE = 9.0
BAIT_BALL_MIN_TIME = 4.0
BAIT_BALL_RADIUS = 2.35
BAIT_BALL_STEER = 8.5
BAIT_BALL_TANGENT = 3.6
...
class BaitBallSystem:
    ...
    def update(self, dt, prey_list, predator_list):
        ...
        # Corner pressure increases near multiple boundaries, especially when backed into x/z walls.
        ...
        packed = sum(1 for f in active_prey if mag(f.pos - self.center) < 4.8) / max(1, len(active_prey))
        close_predators = sum(1 for pr in predator_list if mag(pr.pos - self.center) < 8.0)
        should_form = (threat_dist < BAIT_BALL_TRIGGER_DISTANCE and self.corner_pressure > 0.22 and packed > 0.38) or (close_predators >= 2 and threat_dist < 7.4 and packed > 0.45)
        should_release = threat_dist > BAIT_BALL_RELEASE_DISTANCE and self.timer > BAIT_BALL_MIN_TIME

        if should_form and not self.active and self.cooldown <= 0:
            self.active = True
            self.timer = 0.0
            add_mark(self.center, 1.2, vector(1.0, 0.78, 0.18), 5.0, "bait ball")
            spawn_particles(self.center, vector(1.0, 0.78, 0.18), count=24, speed=2.4)
        elif self.active and should_release:
            self.active = False
            self.cooldown = 4.0
            add_mark(self.center, 0.7, vector(0.38, 0.96, 0.78), 4.0, "scatter out")
        ...
        if self.active:
            ...
            self.rotation += dt * (2.35 + 1.4 * clamp((BAIT_BALL_RELEASE_DISTANCE - threat_dist) / BAIT_BALL_RELEASE_DISTANCE, 0, 1))
            density_t = clamp((len(active_prey) - 4) / max(1, NUM_PREY), 0.25, 1.0)
            self.radius = lerp(self.radius, BAIT_BALL_RADIUS * (0.82 + 0.28 * density_t), 0.05)
    ...
    def steering_for(self, fish, nearest_threat, pred_dist):
        ...
        steer = vector(0, 0, 0)
        steer += safe_norm(to_target, inward) * BAIT_BALL_STEER * clamp(mag(to_target) / 2.0, 0.35, 1.8)
        steer += tangent * BAIT_BALL_TANGENT
        steer += inward * 1.9
        steer += anti_pred * (2.4 + 2.0 * clamp((6.5 - pred_dist) / 6.5, 0, 1))
        self.member_count += 1
        return steer

    def shields_fish(self, fish):
        return self.active and (mag(fish.pos - self.center) < self.radius * 1.28)
    ...
    def update_visual(self, dt):
        ...
        if self.active:
            ...
            self.shell.color = color_lerp(vector(1.0, 0.78, 0.12), vector(1.0, 0.35, 0.12), pulse)
            self.inner.opacity = 0.10 + 0.06 * pulse
            self.core_label.text = "rotating defense"
        else:
...
class PreyFish:
    ...
    def update(self, dt, prey_list, predators):
        ...
        if bait_ball.shields_fish(self):
            # Dense rotation makes the school visually and behaviorally confusing. A predator
            # must get much closer to isolate a fish inside the bait ball.
            capture_radius *= 0.48
...
def secondary_predator_accel(pr, idx, active_prey):
    ...
    if bait_ball.active:
        # Predators have trouble selecting a single fish and are pulled toward the spinning edge.
        offset = pr.pos - bait_ball.center
        tangent = safe_norm(cross(vector(0, 1, 0), offset), random_unit_vector())
        edge = bait_ball.center + safe_norm(offset, random_unit_vector()) * (bait_ball.radius + 0.9)
        acc += (edge - pr.pos) * 0.55 + tangent * 1.4
    ...
    def choose_action(self, dt, state):
        ...
        if self.mode == "HUNT":
            if state.get("bait_ball_active", False):
                # Aim at the rotating edge instead of a single fish; this makes isolation less direct.
                offset = predator.pos - state["bait_ball_center"]
                edge_target = state["bait_ball_center"] + safe_norm(offset, random_unit_vector()) * (bait_ball.radius + 0.65)
                edge_target += safe_norm(cross(vector(0, 1, 0), offset), random_unit_vector()) * 1.3
                acc = self.steering_to(edge_target, arrive_radius=0.8, speed_scale=0.95)
            elif nearest:
...
def update_hud():
    ...
    hud.text = (
        ...
        f"({bait_ball.member_count} fish, pressure {bait_ball.corner_pressure:.2f}) | "
