...
if _csv_output_dir:
    ...
    CSV_OUTPUT_PATH = os.path.join(
        _csv_output_dir,
        f"{_csv_run_id}-mitosis-state-log.csv"
    )
else:
    CSV_OUTPUT_PATH = os.environ.get(
        "SIM_STATE_CSV_PATH",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "mitosis_state_log.csv")
    )
...
def _base_csv_row(sample_index, recording_time, frame):
    state = sim.get_state()
    return {
        "run_id": csv_run_id,
        "sample_index": sample_index,
        "recording_time_seconds": _csv_float(recording_time),
        "frame": frame,
        "round_index": sim.round_index,
        "simulation_time_seconds": _csv_float(sim.time_seconds),
        "progress": _csv_float(sim.progress),
        "stage": sim.stage_name(),
        "paused": _csv_bool(sim.paused),
        "ai_enabled": _csv_bool(ai.enabled),
        "ai_mode": ai.mode,
        "ai_override": _csv_bool(ai.override),
        "ai_mode_timer": _csv_float(ai.mode_timer),
        "ai_stagnation_time": _csv_float(ai.stagnation_time),
        "speed_modifier": _csv_float(sim.speed_modifier),
        "human_speed_bias": _csv_float(sim.human_speed_bias),
        "ai_noise": _csv_float(sim.ai_noise),
        "pull_bias": _csv_float(sim.pull_bias),
        "constriction_bias": _csv_float(sim.constriction_bias),
        "drift_bias": _csv_float(sim.drift_bias),
        "complete_timer": _csv_float(sim.complete_timer),
        "chromosome_separation": _csv_float(state["chromosome_separation"]),
        "attachment_ratio": _csv_float(state["attachment_ratio"]),
        "particle_count": state["particle_count"],
        "marker_count": state["marker_count"],
        "wrapper_count": state["wrapper_count"],
        "daughter_distance": _csv_float(state["daughter_distance"]),
        "entity_type": "",
        "entity_id": "",
        "entity_part": "",
        "x": "",
...
def _add_position(row, pos):
    row["x"] = _csv_float(pos.x)
    row["y"] = _csv_float(pos.y)
    row["z"] = _csv_float(pos.z)


def _write_entity_row(writer, base, entity_type, entity_id, entity_part, pos, radius="", visible="", vel=None, notes=""):
    row = dict(base)
    row["entity_type"] = entity_type
    row["entity_id"] = entity_id
    row["entity_part"] = entity_part
    _add_position(row, pos)
    if vel is not None:
        row["vx"] = _csv_float(vel.x)
        row["vy"] = _csv_float(vel.y)
        row["vz"] = _csv_float(vel.z)
    row["radius"] = _csv_float(radius) if radius != "" else ""
    row["visible"] = _csv_bool(visible) if visible != "" else ""
    row["notes"] = notes
    writer.writerow(row)


def write_csv_snapshot(writer, sample_index, recording_time, frame):
    base = _base_csv_row(sample_index, recording_time, frame)

    summary = dict(base)
    summary["entity_type"] = "simulation"
    summary["entity_id"] = "global"
    summary["entity_part"] = "summary"
    writer.writerow(summary)

    _write_entity_row(writer, base, "cell", "single", "cell_body", sim.single_cell.pos, radius=sim.single_cell.size.x * 0.5, visible=sim.single_cell.visible)
    _write_entity_row(writer, base, "cell", "single", "membrane", sim.single_membrane.pos, radius=sim.single_membrane.size.x * 0.5, visible=sim.single_membrane.visible)
    _write_entity_row(writer, base, "cell", "left_daughter", "body", sim.left_daughter.pos, radius=sim.left_daughter.size.x * 0.5, visible=sim.left_daughter.visible)
    _write_entity_row(writer, base, "cell", "right_daughter", "body", sim.right_daughter.pos, radius=sim.right_daughter.size.x * 0.5, visible=sim.right_daughter.visible)
    _write_entity_row(writer, base, "nucleus", "left", "body", sim.left_nucleus.pos, radius=sim.left_nucleus.radius, visible=sim.left_nucleus.visible)
    _write_entity_row(writer, base, "nucleus", "right", "body", sim.right_nucleus.pos, radius=sim.right_nucleus.radius, visible=sim.right_nucleus.visible)
    _write_entity_row(writer, base, "spindle_pole", "left", "pole", sim.left_pole_obj.pos, radius=sim.left_pole_obj.radius, visible=sim.left_pole_obj.visible)
    _write_entity_row(writer, base, "spindle_pole", "right", "pole", sim.right_pole_obj.pos, radius=sim.right_pole_obj.radius, visible=sim.right_pole_obj.visible)
    _write_entity_row(writer, base, "cleavage_ring", "main", "ring", sim.cleavage_ring.pos, radius=sim.cleavage_ring.radius, visible=sim.cleavage_ring.visible)
    _write_entity_row(writer, base, "bridge", "main", "cytokinetic_bridge", sim.bridge.pos, radius=sim.bridge.radius, visible=sim.bridge.visible)

    for chrom in sim.chromosomes:
        left_row = dict(base)
        left_row["entity_type"] = "chromosome"
        left_row["entity_id"] = chrom.index
        left_row["entity_part"] = "left_chromatid"
        _add_position(left_row, chrom.left.pos)
        left_row["radius"] = _csv_float(chrom.left.radius)
        left_row["visible"] = _csv_bool(chrom.left.visible)
        left_row["left_attached"] = _csv_bool(chrom.left_attached)
        left_row["right_attached"] = _csv_bool(chrom.right_attached)
        left_row["connector_visible"] = _csv_bool(chrom.connector.visible)
        writer.writerow(left_row)

        right_row = dict(base)
        right_row["entity_type"] = "chromosome"
        right_row["entity_id"] = chrom.index
        right_row["entity_part"] = "right_chromatid"
        _add_position(right_row, chrom.right.pos)
        right_row["radius"] = _csv_float(chrom.right.radius)
        right_row["visible"] = _csv_bool(chrom.right.visible)
        right_row["left_attached"] = _csv_bool(chrom.left_attached)
        right_row["right_attached"] = _csv_bool(chrom.right_attached)
        right_row["connector_visible"] = _csv_bool(chrom.connector.visible)
    ...
    for idx, particle in enumerate(sim.particles):
        _write_entity_row(
            ...
            "particle",
            particle.obj.pos,
            radius=particle.obj.radius,
            visible=particle.obj.visible,
            vel=particle.vel,
            notes=f"life={particle.life:.3f}"
        )

    for idx, marker in enumerate(sim.markers):
        _write_entity_row(
            writer,
            base,
            "marker",
            ...
            "glyph",
            marker.dot.pos,
            radius=marker.dot.radius,
            visible=marker.dot.visible,
            notes=marker.lbl.text
        )

    for idx, wrapper in enumerate(sim.wrappers):
        _write_entity_row(
            writer,
            base,
            "membrane_wrapper",
            ...
            "ring",
            wrapper.obj.pos,
            radius=wrapper.obj.radius,
            visible=wrapper.obj.visible,
            notes=f"life={wrapper.life:.3f}"
        )
...
csv_next_flush_time = CSV_FLUSH_INTERVAL
csv_sample_index = 0
csv_frame_index = 0
...
while recording_time < CSV_RUN_SECONDS:
    rate(60)
    csv_frame_index += 1
    ...
    if recording_time + 1e-9 >= csv_next_sample_time:
        write_csv_snapshot(csv_writer, csv_sample_index, recording_time, csv_frame_index)
        csv_sample_index += 1
        csv_next_sample_time += CSV_SAMPLE_INTERVAL

    if recording_time + 1e-9 >= csv_next_flush_time:
        csv_file.flush()
        csv_next_flush_time += CSV_FLUSH_INTERVAL
...
sim.paused = True
sim.stage_label.visible = True
sim.ai_label.visible = True
sim.stage_label.text = (
    f"CSV recording complete: {CSV_RUN_SECONDS:0.0f}s saved to "
    f"{os.path.basename(CSV_OUTPUT_PATH)}"
)
sim.ai_label.text = f"Samples saved: {csv_sample_index} | Run ID: {csv_run_id}"
controls.visible = True
controls.text = "CSV run finished. Close the VPython window or rerun the script to create another CSV."
