...
# Human Controls, an Expressive AI Controller, and Competitive Pathway Pruning.
#
# New behavior:
#   Nearby branches compete for synaptic connections. Synapses that repeatedly
#   carry pulses become stronger, brighter, and harder to prune. Weak nearby
#   links are gradually thinned and removed as stronger circuits emerge.
#
# Controls:
#   SPACE pause/resume
#   A     toggle AI
...
scene.title = "3D Neuron Network with Competitive Circuits + AI Controller"
...
# Competition tuning: nearby synapses compare strength/use and weaker links fade.
COMPETITION_RADIUS = 1.15
COMPETITION_INTERVAL = 0.42
COMPETITION_PRUNE_THRESHOLD = 0.13
COMPETITION_MAX_PAIR_CHECKS = 120
...
class BranchSegment:
    def __init__(self, neuron, parent=None, kind="dendrite", direction=None, max_len=None):
        ...
        self.connection_use = random.uniform(0.0, 0.20)
        self.competition_heat = 0.0
    ...
    def update(self, dt):
        ...
        self.connection_use *= (0.992 ** (dt * 60.0))
        self.competition_heat *= (0.90 ** (dt * 60.0))
        activity_mix = clamp(self.neuron.activity * 0.75 + self.glow + 0.35 * self.connection_use)
        c = lerp_vec(self.base_color, vector(1.0, 0.96, 0.30), activity_mix)
        c = lerp_vec(c, vector(1.0, 0.30, 0.18), clamp(self.competition_heat))
        ...
        self.cyl.color = c
        self.cyl.opacity = 0.38 + 0.42 * activity_mix + 0.15 * clamp(self.competition_heat)
    ...
    def reinforce_use(self, amount=0.08, contested=False):
        self.connection_use = clamp(self.connection_use + amount, 0, 1)
        self.glow = clamp(self.glow + amount * 1.6, 0, 1)
        if contested:
            self.competition_heat = clamp(self.competition_heat + amount * 2.5, 0, 1)
...
class Synapse:
    def __init__(self, sim, seg_a, seg_b):
        ...
        self.use_memory = random.uniform(0.04, 0.16)
    ...
    def update(self, dt):
        ...
        self.age += dt
        self.use_memory *= (0.995 ** (dt * 60.0))
        self.competition_pressure *= (0.90 ** (dt * 60.0))
        if self.use_memory < 0.12:
            self.strength = clamp(self.strength - 0.0035 * dt, 0.05, 1.45)
        p1 = self.p1()
        p2 = self.p2()
        dist = mag(p2 - p1)
        ...
        circuit_power = clamp(0.55 * combined_activity + 0.45 * self.use_memory)
        c = lerp_vec(vector(0.58, 0.46, 1.0), vector(1.0, 0.86, 0.24), circuit_power)
        c = lerp_vec(c, vector(1.0, 0.28, 0.14), clamp(self.competition_pressure))

        self.cyl.pos = p1
        self.cyl.axis = p2 - p1
        self.cyl.radius = 0.014 + 0.025 * self.strength + 0.020 * circuit_power
        self.cyl.color = c
        self.cyl.opacity = 0.15 + 0.42 * circuit_power + 0.16 * clamp(self.strength) + 0.18 * clamp(self.competition_pressure)

        self.mid_glow.pos = (p1 + p2) / 2
        self.mid_glow.color = lerp_vec(vector(0.78, 0.62, 1.0), vector(1.0, 0.95, 0.22), circuit_power)
        self.mid_glow.radius = 0.045 + 0.075 * circuit_power + 0.035 * clamp(self.competition_pressure)
        self.mid_glow.opacity = 0.14 + 0.46 * circuit_power + 0.16 * clamp(self.competition_pressure)
    ...
    def reinforce(self, amount=0.055):
        if not self.alive:
            return
        self.pulse_count += 1
        self.use_memory = clamp(self.use_memory + amount, 0, 1)
        self.strength = clamp(self.strength + amount * 0.42, 0.05, 1.45)
        self.seg_a.reinforce_use(amount * 0.85)
        self.seg_b.reinforce_use(amount * 0.85)

    def competitive_score(self):
        activity = 0.5 * (self.neuron_a.activity + self.neuron_b.activity)
        return self.strength + 0.75 * self.use_memory + 0.20 * activity + 0.015 * self.pulse_count

    def compete_against(self, other):
        if not self.alive or not other.alive:
            return None
        my_score = self.competitive_score()
        other_score = other.competitive_score()
        if abs(my_score - other_score) < 0.05:
            return None
        winner, loser = (self, other) if my_score > other_score else (other, self)
        gap = clamp(abs(my_score - other_score) * 0.24, 0.012, 0.075)
        winner.strength = clamp(winner.strength + gap * 0.32, 0.05, 1.45)
        winner.use_memory = clamp(winner.use_memory + gap * 0.50, 0, 1)
        winner.competition_pressure = clamp(winner.competition_pressure + 0.20, 0, 1)
        loser.strength = clamp(loser.strength - gap, 0.02, 1.45)
        loser.use_memory = clamp(loser.use_memory - gap * 0.90, 0, 1)
        loser.competition_pressure = clamp(loser.competition_pressure + 0.55, 0, 1)
        loser.seg_a.reinforce_use(0.012, contested=True)
        loser.seg_b.reinforce_use(0.012, contested=True)
        return loser
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.competition_timer = 0.0
        self.pruned_by_competition = 0
    ...
    def attach_segments(self, seg_a, seg_b):
        ...
        nearby_competitors = self.nearby_competing_synapses(syn)
        if nearby_competitors:
            syn.competition_pressure = 0.55
            seg_a.reinforce_use(0.07, contested=True)
            seg_b.reinforce_use(0.07, contested=True)
        seg_a.pulse(0.9)
        seg_b.pulse(0.9)
    ...
    def nearby_competing_synapses(self, synapse):
        competitors = []
        if synapse is None or not synapse.alive:
            return competitors
        p1 = synapse.p1()
        p2 = synapse.p2()
        for other in self.live_synapses():
            if other is synapse:
                continue
            shared_branch = (
                other.seg_a is synapse.seg_a or other.seg_b is synapse.seg_a or
                other.seg_a is synapse.seg_b or other.seg_b is synapse.seg_b
            )
            nearby_tip = (
                mag(other.p1() - p1) < COMPETITION_RADIUS or
                mag(other.p1() - p2) < COMPETITION_RADIUS or
                mag(other.p2() - p1) < COMPETITION_RADIUS or
                mag(other.p2() - p2) < COMPETITION_RADIUS
            )
            same_neuron_pair = (
                (other.neuron_a is synapse.neuron_a and other.neuron_b is synapse.neuron_b) or
                (other.neuron_a is synapse.neuron_b and other.neuron_b is synapse.neuron_a)
            )
            if shared_branch or nearby_tip or same_neuron_pair:
                competitors.append(other)
        return competitors

    def run_connection_competition(self, dt):
        self.competition_timer += dt
        if self.competition_timer < COMPETITION_INTERVAL:
            return
        self.competition_timer = 0.0
        ...
        pair_checks = 0
        prunes_this_round = 0
        sample = live if len(live) <= 36 else random.sample(live, 36)
        for i, a in enumerate(sample):
            if not a.alive:
                continue
            for b in sample[i + 1:]:
                if not b.alive:
                    continue
                shared_branch = (
                    a.seg_a is b.seg_a or a.seg_a is b.seg_b or
                    a.seg_b is b.seg_a or a.seg_b is b.seg_b
                )
                nearby_tip = (
                    mag(a.p1() - b.p1()) < COMPETITION_RADIUS or
                    mag(a.p1() - b.p2()) < COMPETITION_RADIUS or
                    mag(a.p2() - b.p1()) < COMPETITION_RADIUS or
                    mag(a.p2() - b.p2()) < COMPETITION_RADIUS
                )
                same_neuron_pair = (
                    (a.neuron_a is b.neuron_a and a.neuron_b is b.neuron_b) or
                    (a.neuron_a is b.neuron_b and a.neuron_b is b.neuron_a)
                )
                if not (shared_branch or nearby_tip or same_neuron_pair):
                    continue

                loser = a.compete_against(b)
                pair_checks += 1
                if loser is not None and loser.alive and loser.strength <= COMPETITION_PRUNE_THRESHOLD:
                    pos = (loser.p1() + loser.p2()) / 2
                    loser.detach()
                    self.pruned_by_competition += 1
                    prunes_this_round += 1
                    self.spill_particles(pos=pos, count=7, color=vector(1.0, 0.38, 0.22))
                    # Stop after a few removals so pruning is visible instead of instantaneous.
                    if prunes_this_round >= 3:
                        return
                if pair_checks >= COMPETITION_MAX_PAIR_CHECKS:
                    return
    ...
    def update_meter(self):
        ...
        self.status_label.text = (
            ...
            f"Competitive prunes {self.pruned_by_competition} | "
    ...
    def state_snapshot(self):
        return {
            ...
            "competitive_prunes": self.pruned_by_competition,
            "activity": self.network_activity(),
    ...
    def update(self, dt):
        ...
        for s in self.synapses:
            s.update(dt)
        self.run_connection_competition(dt)
