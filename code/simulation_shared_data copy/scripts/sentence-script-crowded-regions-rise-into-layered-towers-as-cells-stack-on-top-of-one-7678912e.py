...
scene = canvas(
    title="3D Bacterial Colony Growth - Layered Slime Towers",
    width=1200,
    height=780,
...
Tower behavior:
  Crowded, slimed regions rise into layered towers as bacteria stack upward.
  Tower columns glow under dense clusters while outer cells continue crawling on the surface.
"""
...
# Layered tower behavior
TOWER_DENSITY_RADIUS = 0.92
TOWER_MIN_NEIGHBORS = 5
TOWER_LAYER_HEIGHT = 0.22
MAX_TOWER_LAYERS = 7
TOWER_COLUMN_STEP = 2
TOWER_UPDATE_EVERY = 5
...
def create_tower_columns():
    global tower_columns
    tower_columns[:] = []
    for i in range(0, GRID, TOWER_COLUMN_STEP):
        for j in range(0, GRID, TOWER_COLUMN_STEP):
            cp = cell_center(i, j)
            col = cylinder(
                pos=vector(cp.x, 0.012, cp.z),
                axis=vector(0, 0.01, 0),
                radius=CELL * 0.62,
                color=vector(0.48, 0.36, 0.90),
                opacity=0.0,
                shininess=0.05
            )
            tower_columns.append({"obj": col, "i": i, "j": j, "height": 0.0})


def update_tower_structure(dt, force=False):
    if not force and frame_count % TOWER_UPDATE_EVERY != 0:
        return

    # First estimate local crowding around each bacterium. The physical motion
    # still happens mostly on the surface, but visuals lift into vertical layers.
    active_tower_count = 0
    for b in bacteria:
        if b.dead:
            b.tower_level = 0
            b.stack_height += (0.0 - b.stack_height) * clamp(5.0 * dt, 0, 1)
            continue
        ...
        for other in bacteria:
            if other is b or other.dead:
                continue
            d = mag(flat(other.pos - b.pos))
            if d < TOWER_DENSITY_RADIUS:
                neighbor_count += 1
                crowd_weight += (1.0 - d / TOWER_DENSITY_RADIUS)

        local_slime = sample_field(S, b.pos)
        local_nutrient = sample_field(N, b.pos)
        crowd_drive = max(0.0, crowd_weight - 1.4)
        slime_drive = max(0.0, local_slime - 0.36) * 3.0
        nutrient_bonus = 0.55 if local_nutrient > 0.42 else 0.0

        raw_layers = int(crowd_drive + slime_drive + nutrient_bonus)
        if neighbor_count < TOWER_MIN_NEIGHBORS and local_slime < 0.72:
            raw_layers = 0

        target_level = max(0, min(MAX_TOWER_LAYERS, raw_layers))
        b.tower_level = target_level
        if target_level > 0:
            active_tower_count += 1
            # Add a small deterministic offset so a dense region looks like stacked cells
            # rather than a flat shelf.
            stagger = ((b.id % 5) - 2) * 0.018
            target_height = target_level * TOWER_LAYER_HEIGHT + stagger
            add_to_field(S, b.pos, 0.010 * target_level, radius=0.44, cap=1.35)
        else:
            target_height = 0.0

        b.stack_height += (target_height - b.stack_height) * clamp(3.8 * dt, 0, 1)

    # Show broader tower bases as translucent vertical columns under crowded patches.
    for entry in tower_columns:
        i0 = entry["i"]
        j0 = entry["j"]
        cp = cell_center(i0, j0)
        count = 0
        slime_total = 0.0
        for b in bacteria:
            if not b.dead and mag(flat(b.pos - cp)) < CELL * 1.55:
                count += 1
                slime_total += sample_field(S, b.pos)
        avg_slime = slime_total / count if count else 0.0
        tower_strength = max(0.0, count - 2.5) + max(0.0, avg_slime - 0.42) * 4.0
        target_h = clamp(tower_strength / 8.0, 0.0, 1.0) * (MAX_TOWER_LAYERS * TOWER_LAYER_HEIGHT + 0.22)
        entry["height"] += (target_h - entry["height"]) * clamp(2.6 * dt, 0, 1)

        obj = entry["obj"]
        if entry["height"] > 0.035:
            obj.visible = True
            obj.pos = vector(cp.x, 0.012, cp.z)
            obj.axis = vector(0, entry["height"], 0)
            obj.radius = CELL * (0.48 + 0.16 * clamp(count / 9.0))
            obj.color = mix_color(vector(0.32, 0.78, 0.82), vector(0.72, 0.44, 1.0), clamp(entry["height"] / (MAX_TOWER_LAYERS * TOWER_LAYER_HEIGHT)))
            obj.opacity = clamp(0.07 + 0.20 * entry["height"] / (MAX_TOWER_LAYERS * TOWER_LAYER_HEIGHT), 0.0, 0.32)
        else:
            obj.opacity = 0.0

    return active_tower_count
...
class Bacterium:
    ...
    def __init__(self, pos=None, theta=None, energy=1.0, attached=False):
        ...
        self.stack_phase = random.random() * math.tau
    ...
    def update_visual(self):
        axis = heading(self.theta) * self.length
        visual_y = BACTERIA_Y + self.stack_height
        if self.tower_level > 0:
            visual_y += 0.012 * math.sin(sim_time * 2.8 + self.stack_phase)
        visual_pos = vector(self.pos.x, visual_y, self.pos.z)

        self.body.pos = visual_pos - axis * 0.5
        self.body.axis = axis
        self.cap1.pos = visual_pos - axis * 0.5
        self.cap2.pos = visual_pos + axis * 0.5
        self.halo.pos = visual_pos
        ...
        if self.tower_level > 0:
            tower_t = clamp(self.tower_level / MAX_TOWER_LAYERS)
            c = mix_color(c, vector(0.76, 0.46, 1.0), 0.24 + 0.22 * tower_t)
            self.halo.opacity = max(self.halo.opacity, 0.10 + 0.06 * tower_t)
            self.halo.radius = self.radius * (2.0 + 0.34 * tower_t)
    ...
    def update(self, dt):
        ...
        if self.tower_level > 0:
            drag += 0.20 * self.tower_level
        self.vel *= max(0.0, 1.0 - drag * dt)

        if self.attached:
            tower_slow = 1.0 / (1.0 + 0.22 * self.tower_level)
            crawl = BASE_SPEED * 0.045 * (0.6 + local_n) * tower_slow
            ...
            tower_slow = 1.0 / (1.0 + 0.28 * self.tower_level)
            speed = BASE_SPEED * (0.25 + 0.92 * local_n) * (1.0 - 0.36 * clamp(local_s)) * tower_slow
...
def reset_simulation(seed_count=START_BACTERIA, from_ai=False):
    ...
    update_tower_structure(1.0, force=True)
...
def update_labels():
    ...
    stats_label.text = (
        ...
        f"Towered: {sum(1 for b in bacteria if getattr(b, 'tower_level', 0) > 0)}\n"
