...
scene = canvas(
    title="3D Bacterial Colony Growth - Sticky Slime Highways",
    width=1200,
    height=780,
...
# Sticky highway field: repeated slime trails reinforce this slower-decaying path map.
HIGHWAY_DIFFUSION = 0.010
HIGHWAY_DECAY = 0.0011
HIGHWAY_REINFORCE = 0.090
HIGHWAY_THRESHOLD = 0.42
HIGHWAY_LIMIT = 240
...
H = [[0.0 for _ in range(GRID)] for _ in range(GRID)]  # reinforced sticky highways
...
def update_tile_colors(force=False):
    ...
    highway_col = vector(1.0, 0.88, 0.30)
    ...
    for i in range(GRID):
        for j in range(GRID):
            ...
            highwayed = mix_color(slimed, highway_col, clamp(H[i][j] * 0.78))
            tiles[i][j].color = highwayed
...
def diffuse_fields(dt):
    ...
    newH = [[H[i][j] for j in range(GRID)] for i in range(GRID)]
    ...
    for i in range(GRID):
        for j in range(GRID):
            ...
            newH[i][j] = clamp(H[i][j] + (avgH - H[i][j]) * HIGHWAY_DIFFUSION * dt * 8 - HIGHWAY_DECAY * dt, 0.0, 1.0)
...
# -----------------------------
# Sticky slime highways
# -----------------------------

def reinforce_highway(start, end, strength=0.18):
    """Repeated trails raise H, a slow-decaying field that later cells can follow."""
    steps = max(2, int(mag(flat(end - start)) / (CELL * 0.45)) + 1)
    for k in range(steps + 1):
        t = k / steps
        p = start * (1 - t) + end * t
        add_to_field(H, p, HIGHWAY_REINFORCE * strength, radius=0.46 + 0.20 * strength, cap=1.0)


def highway_cell_count():
    count = 0
    for i in range(GRID):
        for j in range(GRID):
            if H[i][j] > HIGHWAY_THRESHOLD:
                count += 1
    return count


def add_highway_visual(start, end, intensity):
    if mag(end - start) < 0.05:
        return
    if len(highways) >= HIGHWAY_LIMIT:
        old = highways.pop(0)
        hide_object(old["obj"])
    c = curve(
        pos=[vector(start.x, 0.052, start.z), vector(end.x, 0.052, end.z)],
        radius=0.022 + 0.020 * clamp(intensity),
        color=mix_color(vector(0.20, 0.86, 0.80), vector(1.0, 0.84, 0.24), clamp(intensity)),
    )
    try:
        c.opacity = 0.20 + 0.28 * clamp(intensity)
    except Exception:
        pass
    highways.append({"obj": c, "life": 46.0 + 56.0 * clamp(intensity), "maxlife": 46.0 + 56.0 * clamp(intensity)})


def update_highways(dt):
    alive = []
    for hw in highways:
        hw["life"] -= dt * 0.38
        if hw["life"] <= 0:
            ...
            continue
        try:
            hw["obj"].opacity = max(0.04, 0.40 * hw["life"] / hw["maxlife"])
        except Exception:
            pass
        alive.append(hw)
    highways[:] = alive
...
def add_slime_trail(start, end, strength=0.16):
    ...
    if len(trails) >= TRAIL_LIMIT:
        old = trails.pop(0)
        hide_object(old["obj"])
    reinforce_highway(start, end, strength)
    local_h = sample_field(H, end)
    c = curve(
        pos=[vector(start.x, 0.034, start.z), vector(end.x, 0.034, end.z)],
        radius=0.012 + 0.008 * strength + 0.010 * clamp(local_h),
        color=mix_color(vector(0.20, 0.86, 0.80), vector(1.0, 0.84, 0.24), clamp(local_h))
    )
    try:
    ...
    if local_h > HIGHWAY_THRESHOLD and random.random() < 0.45:
        add_highway_visual(start, end, local_h)
...
class Bacterium:
    ...
    def update(self, dt):
        ...
        if mag(grad_h) > 0.01:
            desired += safe_norm(grad_h) * (0.40 + 0.95 * clamp(local_h))
        ...
        if self.attached:
            crawl = BASE_SPEED * 0.045 * (0.6 + local_n) * (1.0 + 0.30 * clamp(local_h))
            ...
            speed = BASE_SPEED * (0.25 + 0.92 * local_n) * (1.0 - 0.36 * clamp(local_s)) * (1.0 + 0.55 * clamp(local_h))
            self.theta += self.omega * dt
            self.pos += self.forward() * speed * dt + self.vel * dt
            slime_rate = 0.030 + 0.030 * clamp(1.0 - local_n) + 0.018 * clamp(local_h)
        ...
        if self.trail_clock <= 0 and moved > 0.025:
            add_slime_trail(self.prev_pos, self.pos, strength=0.22 + local_s * 0.3 + local_h * 0.55)
    ...
    def perform_mode_action(self, state, dt):
        ...
        if self.mode == "SEED":
            ...
            if self.action_timer <= 0:
                ...
                add_slime_trail(last, self.cursor_pos, strength=0.78)
...
def reset_simulation(seed_count=START_BACTERIA, from_ai=False):
    global bacteria, trails, highways, particles, sim_time, frame_count
...
def update_labels():
    ...
    stats_label.text = (
        ...
        f"Sticky highway cells: {highway_cell_count()}\n"
