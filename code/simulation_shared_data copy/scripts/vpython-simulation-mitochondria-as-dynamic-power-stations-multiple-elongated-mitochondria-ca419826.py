...
# Includes:
# - elongated moving mitochondria
# - fusion / fission
# - fuel particles streaming inward
# - dim waste particles emitted outward
# - ATP level label
# - rule-based + expressive AI controller
# - keyboard control with pause / resume / override
# ------------------------------------------------------------

scene = canvas(
    title="Mitochondria as Dynamic Power Stations - AI Controlled 3D Simulation",
    width=1200,
    height=760,
...
cell_shell = sphere(
    pos=vector(0, 0, 0),
    radius=CELL_RADIUS * 1.008,
    color=vector(0.40, 0.72, 0.95),
    opacity=0.055,
    shininess=0.55
)

nucleus = sphere(
    pos=vector(-2.15, 1.75, -0.45),
    radius=NUCLEUS_RADIUS,
    color=vector(0.75, 0.70, 1.0),
    opacity=0.23,
    shininess=0.4
)

nucleus_label = label(
    pos=nucleus.pos + vector(0, 0, NUCLEUS_RADIUS + 0.35),
    text="stationary nucleus",
    height=11,
    color=vector(0.38, 0.28, 0.7),
    box=False,
    opacity=0
)

atp_label = label(
    pos=vector(-6.4, -6.9, 4.0),
    text="ATP level: 0",
    height=18,
    color=vector(0.05, 0.22, 0.32),
    box=True,
    border=7,
...
mode_label = label(
    pos=vector(2.1, -6.9, 4.0),
    text="AI: starting",
    height=14,
    color=vector(0.10, 0.22, 0.32),
    box=True,
    border=6,
    background=vector(0.96, 1.0, 0.94),
    opacity=0.50
)

help_label = label(
    pos=vector(0, 7.4, 4.8),
    text="Keys: A AI on/off | P pause | R reset | TAB select | arrows move | F split | G fuse | O orbit | D spill | W wrap | M mark | N fuel",
    height=11,
    color=vector(0.08, 0.20, 0.30),
    box=False,
    opacity=0
)
...
def segment_distance_to_point(a, b, p):
    ...
    if denom < 1e-8:
        return mag(p - a)
    t = clamp(dot(p - a, ab) / denom, 0, 1)
    closest = a + t * ab
    return mag(p - closest)
...
def scaled_color(base, factor):
    return vector(
        clamp(base.x * factor, 0, 1),
        clamp(base.y * factor, 0, 1),
        clamp(base.z * factor, 0, 1)
    )


# ------------------------------------------------------------
# Mitochondrion object
# ------------------------------------------------------------
...
class Mitochondrion:
    ...
    def __init__(self, pos=None, axis=None, length=None, radius=None, activity=None):
        ...
        self.carefulness = random.uniform(0.6, 1.4)

        self.outer_base = vector(1.0, 0.50, 0.16)
        self.inner_base = vector(1.0, 0.86, 0.30)

        self.body = cylinder(
            pos=self.pos - self.axis * self.length * 0.5,
            axis=self.axis * self.length,
            radius=self.radius,
            color=self.outer_base,
            opacity=0.62,
            shininess=0.75
        )
        ...
        self.inner_curve = curve(
            pos=self._inner_curve_points(),
            radius=max(0.018, self.radius * 0.045),
            color=self.inner_base,
            opacity=0.82
        )

        self.marker_ring = ring(
            pos=self.pos,
            axis=self.axis,
        ...
        self.wrapper = helix(
            pos=self.pos - self.axis * self.length * 0.55,
            axis=self.axis * self.length * 1.10,
            radius=self.radius * 1.22,
            coils=5,
            thickness=0.018,
            color=vector(0.12, 0.75, 0.55),
            opacity=0.0
        )
    ...
    def endpoints(self):
        return (
            self.pos - self.axis * self.length * 0.5,
            self.pos + self.axis * self.length * 0.5
        )
    ...
    def contains_point(self, p, extra=0.0):
        return self.distance_to_point(p) <= self.radius + extra
    ...
    def update(self, dt, t):
        ...
        # Activity flicker shows changing power station output
        ...
        # Gentle self-motion
        swim = vector(
            math.sin(t * 0.43 + self.phase),
            math.cos(t * 0.37 + self.phase * 0.7),
            math.sin(t * 0.31 + self.phase * 1.3)
        ) * 0.018

        self.vel += swim
        self.vel += self.ai_push * dt
        self.ai_push *= 0.86
        ...
        if self.orbiting:
            ...
            self.vel += tangent * 0.075
            self.vel += -safe_norm(radial) * 0.026
    ...
    def _update_visuals(self, visible_activity):
        ...
        self.glow.color = vector(1.0, 0.75 + 0.12 * clamp(visible_activity, 0, 1), 0.24)
...
class Particle:
    ...
    def __init__(self, kind="fuel", source=None, target=None):
        ...
        self.obj = sphere(
            ...
            emissive=(self.kind == "fuel"),
            make_trail=True,
            retain=18 if self.kind == "fuel" else 9,
            trail_radius=self.radius * 0.36,
    ...
    def choose_target(self):
        ...
        if not living:
            self.target = None
            return
        weighted = sorted(living, key=lambda m: mag(m.pos - self.pos) - m.activity * 1.3)
        self.target = weighted[0]
    ...
    def update(self, dt, t):
        ...
        if self.kind == "fuel":
            ...
            self.vel += safe_norm(self.pos, random_unit_vector()) * 0.006
            self.vel += random_unit_vector() * 0.006
            self.pos += self.vel * dt
            self.vel *= 0.992
...
def split_mitochondrion(m):
    if m is None or not m.alive:
        return None
    if len(living_mitochondria()) >= MAX_MITOCHONDRIA:
        return None
    if m.length < 1.05:
        return None
    ...
    parent_pos = m.pos - split_dir * (m.radius * 1.25)

    m.length = parent_length
    m.pos = parent_pos
    m.radius *= random.uniform(0.96, 1.02)
    m.activity = clamp(m.activity * 0.86, 0.05, 1.6)
    m.atp *= 0.62
    m.vel += -split_dir * 0.28

    child = spawn_mitochondrion(
        pos=child_pos,
        axis=safe_norm(rotate(m.axis, angle=random.uniform(-0.55, 0.55), axis=split_dir)),
        length=child_length,
        radius=m.radius * random.uniform(0.90, 1.02),
        activity=clamp(m.activity * random.uniform(0.75, 1.1), 0.05, 1.4)
    )
...
def merge_mitochondria(a, b):
    ...
    combined_activity = clamp((a.activity + b.activity) * 0.58 + 0.08, 0.05, 1.8)
    ...
    heavier.activity = combined_activity
    ...
    for i in range(len(living)):
        for j in range(i + 1, len(living)):
            ...
            if d < min_d:
                ...
                should_merge = (
                    d < (a.radius + b.radius) * 1.05 and
                    len(living_mitochondria()) > MIN_MITOCHONDRIA and
                    (a.marked or b.marked or a.wrapped or b.wrapped or random.random() < 0.012)
                )
                if should_merge:
...
class MitoAIController:
    ...
    def detect_stagnation_or_completion(self, state, dt):
        snapshot = (
            round(state["atp"], 1),
            state["mitochondria_count"],
            state["fuel_count"],
            state["waste_count"],
            round(state["avg_activity"], 2),
            state["close_pairs"]
        )
        ...
        change = (
            abs(snapshot[0] - self.last_snapshot[0]) +
            abs(snapshot[1] - self.last_snapshot[1]) * 12 +
            abs(snapshot[2] - self.last_snapshot[2]) * 0.35 +
            abs(snapshot[3] - self.last_snapshot[3]) * 0.25 +
            abs(snapshot[4] - self.last_snapshot[4]) * 20 +
            abs(snapshot[5] - self.last_snapshot[5]) * 3
        )
        ...
        complete = (
            state["atp"] > ATP_COMPLETE_LEVEL or
            state["mitochondria_count"] <= 0 or
            (
                state["fuel_count"] <= 0 and
                state["atp"] < ATP_EMPTY_LEVEL and
                sim_time - last_reset_time > 8
            )
        )
    ...
    def choose_mode(self, state, force=False):
        ...
        candidates = [c for c in candidates if recent.count(c) < 2]
        if not candidates:
            candidates = self.behavior_modes[:]
    ...
    def update(self, dt):
        ...
        if self.mode == "NOURISH":
            self.perform_nourish(state)
        elif self.mode == "FUSION":
            self.perform_fusion(state)
        elif self.mode == "FISSION":
            self.perform_fission(state)
        elif self.mode == "ORBIT":
            self.perform_orbit(state)
        elif self.mode == "ORGANIZE":
            self.perform_organize(state)
        elif self.mode == "MARK_AND_WRAP":
            self.perform_mark_and_wrap(state)
        elif self.mode == "SPILL":
            self.perform_spill(state)
        elif self.mode == "CHAOS":
            self.perform_chaos(state)
        elif self.mode == "CALM":
            self.perform_calm(state)
        elif self.mode == "ARTIST":
            self.perform_artist(state)

        mode_label.text = (
            f"AI: {self.mode} | stagnation {self.stagnation_time:0.1f}s | "
            f"override {'on' if human_override_timer > 0 else 'off'}"
        )
    ...
    def perform_nourish(self, state):
        ...
        hungry = sorted(living, key=lambda m: m.atp + m.activity * 8)
        target = hungry[0]
    ...
    def perform_fusion(self, state):
        ...
        for m in pair:
            m.mark(True)
            m.wrap(True)
            direction = safe_norm(center - m.pos, random_unit_vector())
            m.add_force(direction * 2.8)
    ...
    def perform_fission(self, state):
        ...
        candidates = sorted(living, key=lambda m: m.length + m.atp * 0.01, reverse=True)
        target = candidates[0]
        target.mark(True)
        target.wrap(False)
    ...
    def perform_orbit(self, state):
        ...
        if sim_time - self.last_action_time > 0.65:
    ...
    def perform_organize(self, state):
        ...
        for i, m in enumerate(living):
            ...
            z = 0.9 * math.sin(angle * 2 + sim_time * 0.35)
            target = vector(ring_radius * math.cos(angle), ring_radius * math.sin(angle), z)
            m.organize_target = target
    ...
    def perform_spill(self, state):
        ...
        energetic = sorted(living, key=lambda m: m.activity + m.atp * 0.01, reverse=True)
        target = energetic[0]
        target.mark(True)
        target.dipping = True
        ...
        for m in energetic[1:]:
            m.dipping = random.random() < 0.12
    ...
    def perform_chaos(self, state, strong=False):
        ...
        if sim_time - self.last_action_time > 0.55:
    ...
    def perform_calm(self, state):
        ...
        if state["fuel_count"] < 12 and sim_time - self.last_action_time > 1.5:
            spawn_fuel(2, target=random.choice(living) if living else None)
            self.last_action_time = sim_time
...
while True:
    ...
    # Occasional natural fission/fusion dynamics even without direct AI
    ...
    # Ensure minimum life in the loop
    ...
    # Selection ring if human selected object exists
    sm = selected_mito()
