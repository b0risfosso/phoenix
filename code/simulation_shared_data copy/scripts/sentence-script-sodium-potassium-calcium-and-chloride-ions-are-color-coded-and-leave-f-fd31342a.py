...
# Scene styling
# -----------------------------
scene.title = "Cell Membrane with Na+, K+, Ca2+, Cl- Ions, Fading Trails, Channels, and AI Controller"
scene.width = 1200
scene.height = 760
...
Controls:
  ...
  T = toggle fading ion trails  P = spill/spawn mixed ions
...
# Ion species colors. Each ion now has a stable biological identity,
# charge, radius, color, and fading trail color.
ION_SPECIES = {
    "Na+": {
        "name": "Sodium",
        "charge": 1,
        "color": vector(1.0, 0.78, 0.18),
        "trail": vector(1.0, 0.86, 0.30),
        "radius": ION_RADIUS * 1.00,
    },
    "K+": {
        "name": "Potassium",
        "charge": 1,
        "color": vector(0.55, 0.22, 1.0),
        "trail": vector(0.68, 0.42, 1.0),
        "radius": ION_RADIUS * 1.08,
    },
    "Ca2+": {
        "name": "Calcium",
        "charge": 2,
        "color": vector(1.0, 0.34, 0.20),
        "trail": vector(1.0, 0.48, 0.32),
        "radius": ION_RADIUS * 1.18,
    },
    "Cl-": {
        "name": "Chloride",
        "charge": -1,
        "color": vector(0.18, 0.70, 1.0),
        "trail": vector(0.28, 0.82, 1.0),
        "radius": ION_RADIUS * 1.04,
    },
}
ION_SPECIES_ORDER = ["Na+", "K+", "Ca2+", "Cl-"]
...
# Ion species legend. The colored spheres make the code usable without
# remembering the mapping while the simulation is running.
legend_title = label(
    pos=vector(7.15, 3.45, 0),
    text="Ion colors",
    height=11,
    color=vector(0.10, 0.18, 0.26),
    box=False,
    opacity=0
)
legend_items = []
for i, symbol in enumerate(ION_SPECIES_ORDER):
    info = ION_SPECIES[symbol]
    y = 3.05 - i * 0.46
    swatch = sphere(
        pos=vector(6.72, y, 0),
        radius=0.09,
    ...
    legend_items.append((swatch, lbl))
...
# -----------------------------
class Ion:
    def __init__(self, pos, species=None, charge=None):
        self.species = species if species in ION_SPECIES else pyrandom.choice(ION_SPECIES_ORDER)
        self.info = ION_SPECIES[self.species]
        self.charge = charge if charge is not None else self.info["charge"]
        self.base_radius = self.info["radius"]
        self.base_color = self.info["color"]
        self.trail_color = self.info["trail"]
        self.pos = vector(pos)
        ...
        # VPython trails do not support per-point alpha fading, but retain creates
        # a fading tail behavior: older trail points are dropped as the ion moves.
        # Shorter trails make fast ions look like they dissolve into the fluid.
        self.trail_retain = pyrandom.randint(55, 95)
        self.trail_radius = 0.012 + 0.004 * abs(self.charge)
        ...
        self.label = label(
            pos=self.pos + vector(0, self.base_radius * 2.6, 0),
            text=self.species,
            height=6,
            color=self.base_color,
            box=False,
    ...
    def apply_species_visual(self):
        if self.marked:
            # Keep the ion's chemical identity visible, but make active/AI-marked
            # ions glow and grow instead of changing them all to one color.
            self.obj.color = self.base_color * 0.70 + ION_MARK_COLOR * 0.30
            self.obj.trail_color = self.trail_color * 0.65 + ION_MARK_COLOR * 0.35
            self.obj.radius = self.base_radius * 1.22
            self.obj.emissive = True
            self.label.color = ION_MARK_COLOR
        else:
            self.obj.color = self.base_color
            self.obj.trail_color = self.trail_color
            self.obj.radius = self.base_radius
            self.obj.emissive = False
            self.label.color = self.base_color

    def set_marked(self, value=True):
        self.marked = bool(value)
        self.apply_species_visual()
    ...
    def update(self, dt):
        ...
        if self.attached_channel is not None:
            ...
            self.label.pos = self.pos + vector(0, self.base_radius * 2.6, 0)
...
# Ion spawning/reset
# -----------------------------
def choose_ion_species():
    # Weighted distribution gives all four ion types a constant visual presence.
    # Calcium is less common but more visually prominent because of its 2+ charge.
    r = pyrandom.random()
    if r < 0.30:
        return "Na+"
    if r < 0.58:
...
def create_ion(distribution="split", species=None):
    ...
    ion = Ion(pos, species=species or choose_ion_species())
...
def update_meter_and_labels():
    ...
    species_counts = {symbol: 0 for symbol in ION_SPECIES_ORDER}
    inside_species_counts = {symbol: 0 for symbol in ION_SPECIES_ORDER}
    outside_species_counts = {symbol: 0 for symbol in ION_SPECIES_ORDER}
    for ion in ions:
        species_counts[ion.species] = species_counts.get(ion.species, 0) + 1
        if ion.is_inside():
            inside_species_counts[ion.species] = inside_species_counts.get(ion.species, 0) + 1
        else:
            outside_species_counts[ion.species] = outside_species_counts.get(ion.species, 0) + 1

    species_line = "  ".join(f"{s}:{species_counts.get(s, 0)}" for s in ION_SPECIES_ORDER)
    meter_count_label.text = f"Ions: {len(ions)}\nIn {inside} | Out {outside}\n{species_line}"

    open_count = sum(1 for ch in channels if ch.open)
    dominant_inside = max(ION_SPECIES_ORDER, key=lambda s: inside_species_counts.get(s, 0)) if ions else "-"
    dominant_outside = max(ION_SPECIES_ORDER, key=lambda s: outside_species_counts.get(s, 0)) if ions else "-"
    status_label.text = (
        f"Round {round_number}   Inside: {inside} ({dominant_inside})   Outside: {outside} ({dominant_outside})   "
