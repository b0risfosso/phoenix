...
#   P     : trigger a surrounding pressure pulse
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.drift_bias = 1.0
        self.environment_pressure_strength = random.uniform(0.28, 0.52)
        self.environment_pressure_phase = random.uniform(0, math.tau)
        self.environment_pressure_angle = random.uniform(0, math.tau)
        self.environment_pressure_manual = 0.0
        self.environment_pressure_level = 0.0
        self.environment_pressure_vector = vector(0, 1, 0)
    ...
    def reset_round(self, full=False):
        ...
        # Surrounding environment pressure field. These objects stay outside the
        # cell and move inward/outward as the external pressure changes.
        self.pressure_pad_a = box(
            pos=vector(0, 2.25, 0),
            size=vector(4.6, 0.08, 1.45),
            color=vector(0.95, 0.54, 0.38),
            opacity=0.16
        )
        self.pressure_pad_b = box(
            pos=vector(0, -2.25, 0),
            size=vector(4.6, 0.08, 1.45),
            color=vector(0.95, 0.54, 0.38),
            opacity=0.16
        )
        self.pressure_arrow_a = cylinder(
            pos=vector(0, 2.60, 0),
            axis=vector(0, -0.42, 0),
            radius=0.035,
            color=vector(1.0, 0.40, 0.22),
            opacity=0.34
        )
        self.pressure_arrow_b = cylinder(
            pos=vector(0, -2.60, 0),
            axis=vector(0, 0.42, 0),
            radius=0.035,
            color=vector(1.0, 0.40, 0.22),
        ...
        self.pressure_ring = ring(
            pos=vector(0, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.82,
            thickness=0.026,
            color=vector(1.0, 0.46, 0.24),
            opacity=0.0
        )
        self.pressure_label = label(
            pos=vector(0, 2.42, 0),
            text="environment pressure",
            box=False,
            opacity=0,
            color=vector(0.62, 0.20, 0.12),
            height=11
        )
        for obj in [
            self.pressure_pad_a, self.pressure_pad_b,
            self.pressure_arrow_a, self.pressure_arrow_b,
            self.pressure_ring, self.pressure_label
        ]:
            self.register(obj)
    ...
    def compute_environment_pressure(self, dt):
        """Return a visible external pressure field for the current frame.

        The pressure field is intentionally external to the cell. It changes
        direction slowly, pulses in strength, and can be manually amplified with
        the P key. The resulting vector is used to squeeze the cell in one
        cross-sectional direction while the perpendicular direction bulges.
        """
        if self.environment_pressure_manual > 0:
            self.environment_pressure_manual = max(0, self.environment_pressure_manual - dt * 0.33)

        t = self.progress
        active_u = smoothstep(0.04, 0.30, t) * (1 - 0.28 * smoothstep(0.88, 1.0, t))
        wave = 0.5 + 0.5 * math.sin(self.time_seconds * 0.92 + self.environment_pressure_phase)
        tremor = 0.5 + 0.5 * math.sin(self.time_seconds * 2.15 + self.environment_pressure_phase * 0.37)
        manual = self.environment_pressure_manual
        level = clamp((0.12 + self.environment_pressure_strength * (0.45 + 0.55 * wave) + 0.10 * tremor) * active_u + manual, 0, 1)

        angle = self.environment_pressure_angle + 0.55 * math.sin(self.time_seconds * 0.31 + self.environment_pressure_phase)
        pressure_dir = safe_norm(vector(0, math.cos(angle), math.sin(angle)), vector(0, 1, 0))
        bulge_dir = safe_norm(vector(0, -pressure_dir.z, pressure_dir.y), vector(0, 0, 1))

        self.environment_pressure_level = level
        self.environment_pressure_vector = pressure_dir

        # Move pressure pads and arrows so the source of deformation is visible.
        distance = lerp(2.90, 2.08, level)
        pad_span = lerp(1.35, 2.05, level)
        pad_opacity = 0.10 + 0.28 * level
        arrow_len = 0.32 + 0.82 * level
        arrow_radius = 0.024 + 0.038 * level

        for sign, pad, arrow in [
            (1, self.pressure_pad_a, self.pressure_arrow_a),
            (-1, self.pressure_pad_b, self.pressure_arrow_b)
        ]:
            pad.pos = pressure_dir * (sign * distance)
            pad.size = vector(4.65, 0.08 + 0.05 * level, pad_span)
            pad.axis = bulge_dir
            pad.opacity = pad_opacity

            arrow.pos = pressure_dir * (sign * (distance + 0.34))
            arrow.axis = pressure_dir * (-sign * arrow_len)
            arrow.radius = arrow_radius
            arrow.opacity = 0.18 + 0.50 * level

        self.pressure_ring.axis = pressure_dir
        self.pressure_ring.radius = lerp(1.74, 1.34, level)
        self.pressure_ring.thickness = lerp(0.018, 0.052, level)
        self.pressure_ring.opacity = 0.08 + 0.42 * level
        self.pressure_label.visible = self.show_labels
        self.pressure_label.pos = pressure_dir * (distance + 0.58)
        self.pressure_label.text = f"environment pressure {int(level * 100)}%"

        return level, pressure_dir, bulge_dir

    def trigger_pressure_pulse(self):
        self.environment_pressure_manual = clamp(self.environment_pressure_manual + 0.55, 0, 1)
        self.environment_pressure_angle += random.uniform(-0.75, 0.75)
        self.mark(
            self.environment_pressure_vector * 1.45,
            "external pressure pulse",
            vector(1.0, 0.42, 0.22),
            life=2.3
        )
    ...
    def update(self, dt):
        ...
        pressure_level, pressure_dir, bulge_dir = self.compute_environment_pressure(dt)

        # External compression slows division slightly when symmetry is highly disturbed.
        pressure_slowdown = lerp(1.0, 0.62, smoothstep(0.55, 0.95, pressure_level))
        speed = self.base_progress_speed * self.speed_modifier * self.human_speed_bias * pressure_slowdown
        if self.progress < 1:
        ...
        base_y = 3.05 - 0.18 * stretch
        base_z = 3.05 - 0.18 * stretch
        # Pressure squeezes the cross-section in the applied direction and lets
        # the perpendicular side bulge. Direction changes make the cell lose
        # perfect symmetry instead of simply scaling uniformly.
        y_squeeze = 1 - 0.24 * pressure_level * abs(pressure_dir.y) + 0.10 * pressure_level * abs(bulge_dir.y)
        z_squeeze = 1 - 0.24 * pressure_level * abs(pressure_dir.z) + 0.10 * pressure_level * abs(bulge_dir.z)
        pressure_skew = pressure_dir * (0.10 * pressure_level * (1 - 0.45 * split))
        self.single_cell.pos = pressure_skew
        self.single_membrane.pos = pressure_skew * 1.08
        self.single_cell.size = vector(cell_x * (1 + 0.05 * pressure_level), base_y * y_squeeze, base_z * z_squeeze)
        self.single_membrane.size = vector((cell_x + 0.1) * (1 + 0.05 * pressure_level), (base_y + 0.1) * y_squeeze, (base_z + 0.1) * z_squeeze)
        ...
        daughter_offset = pressure_dir * (0.22 * pressure_level * split) + bulge_dir * (0.10 * math.sin(self.time_seconds * 1.3) * pressure_level * split)
        left_asym = 1 - 0.12 * pressure_level * pressure_dir.y + 0.05 * math.sin(self.time_seconds * 1.4 + 0.5) * pressure_level
        right_asym = 1 + 0.12 * pressure_level * pressure_dir.y - 0.05 * math.sin(self.time_seconds * 1.4 + 0.5) * pressure_level
        left_size = vector(
            2 * daughter_radius * (1 + 0.05 * pressure_level),
            2 * daughter_radius * clamp(left_asym * (1 - 0.14 * pressure_level * abs(pressure_dir.y)), 0.72, 1.28),
            2 * daughter_radius * clamp((1 / max(0.75, left_asym)) * (1 - 0.14 * pressure_level * abs(pressure_dir.z)), 0.72, 1.28)
        )
        right_size = vector(
            2 * daughter_radius * (1 + 0.05 * pressure_level),
            2 * daughter_radius * clamp(right_asym * (1 - 0.14 * pressure_level * abs(pressure_dir.y)), 0.72, 1.28),
            2 * daughter_radius * clamp((1 / max(0.75, right_asym)) * (1 - 0.14 * pressure_level * abs(pressure_dir.z)), 0.72, 1.28)
        )
        self.left_daughter.pos = vector(-daughter_d, 0, 0) + daughter_offset
        self.right_daughter.pos = vector(daughter_d, 0, 0) - daughter_offset * 0.72
        self.left_daughter.size = left_size
        self.right_daughter.size = right_size
        ...
        self.bridge.pos = vector(-0.16, 0, 0) + pressure_dir * (0.06 * pressure_level * split)
        self.bridge.axis = vector(0.32, 0, 0)
        self.bridge.radius = lerp(1.28, 0.14, pinch)
        self.bridge.opacity = 0.18 * bridge_op

        self.cleavage_ring.pos = pressure_dir * (0.05 * pressure_level * split)
        self.cleavage_ring.radius = lerp(1.47, 0.20, pinch) * (1 - 0.10 * pressure_level)
        ...
        self.left_nucleus.pos = self.left_daughter.pos
        self.right_nucleus.pos = self.right_daughter.pos
        self.left_nucleus.opacity = 0.14 * nuc_op
        self.right_nucleus.opacity = 0.14 * nuc_op

        pole_skew = pressure_dir * (0.12 * pressure_level * smoothstep(0.12, 0.62, t))
        self.left_pole_obj.pos = left_pole + pole_skew
        self.right_pole_obj.pos = right_pole - pole_skew * 0.75
        left_pole = self.left_pole_obj.pos
        right_pole = self.right_pole_obj.pos
        ...
        for c in self.chromosomes:
            c.update(
                ...
                self.ai_noise + 0.06 * pressure_level,
                self.pull_bias
            )
            if pressure_level > 0.18 and t < 0.86:
                # Project a small external-pressure displacement onto chromosomes.
                # This makes pressure visibly change symmetry without overriding
                # the mitosis stage targets.
                asym = math.sin(self.time_seconds * 1.6 + c.index * 0.7)
                offset = pressure_dir * (0.035 * pressure_level * asym) + bulge_dir * (0.020 * pressure_level * math.cos(self.time_seconds + c.index))
                c.left.pos += offset
                c.right.pos += offset * 0.85
                c.connector.pos = c.left.pos
                c.connector.axis = c.right.pos - c.left.pos
        ...
        pressure_state = "symmetric" if pressure_level < 0.28 else ("compressed" if pressure_level < 0.62 else "distorted")
        self.stage_label.text = (
            f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  "
            f"environment pressure {int(pressure_level * 100)}% ({pressure_state})"
        )
...
class AIController:
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if mode == "OBSERVE":
            ...
            sim.ai_noise = lerp(sim.ai_noise, 0.025 + 0.035 * sim.environment_pressure_level, 0.05)
            if sim.environment_pressure_level > 0.68:
                sim.speed_modifier = lerp(sim.speed_modifier, 0.78, 0.06)
                sim.constriction_bias = lerp(sim.constriction_bias, 0.95, 0.05)
            if self.action_timer > 1.9:
                self.action_timer = 0
                sim.spill_particles(8, source=vector(0, 0, 0), mood="ritual")
                if random.random() < 0.35:
                    sim.trigger_pressure_pulse()
                sim.mark(vector(0, 0.95, 0), "furrow adapts to pressure", vector(0.10, 0.58, 0.72), life=2.6)
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | W wrap | P pressure | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
