...
if _csv_output_dir:
    ...
    CSV_OUTPUT_PATH = os.environ.get(
        "SIM_STATE_CSV_PATH",
        os.path.join(_default_script_dir, f"{_csv_run_id}-simulation-state.csv")
    )
...
scene.title = "AI Cell Membrane Transport and Vesicle Traffic"
scene.width = 1250
scene.height = 760
...
scene.range = 9.2
scene.caption = "\nKeyboard: I toggle AI | P pause | R reset | Space wrap vesicle | O pulse nearest channel | M mark | X spill | Arrows/WASD/QE move director\n"
...
mtoc_label = label(pos=mtoc_pos + vector(0, 0.55, 0), text="MTOC", height=10, color=vector(0.1, 0.25, 0.6), box=False, opacity=0)
...
for i, offset in enumerate([-0.28, 0, 0.28]):
    decorations.append(cylinder(
        pos=organelle_list[1].pos + vector(-0.55, offset, -0.18),
        axis=vector(1.1, 0.12 * math.sin(i), 0.22),
        radius=0.045,
        color=vector(1.0, 0.52, 0.15),
        opacity=0.75
    ))


class MembraneChannel:
    def __init__(self, direction, channel_color):
...
for d, c in zip(channel_dirs, channel_colors):
    ch = MembraneChannel(d, c)
    channels.append(ch)

for i, ch in enumerate(channels):
    filament = curve(
        pos=[mtoc_pos, ch.dir * (CELL_R - 0.65)],
        radius=0.023,
        color=vector(0.12, 0.43, 0.92),
        opacity=0.44
    )
    microtubules.append(filament)

for org in organelle_list:
    filament = curve(
        pos=[mtoc_pos, (mtoc_pos + org.pos) * 0.5 + vector(0.18, 0.08, -0.1), org.pos],
        radius=0.032,
        color=vector(0.1, 0.38, 0.9),
        opacity=0.56
    )
    microtubules.append(filament)
...
class Molecule:
    def __init__(self, pos, vel, kind, base_color, inside=True):
        ...
        self.obj = sphere(
            pos=pos,
            radius=MOLECULE_R * randf(0.86, 1.24),
            color=base_color,
            opacity=0.93,
            shininess=0.7
        )
    ...
    def update(self, dt):
        ...
        if self.inside:
            ...
            if r > CELL_R - self.obj.radius:
                ...
                moving_out = dot(self.vel, n) > 0
                if ch and ch.open and dch < ch.radius * 1.7 and moving_out:
                    self.inside = False
            ...
            if r < CELL_R + self.obj.radius:
                n = norm(self.pos)
                ch, dch = self.channel_nearby()
                moving_in = dot(self.vel, n) < 0
                if ch and ch.open and dch < ch.radius * 1.7 and moving_in:
                    self.inside = True
                    self.pos = n * (CELL_R - self.obj.radius - 0.08)
...
class Vesicle:
    def __init__(self, start_pos, cargo, target, export_dir=None):
        ...
        self.obj = sphere(
            pos=self.pos,
            radius=self.radius,
            color=vector(0.25, 0.84, 1.0) if isinstance(target, Organelle) else vector(1.0, 0.55, 0.25),
            opacity=0.27,
            shininess=0.8,
            make_trail=True,
            trail_color=vector(0.32, 0.74, 0.95),
            retain=95
        )
        self.rim_obj = ring(
            pos=self.pos,
    ...
    def detach_all(self, violent=False):
        for mol in self.cargo:
            if violent:
                mol.detach(random_unit() * randf(0.8, 1.7))
            else:
                mol.detach(random_unit() * randf(0.2, 0.7))
        self.cargo = []
...
def make_vesicle_at(direction=None, target=None, prefer_kind=None, max_cargo=7):
    ...
    candidates = []
    for mol in molecules:
        if mol.state == "free" and mol.inside and mag(mol.pos - start) < 1.75:
            if prefer_kind is None or mol.kind == prefer_kind:
    ...
    if target is None:
        export_dir = direction
        ves = Vesicle(start, cargo, "membrane", export_dir=export_dir)
    else:
        ves = Vesicle(start, cargo, target)
...
class AIController:
    def __init__(self):
        ...
        self.mode_label = label(
            pos=vector(0, -7.3, 0),
            text="AI mode: constructive_delivery",
            height=12,
            color=vector(0.1, 0.22, 0.38),
            box=False,
            opacity=0
        )
    ...
    def choose_new_mode(self, reason="time"):
        ...
        if state["free_inside"] < 7:
            ...
            options = [m for m in self.behavior_modes if m != old]
            self.mode = pyrandom.choice(options)
    ...
    def update_director_arrow(self):
        ...
        if mag(director_arrow.axis) < 0.01:
            director_arrow.axis = vector(0.6, 0, 0)
    ...
    def update(self, dt):
        ...
        if self.mode == "careful_import":
            ...
            for mol in molecules:
                if mol.state == "free":
                    mol.vel += cross(norm(mol.pos) if mag(mol.pos) > 0.01 else random_unit(), norm(swirl)) * 0.035
            ...
            flower = vector(
                math.cos(self.orbit_phase) * (3.6 + 0.5 * math.sin(4 * self.orbit_phase)),
                math.sin(self.orbit_phase * 2.0) * 1.8,
                math.sin(self.orbit_phase) * (3.6 + 0.5 * math.cos(3 * self.orbit_phase))
            )
            ...
            for ch in channels:
                ch.pulse(0.32)
            for mol in molecules:
                if mol.state == "free" and mol.inside:
                    mol.vel += norm(mol.pos if mag(mol.pos) > 0.01 else random_unit()) * 0.026
            if self.cooldown <= 0:
                spill_near_director()
...
def reset_simulation(looping=False):
    ...
    for mol in molecules:
        mol.obj.visible = False
    for ves in vesicles:
        ves.obj.visible = False
        ves.rim_obj.visible = False
        if ves.path_curve:
            ves.path_curve.visible = False
...
csv_writer.writerow([
    "run_id", "frame", "elapsed_seconds", "sim_time", "round_number",
    "ai_enabled", "ai_mode", "paused",
    "director_x", "director_y", "director_z",
    "active_channels", "molecules_free_inside", "molecules_free_outside", "molecules_attached",
    "vesicles_alive",
    "first_vesicle_state", "first_vesicle_x", "first_vesicle_y", "first_vesicle_z",
    "first_molecule_kind", "first_molecule_inside", "first_molecule_state",
    "first_molecule_x", "first_molecule_y", "first_molecule_z",
    "first_molecule_vx", "first_molecule_vy", "first_molecule_vz",
    "passes", "bounces", "collisions", "buds", "fusions", "transfers", "marks", "spills", "detaches", "resets",
    "nucleus_absorbed", "golgi_absorbed", "lysosome_absorbed", "er_absorbed"
])
csv_file.flush()
...
try:
    while True:
        ...
        if sim_time - last_csv_log >= 0.25 or frame == 1:
            active_channels = sum(1 for ch in channels if ch.open)
            free_inside = sum(1 for m in molecules if m.state == "free" and m.inside)
            free_outside = sum(1 for m in molecules if m.state == "free" and not m.inside)
            attached = sum(1 for m in molecules if m.state == "attached")
            alive_ves = [v for v in vesicles if v.state != "dead"]
            v0 = alive_ves[0] if alive_ves else None
            m0 = molecules[0] if molecules else None

            csv_writer.writerow([
                _csv_run_id, frame, round(now_elapsed, 4), round(sim_time, 4), round_number,
                int(ai_enabled), ai.mode, int(paused),
                round(director.pos.x, 4), round(director.pos.y, 4), round(director.pos.z, 4),
                active_channels, free_inside, free_outside, attached,
                len(alive_ves),
                v0.state if v0 else "",
                round(v0.pos.x, 4) if v0 else "",
                round(v0.pos.y, 4) if v0 else "",
                round(v0.pos.z, 4) if v0 else "",
                m0.kind if m0 else "",
                int(m0.inside) if m0 else "",
                m0.state if m0 else "",
                round(m0.pos.x, 4) if m0 else "",
                round(m0.pos.y, 4) if m0 else "",
                round(m0.pos.z, 4) if m0 else "",
                round(m0.vel.x, 4) if m0 else "",
                round(m0.vel.y, 4) if m0 else "",
                round(m0.vel.z, 4) if m0 else "",
                counts["passes"], counts["bounces"], counts["collisions"], counts["buds"], counts["fusions"],
                counts["transfers"], counts["marks"], counts["spills"], counts["detaches"], counts["resets"],
                organelle_list[0].absorbed, organelle_list[1].absorbed, organelle_list[2].absorbed, organelle_list[3].absorbed
            ])
            csv_rows_since_flush += 1
            if csv_rows_since_flush >= 12:
                csv_file.flush()
                csv_rows_since_flush = 0
            last_csv_log = sim_time

        if now_elapsed >= CSV_RUN_SECONDS:
            csv_file.flush()
            csv_label.text = f"CSV recording complete: saved run data\n{CSV_OUTPUT_PATH}"
            break
