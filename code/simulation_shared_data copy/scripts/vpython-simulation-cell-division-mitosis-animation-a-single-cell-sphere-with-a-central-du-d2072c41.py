...
class ChromosomePair:
    def __init__(self, sim, index, base_angle, radius_ring):
        ...
        self.left = sphere(
            ...
            shininess=0.55,
            make_trail=True,
            trail_color=vector(0.88, 0.45, 0.95),
            retain=35
        )
        self.right = sphere(
            pos=self.central_pos + vector(0.08, 0, 0),
            ...
            shininess=0.55,
            make_trail=True,
            trail_color=vector(0.62, 0.50, 0.98),
            retain=35
        )
    ...
    def update(self, t, time_seconds, left_pole, right_pole, ai_noise=0.0, pull_bias=1.0):
        ...
        final_left = self.left_final + vector(-0.42 * telophase, 0, 0)
        final_right = self.right_final + vector(0.42 * telophase, 0, 0)
...
class VesicleParticle:
    def __init__(self, sim, pos, vel, radius=0.045, color_value=None, life=8.0):
        ...
        self.obj = sphere(
            ...
            shininess=0.35,
            make_trail=True,
            trail_color=vector(0.70, 0.90, 1.0),
            retain=20
        )
...
class MitosisSimulation:
    ...
    def update(self, dt):
        ...
        for c in self.chromosomes:
            c.update(t, self.time_seconds, left_pole, right_pole, self.ai_noise, self.pull_bias)
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | W wrap | C camera | L labels | X override",
    box=False,
    opacity=0,
