...
SHARED_DATA_DIR = Path(os.environ.get("SIMULATION_SHARED_DATA_DIR") or os.environ.get("SHARED_DATA_DIR") or (APP_DIR.parent / "simulation_shared_data")).expanduser().resolve()
...
def get_node_scripts(node: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for record in node.get("scripts", []):
        ...
        try:
            ...
            scripts.append({"filename": record.get("filename", ""), "missing": True, "note": record.get("note", "")})
    return scripts
...
def api_data_location():
    return jsonify({"ok": True, "shared_data_dir": str(SHARED_DATA_DIR), "scripts_dir": str(SCRIPTS_DIR), "metadata_dir": str(METADATA_DIR), "logs_dir": str(LOGS_DIR), "tree_file": str(TREE_FILE)})
...
def api_node(node_id: str):
    try:
        ...
        return jsonify({"ok": True, "node": node_summary(node), "parent": parent, "children": children, "scripts": get_node_scripts(node), "path": path, "story": story_text_from_path(path)})
    except Exception as exc:
...
def api_create_root_node():
    try:
        ...
        tree["nodes"][node_id] = {"id": node_id, "kind": "core_sentence", "sentence": sentence, "title": title or "Core sentence", "parent_id": None, "children": [], "scripts": [], "created_at": now_iso(), "updated_at": now_iso()}
        save_tree(tree)
...
def api_add_next_sentence(node_id: str):
    try:
        ...
        child = {"id": child_id, "kind": "core_sentence", "sentence": sentence, "title": title or "Next sentence branch", "parent_id": node_id, "children": [], "scripts": [], "created_at": now_iso(), "updated_at": now_iso()}
