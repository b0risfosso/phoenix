...
scene.title = "Multi-Predator Fish School in a Reef - Split/Reform Schooling AI Reef Simulation"
...
class PreyFish:
    ...
    def update(self, dt, prey_list, predators):
        ...
        if pred_dist < 8.0:
            ...
            acceleration += flee * (9.5 + 7.5 * self.fear + 3.0 * pack_fear)
        ...
        if pred_dist < PREDATOR_RADIUS + PREY_RADIUS + 0.23:
            self.mark_caught(nearest_threat)
...
def secondary_predator_accel(pr, idx, active_prey):
    ...
    if not active_prey:
        ...
        if idx % 3 == 1:
            target = nearest.pos + nearest.vel * 0.55 + flank * 2.4
        elif idx % 3 == 2:
            gate = nearest_escape_gate(school_center, pr.pos)
            target = school_center - safe_norm(gate["pos"] - school_center) * 4.0 + flank * 1.2
        else:
            target = school_center + flank * (3.5 + 0.8 * math.sin(sim_time + idx))

    desired = target - pr.pos
    if mag(desired) > EPS:
        desired_vel = safe_norm(desired) * pr.max_speed * predator_speed_multiplier * random.uniform(0.82, 1.05)
    ...
    return limit(acc, pr.max_force * 1.1 * predator_speed_multiplier)
...
scene.caption = (
    ...
    "I toggle AI | P pause | R reset | B bounce/wrap | M new AI mode | "
    "O toggle human override | C clear marks | +/- predator speed\n"
    "School behavior: prey split into smaller clusters when predators get too close, then reform when safe.\n"
)
...
class PreyFish:
    ...
    def update(self, dt, prey_list, predators):
        ...
        split_strength = cluster_controller.split_strength if cluster_controller is not None else 0.0
        max_force = (7.2 + 3.2 * self.fear + SPLIT_FORCE_BOOST * split_strength) * PREY_ESCAPE_DAMPING
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.34 * self.fear + SPLIT_SPEED_BOOST * split_strength) * PREY_ESCAPE_DAMPING)
        self.pos += self.vel * dt
