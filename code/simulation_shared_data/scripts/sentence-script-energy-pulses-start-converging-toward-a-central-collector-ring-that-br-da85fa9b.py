...
Energy pulses converge toward a central collector ring that brightens when nearby blooms collapse.

Run with:
    python vacuum_bloom_reactor_collector_ring.py
...
MAX_PULSES = 85
MAX_SPORES = 160
COLLECTOR_RADIUS = 2.1
COLLECTOR_ATTRACTION_RADIUS = 7.0
DEMAND_MIN = 0.25
DEMAND_MAX = 2.5
...
C_COLLECTOR_FLASH = vector(1.0, 0.72, 0.12)
C_COLLECTOR_STREAM = vector(0.05, 0.55, 0.95)
...
collector_ring_a = ring(pos=vector(0, 0, 0), axis=vector(0, 1, 0), radius=COLLECTOR_RADIUS, thickness=0.05, color=C_HARVEST, opacity=0.62)
...
collector_glow_ring = ring(pos=vector(0, 0.03, 0), axis=vector(0, 1, 0), radius=COLLECTOR_RADIUS * 1.04, thickness=0.09, color=C_COLLECTOR_FLASH, opacity=0.12)
collector_intake_markers = []
for i in range(12):
    a = 2 * math.pi * i / 12
    marker = sphere(
        pos=vector(math.cos(a) * COLLECTOR_RADIUS, 0.06, math.sin(a) * COLLECTOR_RADIUS),
        radius=0.075,
        color=C_COLLECTOR_STREAM,
        emissive=True,
        opacity=0.45,
    )
    collector_intake_markers.append(marker)
...
# -----------------------------
class EnergyPulse:
    def __init__(self, start_pos, energy, color=C_COLLECTOR_STREAM):
        self.energy = energy
        self.age = 0.0
        self.life = random.uniform(1.0, 1.75)
        self.start = vector(start_pos)
        self.collector_angle = math.atan2(self.start.z, self.start.x)
        if abs(self.start.x) + abs(self.start.z) < 0.001:
            self.collector_angle = random.uniform(0, 2 * math.pi)
        # Pulses terminate on the collector ring instead of disappearing into the core.
        self.end = vector(
            math.cos(self.collector_angle) * COLLECTOR_RADIUS,
            0.04,
            math.sin(self.collector_angle) * COLLECTOR_RADIUS,
        )
        self.body = sphere(pos=start_pos, radius=0.11 + 0.026 * energy, color=color, emissive=True, opacity=0.9)
        self.tail = curve(pos=[start_pos, start_pos], radius=0.022, color=color, opacity=0.58)
        self.guide = curve(pos=[start_pos, self.end], radius=0.006, color=C_COLLECTOR_FLASH, opacity=0.15)

    def update(self, dt):
        self.age += dt
        t = clamp(self.age / self.life, 0.0, 1.0)
        eased = 1 - (1 - t) ** 2
        inward = self.start * (1 - eased) + self.end * eased
        tangent = vector(-math.sin(self.collector_angle), 0, math.cos(self.collector_angle))
        lift = vector(0, math.sin(t * math.pi) * 0.65, 0)
        spiral = tangent * (math.sin(self.age * 8.5 + self.energy) * 0.75 * (1 - t))
        self.body.pos = inward + lift + spiral
        self.body.radius = max(0.035, (0.12 + 0.025 * self.energy) * (1 - 0.50 * t))
        self.body.opacity = max(0.0, 0.92 * (1 - 0.12 * t))
        self.tail.modify(0, pos=self.start)
        self.tail.modify(1, pos=self.body.pos)
        self.tail.opacity = max(0.0, 0.58 * (1 - t))
        self.guide.opacity = max(0.0, 0.15 * (1 - t))
        return t >= 1.0
...
def reset_simulation():
    global blooms, pulses, spores, usable_energy, harvested_total, cycle_count, collector_flash, collector_nearby_hits
    for obj in blooms:
        obj.remove()
    for obj in pulses:
        obj.remove()
...
while True:
    ...
    collector_flash = max(0.0, collector_flash - dt * 1.9)
    flash_mix = clamp(collector_flash, 0.0, 1.0)
    ...
    collector_ring_a.color = color_lerp(C_HARVEST, C_COLLECTOR_FLASH, flash_mix)
    collector_ring_a.thickness = 0.05 + 0.09 * flash_mix
    collector_ring_a.opacity = clamp(0.62 + 0.34 * flash_mix, 0.62, 0.96)
    collector_glow_ring.color = color_lerp(C_COLLECTOR_STREAM, C_COLLECTOR_FLASH, flash_mix)
    collector_glow_ring.opacity = clamp(0.10 + 0.50 * flash_mix + usable_energy * 0.006, 0.10, 0.72)
    collector_glow_ring.thickness = 0.07 + 0.16 * flash_mix
    collector_core.radius = 0.68 + 0.16 * math.sin(cycle_count * 0.06) + 0.025 * min(usable_energy, 8) + 0.08 * flash_mix
    collector_core.color = color_lerp(C_HARVEST, C_COLLECTOR_FLASH, flash_mix * 0.75)
    collector_shell.radius = 1.05 + 0.13 * math.sin(cycle_count * 0.04 + 1.3) + 0.025 * min(usable_energy, 8) + 0.18 * flash_mix
    collector_shell.opacity = clamp(0.08 + usable_energy * 0.012 + 0.16 * flash_mix, 0.08, 0.42)
    for i, marker in enumerate(collector_intake_markers):
        a = 2 * math.pi * i / len(collector_intake_markers) + cycle_count * 0.015 * reactor_demand
        marker.pos = vector(math.cos(a) * COLLECTOR_RADIUS, 0.06, math.sin(a) * COLLECTOR_RADIUS)
        marker.radius = 0.07 + 0.045 * flash_mix + 0.012 * math.sin(cycle_count * 0.09 + i)
        marker.opacity = clamp(0.36 + 0.46 * flash_mix, 0.30, 0.9)
        marker.color = color_lerp(C_COLLECTOR_STREAM, C_COLLECTOR_FLASH, flash_mix)
    ...
    for b in expired_blooms:
        ...
        collapse_radius_from_collector = abs(math.sqrt(b.pos.x * b.pos.x + b.pos.z * b.pos.z) - COLLECTOR_RADIUS)
        vertical_offset = abs(b.pos.y)
        collector_nearby = collapse_radius_from_collector < COLLECTOR_ATTRACTION_RADIUS and vertical_offset < 3.2
        if collector_nearby:
            collector_flash = clamp(collector_flash + 0.34 + 0.12 * b.seed_strength, 0.0, 1.6)
            collector_nearby_hits += 1
        b.remove()
        if b in blooms:
            blooms.remove(b)
        for product in products:
            if product[0] == "pulse" and len(pulses) < MAX_PULSES:
                pulse_color = C_COLLECTOR_FLASH if collector_nearby else C_COLLECTOR_STREAM
                pulses.append(EnergyPulse(product[1], product[2], pulse_color))
    ...
    for p in pulses:
        ...
        if arrived:
            ...
            collector_flash = clamp(collector_flash + 0.11 + 0.025 * p.energy, 0.0, 1.6)
    ...
    status.text = (
        "VACUUM BLOOM REACTOR\n"
        f"Blooms: {len(blooms):02d}   Spores: {len(spores):03d}   Pulses: {len(pulses):02d}\n"
        f"Usable Energy: {usable_energy:05.2f}   Harvested Total: {harvested_total:07.2f}   Demand: {reactor_demand:04.2f}x\n"
        f"Collector Ring Hits: {collector_nearby_hits:04d}   Ring Brightness: {clamp(collector_flash, 0.0, 1.0):04.2f}"
    )
