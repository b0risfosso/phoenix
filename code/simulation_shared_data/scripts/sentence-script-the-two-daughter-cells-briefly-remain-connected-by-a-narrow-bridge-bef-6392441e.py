...
scene = canvas(
    title="Mitosis: Daughter Cell Bridge and Final Separation",
    width=1200,
    height=760,
...
class MitosisSimulation:
    ...
    def reset_round(self, full=False):
        ...
        # Late cytokinesis bridge / midbody system.
        # The broad bridge above represents the early shared cytoplasm. These
        # thinner objects persist after furrow closure so the daughter cells
        # visibly remain connected before final abscission separates them.
        self.midbody_bridge = cylinder(
            pos=vector(-0.42, 0, 0),
            axis=vector(0.84, 0, 0),
            radius=0.075,
            color=vector(0.20, 0.72, 0.95),
            opacity=0,
            shininess=0.45
        )
        self.register(self.midbody_bridge)

        self.midbody_core = sphere(
            pos=vector(0, 0, 0),
            radius=0.13,
            color=vector(0.10, 0.58, 0.90),
            opacity=0,
            emissive=True
        )
        self.register(self.midbody_core)

        self.abscission_ring_left = ring(
            pos=vector(-0.25, 0, 0),
            axis=vector(1, 0, 0),
            radius=0.18,
            thickness=0.014,
            color=vector(0.06, 0.52, 0.82),
            opacity=0
        )
        self.abscission_ring_right = ring(
            pos=vector(0.25, 0, 0),
            axis=vector(1, 0, 0),
            radius=0.18,
            thickness=0.014,
        ...
        self.register(self.abscission_ring_left)
        self.register(self.abscission_ring_right)
        ...
        for i in range(9):
            bead = sphere(
                pos=vector(0, 0, 0),
                radius=0.028,
                color=vector(0.36, 0.86, 1.0),
                opacity=0,
                emissive=True
            )
            self.bridge_beads.append(bead)
            self.register(bead)
    ...
    def stage_name(self):
        ...
        if t < 0.91:
            return "Bridge stage: daughter cells remain linked by a narrow midbody"
        if t < 0.98:
            return "Abscission: the bridge thins and separates"
    ...
    def update(self, dt):
        ...
        # Daughter cells separate in two phases: they first pull apart while
        # still linked, then get an extra outward drift only after the narrow
        # bridge has nearly completed abscission.
        bridge_release = smoothstep(0.91, 0.99, t)
        daughter_d = lerp(0.72, 2.16, split) + bridge_release * 0.62 * self.drift_bias
        ...
        # A narrow intercellular bridge forms after the cleavage furrow has
        # mostly closed. It lingers as a bright midbody connection, then snaps
        # away during final abscission.
        bridge_birth = smoothstep(0.68, 0.80, t)
        abscission = smoothstep(0.91, 0.99, t)
        bridge_linger = bridge_birth * (1 - smoothstep(0.965, 0.998, t))
        bridge_length = lerp(0.92, 0.34, abscission)
        bridge_radius = lerp(0.125, 0.035, smoothstep(0.74, 0.97, t))
        bridge_wobble = vector(0, 0.018 * math.sin(self.time_seconds * 5.2), 0.018 * math.cos(self.time_seconds * 4.8))

        self.midbody_bridge.visible = bridge_linger > 0.01
        self.midbody_bridge.pos = vector(-bridge_length / 2, 0, 0) + bridge_wobble
        self.midbody_bridge.axis = vector(bridge_length, 0, 0)
        self.midbody_bridge.radius = bridge_radius
        self.midbody_bridge.opacity = 0.62 * bridge_linger

        self.midbody_core.visible = bridge_linger > 0.01
        self.midbody_core.pos = bridge_wobble
        self.midbody_core.radius = lerp(0.16, 0.055, abscission) + 0.012 * math.sin(self.time_seconds * 8.0) * bridge_linger
        self.midbody_core.opacity = 0.70 * bridge_linger

        ring_sep = lerp(0.34, 0.11, abscission)
        ring_rad = lerp(0.23, 0.055, abscission)
        ring_op = 0.58 * bridge_linger
        self.abscission_ring_left.visible = bridge_linger > 0.01
        self.abscission_ring_right.visible = bridge_linger > 0.01
        self.abscission_ring_left.pos = vector(-ring_sep, 0, 0) + bridge_wobble
        self.abscission_ring_right.pos = vector(ring_sep, 0, 0) + bridge_wobble
        self.abscission_ring_left.radius = ring_rad
        self.abscission_ring_right.radius = ring_rad
        self.abscission_ring_left.thickness = lerp(0.018, 0.032, abscission)
        self.abscission_ring_right.thickness = lerp(0.018, 0.032, abscission)
        self.abscission_ring_left.opacity = ring_op
        self.abscission_ring_right.opacity = ring_op

        for i, bead in enumerate(self.bridge_beads):
            u = 0 if len(self.bridge_beads) == 1 else i / (len(self.bridge_beads) - 1)
            x = lerp(-bridge_length * 0.44, bridge_length * 0.44, u)
            spin = self.time_seconds * (4.0 + 0.18 * i) + i * 0.85
            orbit_r = bridge_radius * (1.65 + 0.22 * math.sin(self.time_seconds * 2.0 + i))
            bead.pos = vector(x, math.sin(spin) * orbit_r, math.cos(spin) * orbit_r) + bridge_wobble
            bead.radius = lerp(0.030, 0.014, abscission)
            bead.opacity = 0.62 * bridge_linger * (0.72 + 0.28 * math.sin(self.time_seconds * 6.0 + i))
            bead.visible = bridge_linger > 0.01
        ...
        bridge_depth = smoothstep(0.68, 0.99, self.progress)
        bridge_state = "bridge open" if self.progress < 0.80 else ("bridge thinning" if self.progress < 0.965 else "bridge separating")
        self.stage_label.text = (
            f"Round {self.round_index} — {self.stage_name()}  |  "
            f"progress {int(self.progress * 100)}%  |  {bridge_state} {int(bridge_depth * 100)}%"
        )
