...
#   V     : pulse cytoplasm/organelle redistribution flow
...
class CytoplasmCargo:
    """Organelle / vesicle / cytoplasm granule that redistributes during cytokinesis."""
    def __init__(self, sim, kind, pos, side_bias):
        self.sim = sim
        self.kind = kind
        self.side_bias = side_bias  # -1 left daughter, +1 right daughter
        self.phase = random.uniform(0, math.tau)
        self.vel = vector(random.uniform(-0.10, 0.10), random.uniform(-0.08, 0.08), random.uniform(-0.08, 0.08))
        self.settled = False

        # Give every cargo item a fixed destination within the daughter-cell cytoplasm,
        # not at the nucleus center.  The previous version used only a small y/z
        # offset around each daughter nucleus, causing organelles and vesicles to
        # cluster near the nuclear region.  These offsets fill the full daughter
        # cell volume so cargo spreads through the whole cell body.
        self.final_offset = self.sample_daughter_cell_offset()
        self.roam_phase = random.uniform(0, math.tau)

        if kind == "mitochondrion":
            self.obj = ellipsoid(
                pos=pos,
                size=vector(0.24, 0.09, 0.09),
                color=vector(0.96, 0.54, 0.28),
                opacity=0.82,
                shininess=0.35
            )
            self.radius = 0.12
            ...
            self.obj = sphere(
                pos=pos,
                radius=random.uniform(0.060, 0.085),
                color=vector(0.42, 0.72, 0.92),
                opacity=0.76,
                shininess=0.40
            )
            self.radius = self.obj.radius
        elif kind == "vesicle":
            self.obj = sphere(
                pos=pos,
                radius=random.uniform(0.030, 0.052),
                color=vector(0.86, 0.76, 1.0),
                opacity=0.58,
                shininess=0.55
            )
            self.radius = self.obj.radius
        ...
        sim.register(self.obj)
        self.trail = ManualTrail(sim, vector(0.70, 0.92, 1.0), radius=max(0.008, self.radius * 0.20), max_points=7, stride=7, opacity=0.16)

    def sample_daughter_cell_offset(self):
        # Uniform-ish sample inside a sphere, scaled for the visible daughter cell.
        # The x component is intentionally allowed to be large so organelles and
        # vesicles occupy the outer body, middle cytoplasm, and furrow-facing side.
        ...
        return vector(
            base_radius * r * u * 0.88,
            base_radius * r * radial * math.cos(theta),
            base_radius * r * radial * math.sin(theta)
        )

    def target_position(self, t, left_center, right_center):
        # Before the cell pinches, cargo circulates through the shared cytoplasm.
        # During cytokinesis, each item receives a stable destination distributed
        # throughout the full daughter-cell volume, not just around the nucleus.
        side_center = left_center if self.side_bias < 0 else right_center
        redistribution = smoothstep(0.46, 0.86, t)
        shared_orbit = vector(
            0.95 * math.sin(self.phase * 1.7),
            0.72 * math.sin(self.phase + 1.1),
            0.72 * math.cos(self.phase * 0.9)
        )
        # A small persistent roaming wobble keeps settled organelles/vesicles alive
        # without collapsing their targets back to the center.
        roam = vector(
            0.05 * math.sin(self.roam_phase + t * math.tau),
            0.04 * math.sin(self.roam_phase * 0.7 + t * math.tau * 1.3),
            0.04 * math.cos(self.roam_phase * 1.1 + t * math.tau)
        )
        return vlerp(shared_orbit, side_center + self.final_offset + roam, redistribution)

    def update(self, dt, t, time_seconds, left_center, right_center):
        redistribution = smoothstep(0.46, 0.86, t)
        target = self.target_position(t, left_center, right_center)
        stirring = vector(
            0.035 * math.sin(time_seconds * 1.6 + self.phase),
            0.030 * math.sin(time_seconds * 2.1 + self.phase * 0.7),
            0.030 * math.cos(time_seconds * 1.9 + self.phase)
        )
        pull_strength = lerp(0.20, 1.15, redistribution)
        self.vel += (target - self.obj.pos) * pull_strength * dt
        self.vel += stirring * dt

        # Keep cargo out of the tightening furrow so the redistribution is visible.
        furrow_depth = smoothstep(0.52, 0.84, t)
        if furrow_depth > 0.10 and abs(self.obj.pos.x) < 0.18:
            self.vel.x += self.side_bias * furrow_depth * 0.42 * dt

        self.vel *= 0.982
        self.obj.pos += self.vel * dt

        # Once daughter cells are emerging, keep redistributed cargo inside the
        # whole daughter-cell boundary rather than letting it collect at the nucleus
        # or leak back through the cleavage furrow.
        if redistribution > 0.28:
            center = left_center if self.side_bias < 0 else right_center
            boundary_radius = 1.28
            rel = self.obj.pos - center
            if mag(rel) > boundary_radius:
                n = safe_norm(rel)
                self.obj.pos = center + n * boundary_radius
                self.vel = self.vel - 1.4 * dot(self.vel, n) * n
                self.vel *= 0.72

        if self.kind == "mitochondrion":
            self.obj.axis = norm(vector(1, 0.25 * math.sin(time_seconds + self.phase), 0.25 * math.cos(time_seconds * 0.8 + self.phase)))
            self.obj.size = vector(0.24, 0.09, 0.09) * (1.0 + 0.04 * math.sin(time_seconds * 2.4 + self.phase))
        else:
            self.obj.radius = self.radius * (1.0 + 0.08 * math.sin(time_seconds * 2.0 + self.phase))

        # Fade cytoplasm grains slightly into daughter cells; keep organelles/vesicles clear.
        if self.kind == "cytoplasm":
            self.obj.opacity = 0.30 + 0.18 * (1 - redistribution)
        elif self.kind == "vesicle":
            self.obj.opacity = 0.50 + 0.12 * math.sin(time_seconds * 2.2 + self.phase)

        self.trail.update(self.obj.pos)
        self.settled = redistribution > 0.92 and mag(self.obj.pos - target) < 0.35
...
def random_seeded_wave(seed_value, amplitude):
    return amplitude * math.sin(seed_value * 12.9898) + amplitude * 0.6 * math.cos(seed_value * 4.1414)
...
class MitosisSimulation:
    ...
    def cleanup_dynamic(self):
        ...
        for w in list(self.wrappers):
            w.destroy()
        for item in list(self.cytoplasm_items):
            item.destroy()
    ...
    def reset_round(self, full=False):
        ...
        self.create_cytoplasm_distribution()
        self.mark(vector(0, 0.95, 0), "duplicated chromosome cluster", vector(0.45, 0.25, 0.85), life=4.0)


    def create_cytoplasm_distribution(self):
        # A mixed internal population: small cytoplasm grains, vesicles, and larger organelles.
        kinds = []
        kinds += ["cytoplasm"] * 34
        kinds += ["vesicle"] * 18
        kinds += ["organelle"] * 10
        kinds += ["mitochondrion"] * 8
        random.shuffle(kinds)

        for i, kind in enumerate(kinds):
            # Start in the shared cytoplasm, then split roughly evenly between daughter cells.
            theta = random.uniform(0, math.tau)
            r = math.sqrt(random.random())
            pos = vector(
                random.uniform(-0.82, 0.82),
                math.cos(theta) * r * random.uniform(0.20, 1.04),
                math.sin(theta) * r * random.uniform(0.20, 1.04)
            )
            side_bias = -1 if i % 2 == 0 else 1
            if random.random() < 0.18:
                side_bias *= -1
            self.cytoplasm_items.append(CytoplasmCargo(self, kind, pos, side_bias))

    def redistribute_score(self):
        if not self.cytoplasm_items:
            return 0.0
        settled = sum(1 for item in self.cytoplasm_items if item.settled)
        return settled / len(self.cytoplasm_items)

    def pulse_cytoplasm_flow(self, count=10):
        if not self.cytoplasm_items:
            return
        for item in random.sample(self.cytoplasm_items, min(count, len(self.cytoplasm_items))):
            # Pulse outward into the daughter-cell body, with y/z spread so the
            # vesicles and organelles do not simply stream toward the nuclei.
            item.vel += vector(
                item.side_bias * random.uniform(0.14, 0.34),
                random.uniform(-0.22, 0.22),
                random.uniform(-0.22, 0.22)
            )
        self.mark(vector(0, -1.05, 0), "whole-cell cytoplasm redistribution pulse", vector(0.20, 0.62, 0.86), life=2.4)
    ...
    def get_state(self):
        ...
        return {
            ...
            "cytoplasm_count": len(self.cytoplasm_items),
            "redistribution_score": self.redistribute_score(),
    ...
    def update(self, dt):
        ...
        for item in self.cytoplasm_items:
            item.update(dt, t, self.time_seconds, self.left_daughter.pos, self.right_daughter.pos)

        self.stage_label.visible = self.show_labels
        self.ai_label.visible = self.show_labels
        redistrib = int(self.redistribute_score() * 100)
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  whole-cell cargo redistribution {redistrib}%"
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | W wrap | V cytoplasm flow | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
