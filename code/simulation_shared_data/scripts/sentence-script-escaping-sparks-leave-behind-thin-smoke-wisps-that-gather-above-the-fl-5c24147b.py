"""
Fire That Wants To Become A Star — Iteration 10
Weak Smoke Boundary Layer

Story:
The flame has a fragile proto-core and faint circular heat paths.
Now escaping sparks leave behind thin smoke wisps. These wisps gather above the
flame into a weak boundary layer that slows later sparks before they disappear.

This is still not a completed star:
- the smoke layer is uneven and weak
- sparks can still escape
- heat paths are not full flame bands
- the proto-core can still collapse
- there is no sealed shell or stable star core
...
# Scene
# -----------------------------
scene.title = "Fire That Wants To Become A Star — Weak Smoke Boundary"
scene.width = 1160
scene.height = 740
...
title_label = label(
    ...
    text="Escaping sparks build a weak smoke boundary",
    height=15,
    box=False,
...
help_label = label(
    ...
    text="H help | P pause | R reset | W will | Space breath | I pull | C cycle | E core | O loop | G guide | S smoke",
    height=9,
    box=False,
    align="left",
...
def evaluate_cycle_survival():
    ...
    if survived:
        ...
        gain = 0.034 + 0.022 * min(proto_core_survival_streak, 5) + 0.014 * min(cycle_stores, 4) + loop_help + path_help + smoke_help
        ...
        if inner_ember_heat > 0.06 or proto_core_persistence > 0.05:
            ...
            collapse_flash = 1.0
            add_collapse_shards()
        inner_ember_heat = clamp(inner_ember_heat * 0.72, 0.0, 0.96)
...
def reset_all_evolving_state():
    ...
    global smoke_memory, smoke_boundary_strength, manual_smoke_boost, smoke_wisp_count, smoke_slow_count
...
def update_learning_state():
    ...
    global smoke_memory, smoke_boundary_strength, manual_smoke_boost
...
def update_flame_shape():
    ...
    proto_core_shell.radius = 0.10 + 0.25 * proto_core_persistence + 0.10 * inner_ember_heat + 0.05 * loop_strength + 0.05 * heat_path_memory + 0.03 * smoke_boundary_strength + 0.03 * proto_pulse
    proto_core_shell.opacity = clamp(0.02 + 0.37 * proto_core_persistence + 0.08 * loop_strength + 0.08 * heat_path_memory + 0.04 * smoke_boundary_strength + 0.12 * survival_flash - 0.12 * collapse_flash, 0.015, 0.55)
    ...
    inward_field_ring.opacity = 0.04 + 0.15 * inward_pull + 0.10 * pull_drive + 0.05 * returned_heat + 0.08 * proto_core_persistence + 0.04 * heat_path_memory + 0.03 * smoke_boundary_strength
    ...
    cycle_halo.opacity = 0.08 + 0.15 * cycle_energy + 0.12 * max(rise_drive, pull_drive, receive_drive) + 0.06 * proto_core_persistence + 0.04 * heat_path_memory + 0.03 * smoke_boundary_strength
...
def update_sparks():
    ...
    for p in sparks:
        ...
        if p["bendable"] and pull_zone:
            pull_strength = (
                inward_pull
                * (0.25 + 0.32 * escape_noticing + 0.24 * returned_heat + 0.18 * inner_ember_heat + 0.24 * proto_core_persistence + 0.10 * heat_path_memory + 0.08 * smoke_boundary_strength + 1.00 * pull_drive)
                / (0.75 + distance)
            )
...
def update_status():
    ...
    status_label.text = (
        ...
        "state: weak smoke boundary, not yet sealed"
    )
