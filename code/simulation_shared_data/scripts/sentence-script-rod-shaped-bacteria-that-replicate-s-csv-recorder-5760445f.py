...
if _csv_output_dir:
    ...
    CSV_OUTPUT_PATH = os.path.join(
        _csv_output_dir,
        f"{_csv_run_id}-bacterial-colony-state-log.csv"
    )
else:
    CSV_OUTPUT_PATH = os.environ.get(
        "SIM_STATE_CSV_PATH",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "bacterial_colony_state_log.csv")
    )

csv_run_id = _csv_run_id
csv_elapsed_time = 0.0
csv_next_sample_time = 0.0
csv_next_field_sample_time = 0.0
...
def _csv_base_row(elapsed_seconds, entity_type, entity_id="", sub_type="", note=""):
    try:
        state = ai.read_state()
    except Exception:
        state = {}
    center = state.get("center", colony_center() if bacteria else vector(0, BACTERIA_Y, 0))
    return {
        "run_id": csv_run_id,
        ...
        "entity_id": entity_id,
        "sub_type": sub_type,
        "round": getattr(ai, "round", ""),
        "ai_enabled": getattr(ai, "enabled", ""),
        "ai_mode": getattr(ai, "mode", ""),
        "mode_time": f"{getattr(ai, 'mode_time', 0.0):.4f}",
        "stagnation_time": f"{getattr(ai, 'stagnation_time', 0.0):.4f}",
        "completion_timer": f"{getattr(ai, 'completion_timer', 0.0):.4f}",
        "population": state.get("population", len(bacteria)),
        "max_population": MAX_BACTERIA,
        "avg_nutrient": f"{state.get('avg_nutrient', avg_field(N)):.6f}",
        "avg_slime": f"{state.get('avg_slime', avg_field(S)):.6f}",
        "total_slime": f"{state.get('total_slime', total_field(S)):.6f}",
        "depleted_fraction": f"{state.get('depleted_fraction', depleted_fraction()):.6f}",
        "attached_fraction": f"{state.get('attached_fraction', 0.0):.6f}",
        "avg_energy": f"{state.get('avg_energy', 0.0):.6f}",
        "colony_center_x": f"{center.x:.6f}",
        "colony_center_y": f"{center.y:.6f}",
        "colony_center_z": f"{center.z:.6f}",
        "spread": f"{state.get('spread', colony_spread(center)):.6f}",
        "boundary_mode": boundary_mode,
        "sim_speed": f"{SIM_SPEED:.4f}",
        "paused": paused,
        "cursor_x": f"{getattr(ai, 'cursor_pos', vector(0,0,0)).x:.6f}",
        "cursor_y": f"{getattr(ai, 'cursor_pos', vector(0,0,0)).y:.6f}",
        "cursor_z": f"{getattr(ai, 'cursor_pos', vector(0,0,0)).z:.6f}",
        "cursor_target_x": f"{getattr(ai, 'cursor_target', vector(0,0,0)).x:.6f}",
        "cursor_target_y": f"{getattr(ai, 'cursor_target', vector(0,0,0)).y:.6f}",
        "cursor_target_z": f"{getattr(ai, 'cursor_target', vector(0,0,0)).z:.6f}",
...
def record_csv_state(elapsed_seconds, include_fields=False):
    """Write one CSV snapshot of the bacterial colony simulation state."""
    csv_writer.writerow(_csv_base_row(elapsed_seconds, "summary", "colony", note="run snapshot"))

    csv_writer.writerow(_csv_base_row(elapsed_seconds, "ai_controller", "ai", note=getattr(ai, "manual_note", "")))

    for b in bacteria:
        row = _csv_base_row(elapsed_seconds, "bacterium", b.id)
        row.update({
            "pos_x": f"{b.pos.x:.6f}",
            "pos_y": f"{b.pos.y:.6f}",
            "pos_z": f"{b.pos.z:.6f}",
            "prev_pos_x": f"{b.prev_pos.x:.6f}",
            "prev_pos_y": f"{b.prev_pos.y:.6f}",
            "prev_pos_z": f"{b.prev_pos.z:.6f}",
            "vel_x": f"{b.vel.x:.6f}",
            "vel_y": f"{b.vel.y:.6f}",
            "vel_z": f"{b.vel.z:.6f}",
            "theta": f"{b.theta:.6f}",
            "omega": f"{b.omega:.6f}",
            "energy": f"{b.energy:.6f}",
            "age": f"{b.age:.6f}",
            "attached": b.attached,
            "dead": b.dead,
            "length": f"{b.length:.6f}",
            "radius": f"{b.radius:.6f}",
            "local_nutrient": f"{sample_field(N, b.pos):.6f}",
            "local_slime": f"{sample_field(S, b.pos):.6f}",
        })
        csv_writer.writerow(row)

    for idx, p in enumerate(particles):
        obj = p.get("obj")
        row = _csv_base_row(elapsed_seconds, "particle", idx, sub_type=p.get("kind", "particle"))
        row.update({
            "pos_x": f"{obj.pos.x:.6f}",
            "pos_y": f"{obj.pos.y:.6f}",
            "pos_z": f"{obj.pos.z:.6f}",
            "vel_x": f"{p.get('vel', vector(0,0,0)).x:.6f}",
            "vel_y": f"{p.get('vel', vector(0,0,0)).y:.6f}",
            "vel_z": f"{p.get('vel', vector(0,0,0)).z:.6f}",
            "age": f"{p.get('maxlife', 0.0) - p.get('life', 0.0):.6f}",
            "note": f"life={p.get('life', 0.0):.4f}",
        })
        csv_writer.writerow(row)
    ...
    if include_fields:
        for i in range(GRID):
            for j in range(GRID):
                cp = cell_center(i, j)
                row = _csv_base_row(elapsed_seconds, "field_cell", f"{i}_{j}")
                row.update({
                    "pos_x": f"{cp.x:.6f}",
                    "pos_y": f"{cp.y:.6f}",
                    "pos_z": f"{cp.z:.6f}",
                    "field_i": i,
                    "field_j": j,
                    "field_nutrient": f"{N[i][j]:.6f}",
                    "field_slime": f"{S[i][j]:.6f}",
                })
                csv_writer.writerow(row)
...
record_csv_state(csv_elapsed_time, include_fields=True)

while csv_elapsed_time < CSV_RUN_SECONDS:
    rate(45)
    now = time.time()
    ...
    if csv_elapsed_time >= csv_next_sample_time:
        include_fields = csv_elapsed_time >= csv_next_field_sample_time
        record_csv_state(csv_elapsed_time, include_fields=include_fields)
        csv_next_sample_time += CSV_SAMPLE_INTERVAL
        if include_fields:
            csv_next_field_sample_time += CSV_FIELD_SAMPLE_INTERVAL
...
stats_label.text = status_message
mode_label.text = status_message
csv_file.flush()
csv_file.close()
