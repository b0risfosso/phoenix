...
class MitosisSimulation:
    def __init__(self):
        ...
        # Internal-stress response state. Stress can slow, pause, or
        # briefly reverse mitosis, but every event cools down so the round
        # cannot become permanently blocked.
        self.internal_stress = 0.0
        self.stress_target = 0.0
        self.stress_mode = "stable"
        self.stress_pause_timer = 0.0
        self.stress_reverse_timer = 0.0
        self.stress_cooldown = 0.0
        self.stress_event_timer = 0.0
        self.reversal_floor = 0.0
        self.last_stress_marker_time = -10.0
    ...
    def reset_round(self, full=False):
        ...
        self.stress_shell = sphere(
            pos=vector(0, 0, 0),
            radius=1,
            size=vector(3.65, 3.20, 3.20),
            color=vector(1.0, 0.36, 0.16),
            opacity=0,
            shininess=0.2
        )
        self.stress_ring = ring(
            pos=vector(0, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.55,
            thickness=0.025,
            color=vector(1.0, 0.45, 0.18),
            opacity=0
        )
        self.stress_reverse_ring = ring(
            pos=vector(0, 0, 0),
            axis=vector(0, 1, 0),
            radius=1.15,
            thickness=0.018,
            color=vector(0.85, 0.20, 0.95),
            opacity=0
        )
        self.register(self.stress_core)
        self.register(self.stress_shell)
        self.register(self.stress_ring)
        self.register(self.stress_reverse_ring)
    ...
    def get_state(self):
        ...
        return {
            ...
            "paused": self.paused,
            "internal_stress": self.internal_stress,
    ...
    def trigger_internal_stress(self, strength=0.85, mode=None):
        # Start a bounded stress event. The event may slow, pause, or
        # reverse visible mitosis, but the timers always expire.
        if mode is None:
            mode = random.choice(["slow", "pause", "reverse"])
        self.stress_target = max(self.stress_target, clamp(strength, 0.0, 1.0))
        self.stress_mode = mode
        self.stress_cooldown = 3.5
        if mode == "pause":
            self.stress_pause_timer = max(self.stress_pause_timer, random.uniform(1.0, 2.2))
            self.mark(vector(0, 1.65, 0), "stress pause", vector(1.0, 0.36, 0.18), life=2.2)
        elif mode == "reverse":
            self.stress_reverse_timer = max(self.stress_reverse_timer, random.uniform(0.75, 1.45))
            self.reversal_floor = max(0.0, self.progress - random.uniform(0.035, 0.085))
            self.mark(vector(0, 1.65, 0), "stress reversal", vector(0.85, 0.20, 0.95), life=2.2)
        else:
            self.mark(vector(0, 1.65, 0), "stress slowdown", vector(1.0, 0.55, 0.22), life=2.2)

        # Stress also produces a small vesicle burst so the internal response
        # is visible even when progress is slowed or paused.
        self.spill_particles(6, source=vector(0, 0, 0), mood="chaotic")

    def update_internal_stress(self, dt):
        t = self.progress
        self.stress_event_timer += dt
        if self.stress_cooldown > 0:
            self.stress_cooldown = max(0, self.stress_cooldown - dt)
        if self.stress_pause_timer > 0:
            self.stress_pause_timer = max(0, self.stress_pause_timer - dt)
        if self.stress_reverse_timer > 0:
            self.stress_reverse_timer = max(0, self.stress_reverse_timer - dt)

        # Natural stress rises when many spindle attachments are missing and
        # during high-force metaphase/anaphase transitions.
        missing = 0
        total = max(1, len(self.chromosomes) * 2)
        for c in self.chromosomes:
            missing += 2 - c.attachment_fraction()
        attachment_stress = missing / total
        force_window = smoothstep(0.26, 0.46, t) * (1 - smoothstep(0.70, 0.88, t))
        oscillation = 0.5 + 0.5 * math.sin(self.time_seconds * 2.2)
        natural = clamp(attachment_stress * 0.90 + force_window * (0.18 + 0.15 * oscillation) + self.ai_noise * 0.45, 0, 0.82)

        # Occasional bounded stress events make the response visible even in
        # otherwise smooth rounds. They are only allowed in the middle of mitosis.
        if 0.22 < t < 0.86 and self.stress_cooldown <= 0 and self.stress_event_timer > 4.0:
            if random.random() < 0.010 + 0.020 * force_window:
                self.stress_event_timer = 0
                self.trigger_internal_stress(random.uniform(0.65, 0.95))

        target = max(self.stress_target, natural)
        self.internal_stress = lerp(self.internal_stress, target, 0.055)
        self.stress_target = max(0.0, self.stress_target - dt * 0.12)

        if self.stress_pause_timer <= 0 and self.stress_reverse_timer <= 0 and self.stress_target < 0.12:
            if self.internal_stress < 0.22:
                self.stress_mode = "stable"

    def stress_progress_multiplier(self):
        if self.progress < 0.08 or self.progress > 0.96:
            return 1.0
        if self.stress_pause_timer > 0:
            return 0.0
        if self.stress_reverse_timer > 0:
            return -0.55
        # High stress slows progress smoothly instead of hard-locking it.
        return clamp(1.0 - self.internal_stress * 0.82, 0.18, 1.0)

    def update_stress_visuals(self):
        s = clamp(self.internal_stress, 0, 1)
        pulse = 0.5 + 0.5 * math.sin(self.time_seconds * (4.0 + 5.0 * s))
        active = s > 0.035
        self.stress_core.visible = active
        self.stress_shell.visible = active
        self.stress_ring.visible = active
        self.stress_reverse_ring.visible = active and self.stress_reverse_timer > 0

        self.stress_core.radius = 0.12 + 0.32 * s + 0.05 * pulse
        self.stress_core.opacity = 0.10 + 0.55 * s if active else 0
        self.stress_core.color = vector(1.0, 0.30 + 0.25 * pulse, 0.15 + 0.20 * (1 - pulse))

        shell_scale = 1.0 + 0.05 * s * math.sin(self.time_seconds * 3.0)
        self.stress_shell.size = vector(3.65 * shell_scale, 3.20 * shell_scale, 3.20 * shell_scale)
        self.stress_shell.opacity = 0.11 * s
        self.stress_shell.color = vector(1.0, 0.38 + 0.12 * pulse, 0.16)

        self.stress_ring.radius = 1.48 + 0.25 * s + 0.08 * pulse
        self.stress_ring.thickness = 0.018 + 0.035 * s
        self.stress_ring.opacity = 0.45 * s
        self.stress_ring.axis = norm(vector(1, 0.18 * math.sin(self.time_seconds * 1.7), 0.18 * math.cos(self.time_seconds * 1.7)))

        self.stress_reverse_ring.radius = 0.85 + 0.35 * pulse
        self.stress_reverse_ring.opacity = 0.52 * s if self.stress_reverse_timer > 0 else 0
        self.stress_reverse_ring.axis = norm(vector(0.12 * math.sin(self.time_seconds), 1, 0.25 * math.cos(self.time_seconds * 1.3)))
    ...
    def update(self, dt):
        ...
        self.update_internal_stress(dt)
        stress_factor = self.stress_progress_multiplier()
        speed = self.base_progress_speed * self.speed_modifier * self.human_speed_bias * stress_factor
        if self.progress < 1:
            next_progress = self.progress + dt * speed
            if stress_factor < 0:
                next_progress = max(self.reversal_floor, next_progress)
            self.progress = clamp(next_progress, 0, 1)
        else:
        ...
        for c in self.chromosomes:
            c.update(
                ...
                self.ai_noise + self.internal_stress * 0.18,
                self.pull_bias
            )
            if self.internal_stress > 0.06:
                heat = clamp(self.internal_stress, 0, 1)
                c.left.color = vlerp(c.color_left, vector(1.0, 0.28, 0.16), heat * 0.65)
                c.right.color = vlerp(c.color_right, vector(1.0, 0.34, 0.18), heat * 0.65)
                c.left.radius *= 1.0 + 0.025 * heat * math.sin(self.time_seconds * 8.0 + c.index)
                c.right.radius *= 1.0 + 0.025 * heat * math.sin(self.time_seconds * 8.0 + c.index + 1.2)
        ...
        stress_pct = int(clamp(self.internal_stress, 0, 1) * 100)
        stress_note = self.stress_mode
        if self.stress_pause_timer > 0:
            stress_note = "paused by stress"
        elif self.stress_reverse_timer > 0:
            stress_note = "reversing under stress"
        elif self.internal_stress > 0.30:
            stress_note = "slowed by stress"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  stress {stress_pct}% ({stress_note})"
...
def keydown(evt):
    ...
    if key == " ":
        ...
        sim.trigger_internal_stress(strength=0.92, mode=random.choice(["slow", "pause", "reverse"]))
    elif key == "up":
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | W wrap | C camera | L labels | X override | T stress",
    box=False,
    opacity=0,
