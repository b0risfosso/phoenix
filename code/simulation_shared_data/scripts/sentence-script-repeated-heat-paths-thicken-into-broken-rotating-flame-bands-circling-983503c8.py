...
"""
Fire That Wants To Become A Star — Iteration 11
Broken Rotating Flame Bands

Story:
The flame has a fragile proto-core and faint circular heat paths.
Escaping sparks leave behind thin smoke wisps, and repeated circular heat paths
now thicken into broken rotating flame bands. The bands circle the proto-core in
uneven arcs, strengthen when sparks follow them, and fade when the fire loses rhythm.

This is still not a completed star:
- the smoke layer is uneven and weak
- sparks can still escape
- flame bands are broken and unstable
...
# Scene
# -----------------------------
scene.title = "Fire That Wants To Become A Star — Broken Rotating Flame Bands"
scene.width = 1160
scene.height = 740
...
help_label = label(
    ...
    text="H help | P pause | R reset | W will | Space breath | I pull | C cycle | E core | O loop | G guide | S smoke | B bands",
    height=9,
    box=False,
    align="left",
...
def evaluate_cycle_survival():
    ...
    if survived:
        ...
        gain = 0.034 + 0.022 * min(proto_core_survival_streak, 5) + 0.014 * min(cycle_stores, 4) + loop_help + path_help + smoke_help + band_help
...
def reset_all_evolving_state():
    ...
    global smoke_memory, smoke_boundary_strength
    global flame_band_memory, flame_band_strength, manual_band_boost, band_reinforce_count, last_cycle_band_gain, cycle_start_band_reinforces, band_break_count, manual_smoke_boost, smoke_wisp_count, smoke_slow_count
...
def update_flame_shape():
    ...
    for idx, band in enumerate(broken_flame_bands):
        ...
        desired = clamp(flame_band_strength * (0.65 + 0.12 * idx) + flame_band_memory * 0.20, 0.0, 1.0)
...
def update_sparks():
    ...
    for p in sparks:
        ...
        if p["bendable"] and pull_zone:
            pull_strength = (
                inward_pull
                * (0.25 + 0.32 * escape_noticing + 0.24 * returned_heat + 0.18 * inner_ember_heat + 0.24 * proto_core_persistence + 0.10 * heat_path_memory + 0.08 * smoke_boundary_strength + 0.10 * flame_band_strength + 1.00 * pull_drive)
                / (0.75 + distance)
            )
...
def update_status():
    ...
    status_label.text = (
        ...
        "state: broken rotating flame bands, not yet stable"
    )
