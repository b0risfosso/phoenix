...
#   D     : force a lagging chromosome recovery event
...
class ChromosomePair:
    def __init__(self, sim, index, base_angle, radius_ring):
        ...
        # Lagging chromosome behavior: a few chromosome pairs hesitate during
        # anaphase, creating visible tension until spindle recovery pulls them
        # into the daughter-cell groups before division finishes.
        self.can_lag = False
        self.lag_delay = 0.0
        self.lag_strength = 0.0
        self.lag_phase = random.uniform(0, math.tau)
        self.lag_active = False
        self.recovered = False

        self.lag_glow = sphere(
        ...
        self.left_tension = cylinder(
            pos=self.left.pos,
            axis=vector(0, 0, 0),
            radius=0.012,
            color=vector(1.0, 0.48, 0.18),
            opacity=0
        )
        self.right_tension = cylinder(
            pos=self.right.pos,
            axis=vector(0, 0, 0),
            radius=0.012,
            color=vector(1.0, 0.48, 0.18),
        ...
        sim.register(self.lag_glow)
        sim.register(self.left_tension)
        sim.register(self.right_tension)
    ...
    def set_visible(self, value):
        ...
        self.lag_glow.visible = value
        self.left_tension.visible = value
        self.right_tension.visible = value
        if not value:
            self.left_trail.clear()
            self.right_trail.clear()

    def update(self, t, time_seconds, left_pole, right_pole, left_nucleus_center, right_nucleus_center, ai_noise=0.0, pull_bias=1.0, lag_recovery_bias=1.0):
        ...
        # Lagging chromosomes resist the first anaphase pull. Their recovery
        # window opens later, so the cell shows tension and then actively
        # catches them up before cytokinesis completes.
        recovery_start = 0.47 + self.lag_delay
        recovery_end = 0.72 - 0.08 * clamp(lag_recovery_bias - 1.0, 0, 1.5)
        if self.can_lag:
            lag_recovery = smoothstep(recovery_start, recovery_end, t)
            effective_pull = pull_u * lerp(1.0 - self.lag_strength, 1.0, lag_recovery)
            self.lag_active = separate > 0.16 and lag_recovery < 0.98
            self.recovered = lag_recovery >= 0.98
        else:
            lag_recovery = 1.0
            effective_pull = pull_u
            self.lag_active = False
            self.recovered = True

        left_target = vlerp(paired_left, final_left, effective_pull)
        right_target = vlerp(paired_right, final_right, effective_pull)

        if self.lag_active:
            tug = 0.035 + 0.025 * math.sin(time_seconds * 8.5 + self.lag_phase)
            left_target += vector(tug, 0.035 * math.sin(time_seconds * 5.0 + self.lag_phase), 0.025 * math.cos(time_seconds * 4.2 + self.lag_phase))
            right_target += vector(-tug, -0.035 * math.sin(time_seconds * 4.7 + self.lag_phase), -0.025 * math.cos(time_seconds * 4.5 + self.lag_phase))
        ...
        if self.lag_active:
            tension_opacity = 0.18 + 0.38 * (0.5 + 0.5 * math.sin(time_seconds * 7.0 + self.lag_phase))
            center = (self.left.pos + self.right.pos) * 0.5
            self.lag_glow.pos = center
            self.lag_glow.opacity = tension_opacity * 0.55
            self.lag_glow.radius = 0.24 + 0.05 * math.sin(time_seconds * 6.0 + self.lag_phase)
            self.left_tension.pos = self.left.pos
            self.left_tension.axis = final_left - self.left.pos
            self.left_tension.opacity = tension_opacity
            self.right_tension.pos = self.right.pos
            self.right_tension.axis = final_right - self.right.pos
            self.right_tension.opacity = tension_opacity
            self.left_fiber.color = vector(1.0, 0.48, 0.18)
            self.right_fiber.color = vector(1.0, 0.48, 0.18)
            self.left_fiber.radius = 0.022
            self.right_fiber.radius = 0.022
        else:
            self.lag_glow.opacity = 0
            self.left_tension.opacity = 0
            self.right_tension.opacity = 0
    ...
    def configure_lagging(self, delay=0.0, strength=0.55):
        self.can_lag = True
        self.lag_delay = delay
        self.lag_strength = clamp(strength, 0.20, 0.82)
        self.recovered = False

    def force_recovery(self):
        if self.can_lag and self.lag_active:
            self.lag_strength = max(0.0, self.lag_strength - 0.22)
            self.lag_delay = max(-0.08, self.lag_delay - 0.05)
            if self.lag_strength <= 0.08:
                self.recovered = True
                self.lag_active = False
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.drift_bias = 1.0
        self.lag_recovery_bias = 1.0
        self.lag_event_timer = 0
    ...
    def reset_round(self, full=False):
        ...
        # Pick a larger group of chromosomes each round to lag during anaphase.
        # Most rounds now show a crowded recovery problem rather than only one
        # or two visible delays. Strengths are slightly staggered so the lagging
        # chromosomes recover in waves instead of all snapping back at once.
        lag_count = random.randint(5, 7)
        lag_count = min(lag_count, len(self.chromosomes))
        for n, chrom in enumerate(random.sample(self.chromosomes, lag_count)):
            chrom.configure_lagging(
                delay=0.025 * n + random.uniform(0.00, 0.045),
                strength=random.uniform(0.42, 0.68)
            )
    ...
    def stage_name(self):
        ...
        if t < 0.58:
            lagging = self.lagging_chromosome_count() if hasattr(self, "chromosomes") else 0
            if lagging:
                return "Anaphase: lagging chromosomes under recovery tension"
    ...
    def lagging_chromosome_count(self):
        return sum(1 for c in self.chromosomes if getattr(c, "lag_active", False))

    def recover_one_lagging_chromosome(self):
        lagging = [c for c in self.chromosomes if getattr(c, "lag_active", False)]
        if not lagging:
            return False
        target = max(lagging, key=lambda c: c.lag_strength)
        for c in self.chromosomes:
            c.selected = False
        target.selected = True
        target.force_recovery()
        self.mark((target.left.pos + target.right.pos) * 0.5, "lagging chromosome recovered", vector(1.0, 0.48, 0.18), life=2.7)
        left_pole, right_pole = self.current_poles()
        self.spill_particles(5, source=random.choice([left_pole, right_pole]), mood="soft")
        return True
    ...
    def get_state(self):
        ...
        return {
            ...
            "lagging_count": self.lagging_chromosome_count(),
    ...
    def update(self, dt):
        ...
        lagging_now = self.lagging_chromosome_count()
        if lagging_now:
            self.lag_event_timer += dt
            # Keep late cytokinesis from rushing ahead while unrecovered
            # chromosomes are still visibly stretched between spindle forces.
            if t > 0.58:
                self.speed_modifier = lerp(self.speed_modifier, 0.62, 0.05)
            if self.lag_event_timer > 1.35:
                self.lag_event_timer = 0
                # With more lagging chromosomes per round, recovery must be
                # more frequent so cytokinesis remains tense but does not drag
                # indefinitely near the end of division.
                self.recover_one_lagging_chromosome()
        else:
            self.lag_event_timer = 0
        ...
        lagging_now = self.lagging_chromosome_count()
        lag_text = f" | lagging chromosomes {lagging_now}" if lagging_now else " | lagging recovered"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%{lag_text}"
...
class AIController:
    ...
    def choose_mode(self, state, stagnating=False):
        ...
        if state.get("lagging_count", 0) > 0:
            return random.choice(["PULL", "CAREFUL", "ORGANIZE"])
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        sim.lag_recovery_bias = lerp(sim.lag_recovery_bias, 1.0, 0.04)
        ...
        if mode == "OBSERVE":
            ...
            if self.action_timer > 1.6:
                ...
                sim.recover_one_lagging_chromosome()
            ...
            sim.lag_recovery_bias = lerp(sim.lag_recovery_bias, 1.55, 0.08)
            sim.ai_noise = lerp(sim.ai_noise, 0.055, 0.04)
            if self.action_timer > 2.2:
                self.action_timer = 0
                sim.attach_all_spindles()
                sim.recover_one_lagging_chromosome()
                if sim.lagging_chromosome_count() > 3:
                    sim.recover_one_lagging_chromosome()
            ...
            sim.lag_recovery_bias = lerp(sim.lag_recovery_bias, 1.35, 0.08)
...
def keydown(evt):
    ...
    if key == " ":
        ...
        if not sim.recover_one_lagging_chromosome():
            sim.toggle_random_spindle_attachment()
    elif key == "w":
        sim.create_wrappers("both")
    elif key == "c":
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D recover lagging chromosome | W wrap | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
