...
# Scene styling
# -----------------------------
scene.title = "Cell Membrane with Gate, Pump, and Orbit-Capture Ion Channels"
scene.width = 1200
scene.height = 760
...
Controls:
  ...
  G = cycle channel roles      H = human override timer    F = clear trails
"""
...
# Channel role colors and behavior constants
GATE_COLOR = vector(0.18, 0.85, 0.42)       # passive open/closed gate
PUMP_COLOR = vector(1.0, 0.64, 0.15)        # active directional transporter
ORBIT_COLOR = vector(0.62, 0.28, 1.0)       # ion-capture orbit channel
PUMP_INWARD_COLOR = vector(0.20, 0.88, 0.55)
PUMP_OUTWARD_COLOR = vector(1.0, 0.50, 0.18)
ORBIT_CAPTURE_RADIUS = CHANNEL_APERTURE * 1.75
ORBIT_RELEASE_TIME = 1.35
ORBIT_RADIUS = 0.58
...
class IonChannel:
    def __init__(self, idx, direction):
        ...
        # Three channel roles:
        #   gate  = passive open/closed pore
        #   pump  = actively pushes ions inward or outward
        #   orbit = captures ions into a short visible orbit, then releases them across the membrane
        self.kind = ["gate", "pump", "orbit"][idx % 3]
        self.pump_direction = -1 if idx % 2 == 0 else 1  # -1 inward, +1 outward
        ...
        self.role_ring = ring(
            pos=self.n * (R + 0.66),
            axis=self.n,
            radius=CHANNEL_RADIUS * 1.95,
            thickness=0.018,
            color=self.role_color(),
            opacity=0.58
        )

        self.pump_arrow = cone(
            pos=self.n * (R + (0.92 if self.pump_direction > 0 else -0.92)),
            axis=self.n * (0.34 if self.pump_direction > 0 else -0.34),
            radius=0.13,
            color=PUMP_OUTWARD_COLOR if self.pump_direction > 0 else PUMP_INWARD_COLOR,
            opacity=0.0 if self.kind != "pump" else 0.82
        )

        self.lbl = label(
            pos=self.n * (R + 0.82),
            text=self.role_label(),
            height=9,
    ...
    def role_label(self):
        prefix = {"gate": "G", "pump": "P", "orbit": "O"}.get(self.kind, "?")
        return f"{prefix}{(self.idx + 1) % 10}"

    def role_color(self):
        if self.kind == "gate":
            return GATE_COLOR
        if self.kind == "pump":
            return PUMP_OUTWARD_COLOR if self.pump_direction > 0 else PUMP_INWARD_COLOR
        if self.kind == "orbit":
            return ORBIT_COLOR
        return REST_COLOR

    def cycle_kind(self):
        order = ["gate", "pump", "orbit"]
        self.kind = order[(order.index(self.kind) + 1) % len(order)]
        self.lbl.text = self.role_label()
        self.update_visual(0)

    def set_pump_direction(self, direction):
        self.pump_direction = 1 if direction >= 0 else -1
        self.update_visual(0)
    ...
    def update_visual(self, dt):
        ...
        if self.open:
            ...
            self.body.color = base_col * 0.72 + role_col * 0.28
        ...
        self.role_ring.color = role_col
        self.role_ring.opacity = 0.42 + 0.18 * math.sin(self.pulse_phase + self.idx)
        self.role_ring.radius = CHANNEL_RADIUS * (1.88 + 0.10 * math.sin(self.pulse_phase * 1.6))

        if self.kind == "pump":
            self.pump_arrow.opacity = 0.82 if self.open else 0.38
            self.pump_arrow.color = PUMP_OUTWARD_COLOR if self.pump_direction > 0 else PUMP_INWARD_COLOR
            self.pump_arrow.pos = self.n * (R + (0.92 if self.pump_direction > 0 else -0.92))
            self.pump_arrow.axis = self.n * (0.34 if self.pump_direction > 0 else -0.34)
        else:
            self.pump_arrow.opacity = 0.0
...
class Ion:
    ...
    def update(self, dt):
        ...
        if self.orbit_channel is not None:
            ch = self.orbit_channel
            self.orbit_timer -= dt
            self.orbit_phase += dt * (5.8 + 1.6 * abs(self.charge))
            radial = ch.n
            tangent_a = cross(radial, vector(0, 1, 0))
            if mag(tangent_a) < 0.02:
                tangent_a = cross(radial, vector(1, 0, 0))
            tangent_a = safe_norm(tangent_a)
            tangent_b = safe_norm(cross(radial, tangent_a))
            orbit_offset = (math.cos(self.orbit_phase) * tangent_a + math.sin(self.orbit_phase) * tangent_b) * ORBIT_RADIUS
            self.pos = radial * (R + self.orbit_side * 0.34) + orbit_offset
            self.vel = tangent_swirl(radial + orbit_offset * 0.2, 0.55)
            self.obj.pos = self.pos

            if self.orbit_timer <= 0 or not ch.open:
                # Release to the opposite side of the membrane.
                release_side = -self.orbit_side
                self.pos = radial * (R + release_side * 0.22)
                self.vel = radial * release_side * pyrandom.uniform(0.75, 1.25) + tangent_swirl(radial, pyrandom.uniform(0.18, 0.48))
                self.obj.pos = self.pos
                self.orbit_channel = None
                self.orbit_timer = 0.0
        ...
        if near_membrane:
            ...
            if nearest is not None:
                ...
                if ch.open and surf_dist < CHANNEL_APERTURE * 2.2:
                    ...
                    if ch.kind == "pump":
                        pull_gain = 0.68
                        channel_pull += ch.n * ch.pump_direction * 0.95
                    elif ch.kind == "orbit":
                        pull_gain = 0.72
                        channel_pull += tangent_swirl(ch.n, 0.72)
                        if surf_dist < ORBIT_CAPTURE_RADIUS and pyrandom.random() < 0.035:
                            self.orbit_channel = ch
                            self.orbit_timer = ORBIT_RELEASE_TIME + pyrandom.uniform(-0.25, 0.35)
                            self.orbit_side = -1 if old_r < R else 1
                            ch.capture_count += 1
                            self.set_marked(True)
                            return
                    channel_pull += safe_norm(tangent_pull) * clamp((CHANNEL_APERTURE * 2.2 - surf_dist), 0, 1.5) * pull_gain
            ...
            if selected is not None:
                ...
                if ch.open:
                    if ch.kind == "orbit":
                        ...
                        self.set_marked(True)
                        self.vel = vector(0, 0, 0)
                        new_pos = ch.n * (R + self.orbit_side * 0.34)
                    elif ch.kind == "pump":
                        ch.transfer_flash()
                        self.recent_transfer_timer = 0.8
                        total_crossings += 1
                        release_side = ch.pump_direction
                        new_pos = ch.n * (R + release_side * 0.22)
                        self.vel = ch.n * release_side * pyrandom.uniform(0.95, 1.45) + tangent_swirl(ch.n, pyrandom.uniform(0.08, 0.32))
                        if pyrandom.random() < 0.16:
                            self.set_marked(True)
                    else:
                        ch.transfer_flash()
                        self.recent_transfer_timer = 0.8
...
class MembraneAI:
    ...
    def step(self, dt):
        ...
        # Homeostatic pump orientation: pump channels reverse direction depending on imbalance.
        # Too many inside -> pump outward. Too many outside -> pump inward.
        if inside > outside + 4:
            for ch in channels:
                if ch.kind == "pump":
                    ch.set_pump_direction(1)
...
def handle_ion_collisions():
    ...
    for i in range(n):
        a = ions[i]
        if a.attached_channel is not None or a.orbit_channel is not None:
            continue
        for j in range(i + 1, n):
            b = ions[j]
            if b.attached_channel is not None or b.orbit_channel is not None:
                continue
            delta = b.pos - a.pos
            d2 = mag2(delta)
            if 1e-8 < d2 < min_d2:
...
def update_meter_and_labels():
    ...
    open_count = sum(1 for ch in channels if ch.open)
    gate_count = sum(1 for ch in channels if ch.kind == "gate")
    pump_count = sum(1 for ch in channels if ch.kind == "pump")
    orbit_count = sum(1 for ch in channels if ch.kind == "orbit")
    orbiting = sum(1 for ion in ions if ion.orbit_channel is not None)
    status_label.text = (
        f"Round {round_number}   Inside: {inside}   Outside: {outside}   "
        f"Open: {open_count}/{len(channels)}   Transfers: {total_crossings}   Orbiting: {orbiting}\n"
        f"Channel roles: Gates {gate_count} | Pumps {pump_count} | Orbit-release {orbit_count}"
    )
    ...
    ai_label.text = (
        ...
        f"{override}{pause_text}   G=gate P=pump O=orbit-release"
    )
