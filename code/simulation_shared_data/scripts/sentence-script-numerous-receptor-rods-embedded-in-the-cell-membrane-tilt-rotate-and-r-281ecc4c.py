...
# Compatible with core sentence branching CSV runs.
# ------------------------------------------------------------
...
def tint(c, amount=0.2):
    return vector(clamp(c.x + amount, 0, 1), clamp(c.y + amount, 0, 1), clamp(c.z + amount, 0, 1))

def csv_float(value, ndigits=6):
    try:
...
class SimulationCSVLogger:
    FIELDNAMES = [
        ...
        "ai_mode",
        "ai_stagnant_time_s",
        "ai_manual_override_s",
        "receptor_count",
        "active_receptor_count",
        "bound_receptor_count",
        "ready_receptor_count",
        "ligand_count",
        "specific_ligand_count",
        "nonspecific_ligand_count",
        "free_ligand_count",
        "free_specific_ligand_count",
        "free_nonspecific_ligand_count",
        "bound_ligand_count",
        "avg_free_ligand_speed",
        "signal_count",
        "messenger_count",
        "membrane_mark_count",
        "delivered_count",
        ...
        "first_free_ligand_y",
        "first_free_ligand_z",
        "first_free_ligand_speed",
        "first_ready_receptor_x",
        "first_ready_receptor_y",
        "first_ready_receptor_z",
    ]

    def __init__(self, label="signal_receptor_scanning"):
        self.label = label
        self.run_id = os.environ.get("SIMULATION_CSV_RUN_ID", "").strip() or time.strftime("%Y%m%d_%H%M%S")
        self.run_seconds = env_float("SIMULATION_CSV_RUN_SECONDS", 60.0)
        self.sample_hz = max(0.1, env_float("SIMULATION_CSV_SAMPLE_HZ", 5.0))
        ...
        output_dir = os.environ.get("SIMULATION_CSV_OUTPUT_DIR", "").strip()
        fallback = os.environ.get("SIM_STATE_CSV_PATH", "").strip()

        if output_dir:
            outdir = Path(output_dir).expanduser()
            outdir.mkdir(parents=True, exist_ok=True)
            self.csv_path = outdir / f"{self.run_id}_{self.label}.csv"
        elif fallback:
            self.csv_path = Path(fallback).expanduser()
            self.csv_path.parent.mkdir(parents=True, exist_ok=True)
        else:
            self.csv_path = Path.cwd() / f"{self.run_id}_{self.label}.csv"
    ...
    def write_metadata(self, status, sim=None):
        data = {
            "status": status,
            "label": self.label,
            "run_id": self.run_id,
            ...
            "source": "full_vpython_script_with_integrated_csv_logging",
            "environment_variables": [
                "SIMULATION_CSV_OUTPUT_DIR",
                "SIMULATION_CSV_RUN_ID",
                "SIMULATION_CSV_RUN_SECONDS",
                "SIMULATION_CSV_SAMPLE_HZ",
                "SIM_STATE_CSV_PATH",
            ],
        }
        if sim is not None:
            data.update({
                "final_sim_time_s": getattr(sim, "t", None),
                "final_round_index": getattr(sim, "round_index", None),
                "final_delivered_count": getattr(sim, "delivered_count", None),
                "final_round_goal": getattr(sim, "round_goal", None),
    ...
    def should_stop(self):
        if self.run_seconds <= 0:
            return False
        return (time.time() - self.start_wall) >= self.run_seconds

    def maybe_log(self, sim, force=False):
        if self.closed:
            return
        now = time.time()
        if force or self.last_sample_wall is None or now - self.last_sample_wall >= self.sample_interval:
            self.last_sample_wall = now
            self.log(sim, now)

    def log(self, sim, now=None):
        now = now if now is not None else time.time()
        receptors = sim.receptors
        ligands = sim.ligands
        free = [l for l in ligands if l.attached_to is None]
        bound_ligands = [l for l in ligands if l.attached_to is not None]
        free_specific = [l for l in free if l.specific]
        free_nonspecific = [l for l in free if not l.specific]
        specific = [l for l in ligands if l.specific]
        nonspecific = [l for l in ligands if not l.specific]
        active = [r for r in receptors if r.activated]
        bound_receptors = [r for r in receptors if r.bound_ligand is not None]
        ready = [r for r in receptors if r.can_bind()]

        avg_speed = sum(mag(l.vel) for l in free) / max(1, len(free))
        first_free = free[0] if free else None
        first_ready = ready[0] if ready else None
        fraction = sim.delivered_count / max(1, sim.round_goal)

        row = {
            "wall_elapsed_s": csv_float(now - self.start_wall),
            "sample_index": self.sample_index,
            "sim_time_s": csv_float(sim.t),
            "round_index": sim.round_index,
            "paused": 1 if sim.paused else 0,
            "ai_enabled": 1 if sim.ai.enabled else 0,
            "ai_mode": sim.ai.mode,
            "ai_stagnant_time_s": csv_float(sim.ai.stagnant_time),
            "ai_manual_override_s": csv_float(sim.ai.manual_override_timer),
            "receptor_count": len(receptors),
            "active_receptor_count": len(active),
            "bound_receptor_count": len(bound_receptors),
            "ready_receptor_count": len(ready),
            "ligand_count": len(ligands),
            "specific_ligand_count": len(specific),
            "nonspecific_ligand_count": len(nonspecific),
            "free_ligand_count": len(free),
            "free_specific_ligand_count": len(free_specific),
            "free_nonspecific_ligand_count": len(free_nonspecific),
            "bound_ligand_count": len(bound_ligands),
            "avg_free_ligand_speed": csv_float(avg_speed),
            "signal_count": len(sim.signals),
            "messenger_count": len(sim.messengers),
            "membrane_mark_count": len(sim.marks),
            "delivered_count": sim.delivered_count,
            "round_goal": sim.round_goal,
            "delivery_fraction": csv_float(fraction),
            "nucleus_opacity": csv_float(sim.nucleus.opacity),
            "nucleus_core_radius": csv_float(sim.nucleus_core.radius),
            "first_free_ligand_x": csv_float(first_free.pos.x) if first_free else "",
            "first_free_ligand_y": csv_float(first_free.pos.y) if first_free else "",
            "first_free_ligand_z": csv_float(first_free.pos.z) if first_free else "",
            "first_free_ligand_speed": csv_float(mag(first_free.vel)) if first_free else "",
            "first_ready_receptor_x": csv_float(first_ready.base.x) if first_ready else "",
            "first_ready_receptor_y": csv_float(first_ready.base.y) if first_ready else "",
            "first_ready_receptor_z": csv_float(first_ready.base.z) if first_ready else "",
        }
    ...
    def close(self, sim=None):
        ...
        try:
            if sim is not None:
                self.maybe_log(sim, force=True)
            self.file.flush()
            self.file.close()
            self.write_metadata("complete", sim=sim)
        finally:
            self.closed = True
...
scene = canvas(
    title="Signal Receptor Scanning and Activation on Membrane - CSV Logging",
    width=1200,
    height=780,
...
class Receptor:
    def __init__(self, sim, base, index):
        ...
        self.socket = cylinder(pos=self.base - vector(0, 0, 0.10), axis=vector(0, 0, 0.22),
                               radius=0.17, color=vector(0.26, 0.38, 0.72), opacity=0.58)
        self.rod = cylinder(pos=self.base, axis=self.direction * self.length, radius=self.radius,
                            color=RECEPTOR_IDLE_COLOR)
        self.tip_sphere = sphere(pos=self.base + self.direction * self.length, radius=0.14,
                                 color=tint(RECEPTOR_IDLE_COLOR, 0.12), shininess=0.75)
        self.scan_whisker = cylinder(pos=self.tip_sphere.pos, axis=vector(0.15, 0, 0.03),
                                     radius=0.014, color=vector(0.72, 0.83, 1.0), opacity=0.50)
        self.ring = ring(pos=self.base + vector(0, 0, 0.015), axis=vector(0, 0, 1),
                         radius=0.26, thickness=0.018, color=vector(0.52, 0.70, 0.94), opacity=0.38)
        self.halo = sphere(pos=self.tip_sphere.pos, radius=0.42, color=vector(1.0, 0.72, 0.20),
                           opacity=0.0, visible=True)
        self.inside_tail = cylinder(pos=self.base - vector(0, 0, 0.08), axis=vector(0, 0, -0.34),
                                    radius=0.04, color=vector(0.36, 0.50, 0.85), opacity=0.55)
    ...
    def update(self, dt):
        ...
        if self.activated:
            ...
            self.tip_sphere.color = tint(RECEPTOR_IDLE_COLOR, 0.08)
        ...
        az = self.scan_phase
        self.direction = safe_norm(vector(math.sin(tilt) * math.cos(az), math.sin(tilt) * math.sin(az), math.cos(tilt)), vector(0, 0, 1))
        self.rod.pos = self.base
...
class Ligand:
    def __init__(self, sim, pos=None, specific=True, vel=None, artistic=False):
        ...
        base_color = SPECIFIC_COLOR if specific else NONSPECIFIC_COLOR
        if artistic and specific:
            base_color = vector(randf(0.15, 0.45), randf(0.65, 1.0), randf(0.40, 0.95))

        self.obj = sphere(pos=self.pos, radius=self.radius, color=base_color, make_trail=specific,
                          retain=35, trail_color=vector(0.40, 0.95, 0.60), trail_radius=0.015,
                          shininess=0.75)
        self.core = sphere(pos=self.pos, radius=self.radius * 0.38, color=vector(1, 1, 1),
    ...
    def update(self, dt):
        ...
        if self.attached_to is not None:
            ...
            self.pos = receptor.tip + orbit * r + receptor.direction * 0.17
            self.vel = vector(0, 0, 0)
            self.obj.color = vector(0.12, 0.95, 0.38)
            self.obj.radius = self.radius * (1.08 + 0.14 * math.sin(self.sim.t * 7.0))
        else:
            self.vel += vector(randf(-0.55, 0.55), randf(-0.55, 0.55), randf(-0.35, 0.35)) * dt
            self.vel *= 0.985 if self.specific else 0.978
            if mag(self.vel) > 2.4:
                self.vel = norm(self.vel) * 2.4
            self.pos += self.vel * dt
            if self.pos.x < -WORLD_BOUND:
                self.pos.x = -WORLD_BOUND; self.vel.x = abs(self.vel.x) * 0.8
            if self.pos.x > WORLD_BOUND:
                self.pos.x = WORLD_BOUND; self.vel.x = -abs(self.vel.x) * 0.8
            if self.pos.y < -WORLD_BOUND:
                self.pos.y = -WORLD_BOUND; self.vel.y = abs(self.vel.y) * 0.8
            if self.pos.y > WORLD_BOUND:
                self.pos.y = WORLD_BOUND; self.vel.y = -abs(self.vel.y) * 0.8
            if self.pos.z < OUTSIDE_Z_MIN:
                self.pos.z = OUTSIDE_Z_MIN; self.vel.z = abs(self.vel.z) * randf(0.55, 0.95)
            if self.pos.z > OUTSIDE_Z_MAX:
                self.pos.z = OUTSIDE_Z_MAX; self.vel.z = -abs(self.vel.z) * randf(0.55, 0.95)
...
class SignalParticle:
    def __init__(self, sim, start, target, delay=0.0, color_value=SIGNAL_COLOR):
        ...
        self.obj = sphere(pos=self.pos, radius=0.075, color=color_value, emissive=True, make_trail=True,
                          retain=45, trail_color=color_value, trail_radius=0.026)
        self.obj.visible = False
        self.glow = sphere(pos=self.pos, radius=0.20, color=color_value, opacity=0.12, emissive=True)
    ...
    def update(self, dt):
        ...
        if mag(to_target) < 0.22:
            self.dead = True
            self.sim.deliver_signal(self.pos)
            self.hide()
            return
        direction = safe_norm(to_target, vector(0, 0, -1))
        side = safe_norm(cross(direction, vector(0, 0, 1)), vector(1, 0, 0))
...
class Messenger:
    def __init__(self, sim, pos, vel=None, color_value=MESSENGER_COLOR):
        ...
        self.dead = False
        self.obj = sphere(pos=self.pos, radius=randf(0.035, 0.065), color=color_value, emissive=True,
                          make_trail=True, retain=18, trail_color=color_value, trail_radius=0.010)
    ...
    def update(self, dt):
        ...
        if self.pos.z > INSIDE_Z_MAX:
            self.pos.z = INSIDE_Z_MAX; self.vel.z = -abs(self.vel.z) * 0.65
        if self.pos.z < INSIDE_Z_MIN:
            self.pos.z = INSIDE_Z_MIN; self.vel.z = abs(self.vel.z) * 0.65
        for axis in ["x", "y"]:
            val = getattr(self.pos, axis)
            if val < -WORLD_BOUND:
                setattr(self.pos, axis, -WORLD_BOUND); setattr(self.vel, axis, abs(getattr(self.vel, axis)) * 0.75)
...
class MembraneMark:
    def __init__(self, pos, color_value):
        ...
        self.max_life = self.life
        self.dead = False
        self.obj = ring(pos=self.pos, axis=vector(0, 0, 1), radius=0.18,
                        thickness=0.014, color=color_value, opacity=0.55)
...
class AIController:
    ...
    def get_state(self):
        ...
        free_specific = [l for l in free if l.specific]
        free_nonspecific = [l for l in free if not l.specific]
        active = [r for r in self.sim.receptors if r.activated]
        bound = [r for r in self.sim.receptors if r.bound_ligand is not None]
        ...
        return {
            ...
            "inactive_ready_count": len(ready),
    ...
    def update_stagnation(self, dt, st):
        metric = (st["free_specific_count"], st["active_count"], st["bound_count"], st["signal_count"], st["delivered_count"])
        if self.last_metric is None:
            self.last_metric = metric
            return
        if metric == self.last_metric and st["avg_ligand_speed"] < 0.16 and st["signal_count"] == 0:
            self.stagnant_time += dt
        else:
            self.stagnant_time = max(0.0, self.stagnant_time - dt * 0.75)
        self.last_metric = metric

    def is_complete_or_empty(self, st):
        complete = st["delivered_count"] >= st["goal"] and st["signal_count"] == 0 and st["active_count"] == 0
        empty = st["free_specific_count"] == 0 and st["active_count"] == 0 and st["signal_count"] == 0
        stable = self.stagnant_time > 8.0
        return complete or empty or stable

    def choose_new_mode(self, force=False):
        st = self.get_state() if hasattr(self, "sim") else None
        if st is not None and self.is_complete_or_empty(st):
            new_mode = "RESET_LOOP"
        else:
            candidates = ["OBSERVE", "CURIOUS_GUIDE", "CAREFUL_BIND", "PLAYFUL_STIR", "RITUAL_ORBIT", "CONSTRUCTIVE_ACTIVATE", "ARTISTIC_MARK"]
            if st:
                if st["free_specific_count"] < 4:
                    candidates += ["CONSTRUCTIVE_ACTIVATE", "ARTISTIC_MARK"]
                if st["active_count"] == 0 and st["free_specific_count"] > 0:
                    candidates += ["CURIOUS_GUIDE", "CAREFUL_BIND"]
                if st["signal_count"] > 12:
                    candidates += ["OBSERVE", "RITUAL_ORBIT"]
                if st["ligand_count"] < 85:
                    candidates += ["CHAOTIC_SPILL"]
            candidates = [m for m in candidates if m != self.mode] or ["OBSERVE"]
    ...
    def choose_ready_receptor(self):
        ready = [r for r in self.sim.receptors if r.can_bind()]
        return random.choice(ready) if ready else (random.choice(self.sim.receptors) if self.sim.receptors else None)
    ...
    def guide_ligand_to_receptor(self, ligand, receptor, speed=1.25, softness=0.55):
        ...
        ligand.vel = ligand.vel * softness + safe_norm(target - ligand.pos, vector(0, 0, -1)) * speed * (1 - softness)
        ligand.ai_mark = True
    ...
    def spawn_specific_near_receptor(self, receptor=None, count=1, artistic=False):
        receptor = receptor or self.choose_ready_receptor()
        if receptor is None:
            return
        for _ in range(count):
            pos = receptor.base + vector(randf(-0.8, 0.8), randf(-0.8, 0.8), randf(1.6, 3.8))
    ...
    def step(self, dt):
        ...
        if self.is_complete_or_empty(st) and self.mode != "RESET_LOOP":
            self.choose_new_mode(force=True)
        if self.mode_timer > self.mode_duration and self.mode != "RESET_LOOP":
            self.choose_new_mode()

        if self.mode == "OBSERVE":
            self.behavior_observe(dt, st)
        elif self.mode == "CURIOUS_GUIDE":
            self.behavior_curious_guide(dt, st)
        elif self.mode == "CAREFUL_BIND":
            self.behavior_careful_bind(dt, st)
        elif self.mode == "PLAYFUL_STIR":
            self.behavior_playful_stir(dt, st)
        elif self.mode == "RITUAL_ORBIT":
            self.behavior_ritual_orbit(dt, st)
        elif self.mode == "CONSTRUCTIVE_ACTIVATE":
            self.behavior_constructive_activate(dt, st)
        elif self.mode == "ARTISTIC_MARK":
            self.behavior_artistic_mark(dt, st)
        elif self.mode == "CHAOTIC_SPILL":
            self.behavior_chaotic_spill(dt, st)
        elif self.mode == "RESET_LOOP":
            self.behavior_reset_loop(dt, st)

    def behavior_observe(self, dt, st):
        if self.action_timer <= 0:
            self.action_timer = randf(1.5, 3.5)
            if st["free_specific_count"] < 5:
    ...
    def behavior_curious_guide(self, dt, st):
        if st["free_specific_count"] < 3 and self.action_timer <= 0:
        ...
        if self.action_timer <= 0:
            ...
            if ready and free:
                ...
                l = min(free, key=lambda x: mag(x.pos - r.tip))
                self.guide_ligand_to_receptor(l, r, 1.55, 0.50)

    def behavior_careful_bind(self, dt, st):
        if self.action_timer <= 0:
            self.action_timer = 0.35
            if st["free_specific_count"] < 2:
                self.spawn_specific_near_receptor(count=1)
            ready = [r for r in self.sim.receptors if r.can_bind()]
            free = [l for l in self.sim.ligands if l.specific and l.attached_to is None]
            if ready and free:
                r = min(ready, key=lambda rr: abs(rr.base.x) + abs(rr.base.y))
                l = min(free, key=lambda x: mag(x.pos - r.tip))
                self.guide_ligand_to_receptor(l, r, 0.95, 0.72)
            for l in self.sim.ligands:
                if l.attached_to is None:
                    l.vel *= 0.985

    def behavior_playful_stir(self, dt, st):
        if self.action_timer <= 0:
            ...
            if random.random() < 0.45 and st["ligand_count"] < 80:
                self.sim.spawn_ligand(specific=random.random() < 0.70)

    def behavior_ritual_orbit(self, dt, st):
        if self.preferred_receptor is None:
        ...
        if st["free_specific_count"] < 5 and self.action_timer <= 0:
        ...
        selected = sorted(free, key=lambda l: mag(l.pos - receptor.base))[:min(14, len(free))]
        radius = 1.25 + 0.25 * math.sin(self.sim.t * 0.7)
        height = 1.6 + 0.45 * math.sin(self.sim.t * 0.5)
        for i, l in enumerate(selected):
            angle = self.sim.t * 0.7 + i * 2 * math.pi / max(1, len(selected))
            desired = receptor.base + vector(math.cos(angle) * radius, math.sin(angle) * radius, height)
            l.vel = l.vel * 0.82 + safe_norm(desired - l.pos, vector(0, 0, 1)) * min(1.35, mag(desired - l.pos) * 0.55) * 0.18
        if self.action_timer <= 0:
    ...
    def behavior_constructive_activate(self, dt, st):
        if self.action_timer <= 0:
            self.action_timer = 0.42
            if st["free_specific_count"] < max(4, st["inactive_ready_count"] // 3):
                self.spawn_specific_near_receptor(count=2)
            ready = [r for r in self.sim.receptors if r.can_bind()]
            free = [l for l in self.sim.ligands if l.specific and l.attached_to is None]
            random.shuffle(ready)
            for r in ready[:min(4, len(ready))]:
                if not free:
                    break
                l = min(free, key=lambda x: mag(x.pos - r.tip))
                self.guide_ligand_to_receptor(l, r, 1.65, 0.45)
                if l in free:
                    free.remove(l)
            for r in random.sample(self.sim.receptors, min(4, len(self.sim.receptors))):
                r.scan_boost_timer = max(r.scan_boost_timer, 0.9)

    def behavior_artistic_mark(self, dt, st):
        if self.action_timer <= 0:
            self.action_timer = randf(0.65, 1.4)
            r = self.choose_ready_receptor()
            if r:
                self.sim.flash_membrane_mark(r.base + vector(randf(-0.15, 0.15), randf(-0.15, 0.15), 0),
                                             vector(randf(0.25, 1.0), randf(0.35, 0.95), randf(0.45, 1.0)))
                r.scan_boost_timer = max(r.scan_boost_timer, 1.0)
                if random.random() < 0.45:
                    r.force_signal_spill()
                if random.random() < 0.65 and st["ligand_count"] < 85:
                    self.spawn_specific_near_receptor(r, count=1, artistic=True)
        center = vector(0, 0, 3.2)
        free = [l for l in self.sim.ligands if l.attached_to is None][:18]
        for i, l in enumerate(free):
            angle = self.sim.t * 0.42 + i * 0.55
            desired = center + vector(math.cos(angle) * (1.0 + 0.08 * i), math.sin(angle) * (1.0 + 0.08 * i), 0.35 * math.sin(angle * 2 + i))
            l.vel = l.vel * 0.90 + safe_norm(desired - l.pos) * 0.10

    def behavior_chaotic_spill(self, dt, st):
        if self.action_timer <= 0:
            self.action_timer = randf(0.30, 0.85)
            if st["ligand_count"] < 95:
                for _ in range(random.randint(1, 4)):
                    self.sim.spawn_ligand(pos=random_outside_position(zmin=2.5, zmax=6.5),
                                          specific=random.random() < 0.55,
                                          vel=random_unit_vector() * randf(0.5, 1.8))
            blast_center = random_outside_position(zmin=1.0, zmax=4.0)
            for l in self.sim.ligands:
                if l.attached_to is None:
                    l.vel += safe_norm(l.pos - blast_center, random_unit_vector()) * randf(0.15, 1.1)
    ...
    def behavior_reset_loop(self, dt, st):
        self.reset_timer -= dt
        if self.action_timer <= 0:
...
class Simulation:
    def __init__(self):
        ...
        self.status_label = label(pos=vector(-6.25, -6.05, 4.15), text="", height=12,
                                  color=vector(0.12, 0.18, 0.26), box=True, border=7,
                                  opacity=0.72, background=vector(0.98, 1.0, 1.0), line=False)
        self.mode_label = label(pos=vector(4.0, -6.1, 3.75), text="", height=14,
                                color=vector(0.10, 0.18, 0.30), box=True, border=8,
                                opacity=0.65, background=vector(0.96, 0.99, 1.0), line=False)
        self.help_label = label(pos=vector(0, 6.25, 4.25), text="", height=11,
                                color=vector(0.16, 0.20, 0.28), box=True, border=8,
                                opacity=0.76, background=vector(1.0, 1.0, 0.96), line=False)
        self.ai = AIController(self)
        scene.bind("keydown", self.on_keydown)
        self.reset_round(initial=True)

    def create_stationary_scene(self):
        self.membrane = box(pos=vector(0, 0, MEMBRANE_Z), size=vector(12.5, 12.5, 0.075),
                            color=vector(0.70, 0.86, 1.0), opacity=0.38)
        self.membrane_core = box(pos=vector(0, 0, MEMBRANE_Z - 0.035), size=vector(12.5, 12.5, 0.018),
                                 color=vector(0.47, 0.68, 0.96), opacity=0.28)
        label(pos=vector(-5.9, 5.75, 3.1), text="outside: ligand space", height=12,
              color=vector(0.20, 0.32, 0.48), box=False, opacity=0)
        label(pos=vector(-5.8, 5.75, -4.4), text="inside: cytosol → nucleus", height=12,
              color=vector(0.34, 0.22, 0.58), box=False, opacity=0)

        self.grid_lines = []
        for v in [i * 1.0 for i in range(-6, 7)]:
            self.grid_lines.append(curve(pos=[vector(-6.2, v, MEMBRANE_Z + 0.06), vector(6.2, v, MEMBRANE_Z + 0.06)],
                                         radius=0.006, color=vector(0.50, 0.67, 0.88)))
            self.grid_lines.append(curve(pos=[vector(v, -6.2, MEMBRANE_Z + 0.061), vector(v, 6.2, MEMBRANE_Z + 0.061)],
                                         radius=0.006, color=vector(0.50, 0.67, 0.88)))
        ...
        for _ in range(150):
            ...
            self.lipid_dots.append(sphere(pos=p, radius=randf(0.018, 0.032),
                                           color=random.choice([vector(0.58, 0.78, 1.0), vector(0.74, 0.88, 1.0), vector(0.90, 0.96, 1.0)]),
                                           opacity=0.50))

        self.nucleus = sphere(pos=vector(0, 0, -6.45), radius=1.18, color=NUCLEUS_COLOR,
                              opacity=0.50, shininess=0.9)
        self.nucleus_core = sphere(pos=vector(0, 0, -6.45), radius=0.65,
                                   color=vector(0.75, 0.58, 1.0), opacity=0.35, emissive=True)
        self.nucleus_ring = ring(pos=vector(0, 0, -6.45), axis=vector(0, 0, 1), radius=1.32,
                                 thickness=0.025, color=vector(0.72, 0.58, 1.0), opacity=0.36)
        self.nuclear_light = local_light(pos=vector(0, 0, -6.45), color=vector(0.35, 0.25, 0.55))
        self.cytosol_haze = sphere(pos=vector(0, 0, -3.1), radius=6.1,
                                   color=vector(0.88, 0.94, 1.0), opacity=0.055)
    ...
    def setup_receptors(self):
        self.receptors = [Receptor(self, p, i) for i, p in enumerate(self.generate_receptor_positions(RECEPTOR_COUNT))]

    def setup_ligands(self):
        self.ligands = []
        for _ in range(SPECIFIC_LIGANDS):
            self.spawn_ligand(specific=True)
        for _ in range(NONSPECIFIC_LIGANDS):
            self.spawn_ligand(specific=False)

    def reset_round(self, initial=False):
        for group in [self.receptors, self.ligands, self.signals, self.messengers, self.marks]:
            for obj in group:
                obj.hide()
        self.receptors = []
        self.ligands = []
    ...
    def spawn_ligand(self, pos=None, specific=True, vel=None, artistic=False):
        ...
        lig = Ligand(self, pos=pos, specific=specific, vel=vel, artistic=artistic)
        self.ligands.append(lig)
        return lig

    def spawn_signal_chain(self, receptor, count=7):
        base = receptor.base - vector(0, 0, 0.22)
        target = self.nucleus.pos + vector(randf(-0.38, 0.38), randf(-0.38, 0.38), randf(-0.22, 0.22))
        for i in range(count):
            self.signals.append(SignalParticle(self, base + vector(randf(-0.12, 0.12), randf(-0.12, 0.12), -0.06 * i),
                                               target, delay=i * randf(0.08, 0.16), color_value=SIGNAL_COLOR))
    ...
    def spill_messengers(self, pos, count=10):
        for _ in range(count):
            ...
            self.messengers.append(Messenger(self, pos + vector(randf(-0.12, 0.12), randf(-0.12, 0.12), randf(-0.05, 0.05)), vel=vel))

    def flash_membrane_mark(self, pos, color_value):
        self.marks.append(MembraneMark(pos, color_value))
    ...
    def deliver_signal(self, pos):
        ...
        for _ in range(4):
            self.messengers.append(Messenger(self, self.nucleus.pos + random_unit_vector() * randf(0.4, 1.0),
    ...
    def handle_collisions(self):
        for ligand in self.ligands:
            if ligand.attached_to is not None:
                continue
            if random.random() < 0.16 and self.ligands:
                other = random.choice(self.ligands)
                if other is not ligand and other.attached_to is None:
            ...
            for receptor in self.receptors:
                ...
                if ligand.specific and receptor.can_bind() and d < bind_radius:
                    ...
                    if ligand.specific and receptor.can_bind() and random.random() < 0.18:
                        receptor.bind_ligand(ligand)
                        break
                    ligand.bounce_from(receptor.tip, strength=randf(0.25, 0.75))
    ...
    def update_stationary_effects(self, dt):
        ...
        if int(self.t * 10) % 7 == 0 and self.lipid_dots:
            for dot in random.sample(self.lipid_dots, min(3, len(self.lipid_dots))):
                dot.color = random.choice([vector(0.58, 0.78, 1.0), vector(0.74, 0.88, 1.0), vector(0.90, 0.96, 1.0), vector(0.82, 0.95, 1.0)])
...
sim = Simulation()
csv_logger = SimulationCSVLogger(label="signal_receptor_scanning")
...
try:
    while True:
        ...
        last_time = now
        sim.update(dt)
        csv_logger.maybe_log(sim)
        if csv_logger.should_stop():
            break
finally:
    csv_logger.close(sim)
