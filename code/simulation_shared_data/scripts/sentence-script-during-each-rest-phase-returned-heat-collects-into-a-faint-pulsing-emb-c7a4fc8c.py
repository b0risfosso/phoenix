"""
Fire That Wants To Become A Star — Iteration 6
Unstable Inner Ember

Story:
The flame has learned a rhythm: rise, pull, receive, rest.
Now the rest phase becomes meaningful.

During each rest phase, returned heat collects into a faint pulsing ember at the base,
giving the flame its first unstable inner center.

This is not a stable star core yet:
- the ember grows only when returned heat exists
- it flickers and fades if the flame fails to receive heat
...
Controls:
    ...
    E       briefly feed the unstable ember
"""
...
# Scene
# -----------------------------
scene.title = "Fire That Wants To Become A Star — Unstable Inner Ember"
scene.width = 1120
scene.height = 720
...
# New: unstable inner center. It is deliberately small, flickering, and not reliable.
...
title_label = label(
    pos=vector(0, 4.25, 0),
    text="Rest phases collect returned heat into an unstable inner ember",
    height=16,
    box=False,
...
help_label = label(
    ...
    text="H help | P pause | R reset | W will | Space breath | I pull | C cycle | E ember",
    height=10,
    box=False,
    align="left",
...
def add_memory_mark(pos):
    ...
    escape_noticing = clamp(escape_noticing + 0.024, 0.0, 1.0)
...
def reward_returning_spark(p):
    ...
    returned_heat = clamp(returned_heat + 0.046, 0.0, 1.0)
    base_brighten = clamp(base_brighten + 0.46, 0.0, 1.0)

    cycle_energy = clamp(cycle_energy + 0.038, 0.0, 1.0)
    inward_pull = clamp(inward_pull + 0.009, 0.02, 0.52)
...
def store_returned_heat_during_rest():
    ...
    # Storage happens only in rest, only if returned heat exists.
    # It is pulsed instead of continuous, so the user can see distinct storage events.
    if cycle_phase != "rest":
        return
    ...
    amount = min(0.030 + 0.040 * returned_heat, returned_heat * 0.42)
    returned_heat = clamp(returned_heat - amount * 0.36, 0.0, 1.0)
    inner_ember_heat = clamp(inner_ember_heat + amount, 0.0, 0.86)
    base_brighten = clamp(base_brighten + amount * 3.5, 0.0, 1.0)
...
def keydown(evt):
    ...
    global cycle_time, cycle_energy, ember_feed_boost, inner_ember_heat
    ...
    if k == "p":
        ...
        inner_ember_heat = clamp(inner_ember_heat + 0.08, 0.0, 0.86)
...
def update_cycle():
    ...
    if progress >= 1.0:
        ...
        cycle_number += 1
        cycle_energy = clamp(
            cycle_energy * 0.70 + returned_heat * 0.22 + inner_ember_heat * 0.08 + will_to_become_star * 0.05,
            0.05,
            1.0
        )
        progress = 0.0
    ...
    if cycle_phase != last_phase:
        if cycle_phase == "rise":
            base_brighten = clamp(base_brighten + 0.08 + 0.08 * inner_ember_heat, 0.0, 1.0)
        elif cycle_phase == "pull":
            base_brighten = clamp(base_brighten + 0.07, 0.0, 1.0)
        elif cycle_phase == "receive":
            base_brighten = clamp(base_brighten + 0.14, 0.0, 1.0)
        elif cycle_phase == "rest":
            base_brighten = clamp(base_brighten + 0.10 + 0.18 * returned_heat, 0.0, 1.0)
...
def update_learning_state():
    ...
    inward_pull = clamp(inward_pull, 0.02, 0.52)

    manual_pull_boost *= 0.94
    ember_feed_boost *= 0.93

    escape_noticing = clamp(escape_noticing - 0.00026, 0.0, 1.0)
    returned_heat = clamp(returned_heat - 0.00018, 0.0, 1.0)
    base_brighten = clamp(base_brighten * 0.945, 0.0, 1.0)

    store_returned_heat_during_rest()

    # The ember is unstable: it fades constantly, with extra flicker when no heat returns.
    loss = 0.00070 + 0.00080 * (1.0 - returned_heat) + 0.00055 * rise_drive
    if cycle_phase == "rest":
        loss *= 0.65
    inner_ember_heat = clamp(inner_ember_heat + 0.0020 * ember_feed_boost - loss, 0.0, 0.86)

    # Instability lowers slightly as stored heat increases, but it never reaches zero here.
    inner_ember_instability = clamp(1.0 - inner_ember_heat * 0.58 + 0.12 * math.sin(t * 3.7), 0.38, 1.0)

    if cycle_phase == "rest":
        cycle_energy = clamp(cycle_energy + 0.00045 * returned_heat + 0.00025 * inner_ember_heat - 0.00015, 0.0, 1.0)
...
def update_flame_shape():
    ...
    ember_pulse = 0.5 + 0.5 * math.sin(t * (4.0 + 8.0 * inner_ember_instability))
    unstable_jitter = random.uniform(-0.04, 0.04) * inner_ember_instability * inner_ember_heat
    notice_tremble = 0.017 * escape_noticing * math.sin(t * 14.0)
    ...
    flame_base.radius = (
        ...
        + 0.016 * inward_pull
        + 0.075 * heat_pulse
        + 0.060 * inner_ember_heat
        + 0.035 * receive_drive
    )
    flame_base.pos = vector(
        0.025 * math.sin(t * 5.0) + notice_tremble + unstable_jitter,
        0.17,
        0.025 * math.cos(t * 4.4) - unstable_jitter * 0.4
    )
    flame_base.color = vector(
        1.0,
        clamp(0.24 + 0.12 * will_to_become_star + 0.10 * inward_pull + 0.38 * heat_pulse + 0.32 * inner_ember_heat, 0.0, 1.0),
        clamp(0.03 + 0.14 * heat_pulse + 0.12 * inner_ember_heat, 0.0, 1.0)
    )

    base_heat_glow.pos = flame_base.pos + vector(0, 0.02, 0)
    base_heat_glow.radius = 0.21 + 0.32 * heat_pulse + 0.08 * returned_heat + 0.10 * inner_ember_heat + 0.08 * receive_drive
    base_heat_glow.opacity = 0.05 + 0.40 * heat_pulse + 0.09 * returned_heat + 0.22 * inner_ember_heat + 0.13 * receive_drive
    base_heat_glow.color = vector(1.0, 0.45 + 0.30 * heat_pulse + 0.22 * inner_ember_heat, 0.05 + 0.12 * heat_pulse)

    # Unstable ember visual.
    unstable_ember.pos = flame_base.pos + vector(0, 0.06 + 0.015 * ember_pulse, 0)
    unstable_ember.radius = 0.045 + 0.16 * inner_ember_heat + 0.035 * ember_pulse * inner_ember_heat
    unstable_ember.opacity = clamp(0.08 + 0.74 * inner_ember_heat + 0.10 * ember_pulse, 0.05, 0.78)
    unstable_ember.color = vector(
        1.0,
        clamp(0.42 + 0.45 * inner_ember_heat + 0.12 * ember_pulse, 0.0, 1.0),
        clamp(0.05 + 0.22 * inner_ember_heat, 0.0, 1.0)
    )

    ember_pulse_ring.pos = unstable_ember.pos
    ember_pulse_ring.radius = 0.14 + 0.38 * inner_ember_heat + 0.05 * ember_pulse
    ember_pulse_ring.thickness = 0.004 + 0.018 * inner_ember_heat
    ember_pulse_ring.opacity = clamp(0.04 + 0.42 * inner_ember_heat * ember_pulse, 0.02, 0.42)
    ...
    flame_tip.radius = clamp(width, 0.12, 0.32)
    flame_tip.color = vector(
        1.0,
        clamp(0.42 + 0.14 * will_to_become_star + 0.08 * inward_pull + 0.18 * heat_pulse + 0.18 * inner_ember_heat + 0.11 * rise_drive, 0.0, 1.0),
        0.06
    )
    ...
    inner_flame.color = vector(
        1.0,
        clamp(0.82 + 0.07 * will_to_become_star + 0.07 * heat_pulse + 0.08 * inner_ember_heat, 0.0, 1.0),
        clamp(0.22 + 0.07 * heat_pulse + 0.08 * inner_ember_heat, 0.0, 1.0)
    )

    flame_light.pos = flame_base.pos + vector(0, 0.45, 0)
    flame_light.color = vector(
        1.0,
        clamp(0.36 + 0.23 * will_to_become_star + 0.10 * inward_pull + 0.22 * heat_pulse + 0.20 * inner_ember_heat, 0.0, 1.0),
        0.08 + 0.05 * inner_ember_heat
    )

    desired_star.opacity = 0.13 + 0.17 * will_to_become_star + 0.05 * slow_pulse + 0.03 * cycle_energy + 0.02 * inner_ember_heat
    ...
    will_line.radius = 0.006 + 0.008 * will_to_become_star + 0.004 * cycle_energy + 0.003 * inner_ember_heat
    will_line.color = vector(1.0, 0.46 + 0.26 * will_to_become_star + 0.08 * inner_ember_heat, 0.04)
    ...
    inward_field_ring.radius = 0.62 + 0.13 * math.sin(t * 3.0) - 0.10 * pull_drive - 0.05 * inner_ember_heat
    inward_field_ring.thickness = 0.006 + 0.020 * inward_pull + 0.010 * pull_drive
    inward_field_ring.opacity = 0.04 + 0.18 * inward_pull + 0.11 * pull_drive + 0.05 * returned_heat + 0.06 * inner_ember_heat

    cycle_halo.pos = vector(0, 0.31 + 0.03 * math.sin(t * 2.5), 0)
    cycle_halo.radius = 0.42 + 0.27 * rise_drive - 0.10 * pull_drive + 0.10 * receive_drive - 0.04 * rest_drive
    cycle_halo.thickness = 0.006 + 0.018 * (rise_drive + pull_drive + receive_drive) + 0.008 * cycle_energy
    cycle_halo.opacity = 0.08 + 0.17 * cycle_energy + 0.12 * max(rise_drive, pull_drive, receive_drive) + 0.04 * inner_ember_heat
    ...
    if cycle_phase == "rise":
        ...
        cycle_halo.color = vector(1.0, 0.45 + 0.25 * inner_ember_heat, 0.06)
...
def update_sparks():
    ...
    for p in sparks:
        ...
        upward_lift = vector(
            0,
            0.10 * will_to_become_star + 0.27 * rise_drive + 0.04 * cycle_energy + 0.035 * inner_ember_heat,
            0
        )
        ...
        if p["bendable"] and pull_zone:
            pull_strength = (
                inward_pull
                * (0.28 + 0.38 * escape_noticing + 0.28 * returned_heat + 0.22 * inner_ember_heat + 1.10 * pull_drive)
                / (0.75 + distance)
            )
            inward_force = safe_norm(rel_to_flame) * pull_strength
            downward_memory = vector(0, -0.034 * inward_pull * (escape_noticing + returned_heat + pull_drive + 0.5 * inner_ember_heat), 0)
        ...
        reward_radius = 0.39 + 0.13 * receive_drive + 0.05 * inner_ember_heat
        if p["has_bent"] and not p["rewarded"] and return_distance < reward_radius and obj.pos.y < 0.88:
        ...
        p["vel"] *= 0.985 - 0.017 * rest_drive

        obj.pos += p["vel"] * dt
        p["return_glow"] *= 0.94

        age_fade = clamp(1.0 - p["age"] / p["life"], 0.0, 0.9)
        obj.opacity = clamp(age_fade + 0.24 * p["return_glow"] + 0.09 * rise_drive + 0.04 * inner_ember_heat, 0.0, 1.0)
        obj.radius = clamp(0.016 + 0.020 * p["return_glow"] + 0.006 * rise_drive + 0.004 * inner_ember_heat, 0.012, 0.062)
        obj.color = vector(
            1.0,
            clamp(0.48 + 0.30 * p["return_glow"] + 0.13 * rise_drive + 0.12 * inner_ember_heat + 0.13 * random.random(), 0.0, 1.0),
            clamp(0.07 + 0.09 * p["return_glow"] + 0.04 * inner_ember_heat, 0.0, 1.0)
        )
...
def update_memory_marks():
    ...
    for m in memory_marks:
        ...
        end_pos = pos - vector(0, length, 0) + toward_flame * (
            0.12 * inward_pull + 0.10 * returned_heat + 0.10 * pull_drive + 0.08 * inner_ember_heat
        )
...
def update_storage_motes():
    ...
    for s in storage_motes:
        ...
        s["obj"].opacity = clamp((1.0 - frac) * (0.22 + 0.42 * inner_ember_heat), 0.0, 0.50)
...
def update_status():
    ...
    status_label.text = (
        ...
        f"unstable inner ember: {inner_ember_heat:0.2f} [{ember_bar[:18]}]\n"
        ...
        "state: unstable inner center, not yet a stable core"
    )
...
while True:
    ...
    t += dt
    will_to_become_star = clamp(will_to_become_star + 0.00028, 0.0, 0.74)
