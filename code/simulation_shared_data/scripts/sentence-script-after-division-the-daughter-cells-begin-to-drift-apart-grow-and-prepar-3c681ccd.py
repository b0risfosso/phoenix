...
#   G     : pulse daughter-cell growth/readiness markers
# ------------------------------------------------------------
...
class MitosisSimulation:
    ...
    def reset_round(self, full=False):
        ...
        # Post-division growth and next-cycle preparation markers. These remain
        # hidden until the daughter cells have nearly separated, then expand and
        # drift with each daughter cell to show early interphase preparation.
        self.left_cycle_ring = ring(
            pos=self.left_daughter.pos,
            axis=vector(1, 0, 0),
            radius=1.18,
            thickness=0.026,
            color=vector(0.26, 0.78, 0.55),
            opacity=0
        )
        self.right_cycle_ring = ring(
            pos=self.right_daughter.pos,
            axis=vector(1, 0, 0),
            radius=1.18,
            thickness=0.026,
            color=vector(0.26, 0.78, 0.55),
        ...
        self.left_cycle_core = sphere(
            pos=self.left_nucleus.pos,
            radius=0.18,
            color=vector(0.48, 0.92, 0.72),
            opacity=0,
            emissive=True
        )
        self.right_cycle_core = sphere(
        ...
        self.register(self.left_cycle_ring)
        self.register(self.right_cycle_ring)
        self.register(self.left_cycle_core)
        self.register(self.right_cycle_core)

        for side in (-1, 1):
            for i in range(10):
                theta = i * math.tau / 10 + random.uniform(-0.10, 0.10)
                phi = random.uniform(-0.70, 0.70)
                node = sphere(
                    pos=vector(0, 0, 0),
                    radius=random.uniform(0.035, 0.055),
                    color=vector(0.34 + random.random() * 0.16, 0.84, 0.62 + random.random() * 0.16),
                    opacity=0,
                    emissive=True
                )
                self.register(node)
                self.cycle_nodes.append({
                    "obj": node,
                    "side": side,
                    "theta": theta,
                    "phi": phi,
                    "phase": random.uniform(0, math.tau),
                    "shell": random.uniform(0.55, 0.95)
                })
    ...
    def stage_name(self):
        ...
        if t < 1.0 or self.complete_timer < 6.0:
            return "Post-division: daughter cells grow and prepare another cycle"
        return "Division complete: next round ready"
    ...
    def update(self, dt):
        ...
        if self.growth_pulse_timer > 0:
            self.growth_pulse_timer -= dt
        ...
        # After division, daughter cells keep drifting apart and growing as they
        # prepare for another cycle. complete_timer keeps this visible after
        # progress reaches 100%, before the AI resets the next round.
        cycle_prepare = smoothstep(0.90, 1.0, t)
        post_cycle = smoothstep(0.0, 7.0, self.complete_timer) if t >= 0.999 else 0
        growth_pulse = smoothstep(0.0, 0.4, self.growth_pulse_timer) * (1 - smoothstep(1.6, 2.2, self.growth_pulse_timer))
        growth_amount = clamp(cycle_prepare + 0.45 * post_cycle + 0.35 * growth_pulse, 0, 1.55)
        daughter_d = lerp(0.72, 2.25, split) + detach * 0.46 * self.drift_bias + 0.55 * post_cycle
        ...
        daughter_radius = lerp(1.22, 1.42, smoothstep(0.55, 0.90, t)) + 0.24 * growth_amount
        ...
        # Daughter-cell growth/readiness system: green rings, cores, and small
        # preparation nodes expand through each daughter cell once separation is
        # nearly complete. Nodes occupy the whole daughter-cell volume shell, not
        # just the nucleus, to show each new cell preparing its own next cycle.
        readiness = clamp(smoothstep(0.88, 1.0, t) + 0.65 * post_cycle + 0.50 * growth_pulse, 0, 1)
        ring_radius = daughter_radius * (0.52 + 0.36 * readiness)
        ring_spin = self.time_seconds * (0.55 + 0.45 * readiness)
        for side, ring_obj, core_obj, center in [
            (-1, self.left_cycle_ring, self.left_cycle_core, self.left_daughter.pos),
            (1, self.right_cycle_ring, self.right_cycle_core, self.right_daughter.pos),
        ]:
            ring_obj.pos = center
            ring_obj.radius = ring_radius
            ring_obj.thickness = 0.020 + 0.018 * readiness
            ring_obj.axis = norm(vector(1, 0.18 * math.sin(ring_spin + side), 0.18 * math.cos(ring_spin + side)))
            ring_obj.opacity = 0.48 * readiness
            core_obj.pos = center + vector(0.08 * side * math.sin(self.time_seconds * 1.3), 0, 0)
            core_obj.radius = 0.15 + 0.13 * readiness + 0.03 * math.sin(self.time_seconds * 4.0) * readiness
            core_obj.opacity = 0.22 * readiness

        for entry in self.cycle_nodes:
            obj = entry["obj"]
            side = entry["side"]
            center = self.left_daughter.pos if side < 0 else self.right_daughter.pos
            theta = entry["theta"] + self.time_seconds * (0.25 + 0.08 * side)
            phi = entry["phi"] + 0.18 * math.sin(self.time_seconds * 0.9 + entry["phase"])
            shell_r = daughter_radius * entry["shell"] * (0.45 + 0.45 * readiness)
            obj.pos = center + vector(
                side * 0.10 * readiness + shell_r * 0.35 * math.sin(phi),
                shell_r * math.cos(theta) * math.cos(phi),
                shell_r * math.sin(theta) * math.cos(phi)
            )
            obj.opacity = 0.68 * readiness
            obj.radius = (0.032 + 0.020 * entry["shell"]) * (0.7 + 0.55 * readiness)
        ...
        growth_label = f" | growth prep {int(clamp((smoothstep(0.88, 1.0, self.progress) + smoothstep(0.0, 7.0, self.complete_timer if self.progress >= 0.999 else 0)) * 50, 0, 100))}%"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%{growth_label}"
    ...
    def pulse_growth_preparation(self):
        self.growth_pulse_timer = 2.4
        for source in [self.left_daughter.pos, self.right_daughter.pos]:
            self.spill_particles(6, source=source, mood="ritual")
        self.mark(vector(0, 1.55, 0), "daughter cells preparing next cycle", vector(0.26, 0.78, 0.55), life=2.8)
...
class AIController:
    ...
    def choose_mode(self, state, stagnating=False):
        ...
        if state["is_complete"] and state["complete_timer"] > 9.0:
            return "RESETTING"
    ...
    def update(self, dt):
        ...
        if s["is_complete"] and s["complete_timer"] > 12.0 and self.mode != "RESETTING":
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if mode == "OBSERVE":
            ...
            if self.action_timer > 1.25:
                ...
                if state["is_complete"] or sim.progress > 0.92:
                    sim.pulse_growth_preparation()
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | W wrap | G grow | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
