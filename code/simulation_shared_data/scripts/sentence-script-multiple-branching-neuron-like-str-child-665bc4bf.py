...
# Controls:
#   SPACE pause/resume
#   Z     toggle sleep mode / low-power long-run mode
#   A     toggle AI
#   R     reset round
...
scene.title = "3D Neuron Network with Firing Signals + AI Controller - Sleep Mode Edition"
...
scene.caption = (
    ...
    "Toggle sleep mode with Z, AI with A, pause with SPACE, reset with R. "
    "Sleep mode is enabled by default for long low-power runs. "
...
# Sleep mode is intended for hours/days-long runs. It reduces frame rate,
# limits short-lived visual effects, disables particle trails, and makes the
# AI act occasionally instead of continuously. Toggle with Z.
SLEEP_MODE_DEFAULT = True
ACTIVE_FPS = 60
SLEEP_FPS = 4
SLEEP_AI_ACTION_PERIOD = 6.0
SLEEP_NATURAL_EVENT_PERIOD = 4.0
SLEEP_MAX_SIGNALS = 6
SLEEP_MAX_FREE_PARTICLES = 18
SLEEP_BRANCH_SOFT_CAP = 52
SLEEP_SYNAPSE_SOFT_CAP = 16
...
class SignalParticle:
    def __init__(self, synapse, source=None):
        ...
        start = self.position()
        trail_on = not synapse.sim.sleep_mode
        self.sphere = sphere(
            pos=start,
...
class NetworkSimulation:
    def __init__(self):
        ...
        self.sleep_mode = SLEEP_MODE_DEFAULT
        self.ai_mode_text = "sleep mode booting" if self.sleep_mode else "AI booting"
        self.time = 0.0
    ...
    def natural_events(self, dt):
        ...
        event_period = SLEEP_NATURAL_EVENT_PERIOD if self.sleep_mode else 0.32
        if self.natural_clock < event_period:
            return
        self.natural_clock = 0.0

        branch_cap = SLEEP_BRANCH_SOFT_CAP if self.sleep_mode else MAX_BRANCHES
        synapse_dist = 1.20 if self.sleep_mode else 1.65

        if not self.ai_enabled:
            if random.random() < (0.06 if self.sleep_mode else 0.22) and self.total_branch_count() < branch_cap:
                random.choice(self.neurons).grow_branch()
            if random.random() < (0.04 if self.sleep_mode else 0.16):
                self.attach_closest(max_dist=synapse_dist, force=False)
            if random.random() < (0.01 if self.sleep_mode else 0.05):
                self.detach_random_synapse()
            if random.random() < (0.03 if self.sleep_mode else 0.12):
                random.choice(self.neurons).fire(0.12 if self.sleep_mode else 0.22)
        else:
            if random.random() < (0.012 if self.sleep_mode else 0.045) and self.total_branch_count() < branch_cap:
                random.choice(self.neurons).grow_branch()
            if random.random() < (0.015 if self.sleep_mode else 0.06):
                self.attach_closest(max_dist=1.10 if self.sleep_mode else 1.35, force=False)
    ...
    def sleep_resource_cleanup(self):
        """Keep object counts low during long unattended runs."""
        if not self.sleep_mode:
            return

        live_signals = [p for p in self.signals if p.alive]
        while len(live_signals) > SLEEP_MAX_SIGNALS:
            old = live_signals.pop(0)
            old.hide()

        live_particles = [p for p in self.free_particles if p.alive]
        while len(live_particles) > SLEEP_MAX_FREE_PARTICLES:
            old = live_particles.pop(0)
            old.hide()

        if len(self.live_synapses()) > SLEEP_SYNAPSE_SOFT_CAP and random.random() < 0.015:
            self.detach_weakest_synapse()

        for n in self.neurons:
            # Sleep mode keeps long-lived decorative rings from accumulating.
            while len(n.marks) > 3:
                old = n.marks.pop(0)
                hide_obj(old)
    ...
    def update_meter(self):
        ...
        self.status_label.text = (
            ...
            f"{'SLEEP ' if self.sleep_mode else 'ACTIVE '}| {'AI ON' if self.ai_enabled else 'AI OFF'}: {self.ai_mode_text}"
        )
...
class ExpressiveAIController:
    ...
    def tick(self, dt):
        ...
        if self.sim.sleep_mode:
            self.sim.ai_mode_text = "sleep:" + self.mode
            if self.mode_timer > self.mode_duration * 2.5:
                self.set_mode(self.choose_next_mode(state, force_variety=True))
                return
            self.execute_sleep_mode(dt, state)
            return
    ...
    def execute_sleep_mode(self, dt, state):
        """Low-frequency AI behavior for unattended long runs."""
        sim = self.sim
        if self.action_timer < SLEEP_AI_ACTION_PERIOD:
            # Very gentle drift keeps the scene alive without constant redraw churn.
            if self.mode in ["orbit", "organize", "ritual"]:
                sim.organize_circle(dt, radius=3.7)
            return
        ...
        if state["branch_count"] < SLEEP_BRANCH_SOFT_CAP and self.mode in ["construct", "curious", "careful"]:
            n = random.choice(sim.neurons)
            n.grow_branch(kind=random.choice(["dendrite", "axon"]))
            n.activity = clamp(n.activity + 0.03)

        elif state["synapse_count"] < SLEEP_SYNAPSE_SOFT_CAP and self.mode in ["connect", "ritual", "organize"]:
            syn = sim.attach_closest(max_dist=1.8, force=False)
            if syn and len([s for s in sim.signals if s.alive]) < SLEEP_MAX_SIGNALS:
                sim.spawn_signal_on_synapse(syn, source=random.choice([syn.neuron_a, syn.neuron_b]))

        elif self.mode in ["pulse", "artistic", "ritual"]:
            n = random.choice(sim.neurons)
            n.fire(0.16)
            if len([p for p in sim.free_particles if p.alive]) < SLEEP_MAX_FREE_PARTICLES and random.random() < 0.25:
                sim.wrap_particles(n, count=2)

        elif self.mode in ["prune", "destructive"]:
            if state["synapse_count"] > SLEEP_SYNAPSE_SOFT_CAP:
                sim.detach_weakest_synapse()
            elif state["branch_count"] > SLEEP_BRANCH_SOFT_CAP:
                random.choice(sim.neurons).prune_leaf()

        elif self.mode == "chaotic":
            n = random.choice(sim.neurons)
            n.vel += random_unit() * 0.12
            if random.random() < 0.20:
                n.fire(0.10)

        else:
            # Fallback: one small visible change per sleep interval.
            n = random.choice(sim.neurons)
            n.activity = clamp(n.activity + 0.05)
...
def keydown(evt):
    ...
    if key == " ":
        ...
        sim.sleep_mode = not sim.sleep_mode
        sim.ai_mode_text = ("sleep:" + ai.mode) if sim.sleep_mode else ai.mode
        # Clearing existing trails prevents old high-detail trails from persisting
        # after switching into low-power mode.
        if sim.sleep_mode:
            for p in sim.signals:
                hide_obj(p.sphere)
            sim.signals = []
            for fp in sim.free_particles:
                hide_obj(fp.s)
            sim.free_particles = []
...
while True:
    fps = SLEEP_FPS if sim.sleep_mode else ACTIVE_FPS
    rate(fps)
    dt = 1.0 / fps
    ...
    if not sim.paused:
        ...
        sim.status_label.text = (
            ...
            f"Synapses {len(sim.live_synapses())} | {'SLEEP ' if sim.sleep_mode else 'ACTIVE '}| {'AI ON' if sim.ai_enabled else 'AI OFF'}: {sim.ai_mode_text}"
        )
