...
scene = canvas(
    title="3D Bacterial Colony Growth - Diffusing Nutrient Particles",
    width=1200,
    height=780,
...
Controls:
  ...
  N: seed bacterium at cursor    F: spill/diffuse nutrient particles    V: mark slime    X: scrape/clear
...
TRAIL_LIMIT = 520
PARTICLE_LIMIT = 250
NUTRIENT_PARTICLE_LIMIT = 180
DEPLETION_HALO_LIMIT = 95
...
def add_nutrient_particle(pos, strength=0.35, life=9.0, vel=None):
    """Visible nutrient speck that drifts, diffuses, and feeds the nutrient field."""
    if len(nutrient_particles) >= NUTRIENT_PARTICLE_LIMIT:
        old = nutrient_particles.pop(0)
        hide_object(old["obj"])

    if vel is None:
        vel = vector(random.uniform(-0.35, 0.35), 0, random.uniform(-0.35, 0.35))

    obj = sphere(
        pos=vector(pos.x, 0.075 + random.uniform(0.0, 0.035), pos.z),
        radius=random.uniform(0.045, 0.075),
        color=vector(1.0, 0.76, 0.26),
        opacity=0.62,
        shininess=0.55
    )
    nutrient_particles.append({
        "obj": obj,
        "vel": flat(vel),
        "life": life,
        "maxlife": life,
...
def seed_nutrient_particles(count=65):
    for _ in range(count):
        pos = richest_cell_pos() + vector(random.uniform(-2.4, 2.4), 0, random.uniform(-2.4, 2.4))
        pos.x = clamp(pos.x, -HALF + 0.3, HALF - 0.3)
        pos.z = clamp(pos.z, -HALF + 0.3, HALF - 0.3)
        add_nutrient_particle(pos, strength=random.uniform(0.20, 0.48), life=random.uniform(7.0, 16.0))


def add_depletion_halo(pos, radius=0.72, strength=0.35):
    """Transient dark halo marking nutrient-depleted ground around feeding cells."""
    if len(depletion_halos) >= DEPLETION_HALO_LIMIT:
        old = depletion_halos.pop(0)
        hide_object(old["obj"])

    obj = ring(
        pos=vector(pos.x, 0.041, pos.z),
        axis=vector(0, 1, 0),
        radius=radius,
        thickness=0.018 + 0.018 * clamp(strength),
        color=vector(0.45, 0.30, 0.64),
        opacity=0.10 + 0.14 * clamp(strength)
    )
    depletion_halos.append({"obj": obj, "life": 8.0, "maxlife": 8.0})


def update_depletion_halos(dt):
    alive = []
    for h in depletion_halos:
        h["life"] -= dt
        if h["life"] <= 0:
            hide_object(h["obj"])
            continue
        t = h["life"] / h["maxlife"]
        try:
            h["obj"].opacity = 0.22 * t
            h["obj"].radius += 0.035 * dt
        except Exception:
            pass
        alive.append(h)
    depletion_halos[:] = alive


def update_nutrient_particles(dt):
    alive = []
    for np in nutrient_particles:
        np["life"] -= dt
        obj = np["obj"]
        if np["life"] <= 0:
            hide_object(obj)
        ...
        pos = obj.pos
        grad = field_gradient(N, pos)
        drift = safe_norm(grad) * 0.08 if mag(grad) > 0.005 else vector(0, 0, 0)
        jitter = vector(math.sin(sim_time * 1.7 + np["phase"]), 0, math.cos(sim_time * 1.3 + np["phase"] * 0.7)) * 0.045
        np["vel"] += (drift + jitter) * dt
        np["vel"] *= max(0.0, 1.0 - 0.22 * dt)
        obj.pos += np["vel"] * dt
        obj.pos.y = 0.075 + 0.018 * math.sin(sim_time * 4.0 + np["phase"])
        ...
        if boundary_mode == "wrap":
            ...
            if obj.pos.x > HALF or obj.pos.x < -HALF:
                obj.pos.x = clamp(obj.pos.x, -HALF, HALF)
                np["vel"].x *= -0.7
            if obj.pos.z > HALF or obj.pos.z < -HALF:
                obj.pos.z = clamp(obj.pos.z, -HALF, HALF)
                np["vel"].z *= -0.7

        # Each visible speck slowly dissolves into the nutrient field as it diffuses.
        add_to_field(N, obj.pos, np["strength"] * 0.010 * dt, radius=0.48, cap=1.0)
        try:
            glow = clamp(sample_field(N, obj.pos))
            obj.color = mix_color(vector(0.92, 0.54, 0.20), vector(1.0, 0.88, 0.30), glow)
            obj.opacity = 0.20 + 0.46 * np["life"] / np["maxlife"]
        except Exception:
            pass
        alive.append(np)
    nutrient_particles[:] = alive


def draw_clusters_toward_nutrient_particles(dt):
    if not nutrient_particles or not bacteria:
        return

    # Use a limited nearest-bright-speck pull so clusters visibly drift toward nutrient-rich patches.
    for b in bacteria:
        if b.dead:
            continue
        best = None
        best_score = -1.0
        for np in nutrient_particles[::max(1, len(nutrient_particles)//45)]:
            offset = flat(np["obj"].pos - b.pos)
            d = mag(offset)
            if d < 4.6 and d > 1e-5:
                score = np["strength"] / (0.35 + d)
                if score > best_score:
                    best_score = score
                    best = offset / d
        if best is not None:
            local_n = sample_field(N, b.pos)
            b.apply_force(best * (0.030 + 0.050 * clamp(1.0 - local_n)) * dt)
            b.rotate_toward(math.atan2(best.z, best.x), 1.2 + 1.4 * clamp(1.0 - local_n), dt)
...
class Bacterium:
    ...
    def update(self, dt):
        ...
        if mag(grad_n) > 0.01:
            # Brighter nutrient regions exert a stronger chemotactic pull.
            desired += safe_norm(grad_n) * (1.55 if local_n < 0.72 else 0.58)
        if mag(grad_s) > 0.01:
        ...
        if consumed > 0.00042 and random.random() < 0.035:
            add_depletion_halo(self.pos, radius=random.uniform(0.36, 0.74), strength=clamp(consumed * 120.0))
...
def spill_nutrients(pos, radius=1.6, amount=0.55, particles_on=True):
    ...
    if particles_on:
        ...
        for _ in range(18):
            rp = pos + vector(random.uniform(-radius * 0.65, radius * 0.65), 0, random.uniform(-radius * 0.65, radius * 0.65))
            add_nutrient_particle(
                rp,
                strength=random.uniform(0.26, 0.52) * amount,
                life=random.uniform(6.0, 14.0),
                vel=vector(random.uniform(-0.55, 0.55), 0, random.uniform(-0.55, 0.55))
            )
...
def reset_simulation(seed_count=START_BACTERIA, from_ai=False):
    global bacteria, trails, particles, nutrient_particles, depletion_halos, sim_time, frame_count
    ...
    seed_nutrient_particles(count=60)
...
def update_labels():
    ...
    stats_label.text = (
        ...
        f"Nutrient particles: {len(nutrient_particles)}\n"
        f"Depleted halos: {len(depletion_halos)}\n"
...
while True:
    ...
    draw_clusters_toward_nutrient_particles(dt)
    ...
    update_particles(dt)
    update_nutrient_particles(dt)
    update_depletion_halos(dt)
    update_trails(dt)
