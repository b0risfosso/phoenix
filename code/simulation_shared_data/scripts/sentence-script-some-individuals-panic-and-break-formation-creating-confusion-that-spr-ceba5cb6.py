...
# Added behavior: individual panic breaks formation and spreads confusion locally
# ------------------------------------------------------------

scene.title = "Two-Predator Fish School in a Reef - Panic Spread Simulation"
...
scene.caption = (
    ...
    "O toggle human override | C clear marks | +/- predator speed | panic spreads locally\n"
)
...
# Panic/confusion behavior: a close predator can make a few fish break formation.
# Panicked fish dart away from predators, pull toward escape gates, and reduce
# normal alignment/cohesion. Nearby swimmers catch partial panic and briefly
# become confused before recovering into the school.
PANIC_TRIGGER_DISTANCE = 6.4
PANIC_FULL_DISTANCE = 2.5
PANIC_SPREAD_RADIUS = 2.55
PANIC_DECAY = 0.55
CONFUSION_DECAY = 0.75
PANIC_DART_STRENGTH = 6.8
PANIC_GATE_PULL = 3.2
PANIC_SEPARATION_BOOST = 2.2
...
class PreyFish:
    def __init__(self, idx):
        ...
        self.panic_mark_cooldown = random.uniform(0.2, 1.4)
        self.fear = 0.0
        self.panic = 0.0
        self.confusion = 0.0
    ...
    def trigger_panic(self, intensity=1.0, source_pos=None):
        ...
        if source_pos is not None:
            ...
            if mag(away) > 0.05:
                self.panic_dir = safe_norm(away + random_unit_vector() * 0.45)
            else:
                self.panic_dir = random_unit_vector()
        else:
            self.panic_dir = random_unit_vector()
        self.panic = max(self.panic, intensity)
        self.confusion = max(self.confusion, intensity * 0.75)
        self.panic_timer = max(self.panic_timer, 0.8 + 2.6 * intensity)

    def spread_panic_to_neighbors(self, prey_list, dt):
        if self.panic < 0.28 or self.caught:
            return
        for other in prey_list:
            if other is self or other.caught:
                continue
            d = mag(other.pos - self.pos)
            if d < PANIC_SPREAD_RADIUS:
                spread = (1.0 - d / PANIC_SPREAD_RADIUS) * self.panic
                if spread > 0.12:
                    other.confusion = max(other.confusion, spread * 0.70)
                    # Most nearby fish become disoriented; some fully panic and peel away.
                    if random.random() < dt * (0.65 + 1.8 * spread):
                        other.trigger_panic(spread * random.uniform(0.55, 0.95), self.pos)

    def recover_from_panic(self, dt):
        self.panic_timer = max(0.0, self.panic_timer - dt)
        if self.panic_timer <= 0:
            self.panic = max(0.0, self.panic - PANIC_DECAY * dt)
        else:
            self.panic = max(0.0, self.panic - PANIC_DECAY * 0.22 * dt)
    ...
    def update(self, dt, prey_list, predators):
        ...
        self.recover_from_panic(dt)
        self.spread_panic_to_neighbors(prey_list, dt)
        ...
        if neighbor_count > 0:
            ...
            # Panic breaks ordinary schooling: alignment and cohesion weaken,
            # while local separation becomes stronger and more irregular.
            school_break = clamp(self.panic * 0.85 + self.confusion * 0.45, 0, 1)
            acceleration += align * (0.52 * (1.0 - 0.78 * school_break))
            acceleration += cohesion * (0.34 * (1.0 - 0.92 * school_break))
            acceleration += separation * (1.35 + PANIC_SEPARATION_BOOST * school_break)
        ...
        if pred_dist < 8.0:
            ...
            # A small number of individuals panic first. Once panicked, the
            # reaction spreads through close neighbors as visible confusion.
            panic_pressure = clamp((PANIC_TRIGGER_DISTANCE - pred_dist) / max(PANIC_TRIGGER_DISTANCE - PANIC_FULL_DISTANCE, EPS), 0, 1)
            if panic_pressure > 0:
                individual_jitter = 0.65 + 0.35 * math.sin(sim_time * 3.1 + self.idx * 0.73)
                trigger_chance = dt * (0.55 + 2.4 * panic_pressure + 0.9 * pack_fear) * individual_jitter
                if random.random() < trigger_chance:
                    self.trigger_panic(0.42 + 0.58 * panic_pressure, nearest_threat.pos)

            gate = nearest_escape_gate(self.pos, nearest_threat.pos)
            gate_pull = safe_norm(gate["pos"] - self.pos)
            acceleration += gate_pull * (0.75 + 1.65 * self.fear + PANIC_GATE_PULL * self.panic)
        ...
        if self.panic > 0.05 or self.confusion > 0.05:
            # Panicked fish make abrupt diagonal darts instead of smooth flocking.
            self.panic_phase += dt * (5.5 + 4.0 * self.panic + 0.05 * self.idx)
            wobble = vector(math.sin(self.panic_phase + self.idx), 0.45 * math.cos(self.panic_phase * 1.3), math.cos(self.panic_phase * 0.9 - self.idx))
            panic_vec = safe_norm(self.panic_dir + wobble * (0.35 + 0.55 * self.confusion))
            acceleration += panic_vec * (PANIC_DART_STRENGTH * self.panic + 2.3 * self.confusion)
            acceleration += safe_norm(to_predator, panic_vec) * (4.5 * self.panic)
            self.panic_mark_cooldown -= dt
            if self.panic > 0.45 and self.panic_mark_cooldown <= 0:
                self.panic_mark_cooldown = random.uniform(0.8, 1.8)
                add_mark(self.pos, 0.35 + self.panic, vector(1.0, 0.64, 0.12), 2.6, "panic")
                spawn_particles(self.pos, vector(1.0, 0.64, 0.12), count=3, speed=1.6)
        ...
        acceleration += random_unit_vector() * (0.12 + 0.75 * self.confusion + 1.05 * self.panic)

        panic_boost = clamp(self.panic + 0.45 * self.confusion, 0, 1)
        max_force = (7.2 + 3.2 * self.fear + 5.4 * panic_boost) * PREY_ESCAPE_DAMPING
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.34 * self.fear + 0.55 * panic_boost) * PREY_ESCAPE_DAMPING)
        self.pos += self.vel * dt
    ...
    def update_visual(self):
        ...
        if not self.caught:
            ...
            panic_col = color_lerp(fear_col, vector(1.0, 0.38, 0.12), self.panic)
            confused_col = color_lerp(panic_col, vector(1.0, 0.70, 0.25), self.confusion * 0.65)
            self.body.color = confused_col
            self.fin.color = color_lerp(confused_col, color.white, 0.30 - 0.12 * self.panic)
            self.trail.color = color_lerp(color_lerp(self.base_col, vector(1, 1, 1), 0.35), vector(1.0, 0.50, 0.12), clamp(self.fear + self.panic, 0, 1))
...
def update_hud():
    ...
    panicked = sum(1 for f in prey if (not f.caught) and f.panic > 0.35)
    confused = sum(1 for f in prey if (not f.caught) and f.confusion > 0.25 and f.panic <= 0.35)
    ...
    hud.text = (
        ...
        f"Panicked: {panicked} | Confused: {confused} | "
