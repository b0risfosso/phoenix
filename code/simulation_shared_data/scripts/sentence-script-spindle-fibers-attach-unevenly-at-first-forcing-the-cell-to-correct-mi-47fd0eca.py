...
class ChromosomePair:
    def __init__(self, sim, index, base_angle, radius_ring):
        ...
        # Each sister chromatid can be correctly attached, missing an attachment,
        # or incorrectly attached to the wrong spindle pole. The checkpoint system
        # uses these states to delay anaphase until mistakes are corrected.
        self.left_attachment_state = "correct"
        self.right_attachment_state = "correct"
        self.left_wrong_pole = False
        self.right_wrong_pole = False
        self.correction_flash = 0.0
        self.tension_pulse = 0.0
        self.selected = False
    ...
    def update(self, t, time_seconds, left_pole, right_pole, left_nucleus_center, right_nucleus_center, ai_noise=0.0, pull_bias=1.0, checkpoint_hold=0.0):
        ...
        if (not self.left_attached or self.left_wrong_pole) and separate > 0.1:
            left_target += vector(0.25 * math.sin(time_seconds * 4.0 + self.index), 0.18 * pulse, 0.12 * math.cos(time_seconds * 3.6))
        if (not self.right_attached or self.right_wrong_pole) and separate > 0.1:
            right_target += vector(-0.25 * math.cos(time_seconds * 3.7 + self.index), -0.16 * pulse, 0.13 * math.sin(time_seconds * 3.2))

        if checkpoint_hold > 0 and 0.22 < t < 0.45:
            wobble = checkpoint_hold * vector(
                0.025 * math.sin(time_seconds * 7.0 + self.index),
                0.050 * math.sin(time_seconds * 5.0 + self.phase_jitter),
                0.050 * math.cos(time_seconds * 4.7 + self.phase_jitter)
            )
            if self.has_mistake():
                left_target += wobble
                right_target -= wobble
        ...
        left_origin = right_pole if self.left_wrong_pole else left_pole
        right_origin = left_pole if self.right_wrong_pole else right_pole

        self.left_fiber.pos = left_origin
        self.left_fiber.axis = self.left.pos - left_origin
        self.left_fiber.opacity = op_base if self.left_attached else op_base * 0.18
        self.left_fiber.radius = 0.024 if self.left_wrong_pole else (0.018 if self.left_attached else 0.012)
        if self.left_wrong_pole:
            self.left_fiber.color = vector(1.0, 0.26, 0.18)
        elif self.left_attached:
            self.left_fiber.color = vector(1.0, 0.84, 0.32)
        else:
            self.left_fiber.color = vector(0.90, 0.72, 0.60)

        self.right_fiber.pos = right_origin
        self.right_fiber.axis = self.right.pos - right_origin
        self.right_fiber.opacity = op_base if self.right_attached else op_base * 0.18
        self.right_fiber.radius = 0.024 if self.right_wrong_pole else (0.018 if self.right_attached else 0.012)
        if self.right_wrong_pole:
            self.right_fiber.color = vector(1.0, 0.26, 0.18)
        elif self.right_attached:
            self.right_fiber.color = vector(1.0, 0.84, 0.32)
        else:
            self.right_fiber.color = vector(0.90, 0.72, 0.60)

        self.correction_flash = max(0.0, self.correction_flash - 0.045)
        mistake_glow = 0.10 if self.has_mistake() and 0.16 < t < 0.45 else 0.0
        repair_glow = self.correction_flash * 0.22
        highlight = 0.12 if self.selected else 0
        self.left.emissive = self.selected or self.correction_flash > 0.1 or (self.has_mistake() and 0.16 < t < 0.45)
        self.right.emissive = self.left.emissive
        self.left.color = self.color_left + vector(highlight + repair_glow, highlight + mistake_glow, highlight + mistake_glow)
        self.right.color = self.color_right + vector(highlight + repair_glow, highlight + mistake_glow, highlight + mistake_glow)

    def attachment_fraction(self):
        return (1 if self.left_attached else 0) + (1 if self.right_attached else 0)

    def correct_attachment_fraction(self):
        return (1 if self.left_attachment_state == "correct" else 0) + (1 if self.right_attachment_state == "correct" else 0)

    def mistake_count(self):
        return (0 if self.left_attachment_state == "correct" else 1) + (0 if self.right_attachment_state == "correct" else 1)

    def has_mistake(self):
        return self.left_attachment_state != "correct" or self.right_attachment_state != "correct"

    def set_attachment_state(self, side, state):
        if state not in ("correct", "missing", "wrong"):
            state = "correct"
        if side == "left":
            self.left_attachment_state = state
            self.left_attached = state != "missing"
            self.left_wrong_pole = state == "wrong"
        else:
            self.right_attachment_state = state
            self.right_attached = state != "missing"
            self.right_wrong_pole = state == "wrong"

    def introduce_initial_spindle_mistake(self):
        # Early metaphase starts uneven: some chromatids are unattached and
        # others are connected to the wrong pole before correction begins.
        ...
        self.set_attachment_state(pattern[0], pattern[1])

    def correct_one_mistake(self):
        wrong_sides = []
        if self.left_attachment_state != "correct":
            wrong_sides.append("left")
        if self.right_attachment_state != "correct":
            wrong_sides.append("right")
        if not wrong_sides:
            return False
        self.set_attachment_state(random.choice(wrong_sides), "correct")
        self.correction_flash = 1.0
        return True

    def toggle_random_attachment(self):
        if random.random() < 0.5:
            if self.left_attachment_state == "correct":
                self.set_attachment_state("left", random.choice(["missing", "wrong"]))
            else:
                self.set_attachment_state("left", "correct")
            return self.left_attached
        if self.right_attachment_state == "correct":
            self.set_attachment_state("right", random.choice(["missing", "wrong"]))
        else:
            self.set_attachment_state("right", "correct")
    ...
    def detach_one(self):
        if random.random() < 0.5:
            self.set_attachment_state("left", "missing")
        else:
            self.set_attachment_state("right", "missing")
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.spindle_checkpoint_unlocked = False
        self.checkpoint_hold_strength = 0.0
        self.spindle_correction_timer = 0.0
        self.spindle_checkpoint_message_timer = 0.0
    ...
    def reset_round(self, full=False):
        ...
        self.seed_uneven_spindle_attachments()
    ...
    def stage_name(self):
        ...
        if t < 0.34:
            return "Metaphase checkpoint: uneven spindle attachments are corrected"
        if t < 0.58:
    ...
    def get_state(self):
        ...
        for c in self.chromosomes:
            ...
            correct += c.correct_attachment_fraction()
        ...
        return {
            ...
            "correct_attachment_ratio": correct / total,
            "spindle_mistakes": mistakes,
            "checkpoint_unlocked": self.spindle_checkpoint_unlocked,
    ...
    def seed_uneven_spindle_attachments(self):
        for c in self.chromosomes:
            c.attach_all()
        mistake_total = max(3, int(len(self.chromosomes) * 0.65))
        for c in random.sample(self.chromosomes, min(mistake_total, len(self.chromosomes))):
            c.introduce_initial_spindle_mistake()
        self.spindle_checkpoint_unlocked = False
        self.checkpoint_hold_strength = 1.0

    def spindle_mistake_count(self):
        return sum(c.mistake_count() for c in self.chromosomes)

    def correct_next_spindle_mistake(self):
        candidates = [c for c in self.chromosomes if c.has_mistake()]
        if not candidates:
            return False
        c = random.choice(candidates)
        did_fix = c.correct_one_mistake()
        if did_fix:
            for ch in self.chromosomes:
                ch.selected = False
            c.selected = True
            pos = (c.left.pos + c.right.pos) * 0.5
            self.mark(pos + vector(0, 0.25, 0), "checkpoint repair", vector(0.16, 0.78, 0.54), life=2.2)
            self.spill_particles(4, source=pos, mood="ritual")
        return did_fix

    def update_spindle_checkpoint(self, dt):
        t = self.progress
        mistakes = self.spindle_mistake_count()
        active_window = 0.16 < t < 0.44
        if active_window and mistakes > 0:
            self.checkpoint_hold_strength = lerp(self.checkpoint_hold_strength, 1.0, 0.08)
            self.spindle_correction_timer += dt
            interval = 0.72 if t < 0.28 else 0.42
            if self.spindle_correction_timer >= interval:
                self.spindle_correction_timer = 0
                self.correct_next_spindle_mistake()
        elif mistakes == 0 and not self.spindle_checkpoint_unlocked and t > 0.20:
            self.spindle_checkpoint_unlocked = True
            self.checkpoint_hold_strength = 0.0
            self.mark(vector(0, 1.48, 0), "checkpoint passed", vector(0.18, 0.70, 0.38), life=3.0)
            self.spill_particles(10, source=vector(0, 0.85, 0), mood="soft")
        else:
            self.checkpoint_hold_strength = lerp(self.checkpoint_hold_strength, 0.0, 0.05)
    ...
    def update(self, dt):
        ...
        mistakes_before = self.spindle_mistake_count()
        if 0.22 < self.progress < 0.42 and mistakes_before > 0:
            # Spindle assembly checkpoint: anaphase cannot proceed normally while
            # missing or wrong-pole attachments remain. The simulation still creeps
            # forward enough to show inspection and repair activity.
            speed *= 0.16
        if self.progress < 1:
            proposed_progress = clamp(self.progress + dt * speed, 0, 1)
            if mistakes_before > 0 and proposed_progress > 0.355 and not self.spindle_checkpoint_unlocked:
                proposed_progress = 0.355
            self.progress = proposed_progress
        else:
            self.complete_timer += dt

        self.update_spindle_checkpoint(dt)
        t = self.progress
        ...
        mistakes = self.spindle_mistake_count()
        checkpoint_text = " | checkpoint repairs needed: " + str(mistakes) if 0.16 < t < 0.46 and mistakes > 0 else (" | checkpoint passed" if self.spindle_checkpoint_unlocked and 0.20 < t < 0.50 else "")
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%{checkpoint_text}"
...
class AIController:
    ...
    def update(self, dt):
        ...
        self.sim.ai_label.text = (
            ...
            f"correct {int(s['correct_attachment_ratio'] * 100)}% | "
            f"mistakes {s['spindle_mistakes']} | "
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if mode == "OBSERVE":
            ...
            if self.action_timer > 1.6:
                self.action_timer = 0
                sim.correct_next_spindle_mistake()
                sim.mark(vector(0, 1.25, 0), "spindle inspection", vector(0.26, 0.72, 0.56), life=2.6)
            ...
            if self.action_timer > 2.2:
                self.action_timer = 0
                if sim.spindle_mistake_count() > 0:
                    sim.correct_next_spindle_mistake()
                else:
