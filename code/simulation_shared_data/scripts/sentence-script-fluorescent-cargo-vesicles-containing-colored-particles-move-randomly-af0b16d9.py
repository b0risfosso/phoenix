...
# Keep VPython/browser rendering stable during long runs. Built-in trails on many
# moving spheres can overload the browser after several seconds, especially when
# many cargos fuse and release/degrade particles at once.
ENABLE_TRAILS = False
    ...
    def execute_herd(self, dt):
        for c in self.active_cargos():
            ...
            if lys:
                ...
                if mag(away) < 0.001:
                    away = random_unit()
                target = lys.pos + norm(away) * (lys.radius * 0.12)
...
class Simulation:
    ...
    def update_status(self):
        ...
        self.status.text = (
            ...
            + "/"
            + str(total)
            + "\nControls: SPACE pause, I/A AI, R reset, TAB select, WASD/arrows/QE move, O orbit attach/detach, X spill, F fuse, M mark, C change AI mode, P pause AI"
            + "\nLast reset: "
    ...
    def update(self, dt):
        ...
        # Remove invisible one-shot effect visuals from the reset bookkeeping list so
        # long runs do not accumulate thousands of stale VPython object references.
