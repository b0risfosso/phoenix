...
# ------------------------------------------------------------
# Predator-Prey Fish School in a Reef
# VPython self-contained simulation with expressive AI controller
# ------------------------------------------------------------

scene.title = "Predator-Prey Fish School in a Reef - AI Reef Simulation"
...
scene.range = 18
scene.caption = (
    "\nControls: W/S forward-back, A/D left-right, Q/E up-down | "
...
class Predator:
    def __init__(self):
        self.pos = vector(0, 1.3, 0)
        self.vel = vector(2.0, 0.1, 0.7)
        ...
        self.trail = curve(color=vector(0.28, 0.44, 0.95), radius=0.035, retain=55)

    def reset(self):
        self.pos = vector(random.uniform(-2, 2), random.uniform(-1, 3), random.uniform(-2, 2))
        self.vel = random_unit_vector() * 2.0
        self.forward = safe_norm(self.vel)
        self.restlessness = 0
        self.trail.clear()
        self.update_visual()
    ...
    def apply_accel(self, accel, dt):
        accel = limit(accel, self.max_force * predator_speed_multiplier)
        self.vel += accel * dt
        self.vel *= 0.995
        self.vel = limit(self.vel, self.max_speed * predator_speed_multiplier)
    ...
    def update_visual(self):
        ...
        self.body.color = color_lerp(vector(0.17, 0.32, 0.78), vector(0.55, 0.15, 0.92), clamp((predator_speed_multiplier - 1.0) * 0.55, 0, 1))
...
class PreyFish:
    ...
    def update(self, dt, prey_list, predator):
        if self.caught:
            self.update_attached(dt, predator)
            return
        ...
        to_predator = self.pos - predator.pos
        pred_dist = mag(to_predator)
        self.fear = clamp((7.7 - pred_dist) / 7.7, 0, 1)
        if pred_dist < 8.0:
            flee = safe_norm(to_predator) * (8.0 - pred_dist) / 8.0
            acceleration += flee * (9.5 + 7.5 * self.fear)

            gate = nearest_escape_gate(self.pos, predator.pos)
            gate_pull = safe_norm(gate["pos"] - self.pos)
            acceleration += gate_pull * (1.1 + 2.7 * self.fear)
        ...
        max_force = 8.0 + 5.5 * self.fear
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.58 * self.fear))
        self.pos += self.vel * dt

        self.check_escape_gates()
        self.handle_boundaries()

        if pred_dist < PREDATOR_RADIUS + PREY_RADIUS + 0.23:
            self.mark_caught(predator)
...
def update_hud():
    ...
    hud.text = (
        f"Round {round_number} | Active prey: {active}/{NUM_PREY} | "
