...
# Behavior added: thick slime can trap individuals while others ride
# along the edge of expanding slime patches.
# ------------------------------------------------------------

scene = canvas(
    title="3D Bacterial Colony Growth - Slime Trapping and Edge Riding",
    width=1200,
    height=780,
...
Controls:
  ...
  C: attach nearby bacteria      U: detach/free nearby bacteria  O: orbit pulse
  +/-: speed up / slow down

Behavior: thick slime can trap individuals; bacteria on slime boundaries may ride along patch edges.
"""
...
# Slime-patch behavior thresholds. Thick matrix can immobilize some
# bacteria, while the steep boundary of a patch becomes a rideable edge.
TRAP_SLIME_THRESHOLD = 0.82
TRAP_RELEASE_THRESHOLD = 0.54
EDGE_SLIME_MIN = 0.22
EDGE_SLIME_MAX = 0.78
EDGE_GRADIENT_THRESHOLD = 0.018
...
class Bacterium:
    ...
    def set_trapped(self, value=True):
        self.trapped = value
        if value:
            self.edge_rider = False
            self.trap_clock = random.uniform(3.0, 7.0)
            self.vel *= 0.04
            self.omega *= 0.08
            add_to_field(S, self.pos, 0.045, radius=0.55, cap=1.35)
        else:
            self.trap_clock = 0.0
            self.vel += heading(self.theta) * random.uniform(0.04, 0.12)
            self.omega += random.uniform(-0.6, 0.6)

    def update_slime_patch_state(self, local_s, grad_s, dt):
        grad_mag = mag(grad_s)
        on_edge = EDGE_SLIME_MIN < local_s < EDGE_SLIME_MAX and grad_mag > EDGE_GRADIENT_THRESHOLD

        if self.trapped:
            self.trap_clock -= dt
            # Very thick slime keeps individuals stuck, but decay, scraping,
            # or a lucky struggle can free them after a short delay.
            if local_s < TRAP_RELEASE_THRESHOLD or (self.trap_clock <= 0 and random.random() < 0.10 * dt):
                self.set_trapped(False)
            return on_edge, grad_mag

        if local_s > TRAP_SLIME_THRESHOLD:
            trap_chance = (0.10 + 0.38 * clamp((local_s - TRAP_SLIME_THRESHOLD) / (1.35 - TRAP_SLIME_THRESHOLD))) * dt
            if self.attached:
                trap_chance *= 1.65
            if random.random() < trap_chance:
                self.set_trapped(True)
                return on_edge, grad_mag

        self.edge_rider = False
        if on_edge and not self.attached:
            ride_chance = (0.20 + 0.25 * clamp(grad_mag * 10.0)) * dt
            if self.edge_clock > 0 or random.random() < ride_chance:
                self.edge_rider = True
                self.edge_clock = random.uniform(1.2, 3.6)
        if self.edge_clock > 0:
            self.edge_clock -= dt
    ...
    def update_visual(self):
        ...
        if self.trapped:
            c = mix_color(c, vector(0.12, 0.62, 0.58), 0.58)
            self.halo.color = vector(0.12, 0.82, 0.76)
            self.halo.opacity = 0.20
            self.halo.radius = self.radius * (2.55 + 0.20 * math.sin(sim_time * 5.0 + self.id))
        elif self.edge_rider:
            c = mix_color(c, vector(1.0, 0.82, 0.28), 0.48)
            self.halo.color = vector(1.0, 0.78, 0.24)
            self.halo.opacity = 0.16
            self.halo.radius = self.radius * (2.25 + 0.22 * math.sin(sim_time * 4.0 + self.id))
        elif self.attached:
    ...
    def update(self, dt):
        ...
        on_slime_edge, slime_edge_strength = self.update_slime_patch_state(local_s, grad_s, dt)
        ...
        if self.edge_rider and mag(grad_s) > 0.001:
            # Ride along the boundary of the growing slime patch instead of
            # cutting straight into the thickest matrix.
            tangent = safe_norm(perpendicular_xz(grad_s))
            if (self.id % 2) == 0:
                tangent *= -1
            desired += tangent * (1.35 + 0.55 * clamp(slime_edge_strength * 10.0))
        ...
        if self.trapped:
            # Trapped bacteria twitch and slowly thicken the local slime rather
            # than crawling away. They still consume a little nutrient, leaving
            # stuck, darker cores inside dense patches.
            self.theta += self.omega * dt * 0.08 + random.uniform(-0.04, 0.04) * dt
            self.pos += self.vel * dt * 0.08
            self.vel *= max(0.0, 1.0 - 2.8 * dt)
            slime_rate = 0.075
            consume_rate = 0.007
        elif self.attached:
            ...
            if self.edge_rider:
                edge_tangent = safe_norm(perpendicular_xz(grad_s))
                if (self.id % 2) == 0:
                    edge_tangent *= -1
                self.vel += edge_tangent * (0.11 + 0.12 * clamp(slime_edge_strength * 8.0)) * dt
                speed *= 1.22
                slime_rate = 0.020
                consume_rate = 0.015 + 0.020 * clamp(self.energy)
            else:
        ...
        if self.trail_clock <= 0 and moved > 0.025:
            trail_strength = 0.22 + local_s * 0.3
            if self.edge_rider:
                trail_strength += 0.35
            if self.trapped:
                trail_strength += 0.15
            add_slime_trail(self.prev_pos, self.pos, strength=trail_strength)
...
class AIController:
    ...
    def read_state(self):
        ...
        return {
            ...
            "trapped_fraction": trapped / pop if pop else 0.0,
            "edge_riders": edge_riders,
    ...
    def choose_mode(self, state):
        ...
        if state["trapped_fraction"] > 0.35:
            return random.choice(["SCRAPE", "STIR", "FEED"])
...
def update_labels():
    ...
    stats_label.text = (
        ...
        f"Trapped: {state['trapped_fraction']*100:.0f}%\n"
        f"Edge riders: {state['edge_riders']}\n"
