...
scene.title = "Two-Predator Fish School with Drifting Currents - AI Reef Simulation"
...
MAX_PARTICLES = 220
MAX_MARKS = 70
CURRENT_DRIFT_SCALE = 0.55
CURRENT_STEER_SCALE = 1.55
...
# -----------------------------
# Drifting current fields
# -----------------------------

class CurrentField:
    """A visible sideways flow patch that bends movement through the reef."""
    def __init__(self, idx, center, direction, radius, strength):
        self.idx = idx
        self.center = center
        self.base_center = vector(center.x, center.y, center.z)
        self.direction = safe_norm(direction, vector(1, 0, 0))
        self.radius = radius
        self.strength = strength
        self.phase = random.uniform(0, 2 * math.pi)
        self.drift_axis = safe_norm(cross(self.direction, vector(0, 1, 0)), vector(0, 0, 1))
        self.zone = sphere(
            pos=self.center,
            radius=self.radius,
            color=vector(0.38, 0.78, 1.0),
            opacity=0.075,
        )
        self.ring = ring(
            pos=self.center,
            axis=self.direction,
            radius=self.radius * 0.72,
            thickness=0.035,
            color=vector(0.20, 0.66, 1.0),
            opacity=0.28,
        )
        self.arrow = arrow(
            pos=self.center - self.direction * self.radius * 0.48,
            axis=self.direction * self.radius * 0.95,
            shaftwidth=0.08,
            color=vector(0.18, 0.58, 0.96),
            opacity=0.46,
        )
        self.label = label(
            pos=self.center + vector(0, self.radius * 0.62, 0),
            text="current",
            height=8,
            color=vector(0.08, 0.35, 0.60),
            box=False,
            opacity=0,
        )

    def update(self, t):
        # The field drifts slowly, so the chase has to bend around moving water rather than a fixed obstacle.
        drift = self.drift_axis * math.sin(t * (0.22 + 0.035 * self.idx) + self.phase) * 1.15
        bob = vector(0, math.sin(t * (0.35 + 0.04 * self.idx) + self.phase * 0.7) * 0.55, 0)
        self.center = self.base_center + drift + bob
        pulse = 0.72 + 0.28 * (0.5 + 0.5 * math.sin(t * (1.4 + 0.08 * self.idx) + self.phase))
        self.zone.pos = self.center
        self.zone.radius = self.radius * (0.90 + 0.10 * pulse)
        self.zone.opacity = 0.045 + 0.055 * pulse
        self.ring.pos = self.center
        self.ring.radius = self.radius * (0.60 + 0.22 * pulse)
        self.ring.opacity = 0.18 + 0.17 * pulse
        self.arrow.pos = self.center - self.direction * self.radius * 0.50
        self.arrow.axis = self.direction * self.radius * (0.80 + 0.24 * pulse)
        self.arrow.opacity = 0.34 + 0.20 * pulse
        self.label.pos = self.center + vector(0, self.radius * 0.66, 0)
    ...
    def velocity_at(self, pos):
        ...
        if d >= self.radius:
            return vector(0, 0, 0)
        falloff = (1.0 - d / self.radius) ** 1.45
        # A small curl term makes fish and predators bend around the flow instead of sliding in a straight line.
        radial = safe_norm(vector(offset.x, 0, offset.z), self.drift_axis)
        curl = safe_norm(cross(vector(0, 1, 0), radial), self.direction)
        return (self.direction * self.strength + curl * self.strength * 0.32) * falloff


def make_current_fields():
    specs = [
        (vector(-8.6, -1.6, -5.5), vector(1.0, 0.05, 0.35), 4.7, 3.15),
        (vector(6.2, 0.7, -2.3), vector(-0.55, 0.03, 1.0), 4.1, 2.85),
        (vector(-2.0, 2.8, 5.2), vector(0.9, -0.02, -0.45), 4.4, 2.65),
        (vector(8.7, -2.4, 5.5), vector(-1.0, 0.04, -0.2), 3.9, 3.05),
        (vector(0.0, 0.4, -8.3), vector(0.35, 0.02, 1.0), 3.6, 2.75),
    ]
    return [CurrentField(i, center, direction, radius, strength) for i, (center, direction, radius, strength) in enumerate(specs[:NUM_CURRENTS])]


current_fields = make_current_fields()


def flow_at(pos):
    flow = vector(0, 0, 0)
    for field in current_fields:
        flow += field.velocity_at(pos)
    return limit(flow, 4.2)


def current_intensity_at(pos):
    return clamp(mag(flow_at(pos)) / 4.2, 0, 1)


def update_current_fields(t):
    for field in current_fields:
        field.update(t)
...
class Predator:
    ...
    def update(self, dt):
        local_flow = flow_at(self.pos)
        self.vel += local_flow * (CURRENT_STEER_SCALE * 0.20) * dt
        self.pos += (self.vel + local_flow * CURRENT_DRIFT_SCALE * 0.42) * dt
...
class PreyFish:
    ...
    def update(self, dt, prey_list, predators):
        ...
        local_flow = flow_at(self.pos)
        flow_strength = clamp(mag(local_flow) / 4.2, 0, 1)
        if flow_strength > 0.08:
            acceleration += local_flow * (CURRENT_STEER_SCALE * (0.75 + 0.45 * self.fear))
            # In strong current, fish lean into the flow but still try to school and flee.
            if self.mark_cooldown <= 0.15 and random.random() < 0.12:
                add_mark(self.pos, 0.22 + flow_strength * 0.35, vector(0.22, 0.68, 1.0), 2.2, "flow")

        acceleration += self.boundary_steering() * 4.0
        acceleration += random_unit_vector() * 0.12

        max_force = (7.2 + 3.2 * self.fear + 1.8 * flow_strength) * PREY_ESCAPE_DAMPING
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.34 * self.fear + 0.16 * flow_strength) * PREY_ESCAPE_DAMPING)
        self.pos += (self.vel + local_flow * CURRENT_DRIFT_SCALE) * dt
    ...
    def update_visual(self):
        self.body.pos = self.pos
        if not self.caught:
            flow_t = current_intensity_at(self.pos)
            fear_col = color_lerp(self.base_col, vector(1.0, 0.92, 0.30), self.fear * 0.85)
            current_col = color_lerp(fear_col, vector(0.32, 0.72, 1.0), flow_t * 0.45)
            self.body.color = current_col
            self.fin.color = color_lerp(current_col, color.white, 0.30)
            self.trail.color = color_lerp(color_lerp(self.base_col, vector(1, 1, 1), 0.35), vector(0.25, 0.66, 1.0), max(self.fear * 0.45, flow_t))
...
def secondary_predator_accel(pr, idx, active_prey):
    ...
    acc += flow_at(pr.pos) * (CURRENT_STEER_SCALE * 0.45)
...
class ReefAIController:
    ...
    def choose_action(self, dt, state):
        ...
        acc += flow_at(predator.pos) * (CURRENT_STEER_SCALE * 0.45)
...
def reset_round(looped=False):
    ...
    add_mark(vector(-3.0, 1.0, -2.0), 0.8, vector(0.22, 0.68, 1.0), 4.0, "currents")
...
def update_hud():
    ...
    in_current = sum(1 for f in prey if (not f.caught) and current_intensity_at(f.pos) > 0.12)
    predator_current = sum(1 for pr in predators if current_intensity_at(pr.pos) > 0.12)
    ...
    hud.text = (
        ...
        f"Currents: {len(current_fields)} fields | Fish in flow: {in_current} | Predators in flow: {predator_current}\n"
...
while True:
    ...
    update_current_fields(sim_time)
