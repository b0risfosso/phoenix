...
Some failed collapses leave unstable glowing seeds that drift, flicker, and later flower again.

Run with:
    python vacuum_bloom_reactor_unstable_seeds.py
...
C_UNSTABLE_SEED = vector(1.0, 0.18, 0.56)
C_UNSTABLE_HALO = vector(0.92, 0.42, 1.0)
...
class UnstableSeed:
    """A failed-collapse remnant that keeps moving instead of disappearing."""
    def __init__(self, pos, velocity, potency=1.0):
        self.pos = vector(pos)
        self.velocity = vector(velocity)
        self.potency = clamp(potency, 0.45, 2.2)
        self.age = 0.0
        self.life = random.uniform(7.0, 14.0)
        self.phase = random.uniform(0, 2 * math.pi)
        self.reseed_charge = 0.0
        self.body = sphere(
            pos=self.pos,
            radius=0.16 + 0.055 * self.potency,
            color=C_UNSTABLE_SEED,
            emissive=True,
            opacity=0.92,
        )
        self.halo = sphere(
            pos=self.pos,
            radius=0.55 + 0.16 * self.potency,
            color=C_UNSTABLE_HALO,
            opacity=0.16,
        )
        self.ring = ring(
            pos=self.pos,
            axis=safe_unit(vector(random.uniform(-1, 1), random.uniform(0.2, 1), random.uniform(-1, 1))),
            radius=0.38 + 0.10 * self.potency,
            thickness=0.016,
            color=C_UNSTABLE_SEED,
            opacity=0.72,
        )
        self.last_trail_pos = vector(self.pos)
        self.trail = curve(pos=[self.pos, self.pos], radius=0.012, color=C_UNSTABLE_SEED, opacity=0.38)

    def update(self, dt):
        self.age += dt
        flicker = 0.5 + 0.5 * math.sin(self.phase + self.age * (8.0 + self.potency * 2.0))

        # Failed-collapse seeds drift like charged remnants, orbiting weakly around the collector
        # while still exploring the open field.
        center_pull = safe_unit(-self.pos) * (0.08 + 0.025 * reactor_demand)
        sideways = safe_unit(vector(-self.pos.z, 0.25 * math.sin(self.age + self.phase), self.pos.x)) * 0.11
        jitter = vector(
            math.sin(self.age * 4.1 + self.phase),
            math.sin(self.age * 3.3 + self.phase * 0.4) * 0.35,
            math.cos(self.age * 3.7 + self.phase),
        ) * 0.08
        self.velocity += (center_pull + sideways + jitter) * dt
        self.velocity *= 0.992
        if mag(self.pos) > FIELD_RADIUS + 1.5:
            self.velocity += safe_unit(-self.pos) * 0.65 * dt
        self.pos += self.velocity * dt

        self.body.pos = self.pos
        self.body.radius = (0.15 + 0.055 * self.potency) * (0.82 + 0.45 * flicker)
        self.body.opacity = 0.65 + 0.30 * flicker
        self.body.color = color_lerp(C_UNSTABLE_SEED, C_COLLAPSE, flicker)

        self.halo.pos = self.pos
        self.halo.radius = (0.52 + 0.18 * self.potency) * (0.85 + 0.40 * flicker)
        self.halo.opacity = 0.08 + 0.15 * flicker

        self.ring.pos = self.pos
        self.ring.radius = (0.34 + 0.11 * self.potency) * (0.92 + 0.18 * flicker)
        self.ring.opacity = 0.35 + 0.35 * flicker
        self.ring.rotate(angle=0.06 + 0.025 * reactor_demand, axis=self.ring.axis, origin=self.pos)

        self.trail.modify(0, pos=self.last_trail_pos)
        self.trail.modify(1, pos=self.pos)
        self.trail.opacity = 0.22 + 0.22 * flicker
        self.last_trail_pos = vector(self.pos)

        # Charge rises with age. Once charged enough, the seed may become a new bloom.
        self.reseed_charge += dt * (0.13 + 0.055 * reactor_demand + 0.025 * self.potency)
        ready_to_flower = self.reseed_charge > random.uniform(0.85, 1.35) and mag(self.pos) < FIELD_RADIUS
        expired = self.age >= self.life
        return ready_to_flower, expired

    def burst_products(self):
        products = []
        # Emit a few weak spores as the unstable seed either flowers or burns out.
        for _ in range(random.randint(2, 5)):
            direction = safe_unit(vector(random.uniform(-1, 1), random.uniform(-0.2, 0.2), random.uniform(-1, 1)))
            velocity = direction * random.uniform(0.8, 2.2)
            potency = clamp(self.potency * random.uniform(0.35, 0.75), 0.25, 1.2)
            products.append((self.pos + direction * 0.35, velocity, potency))
        return products
...
class QuantumBloom:
    def __init__(self, pos=None, seed_strength=None):
        self.pos = rand_field_pos() if pos is None else vector(pos)
    ...
    def collapse_products(self):
        ...
        # Some collapses fail to finish. Instead of vanishing, they leave unstable
        # glowing seeds that drift through the field and may later flower again.
        failure_chance = clamp(0.16 + 0.06 * reactor_demand + 0.04 * self.seed_strength, 0.12, 0.38)
        if random.random() < failure_chance:
            direction = safe_unit(vector(random.uniform(-1, 1), random.uniform(-0.15, 0.15), random.uniform(-1, 1)))
            velocity = direction * random.uniform(0.55, 1.75) + safe_unit(self.pos) * random.uniform(0.10, 0.45)
            potency = clamp(self.seed_strength * random.uniform(0.65, 1.25), 0.45, 2.2)
            products.append(("unstable_seed", self.pos + direction * 0.45, velocity, potency))
...
def reset_simulation():
    global blooms, pulses, spores, unstable_seeds, usable_energy, harvested_total, cycle_count
    for obj in blooms:
        obj.remove()
    for obj in pulses:
        obj.remove()
...
while True:
    ...
    for b in expired_blooms:
        ...
        for product in products:
            if product[0] == "pulse" and len(pulses) < MAX_PULSES:
            ...
            elif product[0] == "unstable_seed" and len(unstable_seeds) < MAX_UNSTABLE_SEEDS:
                unstable_seeds.append(UnstableSeed(product[1], product[2], product[3]))
    ...
    # Update failed-collapse unstable seeds. Charged seeds can become new blooms;
    # expired ones shed smaller spores instead of simply disappearing.
    expired_unstable = []
    for u in unstable_seeds:
        ready_to_flower, expired = u.update(dt)
        if ready_to_flower and len(blooms) < MAX_BLOOMS:
            blooms.append(QuantumBloom(pos=u.pos, seed_strength=clamp(u.potency * random.uniform(0.80, 1.30), 0.55, 1.9)))
            pulses.append(EnergyPulse(u.pos, max(0.35, u.potency * 0.65), color=C_UNSTABLE_SEED))
            expired_unstable.append(u)
        elif expired:
            for pos, vel, potency in u.burst_products():
                if len(spores) < MAX_SPORES:
                    spores.append(VacuumSpore(pos, vel, potency))
            expired_unstable.append(u)
    for u in expired_unstable:
        u.remove()
        if u in unstable_seeds:
            unstable_seeds.remove(u)

    # Unstable seeds stimulate nearby vacuum, making failed collapses visibly affect the field.
    if unstable_seeds and len(blooms) < MAX_BLOOMS and random.random() < 0.006 * len(unstable_seeds):
        parent = random.choice(unstable_seeds)
        offset = safe_unit(vector(random.uniform(-1, 1), random.uniform(-0.15, 0.15), random.uniform(-1, 1))) * random.uniform(0.8, 2.0)
        if mag(parent.pos + offset) < FIELD_RADIUS:
            blooms.append(QuantumBloom(pos=parent.pos + offset, seed_strength=clamp(parent.potency * random.uniform(0.55, 0.95), 0.45, 1.6)))

    # Vacuum occasionally births spontaneous fluctuations when energy is low or demand rises.
    vacuum_pressure = clamp(reactor_demand * 0.015 + max(0, 3.0 - usable_energy) * 0.003, 0.004, 0.055)
    if len(blooms) < MAX_BLOOMS and random.random() < vacuum_pressure:
        blooms.append(QuantumBloom(seed_strength=random.uniform(0.55, 1.25 + reactor_demand * 0.2)))

    # Keep minimum activity alive.
    if len(blooms) < 6 and random.random() < 0.08:
        blooms.append(QuantumBloom(seed_strength=random.uniform(0.7, 1.4)))
    ...
    while len(spores) > MAX_SPORES:
        old = spores.pop(0)
        old.remove()
    while len(unstable_seeds) > MAX_UNSTABLE_SEEDS:
        old = unstable_seeds.pop(0)
        old.remove()
    ...
    status.text = (
        "VACUUM BLOOM REACTOR\n"
        f"Blooms: {len(blooms):02d}   Unstable Seeds: {len(unstable_seeds):02d}   Spores: {len(spores):03d}   Pulses: {len(pulses):02d}\n"
