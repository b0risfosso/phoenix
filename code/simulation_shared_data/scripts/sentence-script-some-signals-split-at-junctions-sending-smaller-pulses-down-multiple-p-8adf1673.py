...
# Human Controls, an Expressive AI Controller, and split signal pulses.
#
# Controls:
#   SPACE pause/resume
#   A     toggle AI
...
scene.title = "3D Neuron Network with Splitting Signals + AI Controller"
...
scene.range = 10.5
scene.caption = (
    "\nLight 3D neural network simulation with splitting signal pulses. "
...
# Signal splitting behavior:
# A full pulse can divide at a neuron/junction into several smaller pulses.
# These caps keep branching visually active without letting the signal count explode.
MAX_ACTIVE_SIGNALS = 64
MAX_SIGNAL_GENERATION = 3
SPLIT_CHANCE = 0.62
...
class SignalParticle:
    def __init__(self, synapse, source=None, amplitude=1.0, generation=0, speed_multiplier=None):
        self.synapse = synapse
        self.alive = True
        self.amplitude = clamp(amplitude, 0.18, 1.25)
        self.generation = generation
        speed_factor = speed_multiplier if speed_multiplier is not None else random.uniform(0.78, 1.28)
        self.speed = random.uniform(0.55, 1.15) * (0.75 + synapse.strength) * speed_factor
        self.t = 0.0

        # source controls direction. If the pulse starts at neuron_b, it travels
        # toward neuron_a; otherwise it travels toward neuron_b.
        if source is synapse.neuron_b:
            self.forward = False
            self.source = synapse.neuron_b
            self.target = synapse.neuron_a
        else:
            self.forward = True
            self.source = synapse.neuron_a
            self.target = synapse.neuron_b

        start = self.position()
        base_radius = (0.045 + 0.035 * synapse.strength) * (0.55 + 0.55 * self.amplitude)
        trail_radius = max(0.005, base_radius * (0.16 + 0.18 * self.amplitude))
        self.sphere = sphere(
            pos=start,
            radius=base_radius,
            color=lerp_vec(vector(1.0, 0.58, 0.14), vector(1.0, 0.96, 0.24), self.amplitude),
            opacity=0.50 + 0.42 * self.amplitude,
            emissive=True,
            make_trail=True,
            retain=8 + int(8 * self.amplitude),
            trail_radius=trail_radius,
            trail_color=lerp_vec(vector(1.0, 0.52, 0.12), vector(1.0, 0.86, 0.22), self.amplitude),
        )
    ...
    def update(self, dt):
        ...
        if self.t >= 1.0:
            self.target.receive_signal((0.08 + 0.18 * self.synapse.strength) * self.amplitude)

            # At the arrival neuron, some pulses split into smaller pulses that
            # continue through other connected synapses at different speeds.
            self.synapse.seg_a.pulse(0.28 * self.amplitude)
            self.synapse.seg_b.pulse(0.28 * self.amplitude)
            self.sim = self.synapse.sim
            self.sim.split_signal_at_junction(self)
            self.hide()
            return

        self.sphere.pos = self.position()
        pulse = 0.5 + 0.5 * math.sin(self.t * math.pi * 8.0 + self.generation)
        self.sphere.radius = (0.042 + 0.052 * pulse) * (0.55 + 0.60 * self.amplitude)
        self.sphere.opacity = 0.34 + 0.58 * self.amplitude
        self.sphere.color = lerp_vec(
            vector(1.0, 0.48 + 0.16 * self.generation / max(1, MAX_SIGNAL_GENERATION), 0.12),
            vector(1.0, 1.0, 0.28),
...
class NetworkSimulation:
    ...
    def spawn_signal_on_synapse(self, synapse, source=None, amplitude=1.0, generation=0, speed_multiplier=None):
        if not synapse.alive:
            return
        if len([p for p in self.signals if p.alive]) >= MAX_ACTIVE_SIGNALS:
            return
        self.signals.append(
            SignalParticle(
                synapse,
    ...
    def split_signal_at_junction(self, signal):
        """Split an arriving signal into smaller pulses through nearby paths.

        The arrival neuron acts as a junction. A strong early-generation pulse can
        divide into two to four lower-amplitude pulses. Each child pulse begins at
        the arrival neuron and travels along a different live synapse with its own
        speed multiplier, making split traffic visibly fan out through the network.
        """
        if signal.generation >= MAX_SIGNAL_GENERATION:
            return
        if signal.amplitude < 0.30:
            return
        if len([p for p in self.signals if p.alive]) >= MAX_ACTIVE_SIGNALS - 2:
            return
        if random.random() > SPLIT_CHANCE:
            return

        junction = signal.target
        outgoing = [
            s for s in self.live_synapses()
            if s is not signal.synapse and (s.neuron_a is junction or s.neuron_b is junction)
        ]
        if not outgoing:
            return

        random.shuffle(outgoing)
        child_count = min(len(outgoing), random.randint(2, 4), MAX_ACTIVE_SIGNALS - len([p for p in self.signals if p.alive]))
        if child_count <= 0:
            return

        junction.activity = clamp(junction.activity + 0.10 * signal.amplitude)
        junction.mark(vector(1.0, 0.86, 0.28))
        self.spill_particles(neuron=junction, count=3 + child_count, color=vector(1.0, 0.78, 0.20))

        for syn in outgoing[:child_count]:
            child_amp = signal.amplitude * random.uniform(0.42, 0.68)
            child_speed = random.uniform(0.58, 1.65)
            self.spawn_signal_on_synapse(
                syn,
                source=junction,
                amplitude=child_amp,
                generation=signal.generation + 1,
                speed_multiplier=child_speed,
            )

        self.split_count += child_count
