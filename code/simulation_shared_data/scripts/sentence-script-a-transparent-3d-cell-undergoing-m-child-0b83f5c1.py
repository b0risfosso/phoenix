...
# SLEEP MODE / LOW-POWER LONG-RUN VERSION
# VPython self-contained source file
#
# Sleep mode is designed for leaving the simulation open for hours or days:
#   - low frame rate
#   - very slow biological progress
#   - labels hidden by default
#   - scripted camera off
#   - no automatic particle, marker, wrapper, or trail growth
#   - capped temporary object counts when manually triggered
#
# Controls:
#   Space : pause / resume
#   A     : toggle AI on/off
...
# Long-run power profile. The simulation starts in sleep mode by default.
SLEEP_MODE_START_ENABLED = True
ACTIVE_FPS = 60
SLEEP_FPS = 6
ACTIVE_DT = 1 / ACTIVE_FPS
SLEEP_DT = 1 / SLEEP_FPS
ACTIVE_PROGRESS_SPEED = 0.045       # roughly original pacing
SLEEP_PROGRESS_SPEED = 0.00075      # low-power visible cycle: mitosis completes instead of lingering at metaphase
MAX_SLEEP_PARTICLES = 0             # automatic sleep-mode bursts are disabled
MAX_ACTIVE_PARTICLES = 140
MAX_ACTIVE_MARKERS = 36
MAX_ACTIVE_WRAPPERS = 20

scene = canvas(
    title="Mitosis: Cell Division with AI Controller — Sleep Mode",
    width=980,
    height=620,
...
class ManualTrail:
    ...
    def update(self, pos):
        # In sleep mode, do not add or refresh trail dots. Keeping the same
        # preallocated objects hidden avoids long-run CPU/GPU work.
        if getattr(self.sim, "sleep_mode", False):
            return
        self.frame += 1
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.paused = False
        self.sleep_mode = SLEEP_MODE_START_ENABLED
        self.show_labels = not self.sleep_mode
        self.camera_orbit = False  # default off so mouse drag/pan/rotate controls are not overwritten
        self.sleep_visual_tick = 0
        self.time_seconds = 0
        self.progress = 0
        self.round_index = 1
        self.base_progress_speed = SLEEP_PROGRESS_SPEED if self.sleep_mode else ACTIVE_PROGRESS_SPEED
    ...
    def update(self, dt):
        ...
        if self.sleep_mode:
            # Sleep mode keeps the scene stable by removing temporary visual
            # helpers instead of updating many short-lived objects for hours.
            for p in list(self.particles):
                p.destroy()
            for m in list(self.markers):
                m.destroy()
        ...
        profile = "sleep" if self.sleep_mode else "active"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}% | {profile} mode | Z toggles active/sleep"
    ...
    def spill_particles(self, count=16, source=None, mood="soft"):
        ...
        if len(self.particles) >= MAX_ACTIVE_PARTICLES:
            return
        count = min(count, max(0, MAX_ACTIVE_PARTICLES - len(self.particles)))
        if source is None:
    ...
    def create_wrappers(self, target="both"):
        if self.sleep_mode:
            return
        if len(self.wrappers) >= MAX_ACTIVE_WRAPPERS:
            self.wrappers[0].destroy()
            self.wrappers.pop(0)
        t = self.progress
        split = smoothstep(0.50, 0.78, t)
    ...
    def set_sleep_mode(self, enabled):
        ...
        self.base_progress_speed = SLEEP_PROGRESS_SPEED if enabled else ACTIVE_PROGRESS_SPEED
        self.camera_orbit = False if enabled else self.camera_orbit
        self.show_labels = False if enabled else True
        self.ai_noise = 0.0 if enabled else self.ai_noise
        self.speed_modifier = 1.0

        # Clear temporary effects immediately when entering sleep mode.
        if enabled:
            for p in list(self.particles):
                p.destroy()
            for m in list(self.markers):
            ...
            self.mark(vector(0, -1.75, 0), "active mode restored", vector(0.2, 0.5, 0.9), life=2.5)

    def toggle_sleep_mode(self):
        self.set_sleep_mode(not self.sleep_mode)
...
class AIController:
    ...
    def update(self, dt):
        ...
        reset_delay = 900.0 if self.sim.sleep_mode else 7.0
        if s["is_complete"] and s["complete_timer"] > reset_delay and self.mode != "RESETTING":
            self.set_mode("RESETTING")

        self.apply_behavior(dt, s, stagnating)

        profile = "sleep" if self.sim.sleep_mode else "active"
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if sim.sleep_mode:
            # Low-power behavior: preserve the biological cycle, but avoid
            # decorative object creation, camera motion, high noise, and bursts.
            # Keep progress moving in sleep mode. The earlier sleep version
            # reduced speed so much that chromosomes could sit near the
            # metaphase plate for a very long time. This still runs at low FPS,
            # but uses enough biological progress drive for visible anaphase,
            # cytokinesis, and reset cycles during long unattended runs.
            sim.speed_modifier = lerp(sim.speed_modifier, 1.0, 0.04)
            if 0.30 <= state["progress"] <= 0.62:
                sim.pull_bias = lerp(sim.pull_bias, 1.18, 0.05)
            sim.ai_noise = lerp(sim.ai_noise, 0.0, 0.10)
            sim.pull_bias = lerp(sim.pull_bias, 1.0, 0.04) if not (0.30 <= state["progress"] <= 0.62) else sim.pull_bias
            sim.constriction_bias = lerp(sim.constriction_bias, 1.08 if state["progress"] >= 0.52 else 1.0, 0.04)
            sim.drift_bias = lerp(sim.drift_bias, 1.0, 0.04)
            if state["progress"] > 0.12 and self.action_timer > 180.0:
                self.action_timer = 0
                sim.attach_all_spindles()
            if mode == "RESETTING":
                if self.reset_countdown is None:
                    self.reset_countdown = 12.0
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | Z sleep/active | R reset | A AI | M mode | S particles | D spindle | W wrap | C camera | L labels | X override",
    box=False,
    opacity=0,
...
while True:
    ...
    dt = SLEEP_DT if sim.sleep_mode else ACTIVE_DT
    rate(fps)
    ai.update(dt)
    sim.update(dt)
