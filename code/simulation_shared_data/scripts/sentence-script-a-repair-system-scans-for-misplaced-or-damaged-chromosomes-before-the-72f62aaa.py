...
#   K     : force chromosome repair scan
...
class ChromosomePair:
    def __init__(self, sim, index, base_angle, radius_ring):
        ...
        # Repair-system state. A few chromosome pairs are marked as misplaced or
        # damaged each round. Before the daughter nuclei fully re-form, a scanner
        # finds these pairs, holds back nuclear opacity, and repairs them one by
        # one. The offset makes damaged chromosomes visibly imperfect until fixed.
        self.repair_needed = False
        self.repairing = False
        self.repair_progress = 0.0
        self.damage_phase = random.uniform(0, math.tau)
        self.damage_offset_left = vector(0, 0, 0)
        self.damage_offset_right = vector(0, 0, 0)
        self.repair_note = ""

        self.repair_halo = ring(
        ...
        self.damage_marker = sphere(
            pos=self.central_pos,
            radius=0.055,
            color=vector(1.0, 0.30, 0.15),
            opacity=0,
            emissive=True
        )
        ...
        sim.register(self.repair_halo)
        sim.register(self.damage_marker)
    ...
    def set_visible(self, value):
        ...
        self.repair_halo.visible = value
        self.damage_marker.visible = value
        if not value:
    ...
    def update(self, t, time_seconds, left_pole, right_pole, left_nucleus_center, right_nucleus_center, ai_noise=0.0, pull_bias=1.0):
        ...
        # Before the new nuclei fully form, unrepaired chromosomes look slightly
        # misplaced or damaged. As repair_progress rises, the offset fades away.
        repair_window = smoothstep(0.54, 0.66, t) * (1 - smoothstep(0.88, 0.98, t))
        if self.repair_needed:
            offset_strength = repair_window * (1 - clamp(self.repair_progress, 0, 1))
            jitter = vector(
                0,
                0.035 * math.sin(time_seconds * 6.0 + self.damage_phase),
                0.035 * math.cos(time_seconds * 5.2 + self.damage_phase)
            )
            left_target += (self.damage_offset_left + jitter) * offset_strength
            right_target += (self.damage_offset_right - jitter) * offset_strength
        ...
        self.update_repair_visuals(t, time_seconds)

    def mark_for_repair(self, note="misplaced"):
        self.repair_needed = True
        self.repairing = False
        self.repair_progress = 0.0
        self.repair_note = note
        side = random.choice([-1, 1])
        self.damage_offset_left = vector(
            random.uniform(-0.16, 0.16),
            side * random.uniform(0.18, 0.34),
            random.uniform(-0.20, 0.20)
        )
        self.damage_offset_right = vector(
            random.uniform(-0.16, 0.16),
            -side * random.uniform(0.18, 0.34),
            random.uniform(-0.20, 0.20)
        )
    ...
    def repair_step(self, dt, amount=0.42):
        if not self.repair_needed:
            return False
        self.repairing = True
        self.repair_progress = clamp(self.repair_progress + dt * amount, 0, 1)
        if self.repair_progress >= 1:
            self.repair_needed = False
            self.repairing = False
            self.selected = False
            self.damage_offset_left = vector(0, 0, 0)
    ...
    def update_repair_visuals(self, t, time_seconds):
        active = self.repair_needed or self.repairing
        repair_window = smoothstep(0.52, 0.66, t) * (1 - smoothstep(0.90, 0.99, t))
        midpoint = (self.left.pos + self.right.pos) * 0.5
        self.repair_halo.pos = midpoint
        self.repair_halo.axis = norm(vector(1, 0.35 * math.sin(time_seconds + self.index), 0.35 * math.cos(time_seconds + self.index)))
        pulse = 0.5 + 0.5 * math.sin(time_seconds * 7.0 + self.damage_phase)
        self.repair_halo.radius = 0.26 + 0.05 * pulse
        self.repair_halo.opacity = (0.12 + 0.55 * pulse) * repair_window if active else 0
        self.repair_halo.color = vector(0.25, 0.95, 1.0) if self.repairing else vector(1.0, 0.35, 0.18)

        self.damage_marker.pos = midpoint + vector(0, 0.24 + 0.04 * pulse, 0)
        self.damage_marker.opacity = (0.22 + 0.50 * pulse) * repair_window if active else 0
        self.damage_marker.color = vector(0.35, 1.0, 0.86) if self.repairing else vector(1.0, 0.30, 0.15)
        self.damage_marker.radius = 0.050 + 0.018 * pulse
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.repair_scan_timer = 0
        self.repair_scan_index = 0
        self.repair_scan_complete = False
        self.last_repair_message_timer = 0
    ...
    def reset_round(self, full=False):
        ...
        self.repair_scanner = sphere(
            pos=vector(0, 0, 0),
            radius=0.105,
            color=vector(0.25, 0.96, 1.0),
            opacity=0,
            emissive=True
        )
        self.repair_beam = cylinder(
            pos=vector(0, 0, 0),
        ...
        self.register(self.repair_scanner)
        self.register(self.repair_beam)
        self.register(self.repair_scan_ring)
        ...
        # A small repair workload is created each round so the scan is visible:
        # some chromosomes are misplaced/damaged and must be corrected before
        # the daughter nuclei fully brighten.
        damaged_count = random.randint(3, 4)
        notes = ["misplaced", "damaged", "twisted", "off-center"]
        for ch in random.sample(self.chromosomes, min(damaged_count, len(self.chromosomes))):
            ch.mark_for_repair(random.choice(notes))
    ...
    def get_state(self):
        ...
        repair_count = self.repair_remaining_count()
        return {
    ...
    def repair_remaining_count(self):
        return sum(1 for c in self.chromosomes if getattr(c, "repair_needed", False))

    def current_repair_target(self):
        pending = [c for c in self.chromosomes if getattr(c, "repair_needed", False)]
        if not pending:
            return None
        pending.sort(key=lambda c: (c.repair_progress, c.index))
        return pending[0]

    def force_repair_scan(self):
        target = self.current_repair_target()
        if target is None:
            self.mark(vector(0, 1.38, 0), "repair scan clear", vector(0.25, 0.96, 1.0), life=2.2)
            return
        target.force_repair()
        target.attach_all()
        target.selected = False
        self.mark((target.left.pos + target.right.pos) * 0.5, "manual repair", vector(0.25, 0.96, 1.0), life=2.4)

    def update_repair_system(self, dt, left_pole, right_pole):
        # The scan activates after chromosome groups separate but before nuclei
        # finish re-forming. It does not pause all motion; it delays full nuclear
        # brightness and visibly corrects damaged/misplaced chromosomes.
        t = self.progress
        scan_window = smoothstep(0.52, 0.64, t) * (1 - smoothstep(0.88, 0.98, t))
        pending = [c for c in self.chromosomes if getattr(c, "repair_needed", False)]
        self.repair_scan_ring.pos = vector(0, 0, 0)
        self.repair_scan_ring.axis = norm(vector(1, 0.16 * math.sin(self.time_seconds * 1.4), 0.16 * math.cos(self.time_seconds * 1.2)))
        self.repair_scan_ring.radius = 1.05 + 0.10 * math.sin(self.time_seconds * 2.3)
        self.repair_scan_ring.opacity = 0.24 * scan_window if pending else 0

        if scan_window <= 0.02 or not pending:
            self.repair_scanner.opacity = 0
            self.repair_beam.opacity = 0
            if not pending and not self.repair_scan_complete and t > 0.60:
                self.repair_scan_complete = True
                self.mark(vector(0, 1.38, 0), "repair scan complete", vector(0.25, 0.96, 1.0), life=3.0)
            return
        ...
        midpoint = (target.left.pos + target.right.pos) * 0.5
        orbit = vector(0, 0.18 * math.sin(self.time_seconds * 4.0), 0.18 * math.cos(self.time_seconds * 4.0))
        self.repair_scanner.pos = midpoint + vector(0, 0.36, 0) + orbit
        self.repair_scanner.opacity = 0.78
        self.repair_scanner.radius = 0.095 + 0.025 * math.sin(self.time_seconds * 7.0)

        origin = left_pole if midpoint.x < 0 else right_pole
        self.repair_beam.pos = origin
        self.repair_beam.axis = self.repair_scanner.pos - origin
        self.repair_beam.opacity = 0.34 + 0.20 * math.sin(self.time_seconds * 9.0)
        self.repair_beam.radius = 0.010 + 0.010 * target.repair_progress

        repaired = target.repair_step(dt, amount=0.36 + 0.22 * smoothstep(0.70, 0.88, t))
        if repaired:
            target.attach_all()
            self.mark(midpoint, "chromosome repaired", vector(0.25, 0.96, 1.0), life=2.6)
            self.spill_particles(5, source=midpoint, mood="ritual")
    ...
    def update(self, dt):
        ...
        repair_remaining = self.repair_remaining_count()
        repair_gate = 1.0 if repair_remaining == 0 else 0.28 + 0.10 * math.sin(self.time_seconds * 6.0)
        nuc_op = smoothstep(0.64, 0.88, t)
        self.left_nucleus.pos = vector(-daughter_d, 0, 0)
        self.right_nucleus.pos = vector(daughter_d, 0, 0)
        self.left_nucleus.opacity = 0.14 * nuc_op * repair_gate
        self.right_nucleus.opacity = 0.14 * nuc_op * repair_gate
        ...
        self.update_repair_system(dt, left_pole, right_pole)
        ...
        repair_remaining = self.repair_remaining_count()
        repair_status = "repair scan clear" if repair_remaining == 0 else f"repair scan: {repair_remaining} chromosome groups flagged"
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  {repair_status}"
    ...
    def prioritize_repair_scan(self):
        target = self.current_repair_target()
        if target is not None:
            target.repair_step(0.25, amount=1.0)
            target.selected = True
            self.mark((target.left.pos + target.right.pos) * 0.5, "AI repair focus", vector(0.25, 0.96, 1.0), life=2.0)
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if mode == "OBSERVE":
            ...
            if self.action_timer > 1.7:
                self.action_timer = 0
                target = sim.current_repair_target() or random.choice(sim.chromosomes)
                for c in sim.chromosomes:
                    c.selected = False
                target.selected = True
                sim.mark(target.left.pos, "inspect", vector(0.66, 0.42, 0.94), life=2.1)
                if getattr(target, "repair_needed", False):
                    target.begin_repair()
...
def keydown(evt):
    ...
    if key == " ":
        ...
        if sim.repair_remaining_count() > 0 and sim.progress > 0.52:
            sim.force_repair_scan()
        else:
            sim.toggle_random_spindle_attachment()
    elif key == "k":
        sim.force_repair_scan()
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle/repair | K repair | W wrap | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
