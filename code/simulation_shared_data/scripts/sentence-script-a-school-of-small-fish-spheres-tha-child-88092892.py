...
scene.title = "Multi-Predator Fish School in a Reef - SLEEP MODE"
...
scene.caption = (
    ...
    "O toggle human override | C clear marks | +/- predator speed | F toggle full/sleep\n"
    "Sleep mode is ON by default: low FPS, capped particles/marks, throttled HUD, periodic cleanup.\n"
)
...
# -----------------------------
# Sleep-mode performance controls
# -----------------------------
# This version is designed to run for hours or days with much lower CPU/GPU use.
# It does not write logs, save outputs, or accumulate unbounded visual effects.
SLEEP_MODE = True
SLEEP_RENDER_FPS = 10
FULL_RENDER_FPS = 60
SLEEP_HUD_INTERVAL = 1.0
FULL_HUD_INTERVAL = 0.12
SLEEP_AI_INTERVAL = 0.35
FULL_AI_INTERVAL = 0.0
SLEEP_EFFECT_INTERVAL = 0.20
FULL_EFFECT_INTERVAL = 0.0
SLEEP_CLEANUP_INTERVAL = 240.0
SLEEP_PARTICLE_BURST_CAP = 2
SLEEP_MARK_PROBABILITY = 0.28
SLEEP_PREDATOR_TRAIL_RETAIN = 10
FULL_PREDATOR_TRAIL_RETAIN = 55
SLEEP_PREY_TRAIL_RETAIN = 6
FULL_PREY_TRAIL_RETAIN = 38
...
cleanup_timer = 0.0
last_ai_accel = vector(0, 0, 0)


def current_render_fps():
    return SLEEP_RENDER_FPS if SLEEP_MODE else FULL_RENDER_FPS


def current_hud_interval():
    return SLEEP_HUD_INTERVAL if SLEEP_MODE else FULL_HUD_INTERVAL


def current_ai_interval():
    return SLEEP_AI_INTERVAL if SLEEP_MODE else FULL_AI_INTERVAL


def current_effect_interval():
    return SLEEP_EFFECT_INTERVAL if SLEEP_MODE else FULL_EFFECT_INTERVAL
...
def spawn_particles(pos, col, count=12, speed=2.2):
    global particles
    if SLEEP_MODE:
        count = min(count, SLEEP_PARTICLE_BURST_CAP)
        speed *= 0.55
...
def add_mark(pos, intensity=1.0, col=vector(1.0, 0.45, 0.20), life=7.0, label_text=""):
    global danger_marks
    if SLEEP_MODE:
        if random.random() > SLEEP_MARK_PROBABILITY:
            return
        life = min(life, 2.8)
        intensity *= 0.75
        label_text = ""
...
class Predator:
    def __init__(self, idx=0):
        ...
        self.trail = curve(color=color_lerp(self.base_color, color.white, 0.25), radius=0.025, retain=SLEEP_PREDATOR_TRAIL_RETAIN)
...
class PreyFish:
    def __init__(self, idx):
        ...
        self.trail = curve(pos=[self.body.pos], color=color_lerp(self.base_col, vector(1, 1, 1), 0.35), radius=0.008, retain=SLEEP_PREY_TRAIL_RETAIN)
...
def keydown(evt):
    global paused, boundary_mode, allow_human_override, predator_speed_multiplier, SLEEP_MODE
    k = evt.key.lower()
    keys_down.add(k)
    ...
    if k == "p":
        ...
        for fish in prey:
            fish.trail.clear()
        add_mark(predator.pos, 0.8, vector(0.50, 0.80, 1.0), 2.5, "sleep" if SLEEP_MODE else "full")
    elif k in ["+", "="]:
...
def update_hud():
    ...
    hud.text = (
        ...
        f"AI: {'ON' if ai.enabled else 'OFF'} | Mode: {ai.mode} | Power: {'SLEEP' if SLEEP_MODE else 'FULL'} | "
...
def sleep_mode_cleanup():
    """Periodic cleanup for multi-hour runs: clears visual traces without resetting the ecosystem."""
    clear_marks()
    for pr in predators:
        pr.trail.clear()
    for fish in prey:
...
while True:
    rate(current_render_fps())

    if paused:
        hud_update_timer += DT
        if hud_update_timer >= current_hud_interval():
            hud_update_timer = 0.0
            update_hud()
        continue
    ...
    if ai.enabled and manual_override_timer <= 0:
        ai_update_timer += DT
        if current_ai_interval() <= 0 or ai_update_timer >= current_ai_interval():
            ai_dt = ai_update_timer if current_ai_interval() > 0 else DT
            ai_update_timer = 0.0
            last_ai_accel = ai.update(ai_dt)
        accel = last_ai_accel
    elif ai.enabled:
    ...
    effect_update_timer += DT
    if current_effect_interval() <= 0 or effect_update_timer >= current_effect_interval():
        effect_dt = effect_update_timer if current_effect_interval() > 0 else DT
        effect_update_timer = 0.0
        update_effects(effect_dt)

    cleanup_timer += DT
    if SLEEP_MODE and cleanup_timer >= SLEEP_CLEANUP_INTERVAL:
        cleanup_timer = 0.0
        sleep_mode_cleanup()
