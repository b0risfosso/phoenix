"""
Fire That Wants To Become A Star — Iteration 13
Newborn Star Completion

Story:
The flame has a fragile proto-core and faint circular heat paths.
When the stable glowing core, broken flame bands, smoke boundary, and returning
sparks remain synchronized long enough, the flame completes its transformation
into a newborn star: a steady radiant core surrounded by rotating flame bands
that continuously recycle escaping heat.
...
Controls:
    ...
    N       briefly push toward newborn-star completion
"""
...
# Scene
# -----------------------------
scene.title = "Fire That Wants To Become A Star — Newborn Star Completion"
scene.width = 1160
scene.height = 740
...
# Completion visuals: newborn-star aura and heat-recycling ring.
newborn_star_aura = sphere(
    pos=unstable_ember.pos,
    radius=0.55,
...
title_label = label(
    ...
    text="The flame completes its transformation into a newborn star",
    height=15,
    box=False,
...
help_label = label(
    ...
    text="H help | P pause | R reset | W will | Space breath | I pull | C cycle | E core | O loop | G guide | S smoke | B bands | X sync | N newborn",
    height=9,
    box=False,
    align="left",
...
# Completion state.
newborn_star_progress = 0.0
newborn_star_complete = False
newborn_star_cycles = 0
newborn_star_recycle = 0.0
newborn_star_flash = 0.0
newborn_star_completion_events = 0
manual_newborn_boost = 0.0
...
def reinforce_newborn_star(amount=0.020):
    """Completion grows after synchronized core, bands, smoke, and returns."""
    global newborn_star_progress, newborn_star_recycle, newborn_star_flash

    newborn_star_progress = clamp(newborn_star_progress + amount, 0.0, 1.0)
    newborn_star_recycle = clamp(newborn_star_recycle + amount * 0.75, 0.0, 1.0)
    newborn_star_flash = clamp(newborn_star_flash + amount * 1.8, 0.0, 1.0)
...
def evaluate_cycle_survival():
    ...
    global newborn_star_progress, newborn_star_complete, newborn_star_cycles, newborn_star_recycle
    global newborn_star_flash, newborn_star_completion_events, manual_newborn_boost
    global newborn_star_progress, newborn_star_complete, newborn_star_cycles, newborn_star_recycle
    global newborn_star_flash, newborn_star_completion_events, manual_newborn_boost
    global newborn_star_progress, newborn_star_complete, newborn_star_cycles, newborn_star_recycle
    global newborn_star_flash, newborn_star_completion_events
    ...
    star_ready_cycle = (
        synchronized_cycle
        and stable_core_strength > 0.38
        and stable_core_heat > 0.30
        and core_synchrony > 0.55
        and flame_band_strength > 0.26
        and smoke_boundary_strength > 0.18
    )
    ...
    if star_ready_cycle:
        newborn_star_cycles += 1
        reinforce_newborn_star(0.045 + 0.018 * min(newborn_star_cycles, 7))
    else:
        newborn_star_cycles = 0
        newborn_star_progress = clamp(newborn_star_progress - 0.018 * (1.0 - stable_core_strength), 0.0, 1.0)

    if newborn_star_progress >= 0.92 and not newborn_star_complete:
        newborn_star_complete = True
        newborn_star_completion_events += 1
        newborn_star_flash = 1.0
        newborn_star_recycle = 1.0
    ...
    if newborn_star_complete:
        stable_core_strength = clamp(stable_core_strength + 0.020, 0.0, 1.0)
        stable_core_heat = clamp(stable_core_heat + 0.030, 0.0, 1.0)
        flame_band_strength = clamp(flame_band_strength + 0.018, 0.0, 1.0)
        smoke_boundary_strength = clamp(smoke_boundary_strength + 0.012, 0.0, 0.92)
        newborn_star_recycle = clamp(newborn_star_recycle + 0.020, 0.0, 1.0)
...
def keydown(evt):
    ...
    global manual_newborn_boost, newborn_star_progress, newborn_star_recycle, newborn_star_flash
    ...
    if k == "p":
        ...
        manual_newborn_boost = 0.45
        newborn_star_progress = clamp(newborn_star_progress + 0.10, 0.0, 1.0)
        newborn_star_recycle = clamp(newborn_star_recycle + 0.10, 0.0, 1.0)
        newborn_star_flash = 1.0
...
def update_learning_state():
    ...
    global newborn_star_progress, newborn_star_complete, newborn_star_recycle, newborn_star_flash, manual_newborn_boost
    ...
    completion_target = (
        0.22 * stable_core_strength
        + 0.18 * core_synchrony
        + 0.16 * flame_band_strength
        + 0.12 * smoke_boundary_strength
        + 0.10 * returned_heat
        + manual_newborn_boost
    )
    newborn_star_progress += (completion_target - newborn_star_progress) * 0.005
    newborn_star_progress = clamp(newborn_star_progress, 0.0, 1.0)

    if newborn_star_complete:
        newborn_star_progress = clamp(newborn_star_progress + 0.0008, 0.0, 1.0)
        newborn_star_recycle = clamp(newborn_star_recycle + 0.0012, 0.0, 1.0)
    ...
    manual_newborn_boost *= 0.92
    core_stability_flash *= 0.90
    newborn_star_flash *= 0.90
    ...
    inner_ember_instability = clamp(
        ...
        - newborn_star_recycle * 0.18
        + 0.12 * math.sin(t * 3.7),
        0.18,
        1.0
    )

    stable_core_heat = clamp(stable_core_heat + 0.00055 * stable_core_strength + 0.00080 * newborn_star_recycle - 0.00018 * (1.0 - core_synchrony), 0.0, 1.0)
...
def update_flame_shape():
    ...
    newborn_star_aura.pos = core_pos
    newborn_star_aura.radius = 0.40 + 0.92 * newborn_star_progress + 0.18 * newborn_star_flash
    newborn_star_aura.opacity = clamp(0.02 + 0.34 * newborn_star_progress + 0.18 * newborn_star_recycle + 0.12 * newborn_star_flash, 0.0, 0.62)
    newborn_star_aura.color = vector(1.0, 0.56 + 0.30 * newborn_star_progress, 0.08 + 0.16 * newborn_star_recycle)

    recycling_heat_ring.pos = core_pos + vector(0, 0.42 + 0.04 * math.sin(t * 2.2), 0)
    recycling_heat_ring.radius = 0.70 + 0.72 * newborn_star_recycle + 0.08 * math.sin(t * 2.8)
    recycling_heat_ring.thickness = 0.006 + 0.060 * newborn_star_recycle
    recycling_heat_ring.opacity = clamp(0.02 + 0.52 * newborn_star_recycle + 0.12 * newborn_star_flash, 0.0, 0.66)
    recycling_heat_ring.axis = norm(rotate(recycling_heat_ring.axis, angle=0.030 * (0.3 + newborn_star_recycle), axis=vector(0, 1, 0)))
    ...
    for idx, band in enumerate(broken_flame_bands):
        ...
        desired = clamp(flame_band_strength * (0.65 + 0.12 * idx) + flame_band_memory * 0.20 + stable_core_strength * 0.18 + newborn_star_recycle * 0.25, 0.0, 1.0)
...
def update_sparks():
    ...
    for p in sparks:
        ...
        if escaped_high or burned_out_high:
            if newborn_star_complete and random.random() < 0.42 + 0.30 * newborn_star_recycle:
                # The newborn star recycles escaping heat back into the band/core system.
                p["obj"].pos = flame_base.pos + vector(random.uniform(-0.38, 0.38), random.uniform(0.55, 1.10), random.uniform(-0.38, 0.38))
                p["vel"] = vector(random.uniform(-0.12, 0.12), random.uniform(-0.18, 0.18), random.uniform(-0.12, 0.12))
                p["age"] = 0.0
                p["life"] = random.uniform(1.6, 3.4)
                p["return_glow"] = 1.0
                p["loop_glow"] = 1.0
                p["path_glow"] = 1.0
                reinforce_flame_band(0.012 + 0.018 * newborn_star_recycle)
                reinforce_core_synchrony(0.010 + 0.018 * newborn_star_recycle)
                reinforce_newborn_star(0.006 + 0.012 * newborn_star_recycle)
            else:
                if p["has_looped"]:
                    loop_escape_count += 1
...
def update_status():
    ...
    newborn_bars = int(newborn_star_progress * 18)
    recycle_bars = int(newborn_star_recycle * 18)
    ...
    status_label.text = (
        ...
        f"newborn star progress: {newborn_star_progress:0.2f} [{bar(newborn_bars)}]\n"
        f"newborn complete: {newborn_star_complete} | completion events: {newborn_star_completion_events} | ready cycles: {newborn_star_cycles}\n"
        f"heat recycling: {newborn_star_recycle:0.2f} [{bar(recycle_bars)}]\n"
        ...
        "state: newborn star completion cycle"
    )
