...
class MitosisSimulation:
    ...
    def reset_round(self, full=False):
        ...
        # Cleavage-furrow visual system. These objects make the center of the
        # membrane look like it is tightening into a narrowing contractile belt,
        # rather than simply fading from one whole cell into two cells.
        self.furrow_shadow = ring(
            pos=vector(0, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.55,
            thickness=0.075,
            color=vector(0.03, 0.24, 0.34),
            opacity=0
        )
        self.furrow_glow = ring(
            pos=vector(0, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.50,
            thickness=0.020,
            color=vector(0.26, 0.78, 1.0),
            opacity=0,
            emissive=True
        )
        self.furrow_inner_lip_left = ring(
            pos=vector(-0.22, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.46,
            thickness=0.018,
            color=vector(0.08, 0.58, 0.82),
            opacity=0
        )
        self.furrow_inner_lip_right = ring(
            pos=vector(0.22, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.46,
            thickness=0.018,
        ...
        self.register(self.furrow_shadow)
        self.register(self.furrow_glow)
        self.register(self.furrow_inner_lip_left)
        self.register(self.furrow_inner_lip_right)

        self.furrow_beads = []
        bead_count = 36
        for i in range(bead_count):
            a = i * math.tau / bead_count
            bead = sphere(
                pos=vector(0, math.cos(a) * 1.50, math.sin(a) * 1.50),
                radius=0.025,
                color=vector(0.12, 0.62, 0.86),
                opacity=0,
                emissive=True
            )
            bead.furrow_angle = a
            self.furrow_beads.append(bead)
            self.register(bead)
    ...
    def stage_name(self):
        ...
        if t < 0.82:
            return "Cytokinesis: cleavage furrow slowly deepens"
        if t < 0.98:
    ...
    def update(self, dt):
        ...
        # The cleavage furrow begins as a broad glowing belt and then slowly
        # deepens into a tight central waist. The outer ring, shadow, lips, and
        # moving beads all share the same narrowing radius so the constriction
        # reads as a gradual mechanical tightening of the membrane.
        furrow_birth = smoothstep(0.42, 0.56, t)
        furrow_fade = 1 - smoothstep(0.88, 0.99, t)
        furrow_op = furrow_birth * furrow_fade
        furrow_depth = clamp(pinch, 0, 1)
        furrow_radius = lerp(1.50, 0.18, furrow_depth)
        furrow_pulse = 0.5 + 0.5 * math.sin(self.time_seconds * 5.0)
        wobble_axis = vector(1, 0.05 * math.sin(self.time_seconds * 2.5), 0.05 * math.cos(self.time_seconds * 2.5))

        self.cleavage_ring.radius = furrow_radius
        self.cleavage_ring.thickness = lerp(0.032, 0.075, furrow_depth)
        self.cleavage_ring.opacity = 0.78 * furrow_op
        self.cleavage_ring.color = vector(0.06, 0.42 + 0.18 * furrow_depth, 0.72 + 0.18 * furrow_depth)
        self.cleavage_ring.axis = wobble_axis

        self.furrow_shadow.radius = furrow_radius + lerp(0.10, 0.025, furrow_depth)
        self.furrow_shadow.thickness = lerp(0.050, 0.095, furrow_depth)
        self.furrow_shadow.opacity = 0.24 * furrow_op * furrow_depth
        self.furrow_shadow.axis = wobble_axis

        self.furrow_glow.radius = max(0.08, furrow_radius - 0.025)
        self.furrow_glow.thickness = lerp(0.018, 0.042, furrow_depth)
        self.furrow_glow.opacity = (0.10 + 0.30 * furrow_pulse) * furrow_op
        self.furrow_glow.axis = wobble_axis

        lip_offset = lerp(0.36, 0.07, furrow_depth)
        lip_radius = max(0.10, furrow_radius + lerp(0.03, 0.12, furrow_depth))
        for lip, xsign in [(self.furrow_inner_lip_left, -1), (self.furrow_inner_lip_right, 1)]:
            lip.pos = vector(xsign * lip_offset, 0, 0)
            lip.radius = lip_radius
            lip.thickness = lerp(0.018, 0.034, furrow_depth)
            lip.opacity = 0.42 * furrow_op
            lip.axis = wobble_axis

        for bead in self.furrow_beads:
            a = bead.furrow_angle
            bead_wave = math.sin(self.time_seconds * 4.4 + a * 3.0)
            local_r = furrow_radius + 0.020 * bead_wave * furrow_op
            bead.pos = vector(0, math.cos(a) * local_r, math.sin(a) * local_r)
            bead.radius = lerp(0.018, 0.036, furrow_depth) * (0.82 + 0.18 * furrow_pulse)
            bead.opacity = 0.62 * furrow_op

        self.bridge.radius = min(self.bridge.radius, max(0.08, furrow_radius + 0.10))
        ...
        furrow_percent = int(clamp(smoothstep(0.42, 0.82, self.progress) * 100, 0, 100))
        self.stage_label.text = f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  furrow depth {furrow_percent}%"
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        if mode == "OBSERVE":
            ...
            if self.action_timer > 1.9:
                ...
                sim.mark(vector(0, 0.95, 0), "cleavage furrow deepens", vector(0.10, 0.58, 0.72), life=2.6)
