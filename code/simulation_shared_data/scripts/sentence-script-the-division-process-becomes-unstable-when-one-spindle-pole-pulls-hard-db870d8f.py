...
#   T     : trigger a temporary spindle-pole imbalance
...
class ChromosomePair:
    ...
    def update(self, t, time_seconds, left_pole, right_pole, left_nucleus_center, right_nucleus_center, ai_noise=0.0, pull_bias=1.0, pole_force_imbalance=0.0, instability_level=0.0):
        ...
        # Positive pole_force_imbalance means the right spindle pole is pulling
        # harder; negative means the left spindle pole is stronger. During
        # metaphase/anaphase this makes chromosomes skew away from the precise
        # central line before the cell rebalances the spindle.
        imbalance_active = smoothstep(0.18, 0.38, t) * (1 - smoothstep(0.68, 0.90, t))
        imbalance = clamp(pole_force_imbalance, -0.85, 0.85) * imbalance_active
        left_force = clamp(1.0 - imbalance, 0.35, 1.85)
        right_force = clamp(1.0 + imbalance, 0.35, 1.85)

        # The metaphase plate itself bends slightly toward the stronger pole;
        # this makes the unstable division visibly uneven before correction.
        plate_skew = vector(0.28 * imbalance, 0, 0)
        paired_left += plate_skew
        paired_right += plate_skew

        pull_u_left = clamp(separate * pull_bias * left_force, 0, 1)
        pull_u_right = clamp(separate * pull_bias * right_force, 0, 1)
        left_target = vlerp(paired_left, final_left, pull_u_left)
        right_target = vlerp(paired_right, final_right, pull_u_right)
        ...
        wobble_strength = instability_level * imbalance_active
        wobble = vector(
            0,
            math.sin(time_seconds * 6.3 + self.index * 0.9) * 0.16 * wobble_strength,
            math.cos(time_seconds * 5.7 + self.phase_jitter) * 0.16 * wobble_strength
        )
        left_target += wobble
        right_target -= wobble * 0.72
        ...
        left_fiber_force = clamp(1.0 - imbalance, 0.35, 1.85)
        right_fiber_force = clamp(1.0 + imbalance, 0.35, 1.85)
        left_hot = clamp((left_fiber_force - 1.0) / 0.85, 0, 1)
        right_hot = clamp((right_fiber_force - 1.0) / 0.85, 0, 1)

        self.left_fiber.pos = left_pole
        self.left_fiber.axis = self.left.pos - left_pole
        self.left_fiber.opacity = op_base * (0.82 + 0.28 * left_hot) if self.left_attached else op_base * 0.18
        self.left_fiber.radius = (0.018 + 0.015 * left_hot) if self.left_attached else 0.012
        self.left_fiber.color = vlerp(vector(1.0, 0.84, 0.32), vector(1.0, 0.32, 0.16), left_hot) if self.left_attached else vector(0.90, 0.72, 0.60)

        self.right_fiber.pos = right_pole
        self.right_fiber.axis = self.right.pos - right_pole
        self.right_fiber.opacity = op_base * (0.82 + 0.28 * right_hot) if self.right_attached else op_base * 0.18
        self.right_fiber.radius = (0.018 + 0.015 * right_hot) if self.right_attached else 0.012
        self.right_fiber.color = vlerp(vector(1.0, 0.84, 0.32), vector(1.0, 0.32, 0.16), right_hot) if self.right_attached else vector(0.90, 0.72, 0.60)

        highlight = 0.12 if self.selected else 0
        unstable_glow = clamp(instability_level * imbalance_active, 0, 1)
        self.left.emissive = self.selected or unstable_glow > 0.42
        self.right.emissive = self.selected or unstable_glow > 0.42
        self.left.color = vlerp(self.color_left, vector(1.0, 0.38, 0.20), unstable_glow) + vector(highlight, highlight, highlight)
        self.right.color = vlerp(self.color_right, vector(1.0, 0.38, 0.20), unstable_glow) + vector(highlight, highlight, highlight)
...
class MitosisSimulation:
    def __init__(self):
        ...
        self.drift_bias = 1.0
        self.pole_force_imbalance = 0.0
        self.base_pole_force_imbalance = 0.0
        self.instability_level = 0.0
        self.instability_event_timer = 0.0
        self.ai_force_bias = 0.0
    ...
    def reset_round(self, full=False):
        ...
        self.constriction_bias = 1.0
        self.drift_bias = 1.0
        self.base_pole_force_imbalance = random.choice([-1, 1]) * random.uniform(0.32, 0.58)
        self.pole_force_imbalance = 0.0
        self.instability_level = 0.0
        self.instability_event_timer = 0.0
        self.ai_force_bias = 0.0
        ...
        self.imbalance_bar_base = cylinder(
            pos=vector(-0.72, -2.02, 0),
            axis=vector(1.44, 0, 0),
            radius=0.018,
        ...
        self.instability_wave = ring(
            pos=vector(0, 0, 0),
            axis=vector(1, 0, 0),
            radius=1.0,
            thickness=0.012,
            color=vector(1.0, 0.40, 0.18),
            opacity=0.0,
            emissive=True
        )
        self.left_force_label = label(
            pos=vector(-1.55, -1.70, 0),
            text="",
            box=False,
            opacity=0,
            color=vector(0.95, 0.45, 0.18),
            height=10
        )
        self.right_force_label = label(
            pos=vector(1.55, -1.70, 0),
            text="",
            box=False,
            opacity=0,
        ...
        self.register(self.imbalance_bar_base)
        self.register(self.imbalance_marker)
        self.register(self.instability_wave)
        self.register(self.left_force_label)
        self.register(self.right_force_label)
    ...
    def get_state(self):
        ...
        return {
            ...
            "pole_force_imbalance": self.pole_force_imbalance,
    ...
    def trigger_spindle_imbalance(self, strength=None):
        if strength is None:
            strength = random.choice([-1, 1]) * random.uniform(0.45, 0.75)
        self.base_pole_force_imbalance = clamp(strength, -0.85, 0.85)
        self.instability_event_timer = 3.2
        strong_side = "right" if self.base_pole_force_imbalance > 0 else "left"
        self.mark(vector(0, -1.42, 0), f"{strong_side} pole pulls harder", vector(1.0, 0.42, 0.18), life=3.0)

    def rebalance_spindle_forces(self):
        self.base_pole_force_imbalance *= 0.42
        self.ai_force_bias *= 0.35
        self.instability_event_timer = max(self.instability_event_timer, 1.0)
        self.mark(vector(0, -1.30, 0), "spindle force rebalanced", vector(0.30, 0.72, 0.92), life=2.6)

    def update_spindle_instability(self, dt, t, left_pole, right_pole):
        active = smoothstep(0.18, 0.34, t) * (1 - smoothstep(0.70, 0.92, t))
        recovery = smoothstep(0.48, 0.74, t)
        if self.instability_event_timer > 0:
            self.instability_event_timer -= dt
            event_boost = 1.0
        else:
            event_boost = 0.0

        rhythmic_surge = 0.10 * math.sin(self.time_seconds * 2.6)
        target_imbalance = (self.base_pole_force_imbalance + self.ai_force_bias + rhythmic_surge) * active
        target_imbalance *= (1 - 0.70 * recovery)
        if event_boost > 0:
            target_imbalance += self.base_pole_force_imbalance * 0.35 * active
        self.pole_force_imbalance = lerp(self.pole_force_imbalance, clamp(target_imbalance, -0.85, 0.85), 0.08)
        self.instability_level = clamp(abs(self.pole_force_imbalance) * 1.55 * active, 0, 1)

        # Stronger pole glows larger and hotter. The weaker pole dims so the
        # imbalance can be read immediately in the scene.
        left_power = clamp(1.0 - self.pole_force_imbalance, 0.35, 1.85)
        right_power = clamp(1.0 + self.pole_force_imbalance, 0.35, 1.85)
        left_hot = clamp((left_power - 1.0) / 0.85, 0, 1)
        right_hot = clamp((right_power - 1.0) / 0.85, 0, 1)
        self.pole_glow_left.radius = 0.25 + 0.22 * left_hot + 0.04 * math.sin(self.time_seconds * 7.0)
        self.pole_glow_right.radius = 0.25 + 0.22 * right_hot + 0.04 * math.sin(self.time_seconds * 7.0 + 1.2)
        self.pole_glow_left.opacity = 0.12 + 0.34 * left_hot * active
        self.pole_glow_right.opacity = 0.12 + 0.34 * right_hot * active
        self.pole_glow_left.color = vlerp(vector(1.0, 0.88, 0.42), vector(1.0, 0.30, 0.12), left_hot)
        self.pole_glow_right.color = vlerp(vector(1.0, 0.88, 0.42), vector(1.0, 0.30, 0.12), right_hot)

        self.imbalance_marker.pos = vector(0.72 * self.pole_force_imbalance, -2.02, 0)
        self.imbalance_marker.opacity = 0.16 + 0.72 * active
        self.imbalance_marker.radius = 0.055 + 0.065 * self.instability_level
        self.instability_wave.pos = vector(0.14 * self.pole_force_imbalance, 0, 0)
        self.instability_wave.radius = 0.96 + 0.28 * math.sin(self.time_seconds * 5.5) * self.instability_level
        self.instability_wave.thickness = 0.010 + 0.024 * self.instability_level
        self.instability_wave.opacity = 0.42 * self.instability_level
        self.left_force_label.visible = self.show_labels and active > 0.05
        self.right_force_label.visible = self.show_labels and active > 0.05
        self.left_force_label.pos = left_pole + vector(0, -0.45, 0)
        self.right_force_label.pos = right_pole + vector(0, -0.45, 0)
        self.left_force_label.text = f"left pull {left_power:.2f}x"
        self.right_force_label.text = f"right pull {right_power:.2f}x"
    ...
    def update(self, dt):
        ...
        # Severe pole-force imbalance makes mitosis unstable and temporarily slows
        # stage progression while the cell attempts to restore balanced pulling.
        speed *= lerp(1.0, 0.52, self.instability_level)
        if self.progress < 1:
        ...
        self.update_spindle_instability(dt, t, left_pole, right_pole)
        ...
        strong_side = "right" if self.pole_force_imbalance > 0.04 else "left" if self.pole_force_imbalance < -0.04 else "balanced"
        self.stage_label.text = (
            f"Round {self.round_index} — {self.stage_name()}  |  progress {int(self.progress * 100)}%  |  "
            f"pole pull: {strong_side}  |  instability {int(self.instability_level * 100)}%"
        )
...
class AIController:
    ...
    def apply_behavior(self, dt, state, stagnating):
        ...
        sim.ai_force_bias = lerp(sim.ai_force_bias, 0.0, 0.035)
        ...
        if mode == "OBSERVE":
            ...
            sim.ai_force_bias = lerp(sim.ai_force_bias, -sim.pole_force_imbalance * 0.80, 0.08)
            ...
            sim.ai_noise = lerp(sim.ai_noise, 0.055 + 0.08 * sim.instability_level, 0.04)
            sim.ai_force_bias = lerp(sim.ai_force_bias, sim.base_pole_force_imbalance * 0.32, 0.05)
            ...
            sim.ai_force_bias = lerp(sim.ai_force_bias, random.choice([-1, 1]) * 0.34, 0.04)
            ...
            sim.ai_force_bias = lerp(sim.ai_force_bias, -sim.pole_force_imbalance, 0.12)
            if self.action_timer > 2.4:
                self.action_timer = 0
                sim.attach_all_spindles()
                sim.rebalance_spindle_forces()
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | T imbalance | W wrap | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
