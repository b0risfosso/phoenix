...
scene.title = "Two-Predator Fish School in a Reef - AI Reef Simulation"
...
# Scout behavior: a small number of faster swimmers detect danger early,
# flash visible warnings, and redirect nearby fish before the predator reaches them.
SCOUT_DETECTION_RADIUS = 14.2
SCOUT_WARNING_RADIUS = 9.4
SCOUT_WARNING_TIME = 3.8
SCOUT_WARNING_STRENGTH = 16.5
SCOUT_FLASH_COOLDOWN = 0.24
# Scouts are intentionally faster than predators at default speed; they survive
# by leading the group away instead of acting like ordinary prey.
SCOUT_SPEED_BOOST = 2.75
SCOUT_FORCE_BOOST = 1.75
GROUP_REDIRECT_FORCE_BOOST = 1.45
SCOUT_LEADER_PULL = 8.2
# Scouts guide ordinary fish to gates, but peel away at the threshold instead of escaping.
SCOUT_GATE_PEEL_STRENGTH = 7.5
SCOUT_GATE_PEEL_DISTANCE = 0.72
SCOUT_COORDINATED_CAPTURE_RADIUS = 4.15
SCOUT_COORDINATED_CAPTURE_DOT_MAX = 0.60
DT = 1.0 / 60.0
...
class PreyFish:
    def __init__(self, idx):
        self.idx = idx
        self.is_scout = idx < NUM_SCOUTS
        self.base_col = color_lerp(vector(0.18, 0.78, 0.88), vector(0.60, 0.95, 0.55), random.random())
        if self.is_scout:
            self.base_col = vector(0.08, 0.55, 1.0)
        self.body = sphere(pos=random_swim_pos(), radius=PREY_RADIUS * (1.18 if self.is_scout else 1.0), color=self.base_col, opacity=0.96)
        self.fin = cone(pos=self.body.pos, axis=vector(-0.2, 0, 0), radius=PREY_RADIUS * (0.72 if self.is_scout else 0.55), color=color_lerp(self.base_col, color.white, 0.25), opacity=0.8)
        self.trail = curve(pos=[self.body.pos], color=color_lerp(self.base_col, vector(1, 1, 1), 0.35), radius=0.018 if self.is_scout else 0.012, retain=48 if self.is_scout else 38)
        self.vel = random_unit_vector() * random.uniform(1.4, 2.4) * (1.45 if self.is_scout else 1.0)
        ...
        self.max_speed = random.uniform(3.0, 3.9) * (SCOUT_SPEED_BOOST if self.is_scout else 1.0)
        self.mark_cooldown = random.uniform(0, 2.0)
        self.fear = 0.0
        self.scout_warning_timer = 0.0
        self.scout_flash_timer = random.uniform(0, SCOUT_FLASH_COOLDOWN)
        self.scout_threat_pos = vector(0, 0, 0)
        self.scout_signal = None
        if self.is_scout:
            self.scout_signal = ring(
                pos=self.pos,
                axis=vector(0, 1, 0),
                radius=PREY_RADIUS * 1.75,
                thickness=0.025,
                color=vector(1.0, 0.95, 0.18),
                opacity=0.0,
            )
    ...
    def reset(self, pos=None):
        ...
        self.body.radius = PREY_RADIUS * (1.18 if self.is_scout else 1.0)
        self.body.color = self.base_col
        self.fin.color = color_lerp(self.base_col, color.white, 0.25)
        self.scout_warning_timer = 0.0
        self.scout_flash_timer = random.uniform(0, SCOUT_FLASH_COOLDOWN)
        self.scout_threat_pos = vector(0, 0, 0)
        if self.scout_signal:
            self.scout_signal.opacity = 0.0
            self.scout_signal.pos = self.pos
        self.trail.clear()
        self.trail.color = color_lerp(self.base_col, vector(1, 1, 1), 0.35)
        self.update_visual()

    def is_cooperatively_trapped_by_predators(self, predator_list):
        """Scouts require a visible pincer: both predators close, from different angles.
        This prevents ordinary single-predator contact from catching scouts.
        """
        if not self.is_scout:
            return True
        close = [pr for pr in predator_list if mag(self.pos - pr.pos) < SCOUT_COORDINATED_CAPTURE_RADIUS]
        if len(close) < 2:
            return False
        dirs = [safe_norm(pr.pos - self.pos, random_unit_vector()) for pr in close[:2]]
        opposed_or_crossing = dot(dirs[0], dirs[1]) < SCOUT_COORDINATED_CAPTURE_DOT_MAX
        closing_count = sum(1 for pr in close if dot(pr.vel, self.pos - pr.pos) > 0)
        return opposed_or_crossing and closing_count >= 1
    ...
    def update(self, dt, prey_list, predators):
        ...
        # Scouts sense danger earlier than the rest of the school. They flash a
        # warning and hold the last threat position so nearby fish can redirect.
        if self.is_scout:
            self.scout_warning_timer = max(0, self.scout_warning_timer - dt)
            self.scout_flash_timer = max(0, self.scout_flash_timer - dt)
            predator_approach = dot(nearest_threat.vel, self.pos - nearest_threat.pos) > 0
            if pred_dist < SCOUT_DETECTION_RADIUS or (predator_approach and pred_dist < SCOUT_DETECTION_RADIUS + 2.0):
                self.scout_warning_timer = SCOUT_WARNING_TIME
                self.scout_threat_pos = nearest_threat.pos
                if self.scout_flash_timer <= 0:
                    self.scout_flash_timer = SCOUT_FLASH_COOLDOWN
                    add_mark(self.pos, 0.55, vector(1.0, 0.95, 0.18), 2.2, "scout")
                    spawn_particles(self.pos, vector(1.0, 0.95, 0.18), count=5, speed=1.4)

        scout_push = vector(0, 0, 0)
        scout_count = 0
        for other in prey_list:
            if other is self or other.caught or not getattr(other, "is_scout", False):
                continue
            if other.scout_warning_timer <= 0:
                continue
            offset = self.pos - other.pos
            d = mag(offset)
            if d < SCOUT_WARNING_RADIUS:
                influence = (SCOUT_WARNING_RADIUS - d) / SCOUT_WARNING_RADIUS
                away_from_threat = safe_norm(self.pos - other.scout_threat_pos, safe_norm(offset, random_unit_vector()))
                side_slip = safe_norm(cross(away_from_threat, vector(0, 1, 0)), random_unit_vector())
                if (self.idx + other.idx) % 2 == 0:
                    side_slip *= -1
                leader_gate = nearest_escape_gate(other.pos, other.scout_threat_pos)
                leader_path = safe_norm(leader_gate["pos"] - self.pos, away_from_threat)
                leader_follow = safe_norm((other.pos + leader_path * 2.8 + away_from_threat * 1.4) - self.pos, away_from_threat)
                scout_push += (away_from_threat * 1.35 + side_slip * 0.85 + leader_follow * 1.55) * influence
                scout_count += 1

        if scout_count > 0:
            scout_push /= scout_count
            acceleration += scout_push * SCOUT_WARNING_STRENGTH
            warning_gate = nearest_escape_gate(self.pos, nearest_threat.pos)
            gate_pull = safe_norm(warning_gate["pos"] - self.pos)
            acceleration += gate_pull * (3.8 + 1.05 * scout_count)
            # Briefly loosen formation so the warning can actually redirect motion.
            acceleration -= cohesion * 0.35
            acceleration -= align * 0.12
            self.fear = max(self.fear, clamp(0.35 + 0.18 * scout_count, 0, 0.95))

        if self.is_scout and self.scout_warning_timer > 0:
            warning_gate = nearest_escape_gate(self.pos, self.scout_threat_pos)
            escape_line = safe_norm(warning_gate["pos"] - self.pos, self.pos - self.scout_threat_pos)
            away = safe_norm(self.pos - self.scout_threat_pos, to_predator)
            # Scouts lead instead of merely fleeing: they cut toward a safe exit vector
            # and flash while dragging the school into the redirected path.
            acceleration += away * 5.8
            acceleration += escape_line * SCOUT_LEADER_PULL
            acceleration += random_unit_vector() * 0.80
        ...
        if self.is_scout:
            max_force *= SCOUT_FORCE_BOOST
        if scout_count > 0:
            max_force *= GROUP_REDIRECT_FORCE_BOOST
        acceleration = limit(acceleration, max_force)
        self.vel += acceleration * dt
        alert_boost = 1.0 + (0.55 if self.scout_warning_timer > 0 else 0.0) + (0.22 if scout_count > 0 else 0.0)
        self.vel = limit(self.vel, self.max_speed * (1.0 + 0.38 * self.fear) * PREY_ESCAPE_DAMPING * alert_boost)
        self.pos += self.vel * dt
        ...
        if self.is_scout:
            if self.is_cooperatively_trapped_by_predators(predators):
                captor = nearest_predator_to(self.pos, predators)
                self.mark_caught(captor)
                add_mark(self.pos, 1.1, vector(1.0, 0.55, 0.12), 5.5, "pincer")
        else:
        ...
        for g in escape_gates:
            ...
            if g["axis"].x != 0:
                ...
                if near_wall and within:
                    if self.is_scout:
                        self.peel_scout_away_from_gate(g)
                        return
    ...
    def peel_scout_away_from_gate(self, gate):
        """Scouts point the group at exits but cannot use the exits themselves.

        When a scout reaches a gate, it flashes a guide-only mark, steps back
        inside the reef, and turns sideways/upward so followers can pass through.
        """
        gp = gate["pos"]
        inward = -safe_norm(vector(gp.x, 0, gp.z), vector(1, 0, 0))
        side = safe_norm(cross(inward, vector(0, 1, 0)), vector(0, 0, 1))
        if (self.idx + int(sim_time * 2.0)) % 2 == 0:
            side *= -1
        upward = vector(0, 0.32, 0)
        peel_dir = safe_norm(side * 1.15 + inward * 0.85 + upward, inward)

        if gate["axis"].x != 0:
            self.pos.x = (HALF.x - SCOUT_GATE_PEEL_DISTANCE) * (1 if gp.x > 0 else -1)
        else:
            self.pos.z = (HALF.z - SCOUT_GATE_PEEL_DISTANCE) * (1 if gp.z > 0 else -1)

        self.vel = limit(self.vel * 0.28 + peel_dir * SCOUT_GATE_PEEL_STRENGTH, self.max_speed * 1.12)
        self.scout_warning_timer = max(self.scout_warning_timer, SCOUT_WARNING_TIME * 0.55)
        self.trail.clear()
        add_mark(gp, 0.85, vector(1.0, 0.95, 0.18), 3.6, "guide only")
        spawn_particles(gp, vector(1.0, 0.95, 0.18), count=7, speed=1.6)
    ...
    def update_visual(self):
        ...
        if self.scout_signal:
            self.scout_signal.pos = self.pos
            self.scout_signal.radius = PREY_RADIUS * (2.3 + 3.0 * clamp(self.scout_warning_timer / SCOUT_WARNING_TIME, 0, 1))
            self.scout_signal.opacity = 0.58 * clamp(self.scout_warning_timer / SCOUT_WARNING_TIME, 0, 1)
        if not self.caught:
            fear_col = color_lerp(self.base_col, vector(1.0, 0.92, 0.30), self.fear * 0.85)
            if self.is_scout and self.scout_warning_timer > 0:
                fear_col = color_lerp(fear_col, vector(1.0, 0.98, 0.10), 0.72)
        ...
        self.fin.axis = -f * PREY_RADIUS * (1.25 if self.is_scout else 1.05)
...
def priority_scout_target(reference_pos=None):
    active_scouts = [f for f in prey if getattr(f, "is_scout", False) and not f.caught]
    if not active_scouts:
        return None
    warning_scouts = [f for f in active_scouts if f.scout_warning_timer > 0]
    pool = warning_scouts if warning_scouts else active_scouts
    if reference_pos is None:
        reference_pos = predator.pos
    return min(pool, key=lambda f: mag(f.pos - reference_pos) - (2.5 if f.scout_warning_timer > 0 else 0.0))


def scout_coordination_pressure_count():
    count = 0
    for f in prey:
        if getattr(f, "is_scout", False) and not f.caught:
            close = sum(1 for pr in predators if mag(f.pos - pr.pos) < SCOUT_COORDINATED_CAPTURE_RADIUS)
            if close >= 2:
                count += 1
    return count
...
def secondary_predator_accel(pr, idx, active_prey):
    ...
    if not active_prey:
        ...
        scout_target = priority_scout_target(pr.pos)
        if scout_target is not None and scout_target.scout_warning_timer > 0:
            # The second predator does not simply duplicate the lead predator. It
            # moves to the far side of the scout, forming the required pincer.
            primary = predators[0]
            away_from_primary = safe_norm(scout_target.pos - primary.pos, random_unit_vector())
            cross_cut = safe_norm(cross(away_from_primary, vector(0, 1, 0)), random_unit_vector())
            if idx % 2 == 0:
                cross_cut *= -1
            target = scout_target.pos + scout_target.vel * 0.55 + away_from_primary * 1.85 + cross_cut * 0.65
        else:
            target = None
...
class ReefAIController:
    ...
    def choose_action(self, dt, state):
        ...
        if self.mode == "HUNT":
            scout_target = priority_scout_target(predator.pos)
            if scout_target is not None and scout_target.scout_warning_timer > 0:
                lead = scout_target.pos + scout_target.vel * 0.60
                acc = self.steering_to(lead, arrive_radius=0.55, speed_scale=1.18)
                add_flash = int(self.mode_time * 2.0) != int((self.mode_time - dt) * 2.0)
                if add_flash:
                    add_mark(lead, 0.42, vector(1.0, 0.55, 0.12), 2.0, "pincer")
            elif nearest:
            ...
            if self.mode_time < self.ambush_wait:
                ...
                scout_target = priority_scout_target(predator.pos)
                if scout_target is not None and scout_target.scout_warning_timer > 0:
                    acc = self.steering_to(scout_target.pos + scout_target.vel * 0.50, arrive_radius=0.35, speed_scale=1.22)
                elif nearest:
...
def update_hud():
    ...
    active_scouts = sum(1 for f in prey if getattr(f, "is_scout", False) and not f.caught)
    warning_scouts = sum(1 for f in prey if getattr(f, "is_scout", False) and f.scout_warning_timer > 0 and not f.caught)
    redirected = sum(1 for f in prey if (not f.caught) and any(getattr(s, "is_scout", False) and s.scout_warning_timer > 0 and mag(f.pos - s.pos) < SCOUT_WARNING_RADIUS for s in prey))
    pincer_pressure = scout_coordination_pressure_count()
    ...
    hud.text = (
        ...
        f"Scouts: {active_scouts}/{NUM_SCOUTS} | Warnings: {warning_scouts} | Redirected: {redirected} | Pincers: {pincer_pressure}\n"
