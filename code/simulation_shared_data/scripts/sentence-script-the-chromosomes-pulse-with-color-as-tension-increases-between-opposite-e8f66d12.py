...
#   T     : create a temporary spindle-tension pulse
...
class ChromosomePair:
    def __init__(self, sim, index, base_angle, radius_ring):
        ...
        self.tension_pulse_boost = 0.0

        # Tension visualization: translucent glows sit on each sister chromatid,
        # and a thin line between chromatids brightens as opposite spindle poles
        # pull harder. These are separate from the chromosome bodies so the base
        # simulation geometry remains intact.
        self.left_tension_glow = sphere(
            pos=self.left.pos,
            radius=0.19,
            color=vector(1.0, 0.42, 0.16),
            opacity=0,
            emissive=True,
            shininess=0.1
        )
        self.right_tension_glow = sphere(
            pos=self.right.pos,
            radius=0.19,
            color=vector(1.0, 0.42, 0.16),
            opacity=0,
        ...
        self.tension_bridge = cylinder(
            pos=self.left.pos,
            axis=self.right.pos - self.left.pos,
            radius=0.010,
            color=vector(1.0, 0.50, 0.12),
            opacity=0
        )
        ...
        sim.register(self.left_tension_glow)
        sim.register(self.right_tension_glow)
        sim.register(self.tension_bridge)
    ...
    def set_visible(self, value):
        ...
        self.left_tension_glow.visible = value
        self.right_tension_glow.visible = value
        self.tension_bridge.visible = value
        if not value:
    ...
    def update(self, t, time_seconds, left_pole, right_pole, left_nucleus_center, right_nucleus_center, ai_noise=0.0, pull_bias=1.0):
        ...
        # Spindle tension rises when both sister chromatids are attached and
        # opposite poles are pulling during metaphase/anaphase. The pulsing color
        # gives the user a direct visual signal of mechanical strain.
        stage_tension = smoothstep(0.20, 0.58, t) * (1 - smoothstep(0.78, 0.96, t))
        attachment_tension = self.attachment_fraction() / 2.0
        chromatid_span = clamp(abs(self.right.pos.x - self.left.pos.x) / 2.75, 0, 1)
        pole_span = clamp(mag(right_pole - left_pole) / 3.45, 0, 1)
        alignment_strain = clamp(1.0 - abs((self.left.pos.x + self.right.pos.x) * 0.5) / 1.25, 0, 1)
        raw_tension = stage_tension * attachment_tension * (0.35 + 0.35 * chromatid_span + 0.30 * pole_span) * (0.65 + 0.35 * alignment_strain)
        if self.tension_pulse_boost > 0:
            raw_tension = clamp(raw_tension + self.tension_pulse_boost, 0, 1)
            self.tension_pulse_boost *= 0.965
        self.tension_level = lerp(self.tension_level, raw_tension, 0.10)

        fast_pulse = 0.5 + 0.5 * math.sin(time_seconds * (6.0 + 6.5 * self.tension_level) + self.phase_jitter)
        slow_pulse = 0.5 + 0.5 * math.sin(time_seconds * 2.1 + self.phase_jitter * 0.7)
        pulse_strength = self.tension_level * (0.38 + 0.62 * fast_pulse)
        warm_shift = vector(0.26, 0.08, -0.08) * pulse_strength
        highlight = 0.12 if self.selected else 0

        self.left.emissive = self.selected or self.tension_level > 0.44
        self.right.emissive = self.selected or self.tension_level > 0.44
        self.left.color = self.color_left + warm_shift + vector(highlight, highlight, highlight)
        self.right.color = self.color_right + warm_shift + vector(highlight, highlight, highlight)

        tension_radius = 0.17 + 0.13 * self.tension_level + 0.035 * fast_pulse
        tension_opacity = clamp(0.02 + 0.38 * pulse_strength, 0, 0.42)
        self.left_tension_glow.pos = self.left.pos
        self.right_tension_glow.pos = self.right.pos
        self.left_tension_glow.radius = tension_radius
        self.right_tension_glow.radius = tension_radius
        self.left_tension_glow.opacity = tension_opacity
        self.right_tension_glow.opacity = tension_opacity
        self.left_tension_glow.color = vector(1.0, 0.42 + 0.25 * slow_pulse, 0.10)
        self.right_tension_glow.color = vector(1.0, 0.42 + 0.25 * slow_pulse, 0.10)

        self.tension_bridge.pos = self.left.pos
        self.tension_bridge.axis = self.right.pos - self.left.pos
        self.tension_bridge.radius = 0.008 + 0.020 * self.tension_level
        self.tension_bridge.opacity = clamp(0.08 + 0.58 * pulse_strength, 0, 0.68)
        self.tension_bridge.color = vector(1.0, 0.52 + 0.22 * fast_pulse, 0.12)

        # Spindle fibers also brighten under tension to make the force between
        # poles and chromosomes visible.
        if self.left_attached:
            self.left_fiber.color = vector(1.0, 0.84 - 0.18 * pulse_strength, 0.32 - 0.18 * pulse_strength)
            self.left_fiber.radius = 0.018 + 0.014 * pulse_strength
        if self.right_attached:
            self.right_fiber.color = vector(1.0, 0.84 - 0.18 * pulse_strength, 0.32 - 0.18 * pulse_strength)
            self.right_fiber.radius = 0.018 + 0.014 * pulse_strength

    def add_tension_pulse(self, amount=0.55):
        self.tension_pulse_boost = clamp(self.tension_pulse_boost + amount, 0, 1)
...
class MitosisSimulation:
    ...
    def get_state(self):
        ...
        for c in self.chromosomes:
            ...
            avg_tension += c.tension_level
        avg_tension = avg_tension / max(1, len(self.chromosomes))
        return {
    ...
    def update(self, dt):
        ...
        avg_tension = 0
        if self.chromosomes:
            avg_tension = sum([c.tension_level for c in self.chromosomes]) / len(self.chromosomes)
        tension_bar_count = int(clamp(avg_tension, 0, 1) * 10)
        tension_bar = "█" * tension_bar_count + "·" * (10 - tension_bar_count)
        self.stage_label.text = (
            f"Round {self.round_index} — {self.stage_name()}  |  "
            f"progress {int(self.progress * 100)}%  |  spindle tension [{tension_bar}]"
        )
    ...
    def create_tension_pulse(self, amount=0.65):
        if not self.chromosomes:
            return
        for c in self.chromosomes:
            c.add_tension_pulse(amount * random.uniform(0.65, 1.0))
        self.mark(vector(0, 1.55, 0), "spindle tension pulse", vector(1.0, 0.46, 0.12), life=2.4)
...
class AIController:
    ...
    def update(self, dt):
        ...
        self.sim.ai_label.text = (
            ...
            f"tension {int(s['spindle_tension'] * 100)}% | "
...
controls = label(
    pos=vector(0, -3.18, 0),
    text="Space pause | A AI | M mode | R reset | S particles | D spindle | T tension | W wrap | C scripted camera | L labels | X override",
    box=False,
    opacity=0,
