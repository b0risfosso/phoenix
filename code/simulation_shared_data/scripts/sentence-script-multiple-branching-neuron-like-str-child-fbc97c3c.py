...
#   Y     toggle neuron-controlled keyboard output
...
scene.caption = (
    ...
    "Y toggles the neuron-controlled visual keyboard output.\n"
)
...
KEYBOARD_OUTPUT_LIMIT = 72
KEYBOARD_PRESS_FADE_TIME = 0.75
...
class VisualKeyboard:
    """A visual-only keyboard controlled by neural activity.

    The keyboard output is kept only in the running program's memory and on the
    VPython canvas. Nothing is written to disk, logged, exported, or saved.
    """
    ...
    def __init__(self):
        ...
        self.base_y = -2.85
        self.base_z = 3.85
        self.base_color = vector(0.86, 0.91, 0.98)
        self.active_color = vector(1.0, 0.84, 0.22)
        self.neuron_color = vector(0.34, 0.76, 1.0)

        self.panel = box(
            pos=vector(0, self.base_y - 0.02, self.base_z + 0.02),
            size=vector(6.5, 0.06, 1.75),
            color=vector(0.78, 0.86, 0.95),
            opacity=0.22,
        )
        self.title = label(
            pos=vector(0, self.base_y + 0.55, self.base_z),
            text="Neuron-Controlled Keyboard  |  visual output only, not saved",
            height=12,
            border=5,
            color=vector(0.10, 0.17, 0.24),
            background=vector(0.94, 0.97, 1.0),
            opacity=0.42,
            box=True,
        )
        self.output_label = label(
            pos=vector(0, self.base_y + 0.82, self.base_z),
            text="Output: ",
            height=14,
            border=7,
            color=vector(0.06, 0.10, 0.14),
            background=vector(1.0, 0.98, 0.86),
            opacity=0.52,
            box=True,
        )

        for letters, z_offset, x_offset in self.row_specs:
            row_width = len(letters) * self.key_w + (len(letters) - 1) * self.key_gap
            start_x = -row_width / 2 + self.key_w / 2 + x_offset
            for i, ch in enumerate(letters):
                pos = vector(start_x + i * (self.key_w + self.key_gap), self.base_y, self.base_z + z_offset)
                key_box = box(pos=pos, size=vector(self.key_w, self.key_h, self.key_d), color=self.base_color, opacity=0.70)
                key_label = label(
                    pos=pos + vector(0, 0.075, 0),
                    text=ch,
                    height=11,
                    border=2,
                ...
                self.keys[ch] = (key_box, key_label)
                self.press_fades[ch] = 0.0

        self.special_keys = []
        self._make_special_key("SPACE", vector(-1.05, self.base_y, self.base_z - 5.78), vector(1.75, self.key_h, self.key_d))
        self._make_special_key("BACK", vector(0.55, self.base_y, self.base_z - 5.78), vector(0.76, self.key_h, self.key_d))
        self._make_special_key("CLEAR", vector(1.55, self.base_y, self.base_z - 5.78), vector(0.88, self.key_h, self.key_d))

    def _make_special_key(self, name, pos, size):
        key_box = box(pos=pos, size=size, color=self.base_color, opacity=0.70)
        key_label = label(
            pos=pos + vector(0, 0.075, 0),
            text=name,
            height=10,
        ...
        self.keys[name] = (key_box, key_label)
        self.press_fades[name] = 0.0
        self.special_keys.append(name)
    ...
    def toggle(self):
        ...
        self.title.text = f"Neuron-Controlled Keyboard {state}  |  visual output only, not saved"

    def press(self, key, source_text="network"):
        if not self.enabled:
            return
        key = key.upper()
        if key == "SPACE":
            ...
            self.output = ""
        elif len(key) == 1 and key in self.keys:
            self.output += key.lower()
        else:
            return

        if len(self.output) > KEYBOARD_OUTPUT_LIMIT:
            self.output = self.output[-KEYBOARD_OUTPUT_LIMIT:]
        self.press_fades[key] = KEYBOARD_PRESS_FADE_TIME
        self.update_output_label(source_text=source_text)

    def update_output_label(self, source_text="network"):
        shown = self.output if self.output else ""
        self.output_label.text = f"Output: {shown}  |  source: {source_text}"

    def update(self, dt):
        for key, (key_box, key_label) in self.keys.items():
            fade = max(0.0, self.press_fades.get(key, 0.0) - dt)
            self.press_fades[key] = fade
            mix = clamp(fade / KEYBOARD_PRESS_FADE_TIME)
            key_box.color = lerp_vec(self.base_color, self.active_color, mix)
            key_box.opacity = 0.54 + 0.36 * mix
            key_label.color = lerp_vec(vector(0.08, 0.12, 0.16), vector(0.20, 0.10, 0.00), mix)
...
class NetworkSimulation:
    ...
    def choose_neural_key(self):
        """Translate current network state into a visual keyboard choice."""
        if not self.neurons:
            return None, "empty network"

        active = max(self.neurons, key=lambda n: n.activity)
        state = self.state_snapshot()
        alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"

        # Active neurons choose letter neighborhoods; synapses/signals shift the
        # result so the output changes as the network reorganizes.
        base = (active.index * 4 + int(active.activity * 12)) % len(alphabet)
        shift = (state["synapse_count"] * 2 + state["signal_count"] + state["branch_count"]) % len(alphabet)
        key = alphabet[(base + shift) % len(alphabet)]

        # Use special keys occasionally, but not often enough to erase the output constantly.
        activity = state["activity"]
        if activity > 0.78 and random.random() < 0.10:
            key = "SPACE"
        elif state["signal_count"] > 10 and random.random() < 0.04:
            key = "BACK"
        elif state["activity"] < 0.10 and len(self.keyboard.output) > 28 and random.random() < 0.025:
            key = "CLEAR"

        return key, f"{active.name} A:{active.activity:.2f} syn:{state['synapse_count']} sig:{state['signal_count']}"

    def neural_keyboard_step(self, dt):
        if self.keyboard is None or not self.keyboard.enabled:
            return
        self.keyboard_timer += dt

        # Activity controls typing speed: quiet networks type slowly; active networks type faster.
        activity = self.network_activity()
        interval = lerp(1.35, 0.28, activity)
        if self.keyboard_timer < interval:
            return
        self.keyboard_timer = 0.0

        key, source = self.choose_neural_key()
        if key is not None:
            self.keyboard.press(key, source_text=source)
    ...
    def update_meter(self):
        ...
        self.status_label.text = (
            ...
            f"{'AI ON' if self.ai_enabled else 'AI OFF'}: {self.ai_mode_text} | Keyboard {'ON' if self.keyboard and self.keyboard.enabled else 'OFF'}"
        )
