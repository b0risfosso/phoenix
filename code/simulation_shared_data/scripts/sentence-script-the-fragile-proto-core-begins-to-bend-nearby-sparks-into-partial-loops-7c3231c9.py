"""
Fire That Wants To Become A Star — Iteration 8
Partial Spark Loops

Story:
The flame has a fragile proto-core that can survive across some cycles.
Now the proto-core begins to shape motion around itself.

Nearby sparks can be bent into partial loops. Some circle the flame once before
escaping upward or returning to the base. These are not stable orbits yet.

This is still not a completed star:
- loops are temporary and unstable
- sparks still escape or fade
- there is no smoke boundary
- there are no rotating flame bands
- the proto-core can still collapse
...
# Scene
# -----------------------------
scene.title = "Fire That Wants To Become A Star — Partial Spark Loops"
scene.width = 1120
scene.height = 720
scene.background = vector(0.96, 0.97, 1.0)
scene.forward = vector(-0.35, -0.25, -1.0)
scene.range = 5.9
scene.center = vector(0, 1.40, 0)
...
# New visual: faint loop guide around the proto-core. It appears only when loop strength exists.
...
for _ in range(28):
    ...
    sparks.append({
        ...
        "bendable": random.random() < 0.56,
        "loopable": random.random() < 0.46,
        "return_glow": 0.0,
        "loop_glow": 0.0,
        "has_bent": False,
        "has_looped": False,
...
title_label = label(
    pos=vector(0, 4.30, 0),
    text="The fragile proto-core bends nearby sparks into partial loops",
    height=16,
    box=False,
...
help_label = label(
    pos=vector(-3.95, 4.78, 0),
    text="H help | P pause | R reset | W will | Space breath | I pull | C cycle | E core | O loop",
    height=10,
    box=False,
    align="left",
...
def reset_spark(p):
    ...
    p["bendable"] = random.random() < 0.56
    p["loopable"] = random.random() < 0.46
    p["return_glow"] = 0.0
    p["loop_glow"] = 0.0
    p["has_bent"] = False
    p["has_looped"] = False
...
def add_memory_mark(pos):
    ...
    escape_noticing = clamp(escape_noticing + 0.022, 0.0, 1.0)
...
def reward_returning_spark(p, from_loop=False):
    ...
    returned_heat = clamp(returned_heat + (0.045 + (0.020 if from_loop else 0.0)), 0.0, 1.0)
    base_brighten = clamp(base_brighten + 0.44, 0.0, 1.0)

    cycle_energy = clamp(cycle_energy + 0.036 + (0.018 if from_loop else 0.0), 0.0, 1.0)
    inward_pull = clamp(inward_pull + 0.009, 0.02, 0.58)

    if from_loop:
        loop_return_count += 1
        proto_core_persistence = clamp(proto_core_persistence + 0.012, 0.0, 0.88)

    add_return_ember(p["obj"].pos)

def mark_partial_loop(p):
    global partial_loop_count, loop_strength, survival_flash, proto_core_persistence

    p["has_looped"] = True
    p["loop_glow"] = 1.0
    partial_loop_count += 1
    loop_strength = clamp(loop_strength + 0.020, 0.0, 0.76)
    survival_flash = clamp(survival_flash + 0.20, 0.0, 1.0)
    proto_core_persistence = clamp(proto_core_persistence + 0.006, 0.0, 0.88)
...
def evaluate_cycle_survival():
    global cycle_start_returns, cycle_start_storage, cycle_start_loops
    global last_cycle_return_count, last_cycle_store_count, last_cycle_loop_count
    global proto_core_persistence, proto_core_survival_streak, proto_core_collapse_count
    global collapse_flash, survival_flash, inner_ember_heat, loop_strength
    ...
    if survived:
        ...
        gain = 0.038 + 0.024 * min(proto_core_survival_streak, 5) + 0.016 * min(cycle_stores, 4) + loop_help
        proto_core_persistence = clamp(proto_core_persistence + gain, 0.0, 0.88)
        survival_flash = 1.0
        inner_ember_heat = clamp(inner_ember_heat + 0.017 * proto_core_persistence + 0.008 * min(cycle_loops, 3), 0.0, 0.92)
    else:
        proto_core_survival_streak = 0
        loss = 0.082 + 0.056 * (1.0 - returned_heat)
        proto_core_persistence = clamp(proto_core_persistence - loss, 0.0, 0.88)
        loop_strength = clamp(loop_strength - 0.060, 0.0, 0.76)
...
def reset_all_evolving_state():
    ...
    for group in (memory_marks, return_embers, storage_motes, collapse_shards, loop_echoes):
        for item in group:
            if "dot" in item:
...
def keydown(evt):
    ...
    global manual_loop_boost, loop_strength
    ...
    if k == "p":
        ...
        proto_core_persistence = clamp(proto_core_persistence + 0.025, 0.0, 0.88)
        survival_flash = 0.75
    elif k == "o":
        manual_loop_boost = 0.45
        loop_strength = clamp(loop_strength + 0.08, 0.0, 0.76)
...
def update_learning_state():
    ...
    inward_pull = clamp(inward_pull, 0.02, 0.58)

    target_loop = (
        0.02
        + 0.10 * inner_ember_heat
        + 0.30 * proto_core_persistence
        + 0.12 * pull_drive
        + 0.08 * receive_drive
        + manual_loop_boost
    )
    loop_strength += (target_loop - loop_strength) * 0.018
    loop_strength = clamp(loop_strength, 0.0, 0.76)
    ...
    escape_noticing = clamp(escape_noticing - 0.00024, 0.0, 1.0)
    ...
    loss = (0.00066 + 0.00076 * (1.0 - returned_heat) + 0.00050 * rise_drive) * persistence_protection
    if cycle_phase == "rest":
        loss *= 0.58
    ...
    if cycle_phase != "rest":
        proto_core_persistence = clamp(proto_core_persistence - 0.00010 * (1.0 - inner_ember_heat), 0.0, 0.88)

    inner_ember_instability = clamp(
        1.0 - inner_ember_heat * 0.46 - proto_core_persistence * 0.40 - loop_strength * 0.12 + 0.12 * math.sin(t * 3.7),
        0.22,
        1.0
    )
    ...
    if cycle_phase == "rest":
        cycle_energy = clamp(
            cycle_energy + 0.00040 * returned_heat + 0.00024 * inner_ember_heat + 0.00018 * proto_core_persistence + 0.00010 * loop_strength - 0.00014,
            0.0,
            1.0
        )
...
def update_flame_shape():
    ...
    collapse_jitter = random.uniform(-0.052, 0.052) * inner_ember_instability * (inner_ember_heat + 0.20 * proto_core_persistence)
    notice_tremble = 0.015 * escape_noticing * math.sin(t * 14.0)

    heat_pulse = base_brighten
    cycle_expand = 0.10 * rise_drive - 0.06 * rest_drive
    cycle_contract = 0.08 * pull_drive

    core_effect = inner_ember_heat * 0.62 + proto_core_persistence * 0.72
    ...
    flame_base.radius = (
        ...
        + 0.016 * inward_pull
        + 0.068 * heat_pulse
        + 0.070 * inner_ember_heat
        + 0.064 * proto_core_persistence
        + 0.030 * receive_drive
    )
    ...
    flame_base.color = vector(
        1.0,
        clamp(0.24 + 0.12 * will_to_become_star + 0.10 * inward_pull + 0.34 * heat_pulse + 0.26 * inner_ember_heat + 0.25 * proto_core_persistence, 0.0, 1.0),
    ...
    base_heat_glow.radius = 0.21 + 0.30 * heat_pulse + 0.08 * returned_heat + 0.11 * inner_ember_heat + 0.14 * proto_core_persistence + 0.05 * loop_strength + 0.08 * receive_drive
    base_heat_glow.opacity = clamp(
        0.05 + 0.36 * heat_pulse + 0.09 * returned_heat + 0.18 * inner_ember_heat + 0.20 * proto_core_persistence + 0.07 * loop_strength + 0.12 * receive_drive,
        0.04,
        0.86
    )
    base_heat_glow.color = vector(1.0, 0.45 + 0.26 * heat_pulse + 0.18 * inner_ember_heat + 0.16 * proto_core_persistence, 0.05 + 0.10 * heat_pulse)

    core_pos = flame_base.pos + vector(0, 0.065 + 0.012 * ember_pulse, 0)
    unstable_ember.pos = core_pos
    unstable_ember.radius = 0.045 + 0.14 * inner_ember_heat + 0.075 * proto_core_persistence + 0.020 * loop_strength + 0.030 * proto_pulse * core_effect
    unstable_ember.opacity = clamp(0.08 + 0.60 * inner_ember_heat + 0.48 * proto_core_persistence + 0.10 * loop_strength + 0.08 * proto_pulse - 0.20 * collapse_flash, 0.05, 0.92)
    unstable_ember.color = vector(
        1.0,
        clamp(0.40 + 0.36 * inner_ember_heat + 0.32 * proto_core_persistence + 0.08 * loop_strength + 0.10 * proto_pulse, 0.0, 1.0),
        clamp(0.05 + 0.18 * inner_ember_heat + 0.18 * proto_core_persistence + 0.04 * loop_strength, 0.0, 1.0)
    )

    proto_core_shell.pos = core_pos
    proto_core_shell.radius = 0.10 + 0.26 * proto_core_persistence + 0.10 * inner_ember_heat + 0.05 * loop_strength + 0.03 * proto_pulse
    proto_core_shell.opacity = clamp(0.02 + 0.40 * proto_core_persistence + 0.08 * loop_strength + 0.12 * survival_flash - 0.12 * collapse_flash, 0.015, 0.55)
    proto_core_shell.color = vector(
        1.0,
        clamp(0.42 + 0.38 * proto_core_persistence + 0.08 * loop_strength + 0.12 * survival_flash, 0, 1),
        0.05 + 0.16 * proto_core_persistence
    )

    ember_pulse_ring.pos = unstable_ember.pos
    ember_pulse_ring.radius = 0.14 + 0.34 * inner_ember_heat + 0.20 * proto_core_persistence + 0.08 * loop_strength + 0.05 * ember_pulse
    ember_pulse_ring.thickness = 0.004 + 0.016 * inner_ember_heat + 0.010 * proto_core_persistence + 0.004 * loop_strength
    ember_pulse_ring.opacity = clamp(0.04 + 0.32 * inner_ember_heat * ember_pulse + 0.26 * proto_core_persistence + 0.12 * loop_strength + 0.10 * survival_flash, 0.02, 0.55)

    survival_ring.pos = unstable_ember.pos
    survival_ring.radius = 0.22 + 0.45 * proto_core_persistence + 0.06 * loop_strength + 0.05 * math.sin(t * 2.8)
    survival_ring.thickness = 0.004 + 0.020 * proto_core_persistence + 0.004 * loop_strength
    survival_ring.opacity = clamp(0.02 + 0.34 * proto_core_persistence + 0.07 * loop_strength + 0.20 * survival_flash - 0.14 * collapse_flash, 0.01, 0.48)

    loop_guide_ring.pos = flame_base.pos + vector(0, 0.45 + 0.08 * math.sin(t * 2.0), 0)
    loop_guide_ring.radius = 0.48 + 0.36 * loop_strength + 0.10 * math.sin(t * 2.4)
    loop_guide_ring.thickness = 0.004 + 0.018 * loop_strength
    loop_guide_ring.opacity = clamp(0.02 + 0.38 * loop_strength + 0.08 * pull_drive, 0.01, 0.44)
    loop_guide_ring.color = vector(1.0, clamp(0.42 + 0.36 * loop_strength, 0, 1), 0.05)
    ...
    flame_tip.color = vector(
        1.0,
        clamp(0.42 + 0.14 * will_to_become_star + 0.08 * inward_pull + 0.17 * heat_pulse + 0.14 * core_effect + 0.08 * loop_strength + 0.10 * rise_drive, 0.0, 1.0),
        0.06
    )
    ...
    inner_flame.color = vector(
        1.0,
        clamp(0.82 + 0.07 * will_to_become_star + 0.06 * heat_pulse + 0.10 * core_effect + 0.04 * loop_strength, 0.0, 1.0),
    ...
    flame_light.color = vector(
        1.0,
        clamp(0.36 + 0.22 * will_to_become_star + 0.10 * inward_pull + 0.20 * heat_pulse + 0.20 * core_effect + 0.06 * loop_strength, 0.0, 1.0),
    ...
    inward_field_ring.opacity = 0.04 + 0.16 * inward_pull + 0.10 * pull_drive + 0.05 * returned_heat + 0.08 * proto_core_persistence
...
def update_sparks():
    global bent_spark_count, loop_escape_count
    ...
    for p in sparks:
        ...
        upward_lift = vector(
            0,
            0.10 * will_to_become_star + 0.25 * rise_drive + 0.04 * cycle_energy + 0.030 * inner_ember_heat + 0.020 * proto_core_persistence,
            0
        )
        ...
        if p["bendable"] and pull_zone:
            pull_strength = (
                inward_pull
                * (0.26 + 0.34 * escape_noticing + 0.25 * returned_heat + 0.18 * inner_ember_heat + 0.26 * proto_core_persistence + 1.04 * pull_drive)
                / (0.75 + distance)
            )
            inward_force = safe_norm(rel_to_flame) * pull_strength
            downward_memory = vector(0, -0.032 * inward_pull * (escape_noticing + returned_heat + pull_drive + 0.5 * inner_ember_heat + 0.6 * proto_core_persistence), 0)
        ...
        # New partial-loop behavior.
        # The proto-core creates tangential force near the flame, strongest during pull/receive.
        loop_zone = (
            p["loopable"]
            and (0.30 < obj.pos.y < 1.65)
            and (0.18 < flat_dist < 1.18)
        ...
        if loop_zone:
            ...
            radial_force = -safe_norm(vector(rel_loop.x, 0, rel_loop.z)) * radial_error * (0.07 + 0.22 * loop_strength)

            loop_drive = (
                0.18
                + 0.90 * loop_strength
                + 0.42 * proto_core_persistence
                + 0.22 * pull_drive
                + 0.20 * receive_drive
            )
            ...
            p["loop_time"] += dt
            p["loop_glow"] = clamp(p["loop_glow"] + 0.035, 0.0, 1.0)

            # Count a partial loop when a spark curves around a significant arc.
            if not p["has_looped"] and (p["angle_travel"] > math.pi * 1.25 or p["loop_time"] > 1.10):
                mark_partial_loop(p)

        return_distance = mag(obj.pos - reward_zone_center)
        reward_radius = 0.39 + 0.12 * receive_drive + 0.05 * inner_ember_heat + 0.07 * proto_core_persistence + 0.04 * loop_strength
        if p["has_bent"] and not p["rewarded"] and return_distance < reward_radius and obj.pos.y < 0.91:
        ...
        p["vel"] *= 0.985 - 0.015 * rest_drive

        obj.pos += p["vel"] * dt
        p["return_glow"] *= 0.94
        p["loop_glow"] *= 0.965

        core_glow = 0.5 * inner_ember_heat + 0.7 * proto_core_persistence + 0.35 * loop_strength
        age_fade = clamp(1.0 - p["age"] / p["life"], 0.0, 0.9)
        obj.opacity = clamp(age_fade + 0.22 * p["return_glow"] + 0.14 * p["loop_glow"] + 0.08 * rise_drive + 0.030 * core_glow, 0.0, 1.0)
        obj.radius = clamp(0.016 + 0.019 * p["return_glow"] + 0.012 * p["loop_glow"] + 0.006 * rise_drive + 0.004 * core_glow, 0.012, 0.068)
        obj.color = vector(
            1.0,
            clamp(0.48 + 0.28 * p["return_glow"] + 0.22 * p["loop_glow"] + 0.12 * rise_drive + 0.09 * core_glow + 0.12 * random.random(), 0.0, 1.0),
            clamp(0.07 + 0.08 * p["return_glow"] + 0.10 * p["loop_glow"] + 0.04 * core_glow, 0.0, 1.0)
        )

        # Curved trails only become visible for looped or currently-looping sparks.
        if p["loop_glow"] > 0.12 or p["has_looped"]:
            p["trail"].append(pos=obj.pos)
            p["trail"].color = vector(1.0, clamp(0.38 + 0.35 * p["loop_glow"], 0, 1), 0.05)
            p["trail"].radius = 0.004 + 0.005 * p["loop_glow"]
            if p["trail"].npoints > 26:
                p["trail"].pop(0)

        escaped_high = obj.pos.y > 2.18
...
def update_memory_marks():
    ...
    for m in memory_marks:
        ...
        end_pos = pos - vector(0, length, 0) + toward_flame * (
            0.12 * inward_pull + 0.09 * returned_heat + 0.10 * pull_drive + 0.08 * inner_ember_heat + 0.11 * proto_core_persistence + 0.05 * loop_strength
        )
...
def update_storage_motes():
    ...
    for s in storage_motes:
        ...
        s["angle"] += s["spin"] * dt * (1.0 + 0.35 * proto_core_persistence + 0.20 * loop_strength)
        ...
        s["obj"].opacity = clamp((1.0 - frac) * (0.22 + 0.35 * inner_ember_heat + 0.24 * proto_core_persistence + 0.14 * loop_strength), 0.0, 0.55)
...
def update_status():
    ...
    status_label.text = (
        ...
        f"last cycle returns: {last_cycle_return_count} | stored: {last_cycle_store_count} | loops: {last_cycle_loop_count}\n"
        ...
        "state: partial loops, not yet stable orbit"
    )
...
while True:
    ...
    t += dt
    will_to_become_star = clamp(will_to_become_star + 0.00024, 0.0, 0.78)
