...
# Scene styling
# -----------------------------
scene.title = "Cell Membrane with Homeostatic AI Ion-Channel Controller"
scene.width = 1200
scene.height = 760
...
Controls:
  ...
  I = pump inward             U = pump outward     B = homeostasis/balance mode
...
CSV_SAMPLE_INTERVAL = float(os.environ.get("SIMULATION_CSV_SAMPLE_INTERVAL", "0.10"))
CSV_FLUSH_INTERVAL = float(os.environ.get("SIMULATION_CSV_FLUSH_INTERVAL", "2.0"))
...
if _csv_output_dir:
    ...
    CSV_OUTPUT_PATH = os.path.join(
        _csv_output_dir,
        f"{_csv_run_id}-homeostatic-cell-membrane-state-log.csv"
    )
else:
    CSV_OUTPUT_PATH = os.environ.get(
        ...
        os.path.join(
            ...
            "homeostatic_cell_membrane_state_log.csv"
        )
    )
    fallback_dir = os.path.dirname(os.path.abspath(CSV_OUTPUT_PATH))
    if fallback_dir:
        os.makedirs(fallback_dir, exist_ok=True)

csv_run_id = _csv_run_id
simulation_time = 0.0
next_csv_sample_time = 0.0
last_csv_flush_time = 0.0
csv_file = None
csv_writer = None
...
INITIAL_IONS = 64
MAX_IONS = 95
CONCENTRATION_TOLERANCE = 3       # ion-count difference tolerated before AI intervenes
MAX_HOMEOSTASIS_PRESSURE = 1.35   # strongest inward/outward correction force
DT = 0.020
...
class MembraneAI:
    def __init__(self):
        ...
        self.reset_wait = 0.0
        self.balance_error = 0
        self.balance_action = "watching"
        self.target_inside_fraction = 0.50
    ...
    def read_state(self):
        ...
        avg_speed = sum(mag(ion.vel) for ion in ions) / total
        ...
        inside_fraction = inside / total
        outside_fraction = outside / total
        target_inside = total * self.target_inside_fraction
        balance_error = inside - target_inside
        return {
            "inside": inside,
            "outside": outside,
            "total": len(ions),
    ...
    def set_mode(self, mode):
        ...
        if mode == "HOMEOSTASIS":
            self.mode_duration = pyrandom.uniform(12.0, 22.0)
        if mode == "CHAOS":
    ...
    def choose_new_mode(self, state, force=False):
        ...
        candidates = ["HOMEOSTASIS", "BALANCE", "MIX", "PULSE", "ORBIT", "ARTIST", "CAREFUL"]
        if outside > inside + max(6, total * 0.18):
            candidates += ["HOMEOSTASIS", "PUMP_IN", "PUMP_IN", "CAREFUL"]
        if inside > outside + max(6, total * 0.18):
            candidates += ["HOMEOSTASIS", "PUMP_OUT", "PUMP_OUT", "CAREFUL"]
        if self.stagnant_time > 10:
            candidates += ["HOMEOSTASIS", "CHAOS", "PULSE", "ARTIST"]
        if abs(diff) < total * 0.13:
            candidates += ["HOMEOSTASIS", "MIX", "ORBIT", "ARTIST"]
        if total < 25:
            candidates += ["RESET"]
    ...
    def apply_channel_pattern(self, pattern, state):
        ...
        elif pattern == "homeostasis":
            self.apply_homeostasis_control(state)
        elif pattern == "random":
            for ch in channels:
                if pyrandom.random() < 0.28:
                    ch.toggle()

    def apply_homeostasis_control(self, state):
        """Open/close channels according to concentration imbalance.

        In this simulation, concentration is represented by the visible ion count
        on each side of the membrane. Positive error means too many ions inside;
        negative error means too many outside.
        """
        inside = state["inside"]
        outside = state["outside"]
        total = max(1, state["total"])
        diff = inside - outside
        self.balance_error = diff

        if abs(diff) <= CONCENTRATION_TOLERANCE:
            desired_open = 2
            self.balance_action = "balanced: closing to a small leak"
        else:
            severity = clamp(abs(diff) / total, 0.0, 1.0)
            desired_open = int(clamp(2 + severity * len(channels), 3, len(channels)))
            if diff > 0:
                self.balance_action = "inside high: opening outward gates"
            else:
                self.balance_action = "outside high: opening inward gates"

        # Open a traveling group of channels so the correction is visible, not static.
        center = int((self.phase * 0.9) % len(channels))
        ranked = sorted(
            range(len(channels)),
            key=lambda i: min((i - center) % len(channels), (center - i) % len(channels))
        )
        open_ids = set(ranked[:desired_open])
        for i, ch in enumerate(channels):
            ch.set_open(i in open_ids)
            if i in open_ids:
                ch.flash = max(ch.flash, 0.12)

        # Mark ions on the crowded side so the viewer can see what the AI is correcting.
        for ion in ions:
            if abs(diff) <= CONCENTRATION_TOLERANCE:
                if ion.marked and pyrandom.random() < 0.04:
                    ion.set_marked(False)
            elif diff > 0 and ion.is_inside() and pyrandom.random() < 0.018:
                ion.set_marked(True)
            elif diff < 0 and not ion.is_inside() and pyrandom.random() < 0.018:
                ion.set_marked(True)
            elif ion.marked and pyrandom.random() < 0.006:
                ion.set_marked(False)
    ...
    def step(self, dt):
        ...
        # The AI always falls back to homeostasis when concentration error is large.
        if self.mode not in ["HOMEOSTASIS", "RESET"] and abs(state["imbalance"]) > max(10, state["total"] * 0.24):
            self.set_mode("HOMEOSTASIS")
        ...
        if self.mode == "HOMEOSTASIS":
            self.apply_channel_pattern("homeostasis", state)
            diff = inside - outside
            if abs(diff) <= CONCENTRATION_TOLERANCE:
                global_pressure_bias = 0.04 * math.sin(self.phase * 1.4)
                global_swirl_strength = 0.05
                global_electric_field = vector(0.015 * math.sin(self.phase), 0.015 * math.cos(self.phase), 0)
            else:
                correction = clamp(abs(diff) / total, 0.16, 1.0)
                # positive pressure pushes ions outward; negative pressure pulls inward
                global_pressure_bias = (1 if diff > 0 else -1) * MAX_HOMEOSTASIS_PRESSURE * correction
                global_swirl_strength = 0.10 + 0.22 * correction
                focus_dir = channels[int((self.phase * 0.9) % len(channels))].n
                global_electric_field = safe_norm(focus_dir) * (0.10 * (1 if diff > 0 else -1))
    ...
    def update_wand(self, dt, state):
        mode_col = {
            "HOMEOSTASIS": vector(0.15, 0.95, 0.75),
        ...
        if channels:
            if self.mode in ["PUMP_IN", "PUMP_OUT", "ARTIST", "HOMEOSTASIS"]:
                if self.mode == "HOMEOSTASIS":
                    self.focus_index = int((self.phase * 0.9) % len(channels))
...
def update_meter_and_labels():
    ...
    imbalance = inside - outside
    ai_label.text = (
        f"AI {ai_state}: {ai.mode} | concentration in {inside / total:0.2f} out {outside / total:0.2f} | "
        f"imbalance {imbalance:+d} | {ai.balance_action} | stagnation {ai.stagnant_time:0.1f}s"
...
CSV_FIELDNAMES = [
    ...
    "open_channel_count",
    "total_crossings",
    "global_pressure_bias",
    "global_swirl_strength",
    "global_electric_field_x",
    "global_electric_field_y",
    "global_electric_field_z",
    "channel_open_states",
    "channel_attached_counts",
    "ion_index",
    "ion_charge",
    "ion_marked",
    "ion_inside",
    "ion_attached_channel",
    "ion_attach_timer",
    "ion_recent_transfer_timer",
    "ion_pos_x",
    "ion_pos_y",
    "ion_pos_z",
    "ion_vel_x",
...
def record_csv_snapshot(t):
    ...
    total = max(1, len(ions))
    open_count = sum(1 for ch in channels if ch.open)
    channel_open_states = "|".join("1" if ch.open else "0" for ch in channels)
    ...
    common = {
        ...
        "ai_mode": ai.mode,
        "ai_time_in_mode": round(ai.time_in_mode, 4),
        "ai_stagnant_time": round(ai.stagnant_time, 4),
        "ai_completion_time": round(ai.completion_time, 4),
        "ai_balance_error": round(getattr(ai, "balance_error", 0), 4),
        "ai_balance_action": getattr(ai, "balance_action", ""),
        "inside_count": inside,
        "outside_count": outside,
        "inside_fraction": round(inside_fraction, 6),
        ...
        "global_electric_field_x": round(global_electric_field.x, 6),
        "global_electric_field_y": round(global_electric_field.y, 6),
        "global_electric_field_z": round(global_electric_field.z, 6),
        "channel_open_states": channel_open_states,
        "channel_attached_counts": channel_attached_counts,
    }

    for i, ion in enumerate(ions):
        attached_idx = -1
        if ion.attached_channel is not None:
            attached_idx = ion.attached_channel.idx

        row = dict(common)
        row.update({
            "ion_index": i,
            "ion_charge": ion.charge,
            "ion_marked": int(ion.marked),
            "ion_inside": int(ion.is_inside()),
            "ion_attached_channel": attached_idx,
            "ion_attach_timer": round(ion.attach_timer, 6),
            "ion_recent_transfer_timer": round(ion.recent_transfer_timer, 6),
            "ion_pos_x": round(ion.pos.x, 6),
            "ion_pos_y": round(ion.pos.y, 6),
            "ion_pos_z": round(ion.pos.z, 6),
            "ion_vel_x": round(ion.vel.x, 6),
            "ion_vel_y": round(ion.vel.y, 6),
            "ion_vel_z": round(ion.vel.z, 6),
            "ion_speed": round(mag(ion.vel), 6),
        })
        csv_writer.writerow(row)

def flush_csv_storage():
...
try:
    while simulation_time < CSV_RUN_SECONDS:
        rate(60)
        frame_count += 1
        ...
        if simulation_time + 1e-9 >= next_csv_sample_time:
            record_csv_snapshot(simulation_time)
            next_csv_sample_time += CSV_SAMPLE_INTERVAL

        if simulation_time - last_csv_flush_time >= CSV_FLUSH_INTERVAL:
            flush_csv_storage()
            last_csv_flush_time = simulation_time
    ...
    ai_label.text = f"Run ID: {csv_run_id}"
