...
# Two Bar Magnets Oscillating Between Repulsion and Attraction
# Repulsion: like poles face each other and a central null/repulsion zone is visible.
# Attraction: opposite poles face each other and the central null zone disappears.
# ------------------------------------------------------------

scene = canvas(
    title="Bar Magnets Oscillating Between Repulsion and Attraction",
    width=1200,
    height=760,
...
Controls:
  ...
  T          : toggle trails
  C          : clear trails
  G          : toggle repulsion/attraction oscillation
  H          : hide/show help

Oscillation:
  The right magnet slowly flips between two states.
  Repulsion: N faces N, field lines avoid the center, and the null zone glows.
  Attraction: N faces S, field lines bridge the gap, and the null zone disappears.
"""
...
# Right magnet flip oscillator:
#   right_flip_angle = pi   -> right magnet's N pole faces the gap (repulsion, null zone visible)
#   right_flip_angle = 0    -> right magnet's S pole faces the gap (attraction, null zone disappears)
MAGNET_OSCILLATION_PERIOD = 12.0
...
title_label = label(
    pos=vector(0, 3.55, 0),
    text="Bar Magnets Oscillate: Repulsion Null Zone ⇄ Attraction",
    height=18,
    box=False,
...
bubble_label = label(
    pos=vector(0, 1.75, 0),
    text="REPULSION: central null zone\nvisible",
    height=12,
    box=True,
    border=5,
...
field_label = label(
    ...
    text="repulsion: field lines avoid center\nattraction: field lines cross gap",
    height=12,
    box=False,
...
class RepellingMagnetPair:
    def __init__(self):
        ...
        self.oscillation_time = 0.0
        self.oscillation_enabled = True
        self.right_flip_angle = math.pi
    ...
    def repulsion_strength(self):
        # 1.0 when the right magnet's N pole faces the gap, 0.0 when its S pole faces the gap.
        return clamp((1.0 - math.cos(self.right_flip_angle)) * 0.5, 0.0, 1.0)

    def attraction_strength(self):
        return 1.0 - self.repulsion_strength()

    def polarity_state_name(self):
        r = self.repulsion_strength()
        if r > 0.70:
            return "REPULSION: N faces N | null zone visible"
        if r < 0.30:
            return "ATTRACTION: N faces S | null zone gone"
        return "TRANSITION: right magnet flipping"

    def advance_polarity_oscillation(self, dt):
        if not self.oscillation_enabled:
            return
        self.oscillation_time += dt
        phase = 2.0 * math.pi * self.oscillation_time / MAGNET_OSCILLATION_PERIOD
        self.right_flip_angle = math.pi * 0.5 * (1.0 + math.cos(phase))
        self.update_logical_frames()
        self.visuals_dirty = True
    ...
    def repulsion_bubble_force(self, point):
        ...
        return (sideways + axial) * self.repulsion_strength()
    ...
    def update_logical_frames(self):
        ...
        # The right magnet physically flips around the pair's vertical frame axis.
        # angle=pi: N faces N (repulsion). angle=0: N faces outward, so S faces the gap (attraction).
        right_axis = self.u.rotate(angle=self.right_flip_angle, axis=self.v)
        self.right.set_frame(self.right_center(), right_axis, self.v)
    ...
    def update_visuals(self):
        ...
        repulse = self.repulsion_strength()
        attract = 1.0 - repulse

        # In repulsion the arrows split outward. In attraction they point inward across the gap.
        self.gap_arrow_l.pos = self.pos - self.u * 0.08
        self.gap_arrow_r.pos = self.pos + self.u * 0.08
        self.gap_arrow_l.axis = -self.u * (0.48 * repulse) + self.u * (0.38 * attract)
        self.gap_arrow_r.axis = self.u * (0.48 * repulse) - self.u * (0.38 * attract)
        arrow_color = vector(1.0, 0.30, 0.25) * repulse + vector(0.15, 0.72, 1.0) * attract
        ...
        self.bubble.radius = BUBBLE_RADIUS * pulse * (0.18 + 0.82 * repulse)
        self.bubble.opacity = 0.03 + 0.16 * repulse

        self.bubble_ring_u.pos = self.pos
        self.bubble_ring_u.axis = self.u
        self.bubble_ring_u.radius = BUBBLE_SIDE_RADIUS * pulse * (0.18 + 0.82 * repulse)
        self.bubble_ring_u.opacity = 0.06 + 0.58 * repulse

        self.bubble_ring_v.pos = self.pos
        self.bubble_ring_v.axis = self.v
        self.bubble_ring_v.radius = BUBBLE_SIDE_RADIUS * 0.92 * pulse * (0.18 + 0.82 * repulse)
        self.bubble_ring_v.opacity = 0.04 + 0.42 * repulse

        self.bubble_ring_w.pos = self.pos
        self.bubble_ring_w.axis = self.w
        self.bubble_ring_w.radius = BUBBLE_SIDE_RADIUS * 0.92 * pulse * (0.18 + 0.82 * repulse)
        self.bubble_ring_w.opacity = 0.04 + 0.42 * repulse

        bubble_label.pos = self.pos + self.v * 1.72 + self.w * 0.10
        bubble_label.text = (
            "REPULSION: central null zone\nvisible and glowing"
            if repulse > 0.60
            else "ATTRACTION: central null zone\ndisappears"
            if repulse < 0.28
            else "TRANSITION: null zone\nfading / reforming"
        )
        bubble_label.opacity = 0.28 + 0.42 * repulse
...
def magnetic_field_at(point):
    ...
    repulse = magnet.repulsion_strength()
    if metric < 1.55 and repulse > 0.02:
        radial = magnet.v * local.y + magnet.w * local.z
        if mag(radial) < 0.05:
            radial = magnet.v * 0.05
        radial_dir = safe_norm(radial, magnet.v)
        influence = (1.0 - clamp(metric / 1.55, 0, 1)) ** 2 * repulse
...
class FieldLineSystem:
    ...
    def generate_line_points(self, phase, layer, side):
        ...
        for step in range(FIELD_LINE_STEPS):
            ...
            if magnet.repulsion_strength() > 0.45 and magnet.bubble_metric(p) < 1.02:
                p += safe_norm(ring_dir, magnet.v) * FIELD_LINE_STEP_SIZE * 2.0 * magnet.repulsion_strength()
                last_good = p
...
def read_simulation_state():
    ...
    return {
        ...
        "repulsion_strength": magnet.repulsion_strength(),
        "attraction_strength": magnet.attraction_strength(),
        "polarity_state": magnet.polarity_state_name(),
...
def reset_simulation_round(randomize_orientation=True):
    ...
    magnet.oscillation_time = 0.0
    magnet.right_flip_angle = math.pi
...
def on_keydown(evt):
    ...
    if k in (" ", "p"):
        ...
        magnet.oscillation_enabled = not magnet.oscillation_enabled
    elif k == "h":
...
def update_labels(state):
    ...
    title_label.text = magnet.polarity_state_name()

    ai_status_label.text = (
        f"AI: {ai_text} | {pause_text} | oscillator: {'ON' if magnet.oscillation_enabled else 'OFF'}\n"
        f"mode: {ai.mode}\n"
    ...
    state_label.text = (
        ...
        f"polarity: repulsion {state['repulsion_strength']:.2f} / attraction {state['attraction_strength']:.2f}\n"
        f"in null bubble: {state['bubble_count']}\n"
...
while True:
    ...
    magnet.advance_polarity_oscillation(DT)
