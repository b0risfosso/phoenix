...
Controls:
  ...
  T          : toggle trails
  C          : clear trails
  G          : restart charged particle through central null zone
  H          : hide/show help
...
Variation:
  ...
  A slow positive test charge passes through the central null zone with only
  a small magnetic deflection, leaving a visible cyan trail.
"""
...
CHARGE_PATH_HALF_LENGTH = 3.45
CHARGE_SPEED = 0.34
CHARGE_FIELD_COUPLING = 0.065
CHARGE_TRAIL_RETAIN = 320
CHARGE_MAX_SIDE_DEFLECTION = 0.62
...
charge_label = label(
    pos=vector(0, -1.85, 0),
    text="slow + charge path through central null",
    height=12,
    box=True,
    border=5,
    color=vector(0.03, 0.23, 0.32),
    background=vector(0.88, 1.0, 1.0),
    opacity=0.68,
)
...
# ------------------------------------------------------------
# Slow Charged Particle Through Central Null Zone
# ------------------------------------------------------------

class SlowChargedParticle:
    """Positive test charge moving slowly through the central gap.

    The particle travels along the pair's local w-axis so it passes between
    the two facing north poles without colliding with the bar magnets. Its
    acceleration uses a scaled Lorentz-force term q(v x B). Because the
    magnetic field is weak near the center gap, the visible cyan particle
    keeps an almost straight path through the null zone.
    """

    def __init__(self):
        self.charge_sign = 1.0
        self.mass = 1.0
        self.t = 0.0
        self.local_side_offset = vector(0, 0, 0)
        self.local_side_velocity = vector(0, 0, 0)
        self.pos = vector(0, 0, 0)
        self.vel = vector(0, 0, 0)
        self.last_b_strength = 0.0
        self.last_deflection = 0.0
        self.in_null_zone = False

        self.guide_line = curve(
            pos=[vector(0, 0, 0), vector(0, 0, 0)],
            radius=0.012,
            color=vector(0.20, 0.82, 1.0),
            opacity=0.38,
            emissive=True,
        )
        self.null_ring = ring(
            ...
            radius=0.40,
            thickness=0.012,
            color=vector(0.00, 0.85, 1.0),
            opacity=0.70,
        )
        self.body = sphere(
            pos=vector(0, 0, 0),
            radius=0.105,
            color=vector(0.00, 0.75, 1.0),
            emissive=True,
            shininess=0.9,
        )
        self.core = sphere(
        ...
        self.velocity_arrow = arrow(
            pos=vector(0, 0, 0),
            axis=vector(0, 0, 0.45),
            shaftwidth=0.030,
            headwidth=0.10,
            headlength=0.14,
            color=vector(0.00, 0.55, 1.0),
            opacity=0.72,
        )
        self.deflection_arrow = arrow(
            pos=vector(0, 0, 0),
            axis=vector(0, 0, 0),
            shaftwidth=0.022,
            headwidth=0.075,
            headlength=0.10,
            color=vector(1.0, 0.48, 0.10),
            opacity=0.74,
        )
        self.trail_anchor = sphere(
            pos=vector(0, 0, 0),
            radius=0.010,
            color=vector(0.0, 0.80, 1.0),
            opacity=0.020,
        )
        attach_trail(
            self.trail_anchor,
            radius=0.010,
            color=vector(0.00, 0.68, 1.0),
            retain=CHARGE_TRAIL_RETAIN,
        )
        self.reset(clear_trail=True)

    def start_world(self):
        return magnet.pos - magnet.w * CHARGE_PATH_HALF_LENGTH

    def end_world(self):
        return magnet.pos + magnet.w * CHARGE_PATH_HALF_LENGTH

    def reset(self, clear_trail=True):
        self.t = -CHARGE_PATH_HALF_LENGTH
        self.local_side_offset = vector(0, 0.045, 0)
        self.local_side_velocity = vector(0, 0, 0)
        self.pos = magnet.world_from_pair_local(vector(0, self.local_side_offset.y, self.t))
        self.vel = magnet.w * CHARGE_SPEED
        self.last_b_strength = 0.0
        self.last_deflection = 0.0
        self.in_null_zone = False
        if clear_trail:
    ...
    def update_guide(self):
        start = self.start_world()
        end = self.end_world()
        self.guide_line.modify(0, pos=start)
        self.guide_line.modify(1, pos=end)

        self.null_ring.pos = magnet.pos
        self.null_ring.axis = magnet.w
        self.null_ring.radius = 0.42 + 0.035 * math.sin(time.time() * 3.0)

    def update(self, dt):
        self.t += CHARGE_SPEED * dt
        if self.t > CHARGE_PATH_HALF_LENGTH:
            self.reset(clear_trail=True)
            return

        # Current world velocity includes the forward path plus the small side drift.
        forward_v = magnet.w * CHARGE_SPEED
        side_v_world = magnet.u * self.local_side_velocity.x + magnet.v * self.local_side_velocity.y
        self.vel = forward_v + side_v_world

        b = magnetic_field_at(self.pos)
        self.last_b_strength = mag(b)

        # Scaled Lorentz force: F = q(v x B).  The small coupling keeps the
        # central null-zone pass slow and nearly straight.
        lorentz_world = self.charge_sign * cross(self.vel, b) * CHARGE_FIELD_COUPLING / self.mass

        # Convert only side deflection into pair-local u/v coordinates.
        local_accel_u = dot(lorentz_world, magnet.u)
        local_accel_v = dot(lorentz_world, magnet.v)
        local_accel = vector(local_accel_u, local_accel_v, 0)

        # Extra damping makes the slow pass visually controlled, while still
        # allowing small curvature outside the weakest part of the field.
        self.local_side_velocity = self.local_side_velocity * 0.985 + local_accel * dt
        self.local_side_velocity = limit_vec(self.local_side_velocity, 0.16)

        self.local_side_offset += self.local_side_velocity * dt
        self.local_side_offset.x = clamp(self.local_side_offset.x, -CHARGE_MAX_SIDE_DEFLECTION, CHARGE_MAX_SIDE_DEFLECTION)
        self.local_side_offset.y = clamp(self.local_side_offset.y, -CHARGE_MAX_SIDE_DEFLECTION, CHARGE_MAX_SIDE_DEFLECTION)

        self.pos = magnet.world_from_pair_local(vector(self.local_side_offset.x, self.local_side_offset.y, self.t))
        self.last_deflection = mag(vector(self.local_side_offset.x, self.local_side_offset.y, 0))
        self.in_null_zone = magnet.bubble_metric(self.pos) < 0.85
        self.update_visuals()

    def update_visuals(self):
        self.update_guide()

        pulse = 1.0 + 0.10 * math.sin(time.time() * 8.0)
        self.body.pos = self.pos
        self.body.radius = 0.105 * pulse
        self.core.pos = self.pos

        plus_scale = 0.082
        self.plus_v.modify(0, pos=self.pos - magnet.v * plus_scale)
        self.plus_v.modify(1, pos=self.pos + magnet.v * plus_scale)
        self.plus_h.modify(0, pos=self.pos - magnet.u * plus_scale)
        self.plus_h.modify(1, pos=self.pos + magnet.u * plus_scale)

        self.velocity_arrow.pos = self.pos - safe_norm(self.vel, magnet.w) * 0.22
        self.velocity_arrow.axis = safe_norm(self.vel, magnet.w) * 0.54

        # Orange arrow shows the tiny magnetic deflection direction.  It nearly
        # disappears at the center null region.
        b = magnetic_field_at(self.pos)
        deflect = cross(self.vel, b)
        if mag(deflect) > 1e-6:
            self.deflection_arrow.axis = safe_norm(deflect, magnet.u) * clamp(mag(deflect) * 0.16, 0.02, 0.42)
        else:
            self.deflection_arrow.axis = vector(0, 0, 0)
        self.deflection_arrow.pos = self.pos + magnet.v * 0.17

        self.trail_anchor.pos = self.pos

        if self.in_null_zone:
            self.body.color = vector(0.00, 0.95, 1.0)
            self.body.opacity = 0.72
        else:
            self.body.color = vector(0.00, 0.70, 1.0)
...
def reset_simulation_round(randomize_orientation=True):
    ...
    charged_particle.reset(clear_trail=True)
...
def update_labels(state):
    ...
    state_label.text = (
        ...
        f"+ charge B: {charged_particle.last_b_strength:.3f} | deflect: {charged_particle.last_deflection:.3f}"
    )

    charge_label.text = (
        "slow + charge through central null\n"
        f"{'inside null zone: little deflection' if charged_particle.in_null_zone else 'outside null zone: slight curve'}"
    )
    charge_label.pos = magnet.pos - magnet.v * 1.70 + magnet.w * 0.35
