...
# ------------------------------------------------------------
# VPython Simulation Variation:
# Two Repelling Bar Magnets + Iron Filings + Repulsion Bubble
# Variation: a visible neutral object is placed inside the central null zone.
# ------------------------------------------------------------

scene = canvas(
    title="Two Repelling Bar Magnets - Object Inside the Central Null Zone",
    width=1200,
    height=760,
...
Variation:
  A small golden neutral probe object is placed directly inside the central
  N ⇔ N null/repulsion zone. The glowing rings around it mark its protected
  location in the center gap while iron filings bend outward around it.
"""
...
null_object_label = label(
    pos=vector(0, 0.85, 0.65),
    text="neutral object\ninside null zone",
    height=12,
    box=True,
    border=5,
    color=vector(0.30, 0.19, 0.02),
    background=vector(1.0, 0.96, 0.74),
    opacity=0.72,
)
...
class CentralNullObject:
    def __init__(self):
        self.pos = vector(0, 0, 0)
        self.body = sphere(
            pos=self.pos,
            radius=0.20,
            color=vector(1.0, 0.76, 0.20),
            shininess=0.95,
            emissive=True,
        )
        self.inner_core = sphere(
            pos=self.pos,
            radius=0.115,
            color=vector(1.0, 0.97, 0.72),
            shininess=0.7,
            opacity=0.86,
        )
        self.sample_cube = box(
            pos=self.pos,
            size=vector(0.38, 0.38, 0.38),
            color=vector(1.0, 0.84, 0.32),
            opacity=0.25,
            shininess=0.6,
        )
        self.pin = cylinder(
        ...
        self.ring_w = ring(
            ...
            radius=0.57,
            thickness=0.010,
            color=vector(0.88, 0.58, 1.0),
            opacity=0.58,
        )
        self.zero_arrow_l = arrow(
            pos=self.pos,
            axis=vector(-0.36, 0, 0),
            shaftwidth=0.020,
            headwidth=0.070,
            headlength=0.095,
            color=vector(1.0, 0.35, 0.25),
            opacity=0.75,
        )
        self.zero_arrow_r = arrow(
            pos=self.pos,
            axis=vector(0.36, 0, 0),
            shaftwidth=0.020,
            headwidth=0.070,
    ...
    def update(self, pos, u, v, w, pulse=1.0):
        self.pos = pos
        self.body.pos = pos
        self.body.radius = 0.20 * (1.0 + 0.04 * math.sin(time.time() * 4.4))

        self.inner_core.pos = pos
        self.sample_cube.pos = pos
        self.sample_cube.axis = u
        self.sample_cube.up = v

        self.pin.pos = pos - v * 0.28
        self.pin.axis = v * 0.56

        self.ring_u.pos = pos
        self.ring_u.axis = u
        self.ring_u.radius = 0.43 * pulse

        self.ring_v.pos = pos
        self.ring_v.axis = v
        self.ring_v.radius = 0.50 * pulse

        self.ring_w.pos = pos
        self.ring_w.axis = w
        self.ring_w.radius = 0.57 * pulse

        self.zero_arrow_l.pos = pos + u * 0.07
        self.zero_arrow_l.axis = -u * 0.36
        self.zero_arrow_r.pos = pos - u * 0.07
        self.zero_arrow_r.axis = u * 0.36

        null_object_label.pos = pos + v * 0.92 + w * 0.62
...
class RepellingMagnetPair:
    def __init__(self):
        ...
        self.null_object = CentralNullObject()
        self.update_visuals()
    ...
    def update_visuals(self):
        ...
        self.null_object.update(self.pos, self.u, self.v, self.w, pulse=1.0 + 0.035 * math.sin(time.time() * 3.2))
...
def random_cloud_position():
    for _ in range(500):
        ...
        if magnet.bubble_metric(p) < 1.22:
            continue
        if mag(p - magnet.pos) < NULL_OBJECT_RADIUS + 0.45:
            continue
        return p
...
class FilingParticle:
    ...
    def avoid_central_object(self):
        offset = self.center - magnet.pos
        d = mag(offset)
        if d < NULL_OBJECT_RADIUS + 0.18:
            n = safe_norm(offset, random_perpendicular(magnet.u))
            self.center = magnet.pos + n * (NULL_OBJECT_RADIUS + 0.18)
            self.vel += n * 0.20
...
def read_simulation_state():
    ...
    for p in particles:
        ...
        if magnet.bubble_metric(p.center) < 1.0:
            bubble_count += 1
        if mag(p.center - magnet.pos) < NULL_OBJECT_RADIUS + 0.35:
            near_null_object_count += 1
    ...
    return {
        ...
        "near_null_object_count": near_null_object_count,
        "magnet_u": magnet.u,
...
def spill_cloud(side=None):
    ...
    for p in particles:
        ...
        if mag(p.center - magnet.pos) < NULL_OBJECT_RADIUS + 0.45:
            p.center += safe_norm(p.center - magnet.pos, random_perpendicular(magnet.u)) * 0.8
...
def reset_simulation_round(randomize_orientation=True):
    global round_number, bond_curves
    round_number += 1
...
class AIController:
    ...
    def choose_new_mode(self, state, reason="time"):
        ...
        if bubble_count > 4:
            choices = ["OUTWARD_BEND", "ORBIT_WRAP", "ROTATE_SWEEP"]
        elif attached_ratio > 0.60:
            choices = ["SHAKE_DETACH", "ORBIT_WRAP", "MARK_CHAINS"]
        elif speed < 0.045 and clumps > 20:
            choices = ["MARK_CHAINS", "ARTIST_TRAILS", "ORBIT_WRAP"]
        else:
            choices = ["OBSERVE_NULL_OBJECT", "OUTWARD_BEND", "ROTATE_SWEEP", "ORBIT_WRAP", "CAREFUL_ALIGN", "CHAOTIC_MIX"]

        if reason in ("complete", "stagnant"):
            choices += ["SPILL_FROM_EDGE", "SHAKE_DETACH"]

        previous = self.mode
        choices = [m for m in choices if m != previous] or choices
        self.mode = random.choice(choices)
        self.mode_timer = 0.0

        durations = {
            "OBSERVE_NULL_OBJECT": (2.0, 4.0),
            "OUTWARD_BEND": (4.2, 7.0),
            "ROTATE_SWEEP": (4.0, 6.4),
            "SHAKE_DETACH": (2.0, 3.5),
            "ORBIT_WRAP": (3.0, 5.8),
            "MARK_CHAINS": (1.2, 2.2),
            "SPILL_FROM_EDGE": (2.0, 3.2),
            "ARTIST_TRAILS": (5.0, 8.0),
            "CAREFUL_ALIGN": (4.0, 6.8),
            "CHAOTIC_MIX": (2.5, 4.2),
        }
        lo, hi = durations.get(self.mode, (2.5, 5.0))
        self.mode_duration = rand_range(lo, hi)

        if self.mode == "SPILL_FROM_EDGE":
            spill_cloud(random.choice(["left", "right", "top", "front", "ring"]))
        if self.mode == "ARTIST_TRAILS":
            set_all_trails(True)
    ...
    def update(self, state, dt):
        ...
        if self.mode == "OBSERVE_NULL_OBJECT":
            self.action_rotate(vector(0, 1, 0), 0.0025 * math.sin(phase))
            for p in particles:
                if not p.attached and magnet.bubble_metric(p.center) < 1.6:
                    p.vel += magnet.repulsion_bubble_force(p.center) * 0.0025
...
def update_labels(state):
    ...
    state_label.text = (
        ...
        f"near null object: {state['near_null_object_count']}\n"
