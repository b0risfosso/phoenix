...
# Unequal Repelling Bar Magnets + Iron Filings + Shifting Null Zone + AI Control
# Like north poles face each other; the stronger left magnet shifts the null zone toward the weaker magnet.
# ------------------------------------------------------------

scene = canvas(
    title="Unequal Repelling Bar Magnets - Obvious Shifting Null Zone",
    width=1200,
    height=760,
...
Variation:
  ...
  The left magnet gradually becomes stronger than the right magnet.
  The left magnet pulses from very weak to very strong. The central null/repulsion
  zone visibly slides away from the stronger magnet and toward the weaker magnet.
  A cyan bubble, trail, ruler, and arrow track the shifted null.
"""
...
# Unequal magnet strengths.
# The left magnet now swings from much weaker than the right magnet to much
# stronger than the right magnet. This makes the null-zone shift large enough
# to read immediately in the scene.
LEFT_STRENGTH_MIN = 0.08
LEFT_STRENGTH_MAX = 18.00
RIGHT_STRENGTH = 1.00
STRENGTH_CYCLE_SECONDS = 8.0

# Visual exaggeration for the central cancellation marker. The magnetic field still
# uses the same strength model, but the glowing bubble and arrow are deliberately
# amplified so the displacement is obvious while the simulation is running.
NULL_SHIFT_VISUAL_GAIN = 1.35
MAX_VISIBLE_NULL_SHIFT = CENTER_GAP * 0.52
...
title_label = label(
    pos=vector(0, 3.55, 0),
    text="Unequal Repelling Bar Magnets: Shifted Null Zone",
    height=18,
    box=False,
...
bubble_label = label(
    pos=vector(0, 1.75, 0),
    text="BRIGHT CYAN = shifted null zone\nyellow dot = original midpoint",
    height=12,
    box=True,
    border=5,
...
field_label = label(
    ...
    text="field lines bend around shifted null",
    height=12,
    box=False,
...
class RepellingMagnetPair:
    def __init__(self):
        ...
        self.strength_time = 0.0
        self.left_strength = LEFT_STRENGTH_MIN
        ...
        self.null_marker = sphere(
            pos=self.null_zone_center(),
            radius=0.115,
            color=vector(0.20, 1.0, 0.95),
            opacity=0.88,
            emissive=True,
        )
        self.shift_arrow = arrow(
            pos=self.pos,
            axis=vector(0.01, 0, 0),
            shaftwidth=0.026,
            headwidth=0.13,
            headlength=0.18,
            color=vector(0.16, 0.88, 0.95),
            opacity=0.86,
        )
        self.left_strength_label = label(
            pos=self.left_center() + self.v * 1.05,
            text="",
            height=11,
            box=True,
            border=4,
            color=vector(0.14, 0.04, 0.04),
            background=vector(1.0, 0.88, 0.86),
            opacity=0.72,
        )
        self.right_strength_label = label(
            pos=self.right_center() + self.v * 1.05,
            text="",
            height=11,
            box=True,
            border=4,
            color=vector(0.04, 0.04, 0.14),
            background=vector(0.88, 0.94, 1.0),
            opacity=0.72,
        )

        self.midpoint_ring = ring(
            pos=self.pos,
            axis=self.u,
            radius=0.22,
            thickness=0.010,
            color=vector(1.0, 0.88, 0.12),
            opacity=0.70,
        )
        self.shift_ruler = curve(
            pos=[self.pos, self.null_zone_center()],
            radius=0.025,
            color=vector(0.0, 0.95, 1.0),
            opacity=0.95,
        )
        self.left_limit_marker = sphere(
            pos=self.pos - self.u * MAX_VISIBLE_NULL_SHIFT,
            radius=0.055,
            color=vector(0.45, 0.85, 1.0),
            opacity=0.42,
            emissive=True,
        )
        self.right_limit_marker = sphere(
            pos=self.pos + self.u * MAX_VISIBLE_NULL_SHIFT,
            radius=0.055,
            color=vector(0.45, 0.85, 1.0),
            opacity=0.42,
    ...
    def null_zone_center(self):
        """Approximate zero-field location between two like facing poles.

        For two like poles separated by CENTER_GAP with strengths L and R, the
        cancellation point lies closer to the weaker pole. If the left magnet is
        stronger than the right magnet, the null shifts along +u toward the right.
        The display applies a small visual gain so the shift is unmistakable.
        """
        left_strength = max(self.left_strength, 1e-4)
        right_strength = max(self.right_strength, 1e-4)
        ratio = math.sqrt(left_strength / right_strength)
        physical_shift = 0.5 * CENTER_GAP * (ratio - 1.0) / (ratio + 1.0)
        visible_shift = physical_shift * NULL_SHIFT_VISUAL_GAIN
        visible_shift = clamp(visible_shift, -MAX_VISIBLE_NULL_SHIFT, MAX_VISIBLE_NULL_SHIFT)
        return self.pos + self.u * visible_shift

    def null_shift_local_x(self):
        return dot(self.null_zone_center() - self.pos, self.u)

    def update_strengths(self, dt):
        self.strength_time += dt
        phase = 0.5 + 0.5 * math.sin(2 * math.pi * self.strength_time / STRENGTH_CYCLE_SECONDS)
        self.left_strength = LEFT_STRENGTH_MIN + (LEFT_STRENGTH_MAX - LEFT_STRENGTH_MIN) * phase
        self.right_strength = RIGHT_STRENGTH
        self.visuals_dirty = True

    def null_local_from_world(self, world):
        r = world - self.null_zone_center()
        return vector(dot(r, self.u), dot(r, self.v), dot(r, self.w))

    def bubble_metric(self, point):
        local = self.null_local_from_world(point)
        return math.sqrt(
    ...
    def update_visuals(self):
        ...
        null_center = self.null_zone_center()
        null_shift = null_center - self.pos

        # As the left magnet grows stronger, the real cancellation point shifts
        # toward the weaker right magnet. Move all null-zone visuals to that point.
        self.bubble.pos = null_center
        self.bubble.radius = BUBBLE_RADIUS * pulse

        self.bubble_ring_u.pos = null_center
        self.bubble_ring_u.axis = self.u
        self.bubble_ring_u.radius = BUBBLE_SIDE_RADIUS * pulse

        self.bubble_ring_v.pos = null_center
        self.bubble_ring_v.axis = self.v
        self.bubble_ring_v.radius = BUBBLE_SIDE_RADIUS * 0.92 * pulse

        self.bubble_ring_w.pos = null_center
        self.bubble_ring_w.axis = self.w
        self.bubble_ring_w.radius = BUBBLE_SIDE_RADIUS * 0.92 * pulse

        self.midpoint_marker.pos = self.pos
        self.midpoint_marker.radius = 0.075 + 0.012 * math.sin(time.time() * 5.0)

        self.midpoint_ring.pos = self.pos
        self.midpoint_ring.axis = self.u
        self.midpoint_ring.radius = 0.24 + 0.025 * math.sin(time.time() * 4.0)

        self.left_limit_marker.pos = self.pos - self.u * MAX_VISIBLE_NULL_SHIFT
        self.right_limit_marker.pos = self.pos + self.u * MAX_VISIBLE_NULL_SHIFT

        self.null_marker.pos = null_center
        self.null_marker.radius = 0.155 + 0.055 * pulse

        self.shift_ruler.modify(0, pos=self.pos)
        self.shift_ruler.modify(1, pos=null_center)
        self.shift_ruler.radius = 0.020 + 0.010 * abs(math.sin(time.time() * 4.0))

        self.shift_arrow.pos = self.pos
        self.shift_arrow.axis = null_shift if mag(null_shift) > 0.03 else self.u * 0.03
        self.shift_arrow.shaftwidth = 0.045
        self.shift_arrow.headwidth = 0.22
        self.shift_arrow.headlength = 0.28

        self.left_strength_label.pos = self.left_center() + self.v * 1.05
        if self.left_strength > self.right_strength:
            left_role = "STRONGER"
            right_role = "weaker"
        else:
            left_role = "weaker"
        ...
        self.left_strength_label.text = f"left magnet: {left_role}\nstrength {self.left_strength:.2f}x"

        self.right_strength_label.pos = self.right_center() + self.v * 1.05
        self.right_strength_label.text = f"right magnet: {right_role}\nstrength {self.right_strength:.2f}x"

        bubble_label.pos = null_center + self.v * 1.72 + self.w * 0.10
        direction_text = "right, toward weaker right magnet" if self.null_shift_local_x() > 0 else "left, toward weaker left magnet"
        bubble_label.text = (
            "BRIGHT CYAN = shifted null zone\n"
            "yellow dot/ring = original midpoint\n"
            f"shift {self.null_shift_local_x():+.2f}: {direction_text}"
        )
...
def magnetic_field_at(point):
    ...
    poles = [
        (magnet.left_north_world(), magnet.left_strength),
        (magnet.right_north_world(), magnet.right_strength),
        (magnet.left_south_world(), -magnet.left_strength),
        (magnet.right_south_world(), -magnet.right_strength),
    ]
    ...
    for piece, strength in ((magnet.left, magnet.left_strength), (magnet.right, magnet.right_strength)):
        local = piece.local_from_world(point)
        if (
    ...
    local = magnet.null_local_from_world(point)
...
def read_simulation_state():
    ...
    return {
        ...
        "left_strength": magnet.left_strength,
        "right_strength": magnet.right_strength,
        "null_shift": magnet.null_shift_local_x(),
...
def reset_simulation_round(randomize_orientation=True):
    ...
    magnet.strength_time = 0.0
    magnet.update_strengths(0.0)
...
def update_labels(state):
    ...
    state_label.text = (
        ...
        f"in shifted bubble: {state['bubble_count']}  (should stay low)\n"
        f"L/R strength: {state['left_strength']:.2f} / {state['right_strength']:.2f}\n"
        f"VISIBLE null shift: {state['null_shift']:+.2f}\n"
        f"avg speed: {state['avg_speed']:.3f}\n"
        f"alignment: {state['avg_alignment']:.2f}\n"
        f"trails: {'on' if trails_enabled else 'off'}"
    )

    field_label.pos = magnet.null_zone_center() + magnet.u * 3.2 + magnet.v * 1.85 - magnet.w * 1.15
    field_label.text = "cyan trail/ruler shows null-zone shift"
