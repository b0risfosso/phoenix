...
function variationLink(filename, label = "Build variations") {
  return filename ? `<a href="/variations/${encodeURIComponent(filename)}" target="_blank" rel="noopener">${label}</a>` : "";
}
  ...
  const rows = linked.map(script => `
    <div class="linked-script-row">
      ...
      <span class="downloads">${joinLinks(
        ...
        variationLink(script.filename, "Variations"),
...
function renderScriptDetail(metadata, source, runUrl) {
  ...
  scriptActions.innerHTML = joinLinks(
    ...
    variationLink(metadata.filename)
  );
